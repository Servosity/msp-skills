...
faqs:
  ...
  - q: "Does any data leave my network?"
    a: "No. The CLI and MCP server are binaries on your own machine, and any local data copy sits under your user account. Your AI agent sees the result of a query, not raw bulk data. Credentials come from your own environment, never bundled into the repo or transmitted to us - there are no servers of ours in the path."
