...
howto:
  ...
  - name: "Authenticate"
    text: "Enter your Autotask PSA credentials once, then run autotask-cli doctor to check the install."
...
Yes - there is an MCP server for Autotask PSA. It's free, open source, and runs on your own machine, so your client data stays local unless you route it somewhere yourself. It connects Autotask PSA to Claude, ChatGPT, Copilot, or any MCP-capable agent, and installs in about 60 seconds.
...
Yes, on paid ChatGPT plans. ChatGPT connects to remote MCP servers over HTTPS, but autotask-mcp speaks HTTP natively: run `autotask-mcp --transport http --addr :7777` and put its /mcp endpoint behind an HTTPS tunnel or your own reverse proxy. Step-by-step in the install guide.
