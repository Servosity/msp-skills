...
faqs:
  ...
  - q: "What credentials do I need?"
    a: "An app registration in Microsoft Entra ID. ImmyBot issues no API key of its own: you register an app, create a client secret, then add that Enterprise Application's object ID as an admin person inside ImmyBot. The CLI needs four values, IMMYBOT_SUBDOMAIN, IMMYBOT_TENANT_ID, IMMYBOT_CLIENT_ID and IMMYBOT_CLIENT_SECRET. Run `immybot-pp-cli doctor` to confirm."
...
howto:
  ...
  - name: "Authenticate"
    text: "Enter your ImmyBot credentials once; immybot-pp-cli doctor confirms they work."
  - name: "Ask your first question"
    text: "Ask your AI agent a ImmyBot question in plain language; it runs immybot-pp-cli for you."
---
...
<sub>Your agent runs: <code>immybot-pp-cli session-triage --since 24h</code></sub>
...
<sub>Your agent runs: <code>immybot-pp-cli version-spread "Google Chrome" --min-version 140</code></sub>
...
<sub>Your agent runs: <code>immybot-pp-cli assignment-explain 4821</code></sub>
...
| --- | --- |
| What failed in last night's maintenance window, by root cause? | `immybot-pp-cli session-triage --since 24h` |
| Which tenants are still below a patched version of a title? | `immybot-pp-cli version-spread "Google Chrome" --min-version 140` |
| Why did this computer not get the deployment? | `immybot-pp-cli assignment-explain 4821` |
| What changed across the fleet since yesterday? | `immybot-pp-cli fleet-diff --since 24h` |
| Which computers are stuck part way through onboarding? | `immybot-pp-cli onboarding-stalled --older-than 3d` |
| Which deployments have never actually worked? | `immybot-pp-cli deployment-health --only-failing` |
| What does this script reach before I edit it? | `immybot-pp-cli script-blast-radius "Install Chrome"` |
| Where does ImmyBot disagree with my linked PSA or RMM roster? | `immybot-pp-cli psa-reconcile` |
| Everything known about one computer, in one view? | `immybot-pp-cli computer-dossier 4821` |
| Which tenants are drifting on software compliance? | `immybot-pp-cli drift` |
...
After install, authenticate once with your ImmyBot credentials, then verify with `immybot-pp-cli --version`.
...
| Write (routine) | computers update, tenants update, persons update, preferences update, tags create, target-assignments update | Preview with --dry-run, then a reviewed write |
| Destructive or endpoint-affecting | scripts delete, software delete, target-assignments delete, roles delete, run-immy-service, maintenance-sessions create | Require explicit human approval; never autonomous |
...
An app registration in Microsoft Entra ID. ImmyBot issues no API key of its own: you register an app, create a client secret, then add that Enterprise Application's object ID as an admin person inside ImmyBot. The CLI needs four values, IMMYBOT_SUBDOMAIN, IMMYBOT_TENANT_ID, IMMYBOT_CLIENT_ID and IMMYBOT_CLIENT_SECRET. Run `immybot-pp-cli doctor` to confirm.
