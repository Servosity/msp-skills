...
howto:
  ...
  - name: "Authenticate"
    text: "Enter your MSPbots credentials once, then run mspbots-cli doctor to check the install."
...
Yes - there is an MCP server for MSPbots. It's free, open source, and runs on your own machine, so your client data stays local unless you route it somewhere yourself. It connects MSPbots to Claude, ChatGPT, Copilot, or any MCP-capable agent, and installs in about 60 seconds.
...
Yes, on paid ChatGPT plans. ChatGPT connects to remote MCP servers over HTTPS, but mspbots-mcp speaks HTTP natively: run `mspbots-mcp --transport http --addr :7777` and put its /mcp endpoint behind an HTTPS tunnel or your own reverse proxy. Step-by-step in the install guide.
