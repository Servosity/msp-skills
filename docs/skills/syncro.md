...
howto:
  ...
  - name: "Authenticate"
    text: "Enter your Syncro credentials once, then run syncro-cli doctor to check the install."
...
Yes - there is an MCP server for Syncro. It's free, open source, and runs on your own machine, so your client data stays local unless you route it somewhere yourself. It connects Syncro to Claude, ChatGPT, Copilot, or any MCP-capable agent, and installs in about 60 seconds.
...
Yes, on paid ChatGPT plans. ChatGPT connects to remote MCP servers over HTTPS, but syncro-mcp speaks HTTP natively: run `syncro-mcp --transport http --addr :7777` and put its /mcp endpoint behind an HTTPS tunnel or your own reverse proxy. Step-by-step in the install guide.
...
Run more than one PSA tool, or comparing options? These connectors work the same way: [Autotask PSA](/skills/autotask/) · [ConnectWise PSA (Manage)](/skills/connectwise-manage/) · [HaloPSA](/skills/halopsa/) · [Kaseya BMS](/skills/kaseya-bms/) · [SuperOps](/skills/superops/) · [Zammad](/skills/zammad/)
