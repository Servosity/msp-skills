...
faqs:
  ...
  - q: "TODO: vendor-specific question MSP owners actually search (rate limits, partner requirements, replacing the UniFi portal)"
    a: "TODO"
howto:
...
TODO: <=70 words, MSP-owner language, leads with the outcome. What does UniFi + your AI answer in one sentence that the portal cannot?
...
<sub>Your agent runs: <code>unifi-network-cli TODO</code></sub>
...
| --- | --- |
| TODO: question an MSP keeps asking | `unifi-network-cli TODO` |
...
TODO: one or two sentences vs typical MCP wrappers (generic, no competitor names): most UniFi integrations proxy each question into a live API call ...

TODO: one sentence vs UniFi's own AI features (complements, not replaces). If the vendor has no AI integration, say what this adds that the portal cannot.
...
TODO: 2-3 plain-language sentences from governance.md - what the skill can read, what it can change, and the recommended agent policy per tier. Full details in [governance.md](https://github.com/servosity/msp-skills/blob/main/skills/unifi-network/governance.md).
...
### TODO: vendor-specific question MSP owners actually search (rate limits, partner requirements, replacing the UniFi portal)
