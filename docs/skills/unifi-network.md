...
| --- | --- | --- |
| Read | drift, newcomer, topology, port-audit, guest report, rule-predict, search, analytics, sites devices get-adopted-overview-page, sites firewall get-policies | Allow |
...
Read commands - the local-mirror views, the report commands, search, and every non-mutating site endpoint - cannot change anything and are safe to let an agent run. Writes are grouped by blast radius: routine config writes (firewall policies, ACL rules, VLANs, WiFi broadcasts, DNS policies, guest vouchers) should be previewed with --dry-run and approved before they run. Two tiers should never run unattended: device and port control, where one command can power-cycle a PoE port or kick a client off the network, and destructive commands, where `sites devices remove` factory-resets an online device and the delete commands remove firewall zones, networks, and policies outright. Full details in [governance.md](https://github.com/servosity/msp-skills/blob/main/skills/unifi-network/governance.md).
