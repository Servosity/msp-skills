# Abnormal Security Printed CLI Agent Guide

This directory is a generated `abnormal-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
abnormal-cli doctor --json
abnormal-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
abnormal-cli which "<capability>" --json
abnormal-cli <command> --help
```
...
```bash
abnormal-cli <command> --help
abnormal-cli <command> --dry-run --agent
```
