...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  abnormal-cli workflow archive

  # Full re-archive (ignore previous sync state)
  abnormal-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"abuse-mailbox", "abusecampaigns", "aggregations", "aggregations-attack-frequency", "aggregations-attack-strategy-breakdown", "aggregations-attack-vector-breakdown", "aggregations-attacker-origin", "aggregations-dashboard-summary", "aggregations-most-impersonated-employee", "aggregations-most-impersonated-employee-non-vip", "aggregations-most-impersonated-employee-vip", "aggregations-most-impersonated-vendor", "aggregations-recipient-employees", "aggregations-recipient-employees-non-vip", "aggregations-recipient-employees-vip", "aggregations-sender-impersonation-breakdown", "aggregations-trending-attacks", "api_resources", "auditlogs", "cases", "detection360", "email_search", "roles", "soar", "spm-v2", "spm-v2-reports-summary", "threats", "url-rewrite", "users", "vendor-cases", "vendors"}
			totalSynced := 0
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  abnormal-cli workflow status

  # Show status as JSON
  abnormal-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("abnormal-cli")
			}
