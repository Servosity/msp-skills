...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".local", "share", "abnormal-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("ABNORMAL_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("ABNORMAL_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "abnormal-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.local/share/abnormal-cli/feedback.jsonl.
When ` + "`ABNORMAL_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`ABNORMAL_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  abnormal-cli feedback list
  abnormal-cli feedback list --limit 5
  abnormal-cli feedback list --json`,
