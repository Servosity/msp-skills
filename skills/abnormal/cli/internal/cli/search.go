...
In local mode: searches locally synced data only.`,
  ...
  abnormal-cli search "error timeout"

  # Force local search only
  abnormal-cli search "payment failed" --data-source local

  # Search a specific resource type locally
  abnormal-cli search "critical" --type transactions --data-source local

  # JSON output for piping
  abnormal-cli search "critical" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'abnormal-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
			...
			switch resourceType {
			case "api_resources":
				results, err = db.SearchApiResources(query, limit)
			case "employee":
				results, err = db.SearchEmployee(query, limit)
			case "roles":
				results, err = db.SearchRoles(query, limit)
			case "spm-v2":
				results, err = db.SearchSpmV2(query, limit)
			case "":
				...
				{
					partial, searchErr := db.SearchApiResources(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search api_resources failed: %w", searchErr)
					}
				...
				{
					partial, searchErr := db.SearchEmployee(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search employee failed: %w", searchErr)
					}
					for _, r := range partial {
						key := string(r)
						if !seen[key] {
				...
				{
					partial, searchErr := db.SearchSpmV2(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search spm_v2 failed: %w", searchErr)
					}
					for _, r := range partial {
						key := string(r)
						if !seen[key] {
