...
type Config struct {
	...
	AbnormalApiToken string            `toml:"api_token"`
}
...
func Load(configPath string) (*Config, error) {
	...
	path := configPath
	if path == "" {
		path = os.Getenv("ABNORMAL_CONFIG")
	}
	if path == "" {
		home, _ := os.UserHomeDir()
		path = filepath.Join(home, ".config", "abnormal-cli", "config.toml")
	}
	cfg.Path = path
	...
	// Env var overrides
	if v := os.Getenv("ABNORMAL_API_TOKEN"); v != "" {
		cfg.AbnormalApiToken = v
		cfg.markEnvOverride("AbnormalApiToken")
		cfg.AuthSource = "env:ABNORMAL_API_TOKEN"
	}
	...
	if cfg.AuthSource == "" && cfg.AbnormalApiToken != "" {
		cfg.AuthSource = "config"
	}
	...
	if v := os.Getenv("ABNORMAL_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	return cfg, nil
}
...
func (c *Config) AuthHeader() string {
	...
	if c.AbnormalApiToken != "" {
		if c.AuthSource == "" {
			c.AuthSource = "env:ABNORMAL_API_TOKEN"
		}
		return "Bearer " + c.AbnormalApiToken
	}
	if c.AccessToken != "" {
...
func (c *Config) ClearTokens() error {
	...
	c.updateFileConfigField("ClientSecret")
	c.AbnormalApiToken = ""
	delete(c.envOverrides, "AbnormalApiToken")
	return c.save()
}
...
func (c *Config) configForSave() Config {
	out := *c
	if c.fileConfig != nil {
		if c.envOverrides["AbnormalApiToken"] {
			out.AbnormalApiToken = c.fileConfig.AbnormalApiToken
		}
	}
...
func (c *Config) updateFileConfigField(field string) {
	...
	case "AbnormalApiToken":
		c.fileConfig.AbnormalApiToken = c.AbnormalApiToken
	}
}
