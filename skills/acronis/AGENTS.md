# Acronis Cyber Protect Cloud Printed CLI Agent Guide

This directory is a generated `acronis-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
acronis-cli doctor --json
acronis-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
acronis-cli which "<capability>" --json
acronis-cli <command> --help
```
...
```bash
acronis-cli <command> --help
acronis-cli <command> --dry-run --agent
```
