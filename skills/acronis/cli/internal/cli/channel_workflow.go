...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  acronis-cli workflow archive

  # Full re-archive (ignore previous sync state)
  acronis-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"clients", "task-manager", "task-manager-v2-activities", "tenants"}
			totalSynced := 0
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  acronis-cli workflow status

  # Show status as JSON
  acronis-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("acronis-cli")
			}
