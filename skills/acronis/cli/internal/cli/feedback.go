...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".local", "share", "acronis-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("ACRONIS_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("ACRONIS_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "acronis-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.local/share/acronis-cli/feedback.jsonl.
When ` + "`ACRONIS_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`ACRONIS_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  acronis-cli feedback list
  acronis-cli feedback list --limit 5
  acronis-cli feedback list --json`,
