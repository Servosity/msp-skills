...
Resource scoping:
  ...
  acronis-cli sync --since 7d

  # Parallel sync with 8 workers
  acronis-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  acronis-cli sync --latest-only`,
...
func upsertSingleObject(db *store.Store, resource string, data json.RawMessage) error {
	...
	switch resource {
	case "agent-manager":
		return db.UpsertAgentManager(data)
	case "clients":
		return db.UpsertClients(data)
	case "idp":
		return db.UpsertIdp(data)
	case "task-manager":
		return db.UpsertTaskManager(data)
	case "tenants":
		return db.UpsertTenants(data)
	case "offering_items":
		return db.UpsertOfferingItems(data)
	case "usages":
