...
type Config struct {
	...
	AcronisBearerAuth string            `toml:"bearer_auth"`
...
func Load(configPath string) (*Config, error) {
	cfg := &Config{
		BaseURL: "https://{datacenter}.acronis.com",
	}

	// Resolve config path
	path := configPath
	if path == "" {
		path = os.Getenv("ACRONIS_CONFIG")
	}
	if path == "" {
		home, _ := os.UserHomeDir()
		path = filepath.Join(home, ".config", "acronis-cli", "config.toml")
	}
	cfg.Path = path
	...
	// Env var overrides
	if v := os.Getenv("ACRONIS_BEARER_AUTH"); v != "" {
		cfg.AcronisBearerAuth = v
		cfg.markEnvOverride("AcronisBearerAuth")
		cfg.AuthSource = "env:ACRONIS_BEARER_AUTH"
	}
	...
	if cfg.AuthSource == "" && cfg.AcronisBearerAuth != "" {
		cfg.AuthSource = "config"
	}
	...
	if v := os.Getenv("ACRONIS_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	...
	if v := strings.TrimSpace(os.Getenv("ACRONIS_DATACENTER")); v != "" {
		cfg.TemplateVars["datacenter"] = normalizeEndpointTemplateValue(v)
	} else {
		cfg.TemplateVars["datacenter"] = "eu2-cloud"
	}
	return cfg, nil
}
...
func (c *Config) AuthHeader() string {
	...
	if c.AcronisBearerAuth != "" {
		if c.AuthSource == "" {
			c.AuthSource = "env:ACRONIS_BEARER_AUTH"
		}
		return "Bearer " + c.AcronisBearerAuth
	}
	if c.AccessToken != "" {
...
func (c *Config) ClearTokens() error {
	...
	c.updateFileConfigField("ClientSecret")
	c.AcronisBearerAuth = ""
	delete(c.envOverrides, "AcronisBearerAuth")
	return c.save()
}
...
func (c *Config) configForSave() Config {
	out := *c
	if c.fileConfig != nil {
		if c.envOverrides["AcronisBearerAuth"] {
			out.AcronisBearerAuth = c.fileConfig.AcronisBearerAuth
		}
	}
...
func (c *Config) updateFileConfigField(field string) {
	...
	case "AcronisBearerAuth":
		c.fileConfig.AcronisBearerAuth = c.AcronisBearerAuth
	}
}
