...
func RegisterIntents(s *server.MCPServer) {
	s.AddTool(
		mcplib.NewTool("month_end_billing_reconciliation",
			mcplib.WithDescription("Flag usage with no matching SKU and SKUs with zero usage before invoicing."),
			mcplib.WithString("tenant", mcplib.Required(), mcplib.Description("Override the recipe's --tenant value.")),
		),
		handleMonthEndBillingReconciliation,
	)
}

// handleMonthEndBillingReconciliation runs the month_end_billing_reconciliation recipe intent: Flag usage with no matching SKU and SKUs with zero usage before invoicing.
func handleMonthEndBillingReconciliation(ctx context.Context, req mcplib.CallToolRequest) (*mcplib.CallToolResult, error) {
	...
	args, missingTenant = appendRecipeStringFlag(args, "tenant", input["tenant"], "", true, false)
	if missingTenant {
		return mcplib.NewToolResultError("--tenant is required"), nil
	}
