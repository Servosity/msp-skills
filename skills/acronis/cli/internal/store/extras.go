...
func (s *Store) migrateExtras(ctx context.Context, conn *sql.Conn) error {
	migrations := []string{
		// usage_snapshots: point-in-time copies of the usages table so
		// `usages drift` can diff two dates (the live usages table is
		// overwritten on every sync, so drift needs its own history).
		`CREATE TABLE IF NOT EXISTS "usage_snapshots" (
			...
			PRIMARY KEY ("snapshot_date", "tenant_id", "offering")
		)`,
