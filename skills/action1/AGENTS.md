# Action1 Printed CLI Agent Guide

This directory is a generated `action1-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
action1-cli doctor --json
action1-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
action1-cli which "<capability>" --json
action1-cli <command> --help
```
...
```bash
action1-cli <command> --help
action1-cli <command> --dry-run --agent
```
