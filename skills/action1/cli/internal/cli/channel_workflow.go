...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  action1-cli workflow archive

  # Full re-archive (ignore previous sync state)
  action1-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"audit", "audit-export", "automations", "data-sources", "me", "organizations", "permissions", "reports", "roles", "scripts", "setting-templates", "settings", "subscription", "users"}
			totalSynced := 0
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  action1-cli workflow status

  # Show status as JSON
  action1-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("action1-cli")
			}
