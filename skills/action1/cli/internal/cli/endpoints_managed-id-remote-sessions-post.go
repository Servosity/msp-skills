...
func newEndpointsManagedIdRemoteSessionsPostCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if !stdinBody {
				if !cmd.Flags().Changed("connection-type") && !flags.dryRun {
					return fmt.Errorf("required flag \"%s\" not set", "connection-type")
				}
			}
			...
			if stdinBody {
				...
				body = map[string]any{}
				if bodyConnectionType != "" {
					body["connection_type"] = bodyConnectionType
				}
				if bodyCurrentIp != "" {
					body["current_ip"] = bodyCurrentIp
				}
			}
