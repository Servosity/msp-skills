...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".local", "share", "action1-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("ACTION1_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("ACTION1_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "action1-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.local/share/action1-cli/feedback.jsonl.
When ` + "`ACTION1_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`ACTION1_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  action1-cli feedback list
  action1-cli feedback list --limit 5
  action1-cli feedback list --json`,
