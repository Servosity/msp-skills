...
func newOrganizationsGetCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			if cmd.Flags().Changed("admin") {
				allowedAdmin := []string{"Yes", "No"}
				validAdmin := false
				for _, v := range allowedAdmin {
					if flagAdmin == v {
						validAdmin = true
						break
					}
				...
				if !validAdmin {
					return fmt.Errorf("invalid value %q for --%s: must be one of %v", flagAdmin, "admin", allowedAdmin)
				}
			}
	...
	cmd.Flags().StringVar(&flagAdmin, "admin", "", "Specify 'yes' to get all organizations where the currently authenticated user is at least an organization admin. (one of: Yes, No)")
