...
In local mode: searches locally synced data only.`,
  ...
  action1-cli search "error timeout"

  # Force local search only
  action1-cli search "payment failed" --data-source local

  # Search a specific resource type locally
  action1-cli search "critical" --type transactions --data-source local

  # JSON output for piping
  action1-cli search "critical" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			query := args[0]
			// This API has a search endpoint: GET /search/{orgId}
			if flags.dataSource != "local" {
				c, err := flags.newClient()
				if err != nil {
					return err
				}
				data, getErr := c.Get(cmd.Context(), "/search/{orgId}", map[string]string{
					"query": query,
				})
				if getErr == nil {
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'action1-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
			...
			case "enterprise":
				results, err = db.SearchEnterprise(query, limit)
			case "organizations":
				results, err = db.SearchOrganizations(query, limit)
			case "permissions":
				results, err = db.SearchPermissions(query, limit)
			case "remote_search":
				results, err = db.SearchRemoteSearch(query, limit)
			case "reports":
				results, err = db.SearchReports(query, limit)
			case "roles":
				results, err = db.SearchRoles(query, limit)
			case "scripts":
				results, err = db.SearchScripts(query, limit)
			case "settings":
				results, err = db.SearchSettings(query, limit)
			case "software-repository":
				results, err = db.SearchSoftwareRepository(query, limit)
			case "updates":
				results, err = db.SearchUpdates(query, limit)
			case "":
				...
				}
				{
					...
					if searchErr != nil {
						return fmt.Errorf("search data_sources failed: %w", searchErr)
					}
					for _, r := range partial {
						key := string(r)
						if !seen[key] {
				...
				{
					partial, searchErr := db.SearchEnterprise(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search enterprise failed: %w", searchErr)
					}
					for _, r := range partial {
						key := string(r)
						if !seen[key] {
				...
				{
					partial, searchErr := db.SearchPermissions(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search permissions failed: %w", searchErr)
					}
					for _, r := range partial {
						key := string(r)
						if !seen[key] {
				...
				{
					partial, searchErr := db.SearchRemoteSearch(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search remote_search failed: %w", searchErr)
					}
					for _, r := range partial {
						key := string(r)
						if !seen[key] {
				...
				{
					partial, searchErr := db.SearchScripts(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search scripts failed: %w", searchErr)
					}
					for _, r := range partial {
						key := string(r)
						if !seen[key] {
				...
				{
					partial, searchErr := db.SearchSettings(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search settings failed: %w", searchErr)
					}
					for _, r := range partial {
						key := string(r)
						if !seen[key] {
				...
				{
					partial, searchErr := db.SearchSoftwareRepository(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search software_repository failed: %w", searchErr)
					}
					for _, r := range partial {
						key := string(r)
						if !seen[key] {
				...
				{
					partial, searchErr := db.SearchUpdates(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search updates failed: %w", searchErr)
					}
					for _, r := range partial {
						key := string(r)
						if !seen[key] {
