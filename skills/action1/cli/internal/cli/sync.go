...
Resource scoping:
  ...
  action1-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  action1-cli sync --full

  # Incremental sync: only records from the last 7 days
  action1-cli sync --since 7d

  # Parallel sync with 8 workers
  action1-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  action1-cli sync --latest-only`,
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/action1-cli/data.db)")
...
func syncResourceFieldSelector(resource string) (string, string) {
	switch resource {
	case "audit":
		return "filter", "id,description,event,object_name,object_url,org_id,org_name,self,status_message,timestamp,type,user_id,user_name"
	case "permissions":
		return "filter", "id,description,name"
	case "roles":
		return "filter", "id,description,name,self,type,users"
	case "settings":
		return "filter", "id,description,name,self,template_id,type,value,value_type"
	case "users":
		return "filter", "id,email_verified,impersonating,roles,self,system_role,type"
	case "roles_users":
		return "filter", "id,email_verified,impersonating,roles,self,system_role,type"
	}
	return "", ""
...
func upsertSingleObject(db *store.Store, resource string, data json.RawMessage) error {
	...
	case "cve-descriptions":
		return db.UpsertCveDescriptions(data)
	case "data-sources":
	...
	case "installed_software_data":
		return db.UpsertInstalledSoftwareData(data)
	case "installed_software_errors":
		return db.UpsertInstalledSoftwareErrors(data)
	case "installed_software_requery":
		return db.UpsertInstalledSoftwareRequery(data)
	case "logs":
	...
	case "remote_search":
		return db.UpsertRemoteSearch(data)
	case "reportdata_data":
		return db.UpsertReportdataData(data)
	case "reportdata_errors":
		return db.UpsertReportdataErrors(data)
	case "export":
		return db.UpsertExport(data)
	case "reportdata_requery":
		return db.UpsertReportdataRequery(data)
	case "reports":
		return db.UpsertReports(data)
	case "roles":
		return db.UpsertRoles(data)
	case "roles_clone":
		return db.UpsertRolesClone(data)
	case "roles_users":
		return db.UpsertRolesUsers(data)
	case "scripts":
		return db.UpsertScripts(data)
	case "settings":
		return db.UpsertSettings(data)
	case "software-repository":
		return db.UpsertSoftwareRepository(data)
	case "software_repository_clone":
		return db.UpsertSoftwareRepositoryClone(data)
	case "match_conflicts":
		return db.UpsertMatchConflicts(data)
	case "software_repository_versions":
		return db.UpsertSoftwareRepositoryVersions(data)
	case "subscription":
	...
	case "updates_versions":
		return db.UpsertUpdatesVersions(data)
	case "users":
		return db.UpsertUsers(data)
	case "users_roles":
		return db.UpsertUsersRoles(data)
	case "vulnerabilities":
		return db.UpsertVulnerabilities(data)
	case "vulnerabilities_endpoints":
		return db.UpsertVulnerabilitiesEndpoints(data)
	case "remediations":
...
func syncResourcePath(resource string) (string, error) {
	paths := map[string]string{
		...
		"automations":       "/automations/action-templates",
		"data-sources":      "/data-sources/all",
		"me":                "/me/report-subscriptions",
		"organizations":     "/organizations",
		"permissions":       "/permissions",
		"reports":           "/reports/all",
		"roles":             "/roles",
		"scripts":           "/scripts/all",
		"setting-templates": "/setting-templates/all",
		"settings":          "/settings/all",
		"subscription":      "/subscription/usage/organizations",
		"users":             "/users",
	}
...
func dependentResourceDefs() []dependentResourceDef {
	return []dependentResourceDef{
		{Name: "roles_users", ParentTable: "roles", ParentIDParam: "roleId", PathTemplate: "/roles/{roleId}/users", KeyField: "", PathParams: []dependentPathParamDef{
			{Param: "roleId", Field: "id"},
		}},
		{Name: "users_roles", ParentTable: "users", ParentIDParam: "userId", PathTemplate: "/users/{userId}/roles", KeyField: "", PathParams: []dependentPathParamDef{
