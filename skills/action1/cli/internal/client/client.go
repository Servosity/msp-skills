...
func New(cfg *config.Config, timeout time.Duration, rateLimit float64) *Client {
	...
	cacheDir := filepath.Join(homeDir, ".cache", "action1-cli", "http")
...
func (c *Client) doInternal(ctx context.Context, method, path string, params map[string]string, body any, headerOverrides map[string]string, readOnlyIntent bool) (json.RawMessage, int, error) {
	...
	for attempt := 0; attempt <= maxRetries; attempt++ {
		...
		if req.Header.Get("User-Agent") == "" {
			req.Header.Set("User-Agent", "action1-cli/3.1.0")
		}
...
func (c *Client) authHeader(ctx context.Context) (string, error) {
	...
	if authHeader != "" {
		...
		// Transparently re-mint an expired client-credentials access token.
		if c.Config.AccessTokenExpired() && c.Config.ClientID != "" && c.Config.ClientSecret != "" {
			if h, err := c.mintClientCredentials(ctx); err == nil && h != "" {
				return h, nil
			}
			// Mint failure: fall through to the (expired) header; the request
			// surfaces a 401 with an actionable error rather than swallowing it.
		}
		return authHeader, nil
	}
	// No ready bearer: mint one from client credentials when available.
	if c.Config.ClientID != "" && c.Config.ClientSecret != "" {
		if h, err := c.mintClientCredentials(ctx); err != nil {
			return "", err
		} else if h != "" {
			return h, nil
		}
...
func authPlaceholderCredentialError(cfg *config.Config) error {
	return authPlaceholderCredentialErrorWithSetup(cfg, "export ACTION1_OAUTH2=<your-token> or action1-cli auth set-token <token>")
}
...
func (c *Client) maskCredentialText(text string, extraCredentials ...string) string {
	...
	if c != nil && c.Config != nil {
		...
		addCredential(c.Config.Action1Oauth2)
	}
