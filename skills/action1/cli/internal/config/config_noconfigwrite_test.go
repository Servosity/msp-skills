...
// touching disk. ACTION1_NO_CONFIG_WRITE makes the automatic token cache a
...
const (
	ncwToken   = "minted-token-ACTION1"
	ncwRefresh = "minted-refresh-ACTION1"
	ncwSecret  = "minted-secret-ACTION1"
)
...
func ncwMint(t *testing.T, value string, set bool) map[string]string {
	...
	home := t.TempDir()
	t.Setenv("HOME", home)
	t.Setenv("ACTION1_CONFIG", filepath.Join(home, "config.toml"))
	if set {
...
func TestNoConfigWriteStillAllowsTheWipe(t *testing.T) {
	...
	t.Setenv("HOME", home)
	t.Setenv("ACTION1_CONFIG", cfgPath)
...
func TestNoConfigWriteTruthiness(t *testing.T) {
	...
	if NoConfigWriteEnv != "ACTION1_NO_CONFIG_WRITE" {
		t.Errorf("switch is named %q; the documented name is ACTION1_NO_CONFIG_WRITE", NoConfigWriteEnv)
	}
}
