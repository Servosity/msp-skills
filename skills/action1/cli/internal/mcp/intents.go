...
func RegisterIntents(s *server.MCPServer) {
	s.AddTool(
		mcplib.NewTool("known_exploited_cve_triage",
			mcplib.WithDescription("CISA KEV CVEs across all orgs, ranked by affected endpoints and CVSS."),
			mcplib.WithBoolean("kev_only", mcplib.Description("Override the recipe's --kev-only value.")),
		),
		handleKnownExploitedCveTriage,
	)
}

// handleKnownExploitedCveTriage runs the known_exploited_cve_triage recipe intent: CISA KEV CVEs across all orgs, ranked by affected endpoints and CVSS.
func handleKnownExploitedCveTriage(ctx context.Context, req mcplib.CallToolRequest) (*mcplib.CallToolResult, error) {
	...
	args = appendRecipeBoolFlag(args, "kev-only", input["kev_only"], true)
