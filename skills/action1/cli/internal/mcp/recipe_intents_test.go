...
func TestRecipeIntentHandlerBuildsRecipeArgs(t *testing.T) {
	...
	result, err := handleKnownExploitedCveTriage(context.Background(), req)
	if err != nil {
...
func TestRecipeIntentHandlerOverridesAndRejectsArgs(t *testing.T) {
	...
	got := recipeToolText(t, result)
	if strings.Contains(got, "--kev-only") {
		t.Fatalf("boolean false override should suppress --kev-only: %q", got)
	}
}
