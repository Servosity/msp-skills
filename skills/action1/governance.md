...
The skill drives the `action1-cli` binary (and `action1-mcp`),
authenticating with `ACTION1_CLIENT_ID`, `ACTION1_CLIENT_SECRET`, `ACTION1_REGION`. Credentials are read from the environment only -
never written to disk, never logged, never sent anywhere except the Action1 API.
...
| **Write (routine)** | Day-to-day mutations. | `automations policies-schedules-org-id-id-patch`, `data-sources org-id-id-patch`, `endpoints discovery-org-id-patch`, `endpoints groups-group-id-patch`, `endpoints managed-id-missing-updates`, `endpoints managed-id-patch`, `endpoints managed-id-remote-sessions-session-id-patch`, `enterprise patch`, ... (36 total) | Preview with `--dry-run`, then an approved write (where a command documents its own confirm gate, use it too) |
| **Credential / security** | Touches tokens, keys, MFA. | (none detected) | Human-in-the-loop only |
| **Destructive** | Irreversible data or config loss. | `automations policies-schedules-org-id-id-actions-action-id-delete`, `automations policies-schedules-org-id-id-delete`, `data-sources org-id-id-delete`, `endpoints connectors-org-id-deployer-id-delete`, `endpoints groups-group-id-delete`, `endpoints managed-id-delete`, `me subscriptions-subscription-id-delete`, `organizations org-id-delete`, ... (18 total) | Human-in-the-loop only, explicit confirmation |
