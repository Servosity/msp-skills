...
| --- | --- | --- | --- |
| `ACTION1_CLIENT_ID` | per_call | Yes (recommended pair) | Client ID from the Action1 console's API Credentials page. With `ACTION1_CLIENT_SECRET`, the CLI mints a bearer token automatically. |
| `ACTION1_CLIENT_SECRET` | per_call | Yes (recommended pair) | Client Secret paired with `ACTION1_CLIENT_ID`. |
| `ACTION1_OAUTH2` | per_call | Alternative | Pre-minted bearer token; use instead of the client-credentials pair. |
| `ACTION1_REGION` | optional | No | Data-center shortcut: `us` (default), `eu`, or `au`. |
| `ACTION1_BASE_URL` | optional | No | Explicit API base URL override; wins over `ACTION1_REGION`. |
| `ACTION1_ORG_ID` | optional | No | Default organization filter for `fleet` commands (`--org` overrides). |
