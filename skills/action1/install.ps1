# install.ps1 - install action1-cli and action1-mcp on Windows.
#
# Pulls prebuilt binaries from this skill's latest GitHub Release (action1-v*).
...
$Skill   = "action1"
$CliBin  = "action1-cli.exe"
$McpBin  = "action1-mcp.exe"
...
$ReleaseBase = if ($env:MSP_SKILLS_RELEASE_BASE) {
  ...
  # Each skill is versioned/tagged independently (action1-vX.Y.Z),
...
Write-Host "  action1-cli --version"
Write-Host ""
Write-Host "Next:"
Write-Host "  First command + auth: https://github.com/$Owner/$Repo/tree/main/skills/action1#readme"
Write-Host "  Claude Desktop / ChatGPT wire-up: https://github.com/$Owner/$Repo/blob/main/skills/action1/mcp-install.md"
