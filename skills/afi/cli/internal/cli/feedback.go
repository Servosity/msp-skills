...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".local", "share", "afi-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("AFI_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("AFI_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "afi-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.local/share/afi-cli/feedback.jsonl.
When ` + "`AFI_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`AFI_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  afi-cli feedback list
  afi-cli feedback list --limit 5
  afi-cli feedback list --json`,
