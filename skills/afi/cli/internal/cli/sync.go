...
Resource scoping:
  ...
  afi-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  afi-cli sync --full

  # Incremental sync: only records from the last 7 days
  afi-cli sync --since 7d

  # Parallel sync with 8 workers
  afi-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  afi-cli sync --latest-only`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if len(resources) == 0 {
				if humanFriendly {
					fmt.Fprintln(os.Stderr, "afi-cli sync: no default sync resources in spec; use --resources for explicit sync targets or populate the store via single-fetch commands.")
				} else {
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/afi-cli/data.db)")
...
func upsertSingleObject(db *store.Store, resource string, data json.RawMessage) error {
	...
	case "orgs_orgs":
		return db.UpsertOrgsOrgs(data)
	case "orgs_tenants":
		return db.UpsertOrgsTenants(data)
	case "archives":
	...
	case "protections":
		return db.UpsertProtections(data)
	case "quotas":
		return db.UpsertQuotas(data)
	case "resources":
...
func syncResourcePath(resource string) (string, error) {
	paths := map[string]string{
		"applications": "/api/v1/applications/me/installations",
	}
