...
type Config struct {
	...
	AfiApiKey     string            `toml:"api_key"`
}

func Load(configPath string) (*Config, error) {
	cfg := &Config{
		BaseURL: "https://papi.afi.ai",
	}
	...
	if path == "" {
		...
		path = filepath.Join(home, ".config", "afi-cli", "config.toml")
	}
	cfg.Path = path
	...
	// Env var overrides
	if v := os.Getenv("AFI_API_KEY"); v != "" {
		cfg.AfiApiKey = v
		cfg.markEnvOverride("AfiApiKey")
		cfg.AuthSource = "env:AFI_API_KEY"
	}
	...
	if cfg.AuthSource == "" && cfg.AfiApiKey != "" {
		cfg.AuthSource = "config"
	}
...
func (c *Config) SaveCredential(token string) error {
	...
	c.updateFileConfigField("AccessToken")
	c.AfiApiKey = token
	delete(c.envOverrides, "AfiApiKey")
	c.updateFileConfigField("AfiApiKey")
	return c.save()
}
...
func (c *Config) configForSave() Config {
	out := *c
	if c.fileConfig != nil {
		if c.envOverrides["AfiApiKey"] {
			out.AfiApiKey = c.fileConfig.AfiApiKey
		}
	}
...
func (c *Config) updateFileConfigField(field string) {
	...
	case "AfiApiKey":
		c.fileConfig.AfiApiKey = c.AfiApiKey
	}
}
