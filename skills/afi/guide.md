...
```bash
afi-cli sync --since 7d && afi-cli fleet-health --since 24h --agent
```

Refresh the local store then roll up failures and quota breaches across every tenant in one agent-readable table.
...
Config file: `~/.config/afi-cli/config.toml`
