# install.ps1 - install afi-cli and afi-mcp on Windows.
#
# Pulls prebuilt binaries from this skill's latest GitHub Release (afi-v*).
...
$ReleaseBase = if ($env:MSP_SKILLS_RELEASE_BASE) {
  ...
  # Each skill is versioned/tagged independently (afi-vX.Y.Z),
...
Write-Host "  afi-cli --version"
Write-Host ""
Write-Host "Next:"
Write-Host "  First command + auth: https://github.com/$Owner/$Repo/tree/main/skills/afi#readme"
Write-Host "  Claude Desktop / ChatGPT wire-up: https://github.com/$Owner/$Repo/blob/main/skills/afi/mcp-install.md"
