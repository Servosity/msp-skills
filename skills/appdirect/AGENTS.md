# AppDirect Printed CLI Agent Guide

This directory is a generated `appdirect-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
appdirect-cli doctor --json
appdirect-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
appdirect-cli which "<capability>" --json
appdirect-cli <command> --help
```
...
```bash
appdirect-cli <command> --help
appdirect-cli <command> --dry-run --agent
```
