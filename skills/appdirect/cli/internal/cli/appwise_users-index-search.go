...
func newAppwiseUsersIndexSearchCmd(flags *rootFlags) *cobra.Command {
	var flagQ string
	var flagNumber float64
	...
	cmd.Flags().StringVar(&flagQ, "q", "", "Search query. The term(s) you are searching for. Format: URL-encoded string.")
	cmd.Flags().Float64Var(&flagNumber, "number", 1.000000, "Which page of results to return.")
	cmd.Flags().Float64Var(&flagSize, "size", 50.000000, "Page size: maximum number of search results per page. Max value: 250")
	cmd.Flags().StringVar(&flagSort, "sort", "", "A URL-encoded value that defines how to sort the results.")
