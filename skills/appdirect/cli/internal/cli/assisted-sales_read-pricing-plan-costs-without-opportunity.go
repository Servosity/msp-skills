...
func newAssistedSalesReadPricingPlanCostsWithoutOpportunityCmd(flags *rootFlags) *cobra.Command {
	var flagPricingPlanId string
	...
	cmd.Flags().StringVar(&flagPricingPlanId, "pricing-plan-id", "", "Pricing plan ID")
	cmd.Flags().StringVar(&flagOwnerId, "owner-id", "", "Sales agent's user ID")
	cmd.Flags().StringVar(&flagOwnerCompanyId, "owner-company-id", "", "Sales agent's company ID")
	cmd.Flags().StringVar(&flagCurrency, "currency", "", "Opportunity currency, using the ISO-4217 currency code")
