...
func newAuthCmd(flags *rootFlags) *cobra.Command {
	cmd := &cobra.Command{
		Use:   "auth",
		Short: "Manage authentication for Appdirect",
...
Credentials default to APPDIRECT_CLIENT_ID (Client ID) and APPDIRECT_CLIENT_SECRET (Client Secret) environment variables.
`),
		Example: strings.Trim(`
  # Use env vars
  appdirect-cli auth login

  # Explicit credentials
  appdirect-cli auth login --client-id <id> --client-secret <secret>
`, "\n"),
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if clientID == "" {
				clientID = strings.TrimSpace(os.Getenv("APPDIRECT_CLIENT_ID"))
			}
			if clientSecret == "" {
				clientSecret = strings.TrimSpace(os.Getenv("APPDIRECT_CLIENT_SECRET"))
			}
			if clientID == "" || clientSecret == "" {
				return authErr(fmt.Errorf("client ID and secret required (set --client-id/--client-secret or APPDIRECT_CLIENT_ID/APPDIRECT_CLIENT_SECRET)"))
			}
	...
	cmd.Flags().StringVar(&clientID, "client-id", "", "OAuth2 Client ID; defaults to $APPDIRECT_CLIENT_ID")
	cmd.Flags().StringVar(&clientSecret, "client-secret", "", "OAuth2 Client Secret; defaults to $APPDIRECT_CLIENT_SECRET")
...
func resolveClientCredentialsScope() string {
	if scope := os.Getenv("APPDIRECT_OAUTH_SCOPE"); scope != "" {
		return scope
	}
	return "ROLE_PARTNER ROLE_PARTNER_READ"
}
...
func newAuthStatusCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  appdirect-cli auth status",
		RunE: func(cmd *cobra.Command, args []string) error {
			cfg, err := config.Load(flags.configPath)
			if err != nil {
				return configErr(err)
			...
			if !authed {
				...
				fmt.Fprintln(w, "  export APPDIRECT_CLIENT_ID=\"<client-id>\"")
				fmt.Fprintln(w, "  export APPDIRECT_CLIENT_SECRET=\"<client-secret>\"")
				fmt.Fprintf(w, "  appdirect-cli auth login\n")
				return authErr(fmt.Errorf("no credentials configured"))
			}
...
func newAuthLogoutCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  appdirect-cli auth logout",
		RunE: func(cmd *cobra.Command, args []string) error {
			cfg, err := config.Load(flags.configPath)
			if err != nil {
				return configErr(err)
			...
			envStillSet := ""
			if envStillSet == "" && os.Getenv("APPDIRECT_CLIENT_ID") != "" {
				envStillSet = "APPDIRECT_CLIENT_ID"
			}
			if envStillSet == "" && os.Getenv("APPDIRECT_CLIENT_SECRET") != "" {
				envStillSet = "APPDIRECT_CLIENT_SECRET"
			}
