...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  appdirect-cli doctor
  appdirect-cli doctor --json
  appdirect-cli doctor --fail-on warn`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if cfg != nil {
				...
				if header == "" {
					...
					report["auth_hint"] = "export APPDIRECT_CLIENT_ID=<your-key>"
				} else {
					authConfigured = true
			...
			if os.Getenv("APPDIRECT_CLIENT_ID") != "" {
				authEnvSet = append(authEnvSet, "APPDIRECT_CLIENT_ID")
			} else {
				authEnvInfo = append(authEnvInfo, "APPDIRECT_CLIENT_ID set during auth login")
			}
			if os.Getenv("APPDIRECT_CLIENT_SECRET") != "" {
				authEnvSet = append(authEnvSet, "APPDIRECT_CLIENT_SECRET")
			} else {
				authEnvInfo = append(authEnvInfo, "APPDIRECT_CLIENT_SECRET set during auth login")
			}
			switch {
...
func collectCacheReport(ctx context.Context, staleAfterSpec string) map[string]any {
	...
	if err != nil {
		if os.IsNotExist(err) {
			...
			report["hint"] = "Database not created yet; run 'appdirect-cli sync' to hydrate."
			return report
		}
		report["status"] = "error"
	...
	if qerr != nil {
		...
		report["status"] = "unknown"
		report["hint"] = "No sync state recorded; run 'appdirect-cli sync' to populate."
		return report
	}
	defer rows.Close()
	...
	case !haveAny && len(resources) == 0:
		report["status"] = "unknown"
		report["hint"] = "sync_state is empty; run 'appdirect-cli sync' to hydrate."
	case fresh:
		report["status"] = "fresh"
	default:
		report["status"] = "stale"
		report["oldest_age"] = oldest.Round(time.Minute).String()
		report["hint"] = "Some resources are older than stale_after; run 'appdirect-cli sync' to refresh."
	}
	return report
}
