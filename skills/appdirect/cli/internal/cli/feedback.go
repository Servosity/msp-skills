...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".local", "share", "appdirect-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("APPDIRECT_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("APPDIRECT_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "appdirect-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.local/share/appdirect-cli/feedback.jsonl.
When ` + "`APPDIRECT_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`APPDIRECT_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  appdirect-cli feedback list
  appdirect-cli feedback list --limit 5
  appdirect-cli feedback list --json`,
