...
In local mode: searches locally synced data only.`,
  ...
  appdirect-cli search "error timeout"

  # Force local search only
  appdirect-cli search "payment failed" --data-source local

  # Search a specific resource type locally
  appdirect-cli search "critical" --type transactions --data-source local

  # JSON output for piping
  appdirect-cli search "critical" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			query := args[0]
			// This API has a search endpoint: GET /appwise/v2/search
			if flags.dataSource != "local" {
				c, err := flags.newClient()
				if err != nil {
					return err
				}
				data, getErr := c.Get(cmd.Context(), "/appwise/v2/search", map[string]string{
					"q": query,
				})
				if getErr == nil {
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'appdirect-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
			...
			case "channel":
				results, err = db.SearchChannel(query, limit)
			case "":
				...
				{
					partial, searchErr := db.SearchChannel(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search channel failed: %w", searchErr)
					}
					for _, r := range partial {
						key := string(r)
						if !seen[key] {
