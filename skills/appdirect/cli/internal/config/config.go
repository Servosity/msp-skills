...
func Load(configPath string) (*Config, error) {
	...
	path := configPath
	if path == "" {
		path = os.Getenv("APPDIRECT_CONFIG")
	}
	if path == "" {
		home, _ := os.UserHomeDir()
		path = filepath.Join(home, ".config", "appdirect-cli", "config.toml")
	}
	cfg.Path = path
	...
	// Env var overrides
	if v := os.Getenv("APPDIRECT_CLIENT_ID"); v != "" {
		cfg.ClientID = v
		cfg.markEnvOverride("ClientID")
		cfg.AuthSource = "env:APPDIRECT_CLIENT_ID"
	}
	if v := os.Getenv("APPDIRECT_CLIENT_SECRET"); v != "" {
		cfg.ClientSecret = v
		cfg.markEnvOverride("ClientSecret")
		cfg.AuthSource = "env:APPDIRECT_CLIENT_SECRET"
	}
	...
	if v := os.Getenv("APPDIRECT_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	if v := os.Getenv("APPDIRECT_TOKEN_URL"); v != "" {
		cfg.TokenURL = v
	}
	return cfg, nil
}
...
const NoConfigWriteEnv = "APPDIRECT_NO_CONFIG_WRITE"
...
// material. It is the ONLY save path APPDIRECT_NO_CONFIG_WRITE suppresses;
