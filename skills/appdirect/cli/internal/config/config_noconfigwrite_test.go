...
// touching disk. APPDIRECT_NO_CONFIG_WRITE makes the automatic token cache a
...
const (
	ncwToken   = "minted-token-APPDIRECT"
	ncwRefresh = "minted-refresh-APPDIRECT"
	ncwSecret  = "minted-secret-APPDIRECT"
)
...
func ncwMint(t *testing.T, value string, set bool) map[string]string {
	...
	home := t.TempDir()
	t.Setenv("HOME", home)
	t.Setenv("APPDIRECT_CONFIG", filepath.Join(home, "config.toml"))
	if set {
...
func TestNoConfigWriteStillAllowsTheWipe(t *testing.T) {
	...
	t.Setenv("HOME", home)
	t.Setenv("APPDIRECT_CONFIG", cfgPath)
...
func TestNoConfigWriteTruthiness(t *testing.T) {
	...
	if NoConfigWriteEnv != "APPDIRECT_NO_CONFIG_WRITE" {
		t.Errorf("switch is named %q; the documented name is APPDIRECT_NO_CONFIG_WRITE", NoConfigWriteEnv)
	}
}
