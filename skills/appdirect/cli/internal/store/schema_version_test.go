...
func TestMigrate_AddsColumnsOnUpgrade_Account(t *testing.T) {
	...
	for _, want := range []string{
		...
		"daily_purchase_limit_exempt",
		"daily_purchase_limit_override_amt",
		"daily_purchase_remaining_amt",
		"default_role",
		"email_address",
		"enabled",
		"external_id",
		...
		"reseller",
		"status",
		"thirty_days_purchase_limit",
		"thirty_days_purchase_limit_exempt",
		"thirty_days_purchase_limit_override_amt",
		...
		"source",
		"have_assigned_users",
		"have_completed_company_profile",
		"have_completed_profile",
		"have_invited_users",
		"have_purchased_product",
		"company_default",
		"company_uuid",
		"description",
		"externally_managed",
...
// TestMigrate_AddsColumnsOnUpgrade_AppMarket verifies that opening a
// database created by an older binary succeeds and adds newly generated
// columns before CREATE INDEX runs against the pre-existing table. Regression
// coverage for parent_id upgrades and indexed generated columns.
func TestMigrate_AddsColumnsOnUpgrade_AppMarket(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	...
	if _, err := raw.Exec(`CREATE TABLE "app_market" (
		id TEXT PRIMARY KEY,
		data JSON NOT NULL,
		synced_at DATETIME DEFAULT CURRENT_TIMESTAMP
	)`); err != nil {
	...
	// The migration must have added every generated column.
	rows, err := s.DB().Query(`PRAGMA table_info("app_market")`)
	if err != nil {
		t.Fatalf("table_info: %v", err)
	}
	defer rows.Close()
	...
	for _, want := range []string{
		...
		if !hasColumn[want] {
			t.Fatalf("%s column missing from app_market after migrate", want)
		}
	}
}

// TestMigrate_AddsColumnsOnUpgrade_AppReseller verifies that opening a
// database created by an older binary succeeds and adds newly generated
// columns before CREATE INDEX runs against the pre-existing table. Regression
// coverage for parent_id upgrades and indexed generated columns.
func TestMigrate_AddsColumnsOnUpgrade_AppReseller(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	...
	if _, err := raw.Exec(`CREATE TABLE "app_reseller" (
		id TEXT PRIMARY KEY,
		data JSON NOT NULL,
		synced_at DATETIME DEFAULT CURRENT_TIMESTAMP
	)`); err != nil {
	...
	// The migration must have added every generated column.
	rows, err := s.DB().Query(`PRAGMA table_info("app_reseller")`)
	if err != nil {
		t.Fatalf("table_info: %v", err)
	}
	defer rows.Close()
	...
	for _, want := range []string{
		...
		if !hasColumn[want] {
			t.Fatalf("%s column missing from app_reseller after migrate", want)
		}
	}
}

// TestMigrate_AddsColumnsOnUpgrade_AssistedSales verifies that opening a
// database created by an older binary succeeds and adds newly generated
// columns before CREATE INDEX runs against the pre-existing table. Regression
// coverage for parent_id upgrades and indexed generated columns.
func TestMigrate_AddsColumnsOnUpgrade_AssistedSales(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	...
	if _, err := raw.Exec(`CREATE TABLE "assisted_sales" (
		id TEXT PRIMARY KEY,
		data JSON NOT NULL,
		synced_at DATETIME DEFAULT CURRENT_TIMESTAMP
	)`); err != nil {
	...
	// The migration must have added every generated column.
	rows, err := s.DB().Query(`PRAGMA table_info("assisted_sales")`)
	if err != nil {
		t.Fatalf("table_info: %v", err)
	}
	defer rows.Close()
	...
	for _, want := range []string{
		...
		"action_restrictions",
		"purchase_custom_attributes",
		"purchase_id",
		"purchase_number",
		"amount_due_after_tax",
		"amount_due_before_tax",
		"pricing_plan_id",
	} {
		if !hasColumn[want] {
			t.Fatalf("%s column missing from assisted_sales after migrate", want)
		}
	}
}
...
// TestMigrate_AddsColumnsOnUpgrade_ProductUploaderApi verifies that opening a
// database created by an older binary succeeds and adds newly generated
// columns before CREATE INDEX runs against the pre-existing table. Regression
// coverage for parent_id upgrades and indexed generated columns.
func TestMigrate_AddsColumnsOnUpgrade_ProductUploaderApi(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	...
	if _, err := raw.Exec(`CREATE TABLE "product_uploader_api" (
		id TEXT PRIMARY KEY,
		data JSON NOT NULL,
		synced_at DATETIME DEFAULT CURRENT_TIMESTAMP
	)`); err != nil {
	...
	// The migration must have added every generated column.
	rows, err := s.DB().Query(`PRAGMA table_info("product_uploader_api")`)
	if err != nil {
		t.Fatalf("table_info: %v", err)
	}
	defer rows.Close()
	...
	for _, want := range []string{
		...
		if !hasColumn[want] {
			t.Fatalf("%s column missing from product_uploader_api after migrate", want)
		}
	}
}
