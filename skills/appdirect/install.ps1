# install.ps1 - install appdirect-cli and appdirect-mcp on Windows.
#
# Pulls prebuilt binaries from this skill's latest GitHub Release (appdirect-v*).
...
$Skill   = "appdirect"
$CliBin  = "appdirect-cli.exe"
$McpBin  = "appdirect-mcp.exe"
...
$ReleaseBase = if ($env:MSP_SKILLS_RELEASE_BASE) {
  ...
  # Each skill is versioned/tagged independently (appdirect-vX.Y.Z),
...
Write-Host "  appdirect-cli --version"
Write-Host ""
Write-Host "Next:"
Write-Host "  First command + auth: https://github.com/$Owner/$Repo/tree/main/skills/appdirect#readme"
Write-Host "  Claude Desktop / ChatGPT wire-up: https://github.com/$Owner/$Repo/blob/main/skills/appdirect/mcp-install.md"
