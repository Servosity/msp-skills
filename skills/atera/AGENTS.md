# Atera Printed CLI Agent Guide

This directory is a generated `atera-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
atera-cli doctor --json
atera-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
atera-cli which "<capability>" --json
atera-cli <command> --help
```
...
```bash
atera-cli <command> --help
atera-cli <command> --dry-run --agent
```
