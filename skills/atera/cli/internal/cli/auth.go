...
func newAuthCmd(flags *rootFlags) *cobra.Command {
	cmd := &cobra.Command{
		Use:   "auth",
		Short: "Manage authentication for Atera",
...
func newAuthSetupCmd(_ *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: "  atera-cli auth setup\n  atera-cli auth setup --launch",
		RunE: func(cmd *cobra.Command, args []string) error {
			w := cmd.OutOrStdout()
			fmt.Fprintln(w, "No setup URL is configured for this CLI; check the API's docs.")
			fmt.Fprintln(w, "")
			fmt.Fprintln(w, "Then set:")
			fmt.Fprintln(w, "  export ATERA_API_KEY=\"<your-token>\"")
			fmt.Fprintln(w, "  export ATERA_ACCOUNT_API=\"<your-token>\"")
			fmt.Fprintln(w, "  atera-cli auth set-token <token>")
			if !launch {
				return nil
			}
	...
	return &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if !authed {
				...
				fmt.Fprintln(w, "  export ATERA_API_KEY=\"your-token-here\"")
				fmt.Fprintln(w, "  export ATERA_ACCOUNT_API=\"your-token-here\"")
				fmt.Fprintf(w, "  atera-cli auth set-token <token>\n")
				return authErr(fmt.Errorf("no credentials configured"))
			}
...
func newAuthSetTokenCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  atera-cli auth set-token YOUR_TOKEN_HERE",
...
func newAuthLogoutCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			envStillSet := ""
			if envStillSet == "" && os.Getenv("ATERA_API_KEY") != "" {
				envStillSet = "ATERA_API_KEY"
			}
			if envStillSet == "" && os.Getenv("ATERA_ACCOUNT_API") != "" {
				envStillSet = "ATERA_ACCOUNT_API"
			}
