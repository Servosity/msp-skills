...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  atera-cli workflow archive

  # Full re-archive (ignore previous sync state)
  atera-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"agents", "alerts", "billing", "contacts", "contracts", "customers", "customvalues", "departments", "devices", "devices-genericdevices", "devices-httpdevices", "devices-snmpdevices", "knowledgebases", "rates", "rates-products", "tickets", "tickets-lastmodified", "tickets-statusmodified"}
			totalSynced := 0
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  atera-cli workflow status

  # Show status as JSON
  atera-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("atera-cli")
			}
