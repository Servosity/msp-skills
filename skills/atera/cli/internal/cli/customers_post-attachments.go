...
func newCustomersPostAttachmentsCmd(flags *rootFlags) *cobra.Command {
	var bodyContentBased64 string
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if !stdinBody {
				if !cmd.Flags().Changed("content-based64") && !flags.dryRun {
					return fmt.Errorf("required flag \"%s\" not set", "content-based64")
				}
			...
			if stdinBody {
				...
				body = map[string]any{}
				if bodyContentBased64 != "" {
					body["ContentBased64"] = bodyContentBased64
				}
				if bodyCustomerId != "" {
	...
	cmd.Flags().StringVar(&bodyContentBased64, "content-based64", "", "Content based64")
