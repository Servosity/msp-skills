...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  atera-cli doctor
  atera-cli doctor --json
  atera-cli doctor --fail-on warn`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if strings.Contains("", " OR ") {
				authEnvOptionalNames = append(authEnvOptionalNames, "ATERA_API_KEY")
				if os.Getenv("ATERA_API_KEY") != "" {
					authEnvSet = append(authEnvSet, "ATERA_API_KEY")
					authEnvOptionalSatisfied = true
				}
			} else if os.Getenv("ATERA_API_KEY") != "" {
				authEnvSet = append(authEnvSet, "ATERA_API_KEY")
			} else {
				authEnvInfo = append(authEnvInfo, "ATERA_API_KEY optional")
			}
			if strings.Contains("", " OR ") {
				authEnvOptionalNames = append(authEnvOptionalNames, "ATERA_ACCOUNT_API")
				if os.Getenv("ATERA_ACCOUNT_API") != "" {
					authEnvSet = append(authEnvSet, "ATERA_ACCOUNT_API")
					authEnvOptionalSatisfied = true
				}
			} else if os.Getenv("ATERA_ACCOUNT_API") != "" {
				authEnvSet = append(authEnvSet, "ATERA_ACCOUNT_API")
			} else {
				authEnvInfo = append(authEnvInfo, "ATERA_ACCOUNT_API optional")
			}
			switch {
			...
			if cfg != nil && cfg.BaseURL != "" {
				...
				if clientErr != nil {
					...
					if authHeader == "" {
						...
						authHeaders["User-Agent"] = "atera-cli"
						verifyPath := "/account"
...
func collectCacheReport(ctx context.Context, staleAfterSpec string) map[string]any {
	...
	if err != nil {
		if os.IsNotExist(err) {
			...
			report["hint"] = "Database not created yet; run 'atera-cli sync' to hydrate."
			return report
		}
		report["status"] = "error"
	...
	if qerr != nil {
		...
		report["status"] = "unknown"
		report["hint"] = "No sync state recorded; run 'atera-cli sync' to populate."
		return report
	}
	defer rows.Close()
	...
	case !haveAny && len(resources) == 0:
		report["status"] = "unknown"
		report["hint"] = "sync_state is empty; run 'atera-cli sync' to hydrate."
	case fresh:
		report["status"] = "fresh"
	default:
		report["status"] = "stale"
		report["oldest_age"] = oldest.Round(time.Minute).String()
		report["hint"] = "Some resources are older than stale_after; run 'atera-cli sync' to refresh."
	}
	return report
}
