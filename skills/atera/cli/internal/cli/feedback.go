...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".local", "share", "atera-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("ATERA_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("ATERA_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "atera-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.local/share/atera-cli/feedback.jsonl.
When ` + "`ATERA_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`ATERA_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  atera-cli feedback list
  atera-cli feedback list --limit 5
  atera-cli feedback list --json`,
