...
Resource scoping:
  ...
  atera-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  atera-cli sync --full

  # Incremental sync: only records from the last 7 days
  atera-cli sync --since 7d

  # Parallel sync with 8 workers
  atera-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  atera-cli sync --latest-only`,
...
func resourceSupportsPagination(resource string) bool {
	...
	case "devices":
		return true
	case "devices-genericdevices":
		return true
	case "devices-httpdevices":
		return true
	case "devices-snmpdevices":
		return true
	case "knowledgebases":
		return true
	case "rates":
	...
	case "tickets":
		return true
	case "tickets-lastmodified":
		return true
	case "tickets-statusmodified":
		return true
...
func upsertSingleObject(db *store.Store, resource string, data json.RawMessage) error {
	...
	case "available_patches":
		return db.UpsertAvailablePatches(data)
	case "installed_patches":
		return db.UpsertInstalledPatches(data)
	case "contacts":
	...
	case "billableduration":
		return db.UpsertBillableduration(data)
	case "comments":
		return db.UpsertComments(data)
	case "nonbillableduration":
		return db.UpsertNonbillableduration(data)
	case "relations":
		return db.UpsertRelations(data)
	case "workhours":
		return db.UpsertWorkhours(data)
	case "workhoursrecords":
		return db.UpsertWorkhoursrecords(data)
	default:
...
func syncResourcePath(resource string) (string, error) {
	paths := map[string]string{
		...
		"contracts":              "/contracts",
		"customers":              "/customers",
		"customvalues":           "/customvalues/customfields",
		"departments":            "/departments",
		"devices":                "/devices/tcpdevices",
		"devices-genericdevices": "/devices/genericdevices",
		"devices-httpdevices":    "/devices/httpdevices",
		"devices-snmpdevices":    "/devices/snmpdevices",
		"knowledgebases":         "/knowledgebases",
		"rates":                  "/rates/expenses",
		"rates-products":         "/rates/products",
		"tickets":                "/tickets",
		"tickets-lastmodified":   "/tickets/lastmodified",
		"tickets-statusmodified": "/tickets/statusmodified",
	}
...
func dependentResourceDefs() []dependentResourceDef {
	return []dependentResourceDef{
		{Name: "agents", ParentTable: "customers", ParentIDParam: "customerId", PathTemplate: "/agents/customer/{customerId}", KeyField: "", PathParams: []dependentPathParamDef{
			{Param: "customerId", Field: "customer_id"},
		}},
		{Name: "comments", ParentTable: "tickets", ParentIDParam: "ticketId", PathTemplate: "/tickets/{ticketId}/comments", KeyField: "", PathParams: []dependentPathParamDef{
			{Param: "ticketId", Field: "id"},
		}},
		{Name: "contracts", ParentTable: "customers", ParentIDParam: "customerId", PathTemplate: "/contracts/customer/{customerId}", KeyField: "", PathParams: []dependentPathParamDef{
			{Param: "customerId", Field: "customer_id"},
		}},
		{Name: "workhoursrecords", ParentTable: "tickets", ParentIDParam: "ticketId", PathTemplate: "/tickets/{ticketId}/workhoursrecords", KeyField: "", PathParams: []dependentPathParamDef{
			{Param: "ticketId", Field: "id"},
		}},
	}
}
