...
type Config struct {
	...
	AteraApiKey     string            `toml:"api_key"`
	AteraAccountApi string            `toml:"account_api"`
}
...
func Load(configPath string) (*Config, error) {
	...
	path := configPath
	if path == "" {
		path = os.Getenv("ATERA_CONFIG")
	}
	if path == "" {
		home, _ := os.UserHomeDir()
		path = filepath.Join(home, ".config", "atera-cli", "config.toml")
	}
	cfg.Path = path
	...
	// Env var overrides
	if v := os.Getenv("ATERA_API_KEY"); v != "" {
		cfg.AteraApiKey = v
		cfg.markEnvOverride("AteraApiKey")
		cfg.AuthSource = "env:ATERA_API_KEY"
	}
	if v := os.Getenv("ATERA_ACCOUNT_API"); v != "" {
		cfg.AteraAccountApi = v
		cfg.markEnvOverride("AteraAccountApi")
		cfg.AuthSource = "env:ATERA_ACCOUNT_API"
	}
	...
	if cfg.AuthSource == "" && cfg.AteraApiKey != "" {
		cfg.AuthSource = "config"
	}
	if cfg.AuthSource == "" && cfg.AteraAccountApi != "" {
		cfg.AuthSource = "config"
	}
	...
	if v := os.Getenv("ATERA_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	return cfg, nil
}
...
func (c *Config) AuthHeader() string {
	...
	if c.AteraApiKey != "" {
		return c.AteraApiKey
	}
	if c.AteraAccountApi != "" {
		return c.AteraAccountApi
	}
	return ""
}
...
func (c *Config) SaveCredential(token string) error {
	...
	c.updateFileConfigField("AccessToken")
	c.AteraApiKey = token
	delete(c.envOverrides, "AteraApiKey")
	c.updateFileConfigField("AteraApiKey")
	return c.save()
}
...
func (c *Config) ClearTokens() error {
	...
	delete(c.envOverrides, "AteraApiKey")
	c.AteraAccountApi = ""
	delete(c.envOverrides, "AteraAccountApi")
	return c.save()
}
...
func (c *Config) configForSave() Config {
	out := *c
	if c.fileConfig != nil {
		if c.envOverrides["AteraApiKey"] {
			out.AteraApiKey = c.fileConfig.AteraApiKey
		}
		if c.envOverrides["AteraAccountApi"] {
			out.AteraAccountApi = c.fileConfig.AteraAccountApi
		}
	}
...
func (c *Config) updateFileConfigField(field string) {
	...
	case "AteraApiKey":
		c.fileConfig.AteraApiKey = c.AteraApiKey
	case "AteraAccountApi":
		c.fileConfig.AteraAccountApi = c.AteraAccountApi
	}
}
