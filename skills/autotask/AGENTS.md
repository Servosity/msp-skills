# Autotask PSA Printed CLI Agent Guide

This directory is a generated `autotask-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
autotask-cli doctor --json
autotask-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
autotask-cli which "<capability>" --json
...
```bash
autotask-cli <command> --help
autotask-cli <command> --dry-run --agent
```
