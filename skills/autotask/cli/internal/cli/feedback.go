...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".local", "share", "autotask-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("AUTOTASK_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("AUTOTASK_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "autotask-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.local/share/autotask-cli/feedback.jsonl.
When ` + "`AUTOTASK_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`AUTOTASK_FEEDBACK_AUTO_SEND=true`" + `, the entry is
