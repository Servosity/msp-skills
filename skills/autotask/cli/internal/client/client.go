...
func New(cfg *config.Config, timeout time.Duration, rateLimit float64) *Client {
	...
	cacheDir := filepath.Join(homeDir, ".cache", "autotask-cli", "http")
	...
	httpClient.CheckRedirect = func(req *http.Request, via []*http.Request) error {
		...
		if req.URL.Host == via[0].URL.Host {
			if h, err := c.authHeader(req.Context()); err == nil && h != "" {
				req.Header.Set("ApiIntegrationCode", h)
			}
		} else {
			...
			// HAND-FIX: the sibling per-call credentials (UserName, Secret)
			// became live again when the header-blanking bug was removed, so
			// they must be stripped here too or a cross-host 302 from a
			// compromised/MITM'd zone host harvests the full credential pair.
			// Pinned by TestCrossHostRedirectStripsAutotaskHeaders.
			req.Header.Del("ApiIntegrationCode")
...
func (c *Client) doInternal(ctx context.Context, method, path string, params map[string]string, body any, headerOverrides map[string]string, readOnlyIntent bool) (json.RawMessage, int, error) {
	...
	for attempt := 0; attempt <= maxRetries; attempt++ {
		...
		if authHeader != "" {
			req.Header.Set("ApiIntegrationCode", authHeader)
		}
		...
		if c.Config != nil {
			if v := c.Config.AutotaskPsaSecret; v != "" {
				if authHeaderLooksLikePlaceholderCredential(v) {
					return nil, 0, authPlaceholderCredentialErrorWithSetup(c.Config, "export AUTOTASK_PSA_SECRET=<your-token>")
				}
				req.Header.Set("Secret", v)
			}
			if v := c.Config.AutotaskPsaUserName; v != "" {
				if authHeaderLooksLikePlaceholderCredential(v) {
					return nil, 0, authPlaceholderCredentialErrorWithSetup(c.Config, "export AUTOTASK_PSA_USER_NAME=<your-token>")
				}
				req.Header.Set("UserName", v)
			}
		}
		// NOTE: the generator emitted two unconditional resets here
		//   req.Header.Set("UserName", ""); req.Header.Set("Secret", "")
		// that blanked the sibling credentials immediately after they were set
		// above, so only ApiIntegrationCode reached the wire and every
		// authenticated Autotask call would 401. Autotask requires all three
		// headers (ApiIntegrationCode + UserName + Secret) on every request, so
		// those reset lines are removed. Pinned by
		// internal/client/autotask_headers_test.go. (Generator multi-apiKey-scheme bug.)
		if c.Config != nil {
...
func (c *Client) dryRun(method, targetURL, path string, params map[string]string, body []byte, headerOverrides map[string]string, authHeader string) (json.RawMessage, int, error) {
	...
	if authHeader != "" {
		fmt.Fprintf(os.Stderr, "  %s: %s\n", "ApiIntegrationCode", maskToken(authHeader))
	}
