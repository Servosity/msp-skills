# autotask skill - governance and safety model

> Unofficial. Community-built skill for the Autotask API. Not affiliated with,
> endorsed by, or sponsored by the vendor.
> This page tells an MSP owner exactly what the autotask skill can touch and how to
> scope it, so you can decide what to let an AI agent do.

## What it authenticates as

The skill drives the `autotask-cli` binary (and `autotask-mcp`),
authenticating with `AUTOTASK_PSA_API_INTEGRATION_CODE`, `AUTOTASK_PSA_SECRET`, `AUTOTASK_PSA_USER_NAME`. Credentials are read from the environment only -
never written to disk, never logged, never sent anywhere except the Autotask API.
...
| **Write (routine)** | Day-to-day mutations. | `appointments create-entity`, `appointments patch-entity`, `appointments update-entity`, `companies alerts company-child-create-entity`, `companies alerts company-child-patch-entity`, `companies alerts company-child-update-entity`, `companies attachments company-child-create-entity`, `companies contacts company-child-create-entity`, ... (223 total) | Preview with `--dry-run`, then an approved write (where a command documents its own confirm gate, use it too) |
| **Credential / security** | Touches tokens, keys, MFA. | (none detected) | Human-in-the-loop only |
| **Destructive** | Irreversible data or config loss. | `appointments delete-entity`, `companies alerts company-child-delete-entity`, `companies attachments company-child-delete-entity`, `companies contacts company-child-delete-entity`, `companies locations company-child-delete-entity`, `companies teams company-child-delete-entity`, `companies to-dos company-child-delete-entity`, `company-notes attachments company-note-child-delete-entity`, ... (50 total) | Human-in-the-loop only, explicit confirmation |
...
the Autotask API, and you can read every line of how it does so. The skill is
