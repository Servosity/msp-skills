# install.ps1 - install autotask-cli and autotask-mcp on Windows.
#
# Pulls prebuilt binaries from this skill's latest GitHub Release (autotask-v*).
...
$Skill   = "autotask"
$CliBin  = "autotask-cli.exe"
$McpBin  = "autotask-mcp.exe"
...
$ReleaseBase = if ($env:MSP_SKILLS_RELEASE_BASE) {
  ...
  # Each skill is versioned/tagged independently (autotask-vX.Y.Z),
...
Write-Host "  autotask-cli --version"
Write-Host ""
Write-Host "Next:"
Write-Host "  First command + auth: https://github.com/$Owner/$Repo/tree/main/skills/autotask#readme"
Write-Host "  Claude Desktop / ChatGPT wire-up: https://github.com/$Owner/$Repo/blob/main/skills/autotask/mcp-install.md"
