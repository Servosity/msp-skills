...
func newAuthCmd(flags *rootFlags) *cobra.Command {
	cmd := &cobra.Command{
		Use:         "auth",
		Short:       "Manage authentication for Auvik",
...
func newAuthSetupCmd(_ *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: "  auvik-cli auth setup\n  auvik-cli auth setup --launch",
		RunE: func(cmd *cobra.Command, args []string) error {
			w := cmd.OutOrStdout()
			fmt.Fprintln(w, "No setup URL is configured for this CLI; check the API's docs.")
			fmt.Fprintln(w, "")
			fmt.Fprintln(w, "Then set:")
			fmt.Fprintln(w, "  export AUVIK_USERNAME=\"your-username\"")
			fmt.Fprintln(w, "  export AUVIK_API_KEY=\"your-token-here\"")
			fmt.Fprintln(w, "  auvik-cli auth set-credentials your-username your-token-here")
			if !launch {
				return nil
			}
	...
	return &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if !authed {
				...
				fmt.Fprintln(w, "  export AUVIK_USERNAME=\"your-username\"")
				fmt.Fprintln(w, "  export AUVIK_API_KEY=\"your-token-here\"")
				fmt.Fprintf(w, "  auvik-cli auth set-credentials your-username your-token-here\n")
				return authErr(fmt.Errorf("no credentials configured"))
			}
...
func newAuthSetCredentialsCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		Use:     "set-credentials <username> <api_key>",
		Short:   "Save Basic auth credentials to the credentials file",
		Example: "  auvik-cli auth set-credentials your-username your-token-here",
		Args: func(cmd *cobra.Command, args []string) error {
			if len(args) != 2 {
				return fmt.Errorf("requires exactly 2 args: <username> <api_key>")
			}
			if args[0] == "" {
				return fmt.Errorf("AUVIK_USERNAME is required")
			}
			if args[1] == "" {
				return fmt.Errorf("AUVIK_API_KEY is required")
			}
			return nil
		},
		RunE: func(cmd *cobra.Command, args []string) error {
...
func newAuthLogoutCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			envStillSet := ""
			if envStillSet == "" && os.Getenv("AUVIK_USERNAME") != "" {
				envStillSet = "AUVIK_USERNAME"
			}
			if envStillSet == "" && os.Getenv("AUVIK_API_KEY") != "" {
				envStillSet = "AUVIK_API_KEY"
			}
