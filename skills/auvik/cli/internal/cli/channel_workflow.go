...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  auvik-cli workflow archive

  # Full re-archive (ignore previous sync state)
  auvik-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"alert", "asm-client-info", "auvik-inventory", "auvik-inventory-discovery-status", "auvik-inventory-info", "auvik-inventory-lifecycle", "inventory", "inventory-component-info", "inventory-configuration", "inventory-device-detail", "inventory-device-detail-extended", "inventory-device-lifecycle", "inventory-device-warranty", "inventory-entity-audit", "inventory-entity-note", "inventory-interface-info", "inventory-network-detail", "inventory-network-info", "tenants"}
			archiveMaxPages := 100
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  auvik-cli workflow status

  # Show status as JSON
  auvik-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("auvik-cli")
			}
