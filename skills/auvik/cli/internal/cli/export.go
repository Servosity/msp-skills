...
large datasets as it has no memory pressure.`,
  ...
  auvik-cli export <resource> --format jsonl --output data.jsonl

  # Export with limit
  auvik-cli export <resource> --format jsonl --limit 1000

  # Pipe to another tool
  auvik-cli export <resource> --format jsonl | jq '.id'`,
		Args: cobra.RangeArgs(1, 2),
		RunE: func(cmd *cobra.Command, args []string) (err error) {
			validResources := map[string]bool{
				"alert":           true,
				"asm":             true,
				"auvik-inventory": true,
				"billing":         true,
				"inventory":       true,
				"settings":        true,
				"stat":            true,
