...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("AUVIK_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("AUVIK_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "auvik-cli/feedback")
...
When ` + "`AUVIK_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`AUVIK_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  auvik-cli feedback list
  auvik-cli feedback list --limit 5
  auvik-cli feedback list --json`,
