...
type rootFlags struct {
	...
	// invocation. Mirrors the AUVIK_NO_LEARN env var.
	noLearn bool
...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "auvik-cli",
		Short: `Auvik CLI — Every Auvik endpoint as a command, plus the cross-client answers the Auvik UI and API cannot give you.`,
		Long: `Auvik CLI — Every Auvik endpoint as a command, plus the cross-client answers the Auvik UI and API cannot give you.
...
Health check: run 'auvik-cli doctor' to verify auth and connectivity.
See README.md or the bundled SKILL.md for recipes.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("auvik-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.selectFields, "select", "", "Comma-separated fields to include in output. Paths are relative to the emitted shape: API responses have the JSON:API envelope stripped, so select from attributes (--select attributes.deviceName); local-store reports are envelopes, so select through the row array (--select rows.client)")
	...
	rootCmd.PersistentFlags().StringVar(&flags.runProfileName, "profile", "", "Apply values from a saved run profile; this does not select a client (see 'auvik-cli profile list')")
	...
	rootCmd.AddCommand(newAuvikInventoryCmd(flags))
	...
	rootCmd.AddCommand(newAuthenticationPromotedCmd(flags))
	rootCmd.AddCommand(newAuvikStatPromotedCmd(flags))
	...
	addNovelCommandIfAbsent(rootCmd, newNovelConfigurationCmd(flags))
	addNovelCommandIfAbsent(rootCmd, newNovelDeviceCmd(flags))
	addNovelCommandIfAbsent(rootCmd, newNovelEolCmd(flags))
