...
Run sync first to populate the local search index.`,
  ...
  auvik-cli search "status"

  # Force local search explicitly
  auvik-cli search "status" --data-source local
  # Search a specific resource type locally
  auvik-cli search "status" --type alert --data-source local
  # JSON output for piping
  auvik-cli search "status" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'auvik-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
