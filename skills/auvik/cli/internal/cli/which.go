...
var whichIndex = []whichEntry{
	{Command: "eol", Description: "See every device approaching or past end-of-support across all your clients at once, bucketed by urgency.", Group: "Cross-client answers Auvik cannot give", WhyItMatters: "Reach for this when asked what hardware is aging out across a book of business — it is the one question Auvik's UI cannot answer at all."},
	{Command: "changes", Description: "Merge config revisions, audit entries, notes, and alerts for one device into a single chronological story.", Group: "Cross-client answers Auvik cannot give", WhyItMatters: "Use this to answer 'what happened to this device and who touched it' without opening four screens."},
	{Command: "configuration audit", Description: "Find devices with no configuration backup, a stale backup, or no running-config backup, across every client.", Group: "Config integrity at fleet scale", WhyItMatters: "Reach for this to prove which devices are unprotected across a whole book of business; it cannot search config text, because the API does not expose it."},
	{Command: "inventory diff", Description: "List devices added, removed, or changed fleet-wide since the last sync, attributed to each client.", Group: "Cross-client answers Auvik cannot give", WhyItMatters: "Reach for this when a device count moved and you need to know which devices caused it."},
	{Command: "usage reconcile", Description: "Put each client's billable usage count next to the actual synced inventory and show the device rows behind the difference.", Group: "Billing and count integrity", WhyItMatters: "Use this before invoicing when a client's billable count disagrees with the agreement."},
	{Command: "device discovery-gaps", Description: "List every device Auvik cannot fully poll, per client, with the credential state behind each gap.", Group: "Config integrity at fleet scale", WhyItMatters: "Answers 'why can't we see this box' in one command instead of three screens."},
	{Command: "alert noise", Description: "Rank devices and clients by alert volume over a window, with device names, types, clients and severity mix resolved.", Group: "Alert memory", WhyItMatters: "Use this for the shift handoff or the chronic-offender conversation. Auvik publishes no dismissal timestamp, so this reports dismissal counts, not time-to-dismiss."},
	{Command: "asm shadow", Description: "Surface SaaS apps with active users but no license record, and licenses nobody is using, per client.", Group: "Billing and count integrity", WhyItMatters: "Reach for this when building a client's software-spend narrative."},
}
...
Exit codes:
  ...
  2  no confident match - the query did not score against any indexed capability; fall back to '--help' or 'search' if this CLI has one`,
		Example: `  auvik-cli which "stale tickets"
  auvik-cli which "bottleneck"
  auvik-cli which --limit 1 "send message"
  auvik-cli which                                # list the full capability index`,
