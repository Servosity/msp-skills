...
import (
	...
	"auvik-pp-cli/internal/cliutil/testenv"
...
func resetCredentialEnv(t *testing.T) (home, configPath string) {
	...
	return home, filepath.Join(home, ".config", "auvik-cli", "config.toml")
}
...
func TestExplicitConfigUsesSiblingCredentialsBeforeGlobal(t *testing.T) {
	...
	t.Setenv("AUVIK_CONFIG", configPath)
...
func TestCorruptCredentialsDoesNotOverrideLegacyConfig(t *testing.T) {
	...
	credentialsPath := filepath.Join(home, ".local", "share", "auvik-cli", "credentials.toml")
	if err := os.MkdirAll(filepath.Dir(credentialsPath), 0o700); err != nil {
		t.Fatalf("mkdir credentials dir: %v", err)
	}
...
func TestAuthWriteScrubsLegacyConfigWhenRelocated(t *testing.T) {
	...
	activeConfigPath := filepath.Join(newConfigDir, "auvik-cli", "config.toml")
	...
	credsPath := filepath.Join(newDataDir, "auvik-cli", "credentials.toml")
...
func credentialValue(creds *cliutil.Credentials) string {
	return creds.AuvikUsername
}

func setCredentialValue(creds *cliutil.Credentials, token string) {
	creds.AuvikUsername = token
	creds.AuvikApiKey = token + "-secondary"
}
func setCredentialEnv(t *testing.T, token string) {
	t.Helper()
	t.Setenv("AUVIK_USERNAME", token)
	t.Setenv("AUVIK_API_KEY", token+"-secondary")
}

func configWithCredential(token string) *config.Config {
	cfg := &config.Config{}
	cfg.AuvikUsername = token
	cfg.AuvikApiKey = token + "-secondary"
	return cfg
}

func basicAuthHeaderForCredential(token string) string {
	payload := strings.TrimSpace("Basic {username}:{password}")
	...
	replacements := map[string]string{
		"username":       token,
		"AUVIK_USERNAME": token,
		"user":           token,
		"api_key":        token + "-secondary",
		"AUVIK_API_KEY":  token + "-secondary",
...
func TestAuthHeaderRequiresEveryCredential(t *testing.T) {
	t.Run("missing AUVIK_USERNAME", func(t *testing.T) {
	...
	t.Run("missing AUVIK_API_KEY", func(t *testing.T) {
		cfg := configWithCredential("complete-secret")
		cfg.AuvikApiKey = ""
		if got := cfg.AuthHeader(); got != "" {
			t.Fatalf("AuthHeader() = %q, want empty", got)
