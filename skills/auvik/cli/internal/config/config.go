...
type Config struct {
	...
	AuvikUsername    string          `toml:"username"`
	AuvikApiKey      string          `toml:"api_key"`
}

func Load(configPath string) (*Config, error) {
	cfg := &Config{
		BaseURL: "https://auvikapi.us1.my.auvik.com",
	}
	...
	// Env var overrides
	if v := os.Getenv("AUVIK_USERNAME"); v != "" {
		cfg.AuvikUsername = v
		cfg.markEnvOverride("AuvikUsername")
		cfg.AuthSource = "env:AUVIK_USERNAME"
		cfg.CredentialSource = "env:AUVIK_USERNAME"
	}
	if v := os.Getenv("AUVIK_API_KEY"); v != "" {
		cfg.AuvikApiKey = v
		cfg.markEnvOverride("AuvikApiKey")
		cfg.AuthSource = "env:AUVIK_API_KEY"
		cfg.CredentialSource = "env:AUVIK_API_KEY"
	}
	...
	if v := os.Getenv("AUVIK_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	return cfg, nil
}
...
func resolveConfigPath(configPath string) (string, bool, error) {
	...
	if path := os.Getenv("AUVIK_CONFIG"); path != "" {
...
func LegacyConfigPath() (string, error) {
	...
	return filepath.Join(home, ".config", "auvik-cli", "config.toml"), nil
}
...
func (c *Config) AuthHeader() string {
	...
	credentials := basicAuthPayload("Basic {username}:{password}", map[string]string{
		"username":       c.AuvikUsername,
		"AUVIK_USERNAME": c.AuvikUsername,
		"user":           c.AuvikUsername,
		"api_key":        c.AuvikApiKey,
		"AUVIK_API_KEY":  c.AuvikApiKey,
		"password":       c.AuvikApiKey,
		"secret":         c.AuvikApiKey,
		"token":          c.AuvikApiKey,
		"access_token":   c.AuvikApiKey,
	}, false)
	if credentials == "" {
		return ""
	}
...
func (c *Config) credentials() *cliutil.Credentials {
	return &cliutil.Credentials{
		...
		AuvikUsername: c.AuvikUsername,
		AuvikApiKey:   c.AuvikApiKey,
	}
}
...
func (c *Config) applyCredentials(creds *cliutil.Credentials) {
	...
	if c.AuvikUsername == "" {
		c.AuvikUsername = creds.AuvikUsername
	}
	if c.AuvikApiKey == "" {
		c.AuvikApiKey = creds.AuvikApiKey
	}
}
...
func (c *Config) SaveCredentials(value0, value1 string) error {
	...
	c.updateFileConfigField("AccessToken")
	c.AuvikUsername = value0
	delete(c.envOverrides, "AuvikUsername")
	c.updateFileConfigField("AuvikUsername")
	c.AuvikApiKey = value1
	delete(c.envOverrides, "AuvikApiKey")
	c.updateFileConfigField("AuvikApiKey")
	if err := c.saveCredentialsFirst(); err != nil {
		return err
	}
	return c.save()
...
func (c *Config) configForSave() Config {
	out := *c
	if c.fileConfig != nil {
		if c.envOverrides["AuvikUsername"] {
			out.AuvikUsername = c.fileConfig.AuvikUsername
		}
		if c.envOverrides["AuvikApiKey"] {
			out.AuvikApiKey = c.fileConfig.AuvikApiKey
		}
	}
...
func (c *Config) updateFileConfigField(field string) {
	...
	case "AuvikUsername":
		c.fileConfig.AuvikUsername = c.AuvikUsername
	case "AuvikApiKey":
		c.fileConfig.AuvikApiKey = c.AuvikApiKey
	}
}
