...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  avanan-cli workflow archive

  # Full re-archive (ignore previous sync state)
  avanan-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"msp-addons", "msp-licenses", "msp-msp-partners", "msp-tenants", "msp-users", "sectools", "sectools-click-time-protection-exceptions", "sectools-click-time-protection-exceptions-items"}
			archiveMaxPages := 100
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  avanan-cli workflow status

  # Show status as JSON
  avanan-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("avanan-cli")
			}
