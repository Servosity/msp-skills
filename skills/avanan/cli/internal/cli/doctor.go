...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  avanan-cli doctor
  avanan-cli doctor --json
  avanan-cli doctor --fail-on warn
  avanan-cli doctor --fail-on stale`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if cfg != nil {
				...
				if !configured {
					...
					report["auth_hint"] = "Run 'avanan-cli auth setup' for credential setup steps."
				} else {
					authConfigured = true
			...
			if os.Getenv("AVANAN_APP_ID") != "" {
				authEnvSet = append(authEnvSet, "AVANAN_APP_ID")
			} else {
				authEnvInfo = append(authEnvInfo, "AVANAN_APP_ID set during auth login")
			}
			if os.Getenv("AVANAN_CLIENT_SECRET") != "" {
				authEnvSet = append(authEnvSet, "AVANAN_CLIENT_SECRET")
			} else {
				authEnvInfo = append(authEnvInfo, "AVANAN_CLIENT_SECRET set during auth login")
			}
			if os.Getenv("AVANAN_TOKEN") != "" || authHeader != "" {
				authEnvSet = append(authEnvSet, "AVANAN_TOKEN")
			} else {
				authEnvInfo = append(authEnvInfo, "AVANAN_TOKEN set with auth set-token")
			}
			switch {
			...
			if cfg != nil && doctorBaseURLIsPlaceholder(cfg.BaseURL) {
				...
				if clientErr != nil {
					...
					if authHeader == "" {
						...
						doctorProbeCredentials(cmd.Context(), c, cmd.Root(), "avanan-cli", report)
					}
				}
			} else if cfg != nil && cfg.BaseURL == "" {
...
func collectCacheReport(ctx context.Context, staleAfterSpec string) map[string]any {
	...
	if err != nil {
		if os.IsNotExist(err) {
			...
			report["hint"] = "Database not created yet; run 'avanan-cli sync' to hydrate."
			return report
		}
		report["status"] = "error"
	...
	if qerr != nil {
		...
		report["status"] = "unknown"
		report["hint"] = "No sync state recorded; run 'avanan-cli sync' to populate."
		return report
	}
	defer rows.Close()
	...
	case !haveAny && len(resources) == 0:
		...
		report["hint"] = "No sync recorded; run 'avanan-cli sync' to hydrate API-backed resources. Rows written by other paths are not tracked in sync_state."
	case fresh:
		report["status"] = "fresh"
	default:
		report["status"] = "stale"
		report["oldest_age"] = oldest.Round(time.Minute).String()
		report["hint"] = "Some resources are older than stale_after; run 'avanan-cli sync' to refresh."
	}
	return report
}
