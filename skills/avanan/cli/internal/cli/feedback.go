...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("AVANAN_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("AVANAN_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "avanan-cli/feedback")
...
When ` + "`AVANAN_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`AVANAN_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  avanan-cli feedback list
  avanan-cli feedback list --limit 5
  avanan-cli feedback list --json`,
