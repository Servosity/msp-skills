...
without --wait to get the job ID back immediately and track it later.`,
		Example: `  avanan-cli jobs list --limit 10
  avanan-cli jobs get example-job-id --json`,
...
func newJobsListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example:     `  avanan-cli jobs list --limit 10 --json`,
...
func newJobsGetCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example:     `  avanan-cli jobs get example-job-id --json`,
		Annotations: map[string]string{"mcp:read-only": "true"},
...
func newJobsPruneCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  avanan-cli jobs prune --older-than 168h --json`,
		RunE: func(cmd *cobra.Command, _ []string) error {
			rows, err := readJobRows()
			if err != nil {
				return err
