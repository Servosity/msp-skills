...
type rootFlags struct {
	...
	// invocation. Mirrors the AVANAN_NO_LEARN env var.
	noLearn bool
...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "avanan-cli",
		Short: `Avanan CLI — The first command-line tool for Avanan email security — with an offline mirror that answers cross-tenant and…`,
		Long: `Avanan CLI — The first command-line tool for Avanan email security — with an offline mirror that answers cross-tenant and what-changed questions the API itself cannot.
...
Health check: run 'avanan-cli doctor' to verify auth and connectivity.
See README.md or the bundled SKILL.md for recipes.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("avanan-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.selectFields, "select", "", "Comma-separated fields to include in output (e.g. --select actions,additionalData,availableEventActions)")
	...
	rootCmd.PersistentFlags().StringVar(&flags.runProfileName, "profile", "", "Apply values from a saved run profile; this does not select a client (see 'avanan-cli profile list')")
	...
	rootCmd.AddCommand(newActionCmd(flags))
	rootCmd.AddCommand(newAvananSearchCmd(flags))
	rootCmd.AddCommand(newEventCmd(flags))
	rootCmd.AddCommand(newExceptionsCmd(flags))
	rootCmd.AddCommand(newMspCmd(flags))
	rootCmd.AddCommand(newSectoolExceptionsCmd(flags))
	rootCmd.AddCommand(newSectoolsCmd(flags))
	...
	rootCmd.AddCommand(newDownloadLargeEmailPromotedCmd(flags))
	rootCmd.AddCommand(newReportPromotedCmd(flags))
	...
	addNovelCommandIfAbsent(rootCmd, newNovelCampaignCmd(flags))
	addNovelCommandIfAbsent(rootCmd, newNovelRemediateCmd(flags))
	addNovelCommandIfAbsent(rootCmd, newNovelTimelineCmd(flags))
