...
In local mode: searches locally synced data only.`,
  ...
  avanan-cli search "error timeout"

  # Force local search only
  avanan-cli search "status" --data-source local
  # Search a specific resource type locally
  avanan-cli search "status" --type action --data-source local
  # JSON output for piping
  avanan-cli search "critical" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			query := args[0]
			// This API has a search endpoint: POST /v1.0/search/query
			if flags.dataSource != "local" {
				c, err := flags.newClient()
				if err != nil {
					return err
				}
				data, _, getErr := c.Post(cmd.Context(), "/v1.0/search/query", map[string]any{
					"q":           query,
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'avanan-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
			...
			switch resourceType {
			case "exceptions":
				results, err = db.SearchExceptions(query, limit)
			case "":
				...
				{
					partial, searchErr := db.SearchExceptions(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search exceptions failed: %w", searchErr)
					}
