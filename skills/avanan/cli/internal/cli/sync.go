...
Resource scoping:
  ...
  avanan-cli sync --resources msp,msp-addons

  # Full resync (ignore previous checkpoint)
  avanan-cli sync --full

  # Incremental sync: only records from the last 7 days
  avanan-cli sync --since 7d

  # Parallel sync with 8 workers
  avanan-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  avanan-cli sync --latest-only`,
...
func upsertSingleObject(db *store.Store, resource string, data json.RawMessage) error {
	...
	switch resource {
	case "action":
		return db.UpsertAction(data)
	case "avanan-search":
		return db.UpsertAvananSearch(data)
	case "event":
		return db.UpsertEvent(data)
	case "exceptions":
		return db.UpsertExceptions(data)
	case "delete":
		return db.UpsertDelete(data)
	case "msp":
		return db.UpsertMsp(data)
	case "sectool_exceptions_exceptions":
		return db.UpsertSectoolExceptionsExceptions(data)
	default:
...
func defaultSyncResources() []string {
	return []string{
		...
		"msp-users",
		"sectools",
		"sectools-click-time-protection-exceptions",
		"sectools-click-time-protection-exceptions-items",
	}
}
...
func syncResourcePath(resource string) (string, error) {
	paths := map[string]string{ // #nosec G101 -- endpoint paths, not credentials.
		"msp":              "/v1.0/msp/usage",
		"msp-addons":       "/v1.0/msp/addons",
		"msp-licenses":     "/v1.0/msp/licenses",
		"msp-msp-partners": "/v1.0/msp/msp-partners",
		"msp-tenants":      "/v1.0/msp/tenants",
		"msp-usage-day":    "/v1.0/msp/usage/day",
		"msp-users":        "/v1.0/msp/users",
		"sectools":         "/v1.0/sectools/anomaly/exceptions",
		"sectools-click-time-protection-exceptions":       "/v1.0/sectools/click_time_protection/exceptions",
		"sectools-click-time-protection-exceptions-items": "/v1.0/sectools/click_time_protection/exceptions/items",
	}
...
var reconcileTypedTables = map[string]string{
	"action":                        "action",
	"avanan-search":                 "avanan_search",
	"delete":                        "delete",
	"event":                         "event",
	"exceptions":                    "exceptions",
	"msp":                           "msp",
	"sectool_exceptions_exceptions": "sectool_exceptions_exceptions",
}
...
func responsePathForResource(resource, path string) []string {
	...
	case "action\x00/v1.0/action/entity":
		return []string{"responseData"}
	case "action\x00/v1.0/action/event":
		return []string{"responseData"}
	case "avanan-search\x00/v1.0/search/entity/{entity_id}":
		return []string{"responseData"}
	case "avanan-search\x00/v1.0/search/query":
		return []string{"responseData"}
	case "event\x00/v1.0/event/{event_id}":
		return []string{"responseData"}
	case "event\x00/v1.0/event/query":
		return []string{"responseData"}
	case "exceptions\x00/v1.0/exceptions/blacklist":
		return []string{"responseData"}
	case "exceptions\x00/v1.0/exceptions/whitelist":
		return []string{"responseData"}
	case "exceptions\x00/v1.0/exceptions/{exc_type}":
		return []string{"responseData"}
	case "exceptions\x00/v1.0/exceptions/{exc_type}/{exc_id}":
		return []string{"responseData"}
	case "exceptions\x00/v1.0/exceptions/blacklist/{exc_id}":
		return []string{"responseData"}
	case "exceptions\x00/v1.0/exceptions/whitelist/{exc_id}":
		return []string{"responseData"}
	case "msp\x00/v1.0/msp/msp-partners":
		return []string{"responseData"}
	case "msp\x00/v1.0/msp/addons":
		return []string{"responseData"}
	case "msp\x00/v1.0/msp/usage/day":
		return []string{"responseData"}
	case "msp\x00/v1.0/msp/licenses":
		return []string{"responseData"}
	case "msp\x00/v1.0/msp/usage":
		return []string{"responseData"}
	case "msp\x00/v1.0/msp/tenants":
		return []string{"responseData"}
	case "msp\x00/v1.0/msp/users":
		return []string{"responseData"}
	case "scopes\x00/v1.0/scopes":
		return []string{"responseData"}
	}
	return nil
}
