...
var whichIndex = []whichEntry{
	{Command: "triage", Description: "See everything detected in a window you choose, bucketed by threat type, severity, and state, with the sender domains driving the volume.", Group: "Local state that compounds", WhyItMatters: "Reach for this at the start of a shift instead of re-querying the same window and eyeballing which detections you already handled."},
	{Command: "campaign", Description: "Group detections into candidate phishing campaigns by sender domain and normalized subject, showing recipient spread and how many are still un-remediated.", Group: "Local state that compounds", WhyItMatters: "Reach for this when triage shows volume and you need to know whether forty detections are one campaign or forty problems."},
	{Command: "timeline", Description: "Reconstruct one message's history from the local mirror: detection, the latest mirrored state, actions this CLI submitted, task outcomes, and restore disposition, in order.", Group: "Local state that compounds", WhyItMatters: "Reach for this when a user disputes a quarantine or a ticket needs evidence of what actually happened to a message."},
	{Command: "exceptions find", Description: "Ask whether a domain, sender, URL, or hash is excepted anywhere across all seven engines and nine exception tables at once.", Group: "Collapsing the seven-headed exception system", WhyItMatters: "Reach for this before adding an allowlist entry, or when a phish got through and you need to know whether one of your own exceptions let it."},
	{Command: "exceptions audit", Description: "Flag exceptions that contradict each other across sub-systems, exact duplicates, and entries that have not matched traffic in the mirrored window.", Group: "Collapsing the seven-headed exception system", WhyItMatters: "Reach for this on a policy review to find the contractor domain nobody removed and the string that is allowlisted in one engine and blocked in another."},
	{Command: "msp fleet", Description: "One ranked table across every tenant joining licensed seats, add-ons, usage, and detection volume.", Group: "MSP scale", WhyItMatters: "Reach for this to spot tenants over their seat count or with an unusual detection week without walking the tenant list by hand."},
	{Command: "remediate", Description: "Quarantine or restore a batch, wait for the async task to finish, and report the real per-item outcome.", Group: "Fixing the API's worst footgun", WhyItMatters: "Reach for this instead of firing an action and polling by hand, and to turn the single-scope 400 into an error that names your actual scopes."},
}
...
Exit codes:
  ...
  2  no confident match - the query did not score against any indexed capability; fall back to '--help' or 'search' if this CLI has one`,
		Example: `  avanan-cli which "stale tickets"
  avanan-cli which "bottleneck"
  avanan-cli which --limit 1 "send message"
  avanan-cli which                                # list the full capability index`,
