...
type Config struct {
	...
	AvananAppId      string          `toml:"app_id"`
	AvananToken      string          `toml:"token"`
}

func Load(configPath string) (*Config, error) {
	cfg := &Config{
		BaseURL: "https://smart-api-production-1-us.avanan.net",
	}
	...
	// Env var overrides
	if v := os.Getenv("AVANAN_APP_ID"); v != "" {
		cfg.AvananAppId = v
		cfg.markEnvOverride("AvananAppId")
		cfg.AuthSource = "env:AVANAN_APP_ID"
		cfg.CredentialSource = "env:AVANAN_APP_ID"
	}
	if v := os.Getenv("AVANAN_CLIENT_SECRET"); v != "" {
		cfg.ClientSecret = v
		cfg.markEnvOverride("ClientSecret")
		cfg.AuthSource = "env:AVANAN_CLIENT_SECRET"
		cfg.CredentialSource = "env:AVANAN_CLIENT_SECRET"
	}
	if v := os.Getenv("AVANAN_TOKEN"); v != "" {
		cfg.AvananToken = v
		cfg.markEnvOverride("AvananToken")
		cfg.AuthSource = "env:AVANAN_TOKEN"
		cfg.CredentialSource = "env:AVANAN_TOKEN"
	}
	...
	if v := os.Getenv("AVANAN_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	return cfg, nil
}
...
func resolveConfigPath(configPath string) (string, bool, error) {
	...
	if path := os.Getenv("AVANAN_CONFIG"); path != "" {
...
func LegacyConfigPath() (string, error) {
	...
	return filepath.Join(home, ".config", "avanan-cli", "config.toml"), nil
}
...
func (c *Config) credentials() *cliutil.Credentials {
	return &cliutil.Credentials{
		...
		AvananAppId:   c.AvananAppId,
		AvananToken:   c.AvananToken,
	}
}
...
func (c *Config) applyCredentials(creds *cliutil.Credentials) {
	...
	if c.AvananAppId == "" {
		c.AvananAppId = creds.AvananAppId
	}
	if c.AvananToken == "" {
		c.AvananToken = creds.AvananToken
	}
}
...
func (c *Config) configForSave() Config {
	out := *c
	if c.fileConfig != nil {
		if c.envOverrides["AvananAppId"] {
			out.AvananAppId = c.fileConfig.AvananAppId
		}
		if c.envOverrides["ClientSecret"] {
			out.ClientSecret = c.fileConfig.ClientSecret
		}
		if c.envOverrides["AvananToken"] {
			out.AvananToken = c.fileConfig.AvananToken
		}
	}
...
func (c *Config) updateFileConfigField(field string) {
	...
	case "AvananAppId":
		c.fileConfig.AvananAppId = c.AvananAppId
	case "AvananToken":
		c.fileConfig.AvananToken = c.AvananToken
	}
}
