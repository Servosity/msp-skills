...
func RegisterIntents(s *server.MCPServer) {
	s.AddTool(
		mcplib.NewTool("check_an_allowlist_request_before_you_grant_it",
			mcplib.WithDescription("Requires `mirror --resources exceptions` first. Searches all nine exception tables at once and shows which engine, which list side, and who created each hit — replacing nine separate API calls."),
			mcplib.WithString("domain", mcplib.Required(), mcplib.Description("Override the recipe's positional domain value.")),
		),
		handleCheckAnAllowlistRequestBeforeYouGrantIt,
	)
}

// handleCheckAnAllowlistRequestBeforeYouGrantIt runs the check_an_allowlist_request_before_you_grant_it recipe intent: Requires `mirror --resources exceptions` first. Searches all nine exception tables at once and shows which engine, which list side, and who created each hit — replacing nine separate API calls.
func handleCheckAnAllowlistRequestBeforeYouGrantIt(ctx context.Context, req mcplib.CallToolRequest) (*mcplib.CallToolResult, error) {
