	...
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			if tc.precreateDB {
				...
				// MkdirAll and WriteFile are both masked by the process
				// umask, so under a restrictive umask (0077) the "permissive"
				// precondition this case exists to test is never actually
				// established. Chmod is not masked; set the modes explicitly.
				if err := os.Chmod(filepath.Dir(dbPath), 0o755); err != nil {
					t.Fatalf("relax permissive store dir: %v", err)
				}
				if err := os.Chmod(dbPath, 0o644); err != nil {
					t.Fatalf("relax permissive store: %v", err)
				}
			}
...
func TestHardenSQLiteFilesSkipsSymlinkSidecars(t *testing.T) {
	...
	// WriteFile's mode is masked by the process umask; Chmod is not. Without
	// this the file starts at 0600 under umask 0077 and the assertion below
	// cannot distinguish "hardenSQLiteFiles left the symlink target alone"
	// from "the umask already made it private".
	if err := os.Chmod(targetPath, 0o644); err != nil {
		t.Fatalf("relax unrelated file: %v", err)
	}
...
// TestMigrate_AddsColumnsOnUpgrade_Action verifies that opening a
...
func TestMigrate_AddsColumnsOnUpgrade_Action(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	...
	if _, err := raw.Exec(`CREATE TABLE "action" (
		id TEXT PRIMARY KEY,
		data JSON NOT NULL,
		synced_at DATETIME DEFAULT CURRENT_TIMESTAMP
	)`); err != nil {
	...
	rows, err := s.DB().Query(`PRAGMA table_info("action")`)
	if err != nil {
	...
	for _, want := range []string{
		...
		if !hasColumn[want] {
			t.Fatalf("%s column missing from action after migrate", want)
		}
	}
}

// TestMigrate_AddsColumnsOnUpgrade_AvananSearch verifies that opening a
// database created by an older binary succeeds and adds newly generated
// columns before CREATE INDEX runs against the pre-existing table. Regression
// coverage for parent_id upgrades and indexed generated columns.
func TestMigrate_AddsColumnsOnUpgrade_AvananSearch(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	...
	if _, err := raw.Exec(`CREATE TABLE "avanan_search" (
		id TEXT PRIMARY KEY,
		data JSON NOT NULL,
		synced_at DATETIME DEFAULT CURRENT_TIMESTAMP
	)`); err != nil {
	...
	// The migration must have added every generated column.
	rows, err := s.DB().Query(`PRAGMA table_info("avanan_search")`)
	if err != nil {
		t.Fatalf("table_info: %v", err)
	}
	defer rows.Close()
	...
	for _, want := range []string{
		...
		if !hasColumn[want] {
			t.Fatalf("%s column missing from avanan_search after migrate", want)
		}
	}
}
...
// TestMigrate_AddsColumnsOnUpgrade_Exceptions verifies that opening a
// database created by an older binary succeeds and adds newly generated
// columns before CREATE INDEX runs against the pre-existing table. Regression
// coverage for parent_id upgrades and indexed generated columns.
func TestMigrate_AddsColumnsOnUpgrade_Exceptions(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	...
	if _, err := raw.Exec(`CREATE TABLE "exceptions" (
		id TEXT PRIMARY KEY,
		data JSON NOT NULL,
		synced_at DATETIME DEFAULT CURRENT_TIMESTAMP
	)`); err != nil {
	...
	// The migration must have added every generated column.
	rows, err := s.DB().Query(`PRAGMA table_info("exceptions")`)
	if err != nil {
		t.Fatalf("table_info: %v", err)
	}
	defer rows.Close()
	...
	for _, want := range []string{
		...
		"sender_ip",
		"sender_name",
		"sender_name_matching",
		"subject",
		"subject_matching",
		"update_time",
	} {
		if !hasColumn[want] {
			t.Fatalf("%s column missing from exceptions after migrate", want)
		}
	}
}
...
// TestMigrate_AddsColumnsOnUpgrade_Msp verifies that opening a
// database created by an older binary succeeds and adds newly generated
// columns before CREATE INDEX runs against the pre-existing table. Regression
// coverage for parent_id upgrades and indexed generated columns.
func TestMigrate_AddsColumnsOnUpgrade_Msp(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	...
	if _, err := raw.Exec(`CREATE TABLE "msp" (
		id TEXT PRIMARY KEY,
		data JSON NOT NULL,
		synced_at DATETIME DEFAULT CURRENT_TIMESTAMP
	)`); err != nil {
	...
	// The migration must have added every generated column.
	rows, err := s.DB().Query(`PRAGMA table_info("msp")`)
	if err != nil {
		t.Fatalf("table_info: %v", err)
	}
	defer rows.Close()
	...
	for _, want := range []string{
		...
		if !hasColumn[want] {
			t.Fatalf("%s column missing from msp after migrate", want)
		}
	}
}

// TestMigrate_AddsColumnsOnUpgrade_SectoolExceptionsExceptions verifies that opening a
// database created by an older binary succeeds and adds newly generated
// columns before CREATE INDEX runs against the pre-existing table. Regression
// coverage for parent_id upgrades and indexed generated columns.
func TestMigrate_AddsColumnsOnUpgrade_SectoolExceptionsExceptions(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	...
	if _, err := raw.Exec(`CREATE TABLE "sectool_exceptions_exceptions" (
		id TEXT PRIMARY KEY,
		data JSON NOT NULL,
		synced_at DATETIME DEFAULT CURRENT_TIMESTAMP
	)`); err != nil {
	...
	// The migration must have added every generated column.
	rows, err := s.DB().Query(`PRAGMA table_info("sectool_exceptions_exceptions")`)
	if err != nil {
		t.Fatalf("table_info: %v", err)
	}
	defer rows.Close()
	...
	for _, want := range []string{
		"sectool_exceptions_id",
	} {
		if !hasColumn[want] {
			t.Fatalf("%s column missing from sectool_exceptions_exceptions after migrate", want)
		}
	}
}
