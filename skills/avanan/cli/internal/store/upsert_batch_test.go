...
// TestUpsertBatch_PopulatesActionTable verifies that UpsertBatch
// dispatches paginated items into both the generic resources table AND the
// typed action table. Regression for issue #268: before the fix, paginated
// syncs only filled the generic resources table, so domain commands that
// query the typed table saw zero rows.
func TestUpsertBatch_PopulatesActionTable(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	s, err := Open(dbPath)
	if err != nil {
		t.Fatalf("open: %v", err)
	...
	if _, _, err := s.UpsertBatch("action", items); err != nil {
		t.Fatalf("UpsertBatch: %v", err)
	}
	...
	var generic int
	if err := db.QueryRow(`SELECT COUNT(*) FROM resources WHERE resource_type = ?`, "action").Scan(&generic); err != nil {
		t.Fatalf("count resources: %v", err)
	}
	...
	var typed int
	typedQuery := fmt.Sprintf(`SELECT COUNT(*) FROM "%s"`, "action")
	if err := db.QueryRow(typedQuery).Scan(&typed); err != nil {
		t.Fatalf("count action: %v", err)
	}
	if typed != len(items) {
		t.Fatalf("action count = %d, want %d (typed table not populated by UpsertBatch)", typed, len(items))
	}
}

// TestUpsertBatch_PopulatesAvananSearchTable verifies that UpsertBatch
// dispatches paginated items into both the generic resources table AND the
// typed avanan_search table. Regression for issue #268: before the fix, paginated
// syncs only filled the generic resources table, so domain commands that
// query the typed table saw zero rows.
func TestUpsertBatch_PopulatesAvananSearchTable(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	s, err := Open(dbPath)
	if err != nil {
		t.Fatalf("open: %v", err)
	...
	if _, _, err := s.UpsertBatch("avanan-search", items); err != nil {
		t.Fatalf("UpsertBatch: %v", err)
	}
	...
	var generic int
	if err := db.QueryRow(`SELECT COUNT(*) FROM resources WHERE resource_type = ?`, "avanan-search").Scan(&generic); err != nil {
		t.Fatalf("count resources: %v", err)
	}
	if generic != len(items) {
		t.Fatalf("resources count = %d, want %d", generic, len(items))
	...
	var typed int
	typedQuery := fmt.Sprintf(`SELECT COUNT(*) FROM "%s"`, "avanan_search")
	if err := db.QueryRow(typedQuery).Scan(&typed); err != nil {
		t.Fatalf("count avanan_search: %v", err)
	}
	if typed != len(items) {
		t.Fatalf("avanan_search count = %d, want %d (typed table not populated by UpsertBatch)", typed, len(items))
	}
}
...
// TestUpsertBatch_PopulatesExceptionsTable verifies that UpsertBatch
// dispatches paginated items into both the generic resources table AND the
// typed exceptions table. Regression for issue #268: before the fix, paginated
// syncs only filled the generic resources table, so domain commands that
// query the typed table saw zero rows.
func TestUpsertBatch_PopulatesExceptionsTable(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	s, err := Open(dbPath)
	if err != nil {
		t.Fatalf("open: %v", err)
	...
	if _, _, err := s.UpsertBatch("exceptions", items); err != nil {
		t.Fatalf("UpsertBatch: %v", err)
	}
	...
	var generic int
	if err := db.QueryRow(`SELECT COUNT(*) FROM resources WHERE resource_type = ?`, "exceptions").Scan(&generic); err != nil {
		t.Fatalf("count resources: %v", err)
	}
	if generic != len(items) {
		t.Fatalf("resources count = %d, want %d", generic, len(items))
	...
	var typed int
	typedQuery := fmt.Sprintf(`SELECT COUNT(*) FROM "%s"`, "exceptions")
	if err := db.QueryRow(typedQuery).Scan(&typed); err != nil {
		t.Fatalf("count exceptions: %v", err)
	}
	if typed != len(items) {
		t.Fatalf("exceptions count = %d, want %d (typed table not populated by UpsertBatch)", typed, len(items))
	}
}

func TestSearchExceptionsQuotesFTSQuerySyntax(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	s, err := Open(dbPath)
	if err != nil {
		t.Fatalf("open: %v", err)
	...
	if stored, failed, err := s.UpsertBatch("exceptions", items); err != nil {
		t.Fatalf("UpsertBatch: %v", err)
	} else if failed != 0 || stored != len(items) {
		t.Fatalf("UpsertBatch stored=%d failed=%d, want stored=%d failed=0", stored, failed, len(items))
	}
	...
	for _, query := range []string{
		...
		results, err := s.SearchExceptions(query, 10)
		if err != nil {
			t.Fatalf("SearchExceptions(%q): %v", query, err)
		}
		if len(results) == 0 {
			t.Fatalf("SearchExceptions(%q) returned no results", query)
		}
	}
}
...
func TestUpsertBatch_PopulatesDeleteTable(t *testing.T) {
	...
	items := []json.RawMessage{
		json.RawMessage(`{"id": "test-001", "exceptions_id": "test-parent-001"}`),
		json.RawMessage(`{"id": "test-002", "exceptions_id": "test-parent-001"}`),
		json.RawMessage(`{"id": "test-003", "exceptions_id": "test-parent-001"}`),
	}
...
func TestUpsertBatch_TypedFailureDoesNotStrandDeleteGeneric(t *testing.T) {
	...
	// Items deliberately omit "exceptions_id" so the typed NOT NULL
	// constraint fires.
	items := []json.RawMessage{
	...
	if typed != 0 {
		t.Fatalf("delete count = %d, want 0 (typed insert violated NOT NULL on %q)", typed, "exceptions_id")
	}
}
...
func TestUpsertBatch_KeysDeleteByChildAndParent(t *testing.T) {
	...
	items := []json.RawMessage{
		json.RawMessage(`{"id": "shared_child", "exceptions_id": "parent_A"}`),
		json.RawMessage(`{"id": "shared", "exceptions_id": "child_parent_A"}`),
		json.RawMessage(`{"id": "shared_child", "exceptions_id": "parent_B"}`),
		json.RawMessage(`{"id": "other_child", "exceptions_id": "parent_A"}`),
	}
	stored, extractFailures, err := s.UpsertBatch("delete", items)
	if err != nil {
		t.Fatalf("UpsertBatch: %v", err)
	...
	parentQuery := fmt.Sprintf(`SELECT COUNT(*) FROM "%s" WHERE "exceptions_id" = ?`, "delete")
	if err := db.QueryRow(parentQuery, "parent_A").Scan(&parentMatches); err != nil {
		t.Fatalf("count by exceptions_id: %v", err)
	}
	if parentMatches != 2 {
		t.Fatalf("exceptions_id=parent_A count = %d, want 2", parentMatches)
	}
}

// TestUpsertBatch_PopulatesMspTable verifies that UpsertBatch
// dispatches paginated items into both the generic resources table AND the
// typed msp table. Regression for issue #268: before the fix, paginated
// syncs only filled the generic resources table, so domain commands that
// query the typed table saw zero rows.
func TestUpsertBatch_PopulatesMspTable(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	s, err := Open(dbPath)
	if err != nil {
		t.Fatalf("open: %v", err)
	...
	if _, _, err := s.UpsertBatch("msp", items); err != nil {
		t.Fatalf("UpsertBatch: %v", err)
	}
	...
	var generic int
	if err := db.QueryRow(`SELECT COUNT(*) FROM resources WHERE resource_type = ?`, "msp").Scan(&generic); err != nil {
		t.Fatalf("count resources: %v", err)
	}
	if generic != len(items) {
		t.Fatalf("resources count = %d, want %d", generic, len(items))
	...
	var typed int
	typedQuery := fmt.Sprintf(`SELECT COUNT(*) FROM "%s"`, "msp")
	if err := db.QueryRow(typedQuery).Scan(&typed); err != nil {
		t.Fatalf("count msp: %v", err)
	}
	if typed != len(items) {
		t.Fatalf("msp count = %d, want %d (typed table not populated by UpsertBatch)", typed, len(items))
	}
}

// TestUpsertBatch_PopulatesSectoolExceptionsExceptionsTable verifies that UpsertBatch
// dispatches paginated items into both the generic resources table AND the
// typed sectool_exceptions_exceptions table. Regression for issue #268: before the fix, paginated
// syncs only filled the generic resources table, so domain commands that
// query the typed table saw zero rows.
func TestUpsertBatch_PopulatesSectoolExceptionsExceptionsTable(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	s, err := Open(dbPath)
	if err != nil {
		t.Fatalf("open: %v", err)
	...
	items := []json.RawMessage{
		json.RawMessage(`{"id": "test-001", "sectool_exceptions_id": "test-parent-001"}`),
		json.RawMessage(`{"id": "test-002", "sectool_exceptions_id": "test-parent-001"}`),
		json.RawMessage(`{"id": "test-003", "sectool_exceptions_id": "test-parent-001"}`),
	}
	if _, _, err := s.UpsertBatch("sectool_exceptions_exceptions", items); err != nil {
		t.Fatalf("UpsertBatch: %v", err)
	}
	...
	var generic int
	if err := db.QueryRow(`SELECT COUNT(*) FROM resources WHERE resource_type = ?`, "sectool_exceptions_exceptions").Scan(&generic); err != nil {
		t.Fatalf("count resources: %v", err)
	}
	if generic != len(items) {
		t.Fatalf("resources count = %d, want %d", generic, len(items))
	...
	var typed int
	typedQuery := fmt.Sprintf(`SELECT COUNT(*) FROM "%s"`, "sectool_exceptions_exceptions")
	if err := db.QueryRow(typedQuery).Scan(&typed); err != nil {
		t.Fatalf("count sectool_exceptions_exceptions: %v", err)
	}
	if typed != len(items) {
		t.Fatalf("sectool_exceptions_exceptions count = %d, want %d (typed table not populated by UpsertBatch)", typed, len(items))
	}
}

// TestUpsertBatch_TypedFailureDoesNotStrandSectoolExceptionsExceptionsGeneric exercises
// the savepoint isolation around the typed-table dispatch. The fixture omits
// the NOT NULL parent FK column so the typed insert fails; the savepoint
// rolls back only the typed projection. The generic resources row inserted
// just before must survive. Regression for issue #1392, where a single
// outer transaction caused typed-table failures to cascade and silently
// discard every successfully fetched API row.
func TestUpsertBatch_TypedFailureDoesNotStrandSectoolExceptionsExceptionsGeneric(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	s, err := Open(dbPath)
	if err != nil {
		t.Fatalf("open: %v", err)
	...
	// Items deliberately omit "sectool_exceptions_id" so the typed NOT NULL
	// constraint fires.
	items := []json.RawMessage{
		json.RawMessage(`{"id": "orphan-001"}`),
		json.RawMessage(`{"id": "orphan-002"}`),
	...
	stored, extractFailures, err := s.UpsertBatch("sectool_exceptions_exceptions", items)
	if err != nil {
		t.Fatalf("UpsertBatch: %v (typed-table failure must not propagate)", err)
	}
	if stored != len(items) {
	...
	if typed != 0 {
		t.Fatalf("sectool_exceptions_exceptions count = %d, want 0 (typed insert violated NOT NULL on %q)", typed, "sectool_exceptions_id")
	}
}

// TestUpsertBatch_KeysSectoolExceptionsExceptionsByChildAndParent verifies dependent
// sub-collection rows with the same child id but different parent ids do not
// overwrite one another in either the generic resources table or the typed
// table. Regression for issue #2471.
func TestUpsertBatch_KeysSectoolExceptionsExceptionsByChildAndParent(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	s, err := Open(dbPath)
	if err != nil {
		t.Fatalf("open: %v", err)
	...
	items := []json.RawMessage{
		json.RawMessage(`{"id": "shared_child", "sectool_exceptions_id": "parent_A"}`),
		json.RawMessage(`{"id": "shared", "sectool_exceptions_id": "child_parent_A"}`),
		json.RawMessage(`{"id": "shared_child", "sectool_exceptions_id": "parent_B"}`),
		json.RawMessage(`{"id": "other_child", "sectool_exceptions_id": "parent_A"}`),
	}
	stored, extractFailures, err := s.UpsertBatch("sectool_exceptions_exceptions", items)
	if err != nil {
		t.Fatalf("UpsertBatch: %v", err)
	...
	if typed != len(items) {
		t.Fatalf("sectool_exceptions_exceptions count = %d, want %d (typed rows collapsed on child id)", typed, len(items))
	}

	var parentMatches int
	parentQuery := fmt.Sprintf(`SELECT COUNT(*) FROM "%s" WHERE "sectool_exceptions_id" = ?`, "sectool_exceptions_exceptions")
	if err := db.QueryRow(parentQuery, "parent_A").Scan(&parentMatches); err != nil {
		t.Fatalf("count by sectool_exceptions_id: %v", err)
	}
	if parentMatches != 2 {
		t.Fatalf("sectool_exceptions_id=parent_A count = %d, want 2", parentMatches)
	}
}
