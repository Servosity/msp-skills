...
never written to disk, never logged, never sent anywhere except the Amazon Web Services API.
...
| **Write (routine)** | Day-to-day mutations. | `iam setup` | Preview with `--dry-run`, then an approved write (where a command documents its own confirm gate, use it too) |
...
the Amazon Web Services API, and you can read every line of how it does so. The skill is
