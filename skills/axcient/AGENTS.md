# Axcient x360Recover Printed CLI Agent Guide

This directory is a generated `axcient-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
axcient-cli doctor --json
axcient-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
axcient-cli which "<capability>" --json
axcient-cli <command> --help
```
...
```bash
axcient-cli <command> --help
axcient-cli <command> --dry-run --agent
```
