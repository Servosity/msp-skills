...
func newClientVaultGetD2cAgentTokenByClientAndIdsCmd(flags *rootFlags) *cobra.Command {
	var stdinBody bool
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			path := "/client/{client_id}/vault/{vault_id}/d2c_agent"
			path = replacePathParam(path, "client_id", args[0])
			if len(args) < 2 {
				return usageErr(fmt.Errorf("vault_id is required\nUsage: %s <%s>", cmd.CommandPath(), "vault_id"))
			}
			path = replacePathParam(path, "vault_id", args[1])
			...
			if !flags.dryRun && statusCode >= 200 && statusCode < 300 && (partialFailure == nil || flags.allowPartialFailure) {
				writeMutationResponseToStore(cmd.Context(), "vault", data, "")
			}
				...
				if json.Unmarshal(data, &items) == nil && len(items) > 0 {
					if err := printAutoTable(cmd.OutOrStdout(), items); err != nil {
						...
						if partialFailure != nil && !flags.allowPartialFailure {
							return partialFailureErr(fmt.Errorf("partial failure in %s response: %s", "vault", partialFailure.Message))
						}
						return nil
					}
				} else {
