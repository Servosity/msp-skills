...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".local", "share", "axcient-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("AXCIENT_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("AXCIENT_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "axcient-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.local/share/axcient-cli/feedback.jsonl.
When ` + "`AXCIENT_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`AXCIENT_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  axcient-cli feedback list
  axcient-cli feedback list --limit 5
  axcient-cli feedback list --json`,
