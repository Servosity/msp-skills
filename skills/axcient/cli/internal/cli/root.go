...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "axcient-cli",
		Short: `Axcient CLI — Every x360Recover endpoint, plus the fleet-wide backup-health answers the API alone can't give — offline, joined, and a…`,
		Long: `Axcient CLI — Every x360Recover endpoint, plus the fleet-wide backup-health answers the API alone can't give — offline, joined, and a…
...
Health check: run 'axcient-cli doctor' to verify auth and connectivity.
See README.md or the bundled SKILL.md for recipes.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("axcient-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.profileName, "profile", "", "Apply values from a saved profile (see 'axcient-cli profile list')")
	...
	rootCmd.AddCommand(newApplianceCmd(flags))
	...
	rootCmd.AddCommand(newNovelApplianceMapCmd(flags))
	rootCmd.AddCommand(newNovelBillingCmd(flags))
	rootCmd.AddCommand(newNovelClientRollupCmd(flags))
	rootCmd.AddCommand(newNovelComplianceCmd(flags))
	rootCmd.AddCommand(newNovelHealthCmd(flags))
	rootCmd.AddCommand(newNovelRpoCmd(flags))
