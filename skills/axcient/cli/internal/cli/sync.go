...
Resource scoping:
  ...
  axcient-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  axcient-cli sync --full

  # Incremental sync: only records from the last 7 days
  axcient-cli sync --since 7d

  # Parallel sync with 8 workers
  axcient-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  axcient-cli sync --latest-only`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			// Skip under --dry-run: a preview must not mutate sync-state.
			if full && !c.DryRun {
			...
			if latestOnly {
				if since == "" {
					...
					// a preview must not mutate sync-state.
					if !c.DryRun {
						for _, resource := range resources {
...
func upsertSingleObject(db *store.Store, resource string, data json.RawMessage) error {
	...
	case "client_appliance":
		return db.UpsertClientAppliance(data)
	case "client_device":
		return db.UpsertClientDevice(data)
	case "usage":
		return db.UpsertUsage(data)
	case "client_vault":
		return db.UpsertClientVault(data)
	case "autoverify":
		return db.UpsertAutoverify(data)
	case "restore_point":
		return db.UpsertRestorePoint(data)
	case "threshold":
		return db.UpsertThreshold(data)
	default:
...
func dependentResourceDefs() []dependentResourceDef {
	return []dependentResourceDef{
		...
		// Hand-wired: /device/{device_id}/autoverify items carry no native id
		// (vault_id/appliance_id/autoverify_details only); a synthetic id is
		// injected below in syncDependentResource so UpsertBatch can key them.
		{Name: "autoverify", ParentTable: "device", ParentIDParam: "device_id", PathTemplate: "/device/{device_id}/autoverify", KeyField: "", PathParams: []dependentPathParamDef{
			{Param: "device_id", Field: "id"},
		}},
		// Hand-wired: org-level device rows on the live API may omit client_id
		// (community-documented quirk); walking /client/{id}/device gives the
		// fleet commands an authoritative device->client mapping fallback.
		{Name: "client_device", ParentTable: "clients", ParentIDParam: "client_id", PathTemplate: "/client/{client_id}/device", KeyField: "", PathParams: []dependentPathParamDef{
			{Param: "client_id", Field: "id"},
		}},
	}
}
...
func syncDependentResource(ctx context.Context, c interface {
	...
	for idx, parentRow := range parentRows {
		...
		for {
			...
			// function — mirroring syncResource's guard above.
			if isDryRunResponse(data) {
				if !humanFriendly {
					fmt.Fprintf(syncEvents, `{"event":"sync_dryrun","resource":"%s"}`+"\n", dep.Name)
				}
			...
			for i, item := range items {
				...
				if err := json.Unmarshal(item, &obj); err == nil {
					...
					// Hand-wired: autoverify items have no native identifier.
					// Key on device + vault + appliance so each verification
					// stream upserts to a stable row instead of being dropped
					// by id extraction.
					// Hand-wired: the typed client_device table extracts client_id
					// from the item; the generic injection only adds clients_id
					// (parent-table key), so add the canonical field too.
					if dep.Name == "client_device" {
						if _, ok := obj["client_id"]; !ok {
							obj["client_id"] = parentIDJSON
						}
					}
					if dep.Name == "autoverify" {
						if _, ok := obj["id"]; !ok {
							vaultPart := strings.Trim(string(obj["vault_id"]), `"`)
							appliancePart := strings.Trim(string(obj["appliance_id"]), `"`)
							synth := fmt.Sprintf("av:%s:%s:%s", parentID, vaultPart, appliancePart)
							// Malformed items lacking both vault_id and appliance_id
							// would otherwise collapse onto one key and silently
							// overwrite each other; keep them distinct per item.
							if vaultPart == "" && appliancePart == "" {
								synth = fmt.Sprintf("%s:%d", synth, i)
							}
							idJSON, _ := json.Marshal(synth)
							obj["id"] = idJSON
						}
					}
					for _, pathParam := range pathParams {
