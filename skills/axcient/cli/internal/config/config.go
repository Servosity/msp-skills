...
type Config struct {
	...
	AxcientApiKey string            `toml:"api_key"`
}

func Load(configPath string) (*Config, error) {
	cfg := &Config{
		BaseURL: "https://axapi.axcient.com/x360recover",
	}

	// Resolve config path
	path := configPath
	if path == "" {
		path = os.Getenv("AXCIENT_CONFIG")
	}
	if path == "" {
		home, _ := os.UserHomeDir()
		path = filepath.Join(home, ".config", "axcient-cli", "config.toml")
	}
	cfg.Path = path
	...
	// Env var overrides
	if v := os.Getenv("AXCIENT_API_KEY"); v != "" {
		cfg.AxcientApiKey = v
		cfg.markEnvOverride("AxcientApiKey")
		cfg.AuthSource = "env:AXCIENT_API_KEY"
	}
	...
	if cfg.AuthSource == "" && cfg.AxcientApiKey != "" {
		cfg.AuthSource = "config"
	}
	...
	if v := os.Getenv("AXCIENT_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	return cfg, nil
}
...
func (c *Config) SaveCredential(token string) error {
	...
	c.updateFileConfigField("AccessToken")
	c.AxcientApiKey = token
	delete(c.envOverrides, "AxcientApiKey")
	c.updateFileConfigField("AxcientApiKey")
	return c.save()
}
...
func (c *Config) configForSave() Config {
	out := *c
	if c.fileConfig != nil {
		if c.envOverrides["AxcientApiKey"] {
			out.AxcientApiKey = c.fileConfig.AxcientApiKey
		}
	}
...
func (c *Config) updateFileConfigField(field string) {
	...
	case "AxcientApiKey":
		c.fileConfig.AxcientApiKey = c.AxcientApiKey
	}
}
