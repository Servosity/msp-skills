...
func RegisterIntents(s *server.MCPServer) {
	s.AddTool(
		mcplib.NewTool("month_end_billing_reconciliation",
			mcplib.WithDescription("Protected-system counts and storage per client in one table to match against invoices."),
...
// handleMonthEndBillingReconciliation runs the month_end_billing_reconciliation recipe intent: Protected-system counts and storage per client in one table to match against invoices.
