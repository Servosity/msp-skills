# axcient skill - governance and safety model

> Unofficial. Community-built skill for the Axcient API. Not affiliated with,
> endorsed by, or sponsored by the vendor.
> This page tells an MSP owner exactly what the axcient skill can touch and how to
> scope it, so you can decide what to let an AI agent do.

## What it authenticates as

The skill drives the `axcient-cli` binary (and `axcient-mcp`),
authenticating with `AXCIENT_API_KEY`. Credentials are read from the environment only -
never written to disk, never logged, never sent anywhere except the Axcient API.
...
| **Write (routine)** | Day-to-day mutations, including bulk imports. | `vault threshold set-by-vault-id`, `import <resource>` (POSTs one create/upsert per JSONL record) | Preview with `--dry-run`, then an approved write (where a command documents its own confirm gate, use it too) |
| **Credential / security** | Touches tokens, keys, MFA. | `client vault get-d2c-agent-token-by-client-and-ids` | Human-in-the-loop only |
...
the Axcient API, and you can read every line of how it does so. The skill is
