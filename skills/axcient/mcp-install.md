# Axcient MCP - install for every agent that speaks MCP

This page wires the Axcient MCP server into any MCP client. If you use Claude
...
launch `axcient-mcp` directly on your machine - no hosting. *Remote* agents (ChatGPT,
Microsoft 365 Copilot / Copilot Studio, the Gemini app) only talk to an HTTPS
endpoint, so you expose `axcient-mcp` over HTTPS first.

## Prerequisite: install the MCP binary

Run the install command from [README.md](./README.md). It drops both `axcient-cli`
and `axcient-mcp` on your PATH. `axcient-mcp` is what the agents talk to.
...
Restart Gemini CLI; the Axcient tools appear in its tool list. (The **Gemini app /
...
All remote agents need `axcient-mcp` reachable as a public **HTTPS** endpoint. Run it
...
`axcient-mcp` you installed is not enough on its own. You also need a **Copilot Studio
license** and a **tenant admin** to enable it. This is a build-and-host task, not a
self-serve install.

Once `axcient-mcp` is hosted over HTTPS (above), the lowest-code route:
...
hermes mcp add axcient -- axcient-mcp

# OpenClaw
openclaw mcp set axcient '{"command":"axcient-mcp"}'
```
...
- `axcient-mcp: command not found`: the install dir is not on your PATH (the
