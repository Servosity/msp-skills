# Better Stack Printed CLI Agent Guide

This directory is a generated `betterstack-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
betterstack-cli doctor --json
betterstack-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
betterstack-cli which "<capability>" --json
betterstack-cli <command> --help
```
...
```bash
betterstack-cli <command> --help
betterstack-cli <command> --dry-run --agent
```
