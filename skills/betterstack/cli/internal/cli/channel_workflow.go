...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  betterstack-cli workflow archive

  # Full re-archive (ignore previous sync state)
  betterstack-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"heartbeat-groups", "heartbeats", "incidents", "monitor-groups", "monitors", "on-calls", "policies", "status-pages"}
			totalSynced := 0
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  betterstack-cli workflow status

  # Show status as JSON
  betterstack-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("betterstack-cli")
			}
