...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".local", "share", "betterstack-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("BETTERSTACK_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("BETTERSTACK_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "betterstack-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.local/share/betterstack-cli/feedback.jsonl.
When ` + "`BETTERSTACK_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`BETTERSTACK_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  betterstack-cli feedback list
  betterstack-cli feedback list --limit 5
  betterstack-cli feedback list --json`,
