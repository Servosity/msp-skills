...
Resource scoping:
  ...
  betterstack-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  betterstack-cli sync --full

  # Incremental sync: only records from the last 7 days
  betterstack-cli sync --since 7d

  # Parallel sync with 8 workers
  betterstack-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  betterstack-cli sync --latest-only`,
...
func upsertSingleObject(db *store.Store, resource string, data json.RawMessage) error {
	...
	switch resource {
	case "heartbeat-groups":
		return db.UpsertHeartbeatGroups(data)
	case "heartbeats":
		return db.UpsertHeartbeats(data)
	case "incidents":
		return db.UpsertIncidents(data)
	case "monitor-groups":
		return db.UpsertMonitorGroups(data)
	case "monitors":
	...
	case "status-page-resources":
		return db.UpsertStatusPageResources(data)
	case "status-page-sections":
		return db.UpsertStatusPageSections(data)
	case "status-pages":
