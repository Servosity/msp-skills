...
type Config struct {
	...
	BetterstackApiToken string            `toml:"api_token"`
}
...
func Load(configPath string) (*Config, error) {
	...
	path := configPath
	if path == "" {
		path = os.Getenv("BETTERSTACK_CONFIG")
	}
	if path == "" {
		home, _ := os.UserHomeDir()
		path = filepath.Join(home, ".config", "betterstack-cli", "config.toml")
	}
	cfg.Path = path
	...
	// Env var overrides
	if v := os.Getenv("BETTERSTACK_API_TOKEN"); v != "" {
		cfg.BetterstackApiToken = v
		cfg.markEnvOverride("BetterstackApiToken")
		cfg.AuthSource = "env:BETTERSTACK_API_TOKEN"
	}
	...
	if cfg.AuthSource == "" && cfg.BetterstackApiToken != "" {
		cfg.AuthSource = "config"
	}
	...
	if v := os.Getenv("BETTERSTACK_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	return cfg, nil
}
...
func (c *Config) AuthHeader() string {
	...
	if c.BetterstackApiToken != "" {
		if c.AuthSource == "" {
			c.AuthSource = "env:BETTERSTACK_API_TOKEN"
		}
		return applyAuthFormat("Bearer {token}", map[string]string{
			"api_token":             c.BetterstackApiToken,
			"BETTERSTACK_API_TOKEN": c.BetterstackApiToken,
			"token":                 c.BetterstackApiToken,
		})
	}
	if c.AccessToken != "" {
...
func (c *Config) ClearTokens() error {
	...
	delete(c.envOverrides, "BetterstackApiToken")
	return c.save()
}
...
func (c *Config) configForSave() Config {
	out := *c
	if c.fileConfig != nil {
		if c.envOverrides["BetterstackApiToken"] {
			out.BetterstackApiToken = c.fileConfig.BetterstackApiToken
		}
	}
...
func (c *Config) updateFileConfigField(field string) {
	...
	case "BetterstackApiToken":
		c.fileConfig.BetterstackApiToken = c.BetterstackApiToken
	}
}
