# betterstack skill - governance and safety model

> Unofficial. Community-built skill for the Better Stack API. Not affiliated with,
> endorsed by, or sponsored by the vendor.
> This page tells an MSP owner exactly what the betterstack skill can touch and how to
> scope it, so you can decide what to let an AI agent do.

## What it authenticates as

The skill drives the `betterstack-cli` binary (and `betterstack-mcp`),
authenticating with `BETTERSTACK_API_TOKEN`. Credentials are read from the environment only -
never written to disk, never logged, never sent anywhere except the Better Stack API.
...
the Better Stack API, and you can read every line of how it does so. The skill is
