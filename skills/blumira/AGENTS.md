# Blumira Printed CLI Agent Guide

This directory is a generated `blumira-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
blumira-cli doctor --json
blumira-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
blumira-cli which "<capability>" --json
blumira-cli <command> --help
```
...
```bash
blumira-cli <command> --help
blumira-cli <command> --dry-run --agent
```
