...
func newAuthCmd(flags *rootFlags) *cobra.Command {
	cmd := &cobra.Command{
		Use:   "auth",
		Short: "Manage authentication for Blumira",
		RunE:  parentNoSubcommandRunE(flags),
	}

	cmd.AddCommand(newNovelAuthLoginCmd(flags))
...
func newAuthSetupCmd(_ *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: "  blumira-cli auth setup\n  blumira-cli auth setup --launch",
...
func newAuthStatusCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  blumira-cli auth status",
		RunE: func(cmd *cobra.Command, args []string) error {
...
func newAuthSetTokenCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  blumira-cli auth set-token YOUR_TOKEN_HERE",
...
func newAuthLogoutCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  blumira-cli auth logout",
		RunE: func(cmd *cobra.Command, args []string) error {
			cfg, err := config.Load(flags.configPath)
			if err != nil {
				return configErr(err)
			...
			envStillSet := ""
			if envStillSet == "" && os.Getenv("BLUMIRA_API_TOKEN") != "" {
				envStillSet = "BLUMIRA_API_TOKEN"
			}
			if envStillSet == "" && os.Getenv("BLUMIRA_CLIENT_ID") != "" {
				envStillSet = "BLUMIRA_CLIENT_ID"
			}
			if envStillSet == "" && os.Getenv("BLUMIRA_CLIENT_SECRET") != "" {
				envStillSet = "BLUMIRA_CLIENT_SECRET"
			}
