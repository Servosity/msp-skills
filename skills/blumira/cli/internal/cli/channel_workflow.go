...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  blumira-cli workflow archive

  # Full re-archive (ignore previous sync state)
  blumira-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"health", "msp", "msp-accounts-findings", "msp-basis-detection-rules", "org", "org-agents-devices", "org-agents-keys", "org-detection-rules", "org-findings", "resolutions"}
			totalSynced := 0
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  blumira-cli workflow status

  # Show status as JSON
  blumira-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("blumira-cli")
			}
