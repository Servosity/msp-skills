...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  blumira-cli doctor
  blumira-cli doctor --json
  blumira-cli doctor --fail-on warn`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if cfg != nil {
				...
				if header == "" {
					...
					report["auth_hint"] = "Run 'blumira-cli auth setup' for credential setup steps."
				} else {
					authConfigured = true
			...
			if strings.Contains("Blumira JWT bearer token. Minted+cached by `auth login`, or set directly. Set this OR BLUMIRA_CLIENT_ID + BLUMIRA_CLIENT_SECRET.", " OR ") {
				authEnvOptionalNames = append(authEnvOptionalNames, "BLUMIRA_API_TOKEN")
				if os.Getenv("BLUMIRA_API_TOKEN") != "" {
					authEnvSet = append(authEnvSet, "BLUMIRA_API_TOKEN")
					authEnvOptionalSatisfied = true
				}
			} else if os.Getenv("BLUMIRA_API_TOKEN") != "" {
				authEnvSet = append(authEnvSet, "BLUMIRA_API_TOKEN")
			} else {
				authEnvInfo = append(authEnvInfo, "BLUMIRA_API_TOKEN optional")
			}
			if os.Getenv("BLUMIRA_CLIENT_ID") != "" {
				authEnvSet = append(authEnvSet, "BLUMIRA_CLIENT_ID")
			} else {
				authEnvInfo = append(authEnvInfo, "BLUMIRA_CLIENT_ID set during auth login")
			}
			if os.Getenv("BLUMIRA_CLIENT_SECRET") != "" {
				authEnvSet = append(authEnvSet, "BLUMIRA_CLIENT_SECRET")
			} else {
				authEnvInfo = append(authEnvInfo, "BLUMIRA_CLIENT_SECRET set during auth login")
			}
			switch {
			...
			if cfg != nil && cfg.BaseURL != "" {
				...
				if clientErr != nil {
					...
					if authHeader == "" {
						...
						if suggestion != "" {
							report["credentials"] = fmt.Sprintf("present, not verified. Run `%s %s` to confirm the token works end-to-end.", "blumira-cli", suggestion)
						} else {
...
func collectCacheReport(ctx context.Context, staleAfterSpec string) map[string]any {
	report := map[string]any{}
	dbPath := defaultDBPath("blumira-cli")
	...
	if err != nil {
		if os.IsNotExist(err) {
			...
			report["hint"] = "Database not created yet; run 'blumira-cli sync' to hydrate."
			return report
		}
		report["status"] = "error"
	...
	if qerr != nil {
		...
		report["status"] = "unknown"
		report["hint"] = "No sync state recorded; run 'blumira-cli sync' to populate."
		return report
	}
	defer rows.Close()
	...
	case !haveAny && len(resources) == 0:
		report["status"] = "unknown"
		report["hint"] = "sync_state is empty; run 'blumira-cli sync' to hydrate."
	case fresh:
		report["status"] = "fresh"
	default:
		report["status"] = "stale"
		report["oldest_age"] = oldest.Round(time.Minute).String()
		report["hint"] = "Some resources are older than stale_after; run 'blumira-cli sync' to refresh."
	}
	return report
}
