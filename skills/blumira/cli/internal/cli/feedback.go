...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".local", "share", "blumira-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("BLUMIRA_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("BLUMIRA_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "blumira-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.local/share/blumira-cli/feedback.jsonl.
When ` + "`BLUMIRA_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`BLUMIRA_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  blumira-cli feedback list
  blumira-cli feedback list --limit 5
  blumira-cli feedback list --json`,
