...
In local mode: searches locally synced data only.`,
  ...
  blumira-cli search "error timeout"

  # Force local search only
  blumira-cli search "payment failed" --data-source local

  # Search a specific resource type locally
  blumira-cli search "critical" --type transactions --data-source local

  # JSON output for piping
  blumira-cli search "critical" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			query := args[0]
			// This API has a search endpoint: GET /health
			if flags.dataSource != "local" {
				c, err := flags.newClient()
				if err != nil {
					return err
				}
				data, getErr := c.Get(cmd.Context(), "/health", map[string]string{
					"q": query,
				})
				if getErr == nil {
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'blumira-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
			...
			switch resourceType {
			case "msp":
				results, err = db.SearchMsp(query, limit)
			case "org":
			...
			case "":
				...
				{
					partial, searchErr := db.SearchMsp(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search msp failed: %w", searchErr)
					}
