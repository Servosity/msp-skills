...
Resource scoping:
  ...
  blumira-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  blumira-cli sync --full

  # Incremental sync: only records from the last 7 days
  blumira-cli sync --since 7d

  # Parallel sync with 8 workers
  blumira-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  blumira-cli sync --latest-only`,
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/blumira-cli/data.db)")
...
func resourceSupportsPagination(resource string) bool {
	...
	case "msp":
		return true
	case "msp-accounts-findings":
		return true
	case "msp-basis-detection-rules":
		return true
	case "org":
		return true
	case "org-agents-devices":
		return true
	case "org-agents-keys":
		return true
	case "org-detection-rules":
		return true
	case "org-findings":
		return true
	}
...
func syncResourcePath(resource string) (string, error) {
	paths := map[string]string{
		"health":                    "/health",
		"msp":                       "/msp/accounts",
		"msp-accounts-findings":     "/msp/accounts/findings",
		"msp-basis-detection-rules": "/msp/basis-detection-rules",
		"org":                       "/org/users",
		"org-agents-devices":        "/org/agents/devices",
		"org-agents-keys":           "/org/agents/keys",
		"org-detection-rules":       "/org/detection-rules",
		"org-findings":              "/org/findings",
