...
type Config struct {
	...
	BlumiraApiToken string            `toml:"api_token"`
}
...
func Load(configPath string) (*Config, error) {
	...
	path := configPath
	if path == "" {
		path = os.Getenv("BLUMIRA_CONFIG")
	}
	if path == "" {
		home, _ := os.UserHomeDir()
		path = filepath.Join(home, ".config", "blumira-cli", "config.toml")
	}
	cfg.Path = path
	...
	// Env var overrides
	if v := os.Getenv("BLUMIRA_API_TOKEN"); v != "" {
		cfg.BlumiraApiToken = v
		cfg.markEnvOverride("BlumiraApiToken")
		cfg.AuthSource = "env:BLUMIRA_API_TOKEN"
	}
	if v := os.Getenv("BLUMIRA_CLIENT_ID"); v != "" {
		cfg.ClientID = v
		cfg.markEnvOverride("ClientID")
		cfg.AuthSource = "env:BLUMIRA_CLIENT_ID"
	}
	if v := os.Getenv("BLUMIRA_CLIENT_SECRET"); v != "" {
		cfg.ClientSecret = v
		cfg.markEnvOverride("ClientSecret")
		cfg.AuthSource = "env:BLUMIRA_CLIENT_SECRET"
	}
	...
	if cfg.AuthSource == "" && cfg.BlumiraApiToken != "" {
		cfg.AuthSource = "config"
	}
	...
	if v := os.Getenv("BLUMIRA_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	return cfg, nil
}
...
func (c *Config) AuthHeader() string {
	...
	if c.BlumiraApiToken != "" {
		if c.AuthSource == "" {
			c.AuthSource = "env:BLUMIRA_API_TOKEN"
		}
		return "Bearer " + c.BlumiraApiToken
	}
	return ""
}
...
func (c *Config) ClearTokens() error {
	...
	c.updateFileConfigField("ClientSecret")
	c.BlumiraApiToken = ""
	delete(c.envOverrides, "BlumiraApiToken")
	return c.save()
}
...
func (c *Config) configForSave() Config {
	out := *c
	if c.fileConfig != nil {
		if c.envOverrides["BlumiraApiToken"] {
			out.BlumiraApiToken = c.fileConfig.BlumiraApiToken
		}
...
func (c *Config) updateFileConfigField(field string) {
	...
	case "BlumiraApiToken":
		c.fileConfig.BlumiraApiToken = c.BlumiraApiToken
	}
}
