...
func RegisterIntents(s *server.MCPServer) {
	s.AddTool(
		mcplib.NewTool("find_unprotected_domain_controllers",
			mcplib.WithDescription("List domain controllers across all accounts whose agent check-in is stale or missing."),
			mcplib.WithBoolean("flag_dc_stale", mcplib.Description("Override the recipe's --flag-dc-stale value.")),
		),
		handleFindUnprotectedDomainControllers,
	)
}

// handleFindUnprotectedDomainControllers runs the find_unprotected_domain_controllers recipe intent: List domain controllers across all accounts whose agent check-in is stale or missing.
func handleFindUnprotectedDomainControllers(ctx context.Context, req mcplib.CallToolRequest) (*mcplib.CallToolResult, error) {
	...
	args = appendRecipeBoolFlag(args, "flag-dc-stale", input["flag_dc_stale"], true)
