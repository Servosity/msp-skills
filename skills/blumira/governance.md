# blumira skill - governance and safety model

> Unofficial. Community-built skill for the Blumira API. Not affiliated with,
> endorsed by, or sponsored by the vendor.
> This page tells an MSP owner exactly what the blumira skill can touch and how to
> scope it, so you can decide what to let an AI agent do.

## What it authenticates as

The skill drives the `blumira-cli` binary (and `blumira-mcp`),
authenticating with `BLUMIRA_API_TOKEN`, `BLUMIRA_CLIENT_ID`, `BLUMIRA_CLIENT_SECRET`. Credentials are read from the environment only -
never written to disk, never logged, never sent anywhere except the Blumira API.
...
the Blumira API, and you can read every line of how it does so. The skill is
