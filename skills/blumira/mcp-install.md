# Blumira MCP - install for every agent that speaks MCP

This page wires the Blumira MCP server into any MCP client. If you use Claude
...
launch `blumira-mcp` directly on your machine - no hosting. *Remote* agents (ChatGPT,
Microsoft 365 Copilot / Copilot Studio, the Gemini app) only talk to an HTTPS
endpoint, so you expose `blumira-mcp` over HTTPS first.

## Prerequisite: install the MCP binary

Run the install command from [README.md](./README.md). It drops both `blumira-cli`
and `blumira-mcp` on your PATH. `blumira-mcp` is what the agents talk to.
...
Restart Gemini CLI; the Blumira tools appear in its tool list. (The **Gemini app /
...
All remote agents need `blumira-mcp` reachable as a public **HTTPS** endpoint.
`blumira-mcp` speaks **stdio only** - it has no HTTP mode and opens no listener of
its own - so put a stdio-to-HTTP bridge in front of it:

```bash
# ChatGPT and anything else that accepts SSE:
BLUMIRA_API_TOKEN=<value> BLUMIRA_BASE_URL=<value> BLUMIRA_CLIENT_ID=<value> BLUMIRA_CLIENT_SECRET=<value> npx -y supergateway --stdio "blumira-mcp" --port 7777
...
`blumira-mcp` you installed is not enough on its own. You also need a **Copilot Studio
license** and a **tenant admin** to enable it. This is a build-and-host task, not a
self-serve install.

Once `blumira-mcp` is hosted over HTTPS (above), the lowest-code route:
...
hermes mcp add blumira -- blumira-mcp

# OpenClaw
openclaw mcp set blumira '{"command":"blumira-mcp"}'
```
...
- `blumira-mcp: command not found`: the install dir is not on your PATH (the
