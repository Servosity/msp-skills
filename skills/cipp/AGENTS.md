# CIPP Printed CLI Agent Guide

This directory is a generated `cipp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
cipp-cli doctor --json
cipp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
cipp-cli which "<capability>" --json
cipp-cli <command> --help
```
...
```bash
cipp-cli <command> --help
cipp-cli <command> --dry-run --agent
```
