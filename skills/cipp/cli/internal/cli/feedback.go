...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".local", "share", "cipp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("CIPP_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("CIPP_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "cipp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.local/share/cipp-cli/feedback.jsonl.
When ` + "`CIPP_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`CIPP_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  cipp-cli feedback list
  cipp-cli feedback list --limit 5
  cipp-cli feedback list --json`,
