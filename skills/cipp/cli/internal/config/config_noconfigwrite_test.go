...
// touching disk. CIPP_NO_CONFIG_WRITE makes the automatic token cache a
...
const (
	ncwToken   = "minted-token-CIPP"
	ncwRefresh = "minted-refresh-CIPP"
	ncwSecret  = "minted-secret-CIPP"
)
...
func ncwMint(t *testing.T, value string, set bool) map[string]string {
	...
	home := t.TempDir()
	t.Setenv("HOME", home)
	t.Setenv("CIPP_CONFIG", filepath.Join(home, "config.toml"))
	if set {
...
func TestNoConfigWriteTruthiness(t *testing.T) {
	...
	if NoConfigWriteEnv != "CIPP_NO_CONFIG_WRITE" {
		t.Errorf("switch is named %q; the documented name is CIPP_NO_CONFIG_WRITE", NoConfigWriteEnv)
	}
}
