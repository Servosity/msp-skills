...
var blockedDestinationFlags = map[string]bool{
	"audit-dir":           true,
	"db":                  true,
	"from":                true, // `bulk --from` reads an actions CSV off this account's disk
	"from-csv":            true,
	"home":                true,
	"input":               true,
	"notes-file":          true,
...
var vendorAPIPathFlags = map[string]string{
	"cipp-cli fanout --endpoint": "names a CIPP API endpoint path such as /ListUsers, never a local file",
}
