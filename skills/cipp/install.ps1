# install.ps1 - install cipp-cli and cipp-mcp on Windows.
#
# Pulls prebuilt binaries from this skill's latest GitHub Release (cipp-v*).
...
$ReleaseBase = if ($env:MSP_SKILLS_RELEASE_BASE) {
  ...
  # Each skill is versioned/tagged independently (cipp-vX.Y.Z),
...
Write-Host "  cipp-cli --version"
Write-Host ""
Write-Host "Next:"
Write-Host "  First command + auth: https://github.com/$Owner/$Repo/tree/main/skills/cipp#readme"
Write-Host "  Claude Desktop / ChatGPT wire-up: https://github.com/$Owner/$Repo/blob/main/skills/cipp/mcp-install.md"
