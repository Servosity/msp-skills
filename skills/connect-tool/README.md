...
- "Drives the Chrome you are already logged into" means your Chrome **profile**, not necessarily the tab you had focused. OpenCLI's extension manages its own container windows, so a new, apparently blank Chrome window can appear instead ([opencli#2202](https://github.com/jackwener/opencli/issues/2202)). Your logins still apply - it is the same profile - but it looks wrong the first time, and it is why issue [#266](https://github.com/Servosity/msp-skills/issues/266) was filed. `references/browser-and-keychain.md` has the recovery order.
