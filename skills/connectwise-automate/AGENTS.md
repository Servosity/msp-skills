# ConnectWise Automate Printed CLI Agent Guide

This directory is a generated `connectwise-automate-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
connectwise-automate-cli doctor --json
connectwise-automate-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
connectwise-automate-cli which "<capability>" --json
connectwise-automate-cli <command> --help
```
...
```bash
connectwise-automate-cli <command> --agent
```
...
```bash
connectwise-automate-cli <command> --help
connectwise-automate-cli <command> --dry-run --agent
```
