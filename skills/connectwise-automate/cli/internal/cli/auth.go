...
func newAuthCmd(flags *rootFlags) *cobra.Command {
	cmd := &cobra.Command{
		Use:   "auth",
		Short: "Manage authentication for ConnectWise Automate",
...
func newAuthSetupCmd(_ *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: "  connectwise-automate-cli auth setup\n  connectwise-automate-cli auth setup --launch",
		RunE: func(cmd *cobra.Command, args []string) error {
			w := cmd.OutOrStdout()
			fmt.Fprintln(w, "Get a key at: https://docs.connectwise.com/ConnectWise_Automate")
			fmt.Fprintln(w, "  Mint a token: connectwise-automate-cli apitoken mint --username <u> --password <p> (needs CONNECTWISE_AUTOMATE_SERVER + CONNECTWISE_AUTOMATE_CLIENT_ID set). Then export CONNECTWISE_AUTOMATE_TOKEN=<AccessToken>.")
			fmt.Fprintln(w, "")
			fmt.Fprintln(w, "Then set:")
			fmt.Fprintln(w, "  export CONNECTWISE_AUTOMATE_TOKEN=\"your-token-here\"")
			fmt.Fprintln(w, "  connectwise-automate-cli auth set-token <token>")
			if !launch {
				return nil
			}
			launchURL := "https://docs.connectwise.com/ConnectWise_Automate"
...
func newAuthStatusCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  connectwise-automate-cli auth status",
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if !authed {
				...
				fmt.Fprintln(w, "  export CONNECTWISE_AUTOMATE_TOKEN=\"your-token-here\"")
				fmt.Fprintf(w, "  connectwise-automate-cli auth set-token <token>\n")
				return authErr(fmt.Errorf("no credentials configured"))
			}
...
func newAuthSetTokenCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  connectwise-automate-cli auth set-token YOUR_TOKEN_HERE",
...
func newAuthLogoutCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  connectwise-automate-cli auth logout",
		RunE: func(cmd *cobra.Command, args []string) error {
			cfg, err := config.Load(flags.configPath)
			if err != nil {
				return configErr(err)
			...
			envStillSet := ""
			if envStillSet == "" && os.Getenv("CONNECTWISE_AUTOMATE_TOKEN") != "" {
				envStillSet = "CONNECTWISE_AUTOMATE_TOKEN"
			}
