...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  connectwise-automate-cli workflow archive

  # Full re-archive (ignore previous sync state)
  connectwise-automate-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"alerts", "clients", "commands", "computers", "contacts", "groups", "locations", "monitors", "monitors-monitor-statistics", "network-devices", "patching", "patching-approval-policies", "patching-microsoft-update-policies", "patching-patch-information", "patching-third-party-update-policies", "scripts", "scripts-running-scripts", "scripts-script-schedules"}
			totalSynced := 0
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  connectwise-automate-cli workflow status

  # Show status as JSON
  connectwise-automate-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("connectwise-automate-cli")
			}
