...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  connectwise-automate-cli doctor
  connectwise-automate-cli doctor --json
  connectwise-automate-cli doctor --fail-on warn`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if cfg != nil {
				...
				if header == "" {
					...
					report["auth_hint"] = "Set it with: connectwise-automate-cli auth set-token <token> or export CONNECTWISE_AUTOMATE_TOKEN=\"your-token-here\""
					report["auth_key_url"] = "https://docs.connectwise.com/ConnectWise_Automate"
					report["auth_instructions"] = "Mint a token: connectwise-automate-cli apitoken mint --username <u> --password <p> (needs CONNECTWISE_AUTOMATE_SERVER + CONNECTWISE_AUTOMATE_CLIENT_ID set). Then export CONNECTWISE_AUTOMATE_TOKEN=<AccessToken>."
				} else {
					authConfigured = true
			...
			if os.Getenv("CONNECTWISE_AUTOMATE_TOKEN") != "" {
				authEnvSet = append(authEnvSet, "CONNECTWISE_AUTOMATE_TOKEN")
				...
				authEnvRequiredMissing = append(authEnvRequiredMissing, "CONNECTWISE_AUTOMATE_TOKEN")
			}
			switch {
			...
			if cfg != nil && cfg.BaseURL != "" {
				...
				if clientErr != nil {
					...
					if authHeader == "" {
						...
						authHeaders["User-Agent"] = "connectwise-automate-cli"
						verifyPath := "/Clients"
...
func collectCacheReport(ctx context.Context, staleAfterSpec string) map[string]any {
	...
	if err != nil {
		if os.IsNotExist(err) {
			...
			report["hint"] = "Database not created yet; run 'connectwise-automate-cli sync' to hydrate."
			return report
		}
		report["status"] = "error"
	...
	if qerr != nil {
		...
		report["status"] = "unknown"
		report["hint"] = "No sync state recorded; run 'connectwise-automate-cli sync' to populate."
		return report
	}
	defer rows.Close()
	...
	case !haveAny && len(resources) == 0:
		report["status"] = "unknown"
		report["hint"] = "sync_state is empty; run 'connectwise-automate-cli sync' to hydrate."
	case fresh:
		report["status"] = "fresh"
	default:
		report["status"] = "stale"
		report["oldest_age"] = oldest.Round(time.Minute).String()
		report["hint"] = "Some resources are older than stale_after; run 'connectwise-automate-cli sync' to refresh."
	}
	return report
}
