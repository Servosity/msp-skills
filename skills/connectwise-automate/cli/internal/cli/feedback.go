...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".local", "share", "connectwise-automate-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("CONNECTWISE_AUTOMATE_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("CONNECTWISE_AUTOMATE_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "connectwise-automate-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.local/share/connectwise-automate-cli/feedback.jsonl.
When ` + "`CONNECTWISE_AUTOMATE_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`CONNECTWISE_AUTOMATE_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  connectwise-automate-cli feedback list
  connectwise-automate-cli feedback list --limit 5
  connectwise-automate-cli feedback list --json`,
