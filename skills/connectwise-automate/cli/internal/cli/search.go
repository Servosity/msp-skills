...
In local mode: searches locally synced data only.`,
  ...
  connectwise-automate-cli search "error timeout"

  # Force local search only
  connectwise-automate-cli search "payment failed" --data-source local

  # Search a specific resource type locally
  connectwise-automate-cli search "critical" --type transactions --data-source local

  # JSON output for piping
  connectwise-automate-cli search "critical" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'connectwise-automate-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
			...
			case "commands":
				results, err = db.SearchCommands(query, limit)
			case "computers":
				results, err = db.SearchComputers(query, limit)
			case "patching":
				results, err = db.SearchPatching(query, limit)
			case "scripts":
			...
			case "":
				...
				{
					partial, searchErr := db.SearchCommands(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search commands failed: %w", searchErr)
					}
					for _, r := range partial {
						key := string(r)
						if !seen[key] {
				...
				{
					partial, searchErr := db.SearchPatching(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search patching failed: %w", searchErr)
					}
					for _, r := range partial {
						key := string(r)
						if !seen[key] {
