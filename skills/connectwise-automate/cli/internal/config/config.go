...
type Config struct {
	...
	ConnectwiseAutomateToken string            `toml:"automate_token"`
...
func Load(configPath string) (*Config, error) {
	...
	path := configPath
	if path == "" {
		path = os.Getenv("CONNECTWISE_AUTOMATE_CONFIG")
	}
	if path == "" {
		home, _ := os.UserHomeDir()
		path = filepath.Join(home, ".config", "connectwise-automate-cli", "config.toml")
	}
	cfg.Path = path
	...
	// Env var overrides
	if v := os.Getenv("CONNECTWISE_AUTOMATE_TOKEN"); v != "" {
		cfg.ConnectwiseAutomateToken = v
		cfg.markEnvOverride("ConnectwiseAutomateToken")
		cfg.AuthSource = "env:CONNECTWISE_AUTOMATE_TOKEN"
	}
	...
	if cfg.AuthSource == "" && cfg.ConnectwiseAutomateToken != "" {
		cfg.AuthSource = "config"
	}
	...
	if v := os.Getenv("CONNECTWISE_AUTOMATE_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	...
	if v := strings.TrimSpace(os.Getenv("CONNECTWISE_AUTOMATE_SERVER")); v != "" {
		cfg.TemplateVars["server"] = normalizeEndpointTemplateValue(v)
	} else {
		cfg.TemplateVars["server"] = "YOUR_SERVER.hostedrmm.com"
	}

	// clientId header (hand-wired): ConnectWise Automate v2020.11+ requires a
	// registered integration GUID sent as the `clientId` header on every
	// request. It is per-MSP, so it cannot be a static RequiredHeader baked
	// into the binary — read it from CONNECTWISE_AUTOMATE_CLIENT_ID and inject
	// it into cfg.Headers, which the generated client applies (after the
	// Authorization bearer header) to every outbound request. Older servers
	// (pre-2020.11) don't require it, so the header is only set when present.
	if v := strings.TrimSpace(os.Getenv("CONNECTWISE_AUTOMATE_CLIENT_ID")); v != "" {
		if cfg.Headers == nil {
...
func (c *Config) AuthHeader() string {
	...
	if c.ConnectwiseAutomateToken != "" {
		if c.AuthSource == "" {
			c.AuthSource = "env:CONNECTWISE_AUTOMATE_TOKEN"
		}
		return applyAuthFormat("Bearer {token}", map[string]string{
			"automate_token":             c.ConnectwiseAutomateToken,
			"CONNECTWISE_AUTOMATE_TOKEN": c.ConnectwiseAutomateToken,
			"token":                      c.ConnectwiseAutomateToken,
		})
	}
	if c.AccessToken != "" {
...
func (c *Config) ClearTokens() error {
	...
	c.updateFileConfigField("ClientID")
	c.updateFileConfigField("ClientSecret")
	c.ConnectwiseAutomateToken = ""
	delete(c.envOverrides, "ConnectwiseAutomateToken")
	return c.save()
}
...
func (c *Config) configForSave() Config {
	out := *c
	if c.fileConfig != nil {
		if c.envOverrides["ConnectwiseAutomateToken"] {
			out.ConnectwiseAutomateToken = c.fileConfig.ConnectwiseAutomateToken
		}
	}
...
func (c *Config) updateFileConfigField(field string) {
	...
	case "ConnectwiseAutomateToken":
		c.fileConfig.ConnectwiseAutomateToken = c.ConnectwiseAutomateToken
	}
}
