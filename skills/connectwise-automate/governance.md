...
> Unofficial. Community-built skill for the ConnectWise API. Not affiliated with,
...
authenticating with `CONNECTWISE_AUTOMATE_SERVER`, `CONNECTWISE_AUTOMATE_TOKEN`. Credentials are read from the environment only -
never written to disk, never logged, never sent anywhere except the ConnectWise API.
...
| **Write (routine)** | Day-to-day mutations. | `alert triage`, `computers patching-stats`, `patch compliance`, `patching`, `patching approval-policies`, `patching deploy-approved`, `patching deploy-security`, `patching information`, ... (12 total) | Preview with `--dry-run`, then an approved write (where a command documents its own confirm gate, use it too) |
| **Credential / security** | Touches tokens, keys, MFA. | `apitoken`, `apitoken mint`, `apitoken refresh` | Human-in-the-loop only |
...
the ConnectWise API, and you can read every line of how it does so. The skill is
