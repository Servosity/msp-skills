# Connectwise Control Printed CLI Agent Guide

This directory is a generated `connectwise-control-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
connectwise-control-cli doctor --json
connectwise-control-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
connectwise-control-cli which "<capability>" --json
connectwise-control-cli <command> --help
```
...
```bash
connectwise-control-cli <command> --agent
```
...
```bash
connectwise-control-cli <command> --help
connectwise-control-cli <command> --dry-run --agent
```
