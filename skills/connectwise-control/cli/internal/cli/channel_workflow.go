...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  connectwise-control-cli workflow archive

  # Full re-archive (ignore previous sync state)
  connectwise-control-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"session_groups"}
			totalSynced := 0
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  connectwise-control-cli workflow status

  # Show status as JSON
  connectwise-control-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("connectwise-control-cli")
			}
