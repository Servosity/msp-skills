...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  connectwise-control-cli doctor
  connectwise-control-cli doctor --json
  connectwise-control-cli doctor --fail-on warn`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if cfg != nil {
				...
				if header == "" {
					...
					report["auth_hint"] = "Set Basic credentials with: export CONNECTWISE_CONTROL_USERNAME=\"your-username\" CONNECTWISE_CONTROL_PASSWORD=\"your-password\""
				} else {
					authConfigured = true
			...
			if os.Getenv("CONNECTWISE_CONTROL_USERNAME") != "" {
				authEnvSet = append(authEnvSet, "CONNECTWISE_CONTROL_USERNAME")
				...
				authEnvRequiredMissing = append(authEnvRequiredMissing, "CONNECTWISE_CONTROL_USERNAME")
			}
			if os.Getenv("CONNECTWISE_CONTROL_PASSWORD") != "" {
				authEnvSet = append(authEnvSet, "CONNECTWISE_CONTROL_PASSWORD")
			} else if authConfigured {
				authSource, _ := report["auth_source"].(string)
				if authSource == "" {
					authSource = "config"
				...
				authEnvRequiredMissing = append(authEnvRequiredMissing, "CONNECTWISE_CONTROL_PASSWORD")
			}
			switch {
			...
			if cfg != nil && cfg.BaseURL != "" {
				...
				if clientErr != nil {
					...
					if authHeader == "" {
						...
						if suggestion != "" {
							report["credentials"] = fmt.Sprintf("present, not verified. Run `%s %s` to confirm the token works end-to-end.", "connectwise-control-cli", suggestion)
						} else {
...
func collectCacheReport(ctx context.Context, staleAfterSpec string) map[string]any {
	report := map[string]any{}
	dbPath := defaultDBPath("connectwise-control-cli")
	...
	if err != nil {
		if os.IsNotExist(err) {
			...
			report["hint"] = "Database not created yet; run 'connectwise-control-cli sync' to hydrate."
			return report
		}
		report["status"] = "error"
	...
	if qerr != nil {
		...
		report["status"] = "unknown"
		report["hint"] = "No sync state recorded; run 'connectwise-control-cli sync' to populate."
		return report
	}
	defer rows.Close()
	...
	case !haveAny && len(resources) == 0:
		report["status"] = "unknown"
		report["hint"] = "sync_state is empty; run 'connectwise-control-cli sync' to hydrate."
	case fresh:
		report["status"] = "fresh"
	default:
		report["status"] = "stale"
		report["oldest_age"] = oldest.Round(time.Minute).String()
		report["hint"] = "Some resources are older than stale_after; run 'connectwise-control-cli sync' to refresh."
	}
	return report
}
