...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".local", "share", "connectwise-control-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("CONNECTWISE_CONTROL_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("CONNECTWISE_CONTROL_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "connectwise-control-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.local/share/connectwise-control-cli/feedback.jsonl.
When ` + "`CONNECTWISE_CONTROL_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`CONNECTWISE_CONTROL_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  connectwise-control-cli feedback list
  connectwise-control-cli feedback list --limit 5
  connectwise-control-cli feedback list --json`,
