...
In local mode: searches locally synced data only.`,
  ...
  connectwise-control-cli search "error timeout"

  # Force local search only
  connectwise-control-cli search "payment failed" --data-source local

  # Search a specific resource type locally
  connectwise-control-cli search "critical" --type transactions --data-source local

  # JSON output for piping
  connectwise-control-cli search "critical" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'connectwise-control-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
			...
			switch resourceType {
			case "sessions":
				results, err = db.SearchSessions(query, limit)
			case "":
				...
				{
					partial, searchErr := db.SearchSessions(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search sessions failed: %w", searchErr)
					}
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/connectwise-control-cli/data.db)")
