...
Resource scoping:
  ...
  connectwise-control-cli sync

  # Sync specific resources only
  connectwise-control-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  connectwise-control-cli sync --full

  # Incremental sync: only records from the last 7 days
  connectwise-control-cli sync --since 7d

  # Parallel sync with 8 workers
  connectwise-control-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  connectwise-control-cli sync --latest-only`,
...
func upsertSingleObject(db *store.Store, resource string, data json.RawMessage) error {
	...
	case "session_groups":
		return db.UpsertSessionGroups(data)
	case "sessions":
...
func syncResourcePath(resource string) (string, error) {
	paths := map[string]string{
		"session_groups": "/Services/SessionGroupService.ashx/session-groups",
	}
