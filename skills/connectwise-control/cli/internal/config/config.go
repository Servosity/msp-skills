...
type Config struct {
	...
	ConnectwiseControlUsername string            `toml:"control_username"`
	ConnectwiseControlPassword string            `toml:"control_password"`
}

func Load(configPath string) (*Config, error) {
	cfg := &Config{
		BaseURL: "https://your-instance.screenconnect.com",
	}

	// Resolve config path
	path := configPath
	if path == "" {
		path = os.Getenv("CONNECTWISE_CONTROL_CONFIG")
	}
	if path == "" {
		home, _ := os.UserHomeDir()
		path = filepath.Join(home, ".config", "connectwise-control-cli", "config.toml")
	}
	cfg.Path = path
	...
	// Env var overrides
	if v := os.Getenv("CONNECTWISE_CONTROL_USERNAME"); v != "" {
		cfg.ConnectwiseControlUsername = v
		cfg.markEnvOverride("ConnectwiseControlUsername")
		cfg.AuthSource = "env:CONNECTWISE_CONTROL_USERNAME"
	}
	if v := os.Getenv("CONNECTWISE_CONTROL_PASSWORD"); v != "" {
		cfg.ConnectwiseControlPassword = v
		cfg.markEnvOverride("ConnectwiseControlPassword")
		cfg.AuthSource = "env:CONNECTWISE_CONTROL_PASSWORD"
	}
	...
	if cfg.AuthSource == "" && cfg.ConnectwiseControlUsername != "" {
		cfg.AuthSource = "config"
	}
	if cfg.AuthSource == "" && cfg.ConnectwiseControlPassword != "" {
		cfg.AuthSource = "config"
	}
	...
	if v := os.Getenv("CONNECTWISE_CONTROL_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	return cfg, nil
}
...
func (c *Config) AuthHeader() string {
	...
	if c.ConnectwiseControlUsername == "" || c.ConnectwiseControlPassword == "" {
		return ""
	}
	credentials := c.ConnectwiseControlUsername + ":" + c.ConnectwiseControlPassword
...
func (c *Config) SaveCredential(token string) error {
	...
	c.updateFileConfigField("AccessToken")
	c.ConnectwiseControlUsername = token
	delete(c.envOverrides, "ConnectwiseControlUsername")
	c.updateFileConfigField("ConnectwiseControlUsername")
	return c.save()
}
...
func (c *Config) ClearTokens() error {
	...
	c.updateFileConfigField("ClientID")
	c.updateFileConfigField("ClientSecret")
	c.ConnectwiseControlUsername = ""
	delete(c.envOverrides, "ConnectwiseControlUsername")
	c.ConnectwiseControlPassword = ""
	delete(c.envOverrides, "ConnectwiseControlPassword")
	return c.save()
}
...
func (c *Config) configForSave() Config {
	out := *c
	if c.fileConfig != nil {
		if c.envOverrides["ConnectwiseControlUsername"] {
			out.ConnectwiseControlUsername = c.fileConfig.ConnectwiseControlUsername
		}
		if c.envOverrides["ConnectwiseControlPassword"] {
			out.ConnectwiseControlPassword = c.fileConfig.ConnectwiseControlPassword
		}
	}
...
func (c *Config) updateFileConfigField(field string) {
	...
	case "ConnectwiseControlUsername":
		c.fileConfig.ConnectwiseControlUsername = c.ConnectwiseControlUsername
	case "ConnectwiseControlPassword":
		c.fileConfig.ConnectwiseControlPassword = c.ConnectwiseControlPassword
	}
}
