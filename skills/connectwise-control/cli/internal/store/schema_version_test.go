...
// TestMigrate_AddsColumnsOnUpgrade_SessionGroups verifies that opening a
// database created by an older binary succeeds and adds newly generated
// columns before CREATE INDEX runs against the pre-existing table. Regression
// coverage for parent_id upgrades and indexed generated columns.
func TestMigrate_AddsColumnsOnUpgrade_SessionGroups(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	...
	if _, err := raw.Exec(`CREATE TABLE "session_groups" (
		id TEXT PRIMARY KEY,
		data JSON NOT NULL,
		synced_at DATETIME DEFAULT CURRENT_TIMESTAMP
	)`); err != nil {
	...
	// The migration must have added every generated column.
	rows, err := s.DB().Query(`PRAGMA table_info("session_groups")`)
	if err != nil {
		t.Fatalf("table_info: %v", err)
	}
	defer rows.Close()
	...
	for _, want := range []string{
		...
		if !hasColumn[want] {
			t.Fatalf("%s column missing from session_groups after migrate", want)
		}
	}
}
...
func TestMigrate_AddsColumnsOnUpgrade_Sessions(t *testing.T) {
	...
	for _, want := range []string{
		...
		"url",
		"active_connections",
		"guest_logged_on_user_name",
		"guest_machine_name",
		"guest_network_address",
		"guest_operating_system_name",
		"host",
		"is_public",
		"last_connected_event_time",
		"name",
		"session_id",
		"session_type",
	} {
