# ConnectWise MCP - install for every agent that speaks MCP

This page wires the ConnectWise MCP server into any MCP client. If you use Claude
...
launch `connectwise-control-mcp` directly on your machine - no hosting. *Remote* agents (ChatGPT,
Microsoft 365 Copilot / Copilot Studio, the Gemini app) only talk to an HTTPS
endpoint, so you expose `connectwise-control-mcp` over HTTPS first.

## Prerequisite: install the MCP binary

Run the install command from [README.md](./README.md). It drops both `connectwise-control-cli`
and `connectwise-control-mcp` on your PATH. `connectwise-control-mcp` is what the agents talk to.

```bash
connectwise-control-mcp --help
```
...
```json
{
  "mcpServers": {
    "connectwise-control": {
      "command": "connectwise-control-mcp",
      "env": {
        "CONNECTWISE_CONTROL_BASE_URL": "<your-connectwise_control_base_url>",
        "CONNECTWISE_CONTROL_USERNAME": "<your-connectwise_control_username>",
        "CONNECTWISE_CONTROL_PASSWORD": "<your-connectwise_control_password>"
      }
    }
  }
}
...
Restart Gemini CLI; the ConnectWise tools appear in its tool list. (The **Gemini app /
...
All remote agents need `connectwise-control-mcp` reachable as a public **HTTPS** endpoint. Run it
...
`connectwise-control-mcp` you installed is not enough on its own. You also need a **Copilot Studio
license** and a **tenant admin** to enable it. This is a build-and-host task, not a
self-serve install.

Once `connectwise-control-mcp` is hosted over HTTPS (above), the lowest-code route:
...
hermes mcp add connectwise-control -- connectwise-control-mcp

# OpenClaw
openclaw mcp set connectwise-control '{"command":"connectwise-control-mcp"}'
```
...
- `connectwise-control-mcp: command not found`: the install dir is not on your PATH (the
