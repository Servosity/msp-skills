...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  connectwise-manage-cli doctor
  connectwise-manage-cli doctor --json
  connectwise-manage-cli doctor --fail-on warn`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if cfg != nil {
				cwMissing := []string{}
				cwPresent := []string{}
				checkCW := func(name, val string) {
					if strings.TrimSpace(val) != "" {
						cwPresent = append(cwPresent, name)
					} else {
						cwMissing = append(cwMissing, name)
					}
				}
				checkCW("CW_COMPANY_ID", cfg.CompanyID)
				checkCW("CW_PUBLIC_KEY", cfg.PublicKey)
				checkCW("CW_PRIVATE_KEY", cfg.PrivateKey)
				checkCW("CW_CLIENT_ID", cfg.ClientID)

				switch {
				case len(cwMissing) == 0:
					report["auth"] = "configured"
					report["auth_source"] = cfg.AuthSource
					report["env_vars"] = fmt.Sprintf("OK %d/4 credentials present", len(cwPresent))
				case len(cwPresent) == 0:
					report["auth"] = "not configured"
					report["auth_hint"] = "set CW_COMPANY_ID, CW_PUBLIC_KEY, CW_PRIVATE_KEY, CW_CLIENT_ID — Basic auth is companyId+publicKey:privateKey, plus the clientId header"
					report["env_vars"] = "ERROR missing required: " + strings.Join(cwMissing, ", ")
				default:
					report["auth"] = "incomplete (missing credentials)"
					report["auth_hint"] = "missing " + strings.Join(cwMissing, ", ") + " — all four credentials are required for live calls"
					report["env_vars"] = "ERROR missing required: " + strings.Join(cwMissing, ", ")
				}
			}
			...
			if cfg != nil && cfg.BaseURL != "" {
				...
				if clientErr != nil {
					...
					if authHeader == "" {
						...
						if suggestion != "" {
							report["credentials"] = fmt.Sprintf("present, not verified. Run `%s %s` to confirm the token works end-to-end.", "connectwise-manage-cli", suggestion)
						} else {
...
func collectCacheReport(ctx context.Context, staleAfterSpec string) map[string]any {
	...
	if err != nil {
		if os.IsNotExist(err) {
			...
			report["hint"] = "Database not created yet; run 'connectwise-manage-cli sync' to hydrate."
			return report
		}
		report["status"] = "error"
	...
	if qerr != nil {
		...
		report["status"] = "unknown"
		report["hint"] = "No sync state recorded; run 'connectwise-manage-cli sync' to populate."
		return report
	}
	defer rows.Close()
	...
	case !haveAny && len(resources) == 0:
		report["status"] = "unknown"
		report["hint"] = "sync_state is empty; run 'connectwise-manage-cli sync' to hydrate."
	case fresh:
		report["status"] = "fresh"
	default:
		report["status"] = "stale"
		report["oldest_age"] = oldest.Round(time.Minute).String()
		report["hint"] = "Some resources are older than stale_after; run 'connectwise-manage-cli sync' to refresh."
	}
	return report
}
