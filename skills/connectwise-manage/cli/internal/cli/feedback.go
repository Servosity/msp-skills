...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".local", "share", "connectwise-manage-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("CONNECTWISE_MANAGE_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("CONNECTWISE_MANAGE_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "connectwise-manage-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.local/share/connectwise-manage-pp-cli/feedback.jsonl.
When ` + "`CONNECTWISE_MANAGE_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`CONNECTWISE_MANAGE_FEEDBACK_AUTO_SEND=true`" + `, the entry is
