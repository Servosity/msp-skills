...
In local mode: searches locally synced data only.`,
  ...
  connectwise-manage-cli search "error timeout"

  # Force local search only
  connectwise-manage-cli search "payment failed" --data-source local

  # Search a specific resource type locally
  connectwise-manage-cli search "critical" --type transactions --data-source local

  # JSON output for piping
  connectwise-manage-cli search "critical" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			query := args[0]
			// This API has a search endpoint: POST /project/tickets/search
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'connectwise-manage-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
			...
			case "procurement":
				results, err = db.SearchProcurement(query, limit)
			case "project":
				results, err = db.SearchProject(query, limit)
			case "sales":
				results, err = db.SearchSales(query, limit)
			case "service":
				results, err = db.SearchService(query, limit)
			case "system":
				results, err = db.SearchSystem(query, limit)
			case "time":
				results, err = db.SearchTime(query, limit)
			case "":
				...
				{
					partial, searchErr := db.SearchProcurement(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search procurement failed: %w", searchErr)
					}
					for _, r := range partial {
						key := string(r)
						if !seen[key] {
				...
				{
					partial, searchErr := db.SearchSales(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search sales failed: %w", searchErr)
					}
					for _, r := range partial {
						key := string(r)
						if !seen[key] {
				...
				{
					partial, searchErr := db.SearchTime(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search time failed: %w", searchErr)
					}
					for _, r := range partial {
						key := string(r)
						if !seen[key] {
