...
| **Write (routine)** | Day-to-day mutations. | `company get-companies-by-parent-id-management-report-setup`, `company patch-companies-by-id`, `company patch-companies-statuses-by-id`, `company patch-companies-types-by-id`, `company patch-configurations-by-id`, `company patch-configurations-by-id-change-type`, `company patch-configurations-statuses-by-id`, `company patch-configurations-types-by-id`, ... (39 total) | Preview with `--dry-run`, then an approved write (where a command documents its own confirm gate, use it too) |
