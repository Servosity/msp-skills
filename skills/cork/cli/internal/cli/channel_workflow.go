...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  cork-cli workflow archive

  # Full re-archive (ignore previous sync state)
  cork-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"clients", "distributor", "integrations-connected", "invoices", "software-installer-packages", "warranties"}
			archiveMaxPages := 100
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  cork-cli workflow status

  # Show status as JSON
  cork-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("cork-cli")
			}
