...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  cork-cli doctor
  cork-cli doctor --json
  cork-cli doctor --fail-on warn
  cork-cli doctor --fail-on stale`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if cfg != nil {
				...
				if !configured {
					...
					report["auth_hint"] = "Set it with: cork-cli auth set-token <token> or export CORK_API_KEY=\"your-token-here\""
					report["auth_key_url"] = "https://help.corkinc.com/articles/3092729300-cork-public-api-mcp-server"
				} else {
					authConfigured = true
			...
			if os.Getenv("CORK_API_KEY") != "" {
				authEnvSet = append(authEnvSet, "CORK_API_KEY")
				...
				authEnvRequiredMissing = append(authEnvRequiredMissing, "CORK_API_KEY")
			}
			switch {
			...
			if cfg != nil && doctorBaseURLIsPlaceholder(cfg.BaseURL) {
				...
				if clientErr != nil {
					...
					if authHeader == "" {
						...
						authHeaders["User-Agent"] = "cork-cli"
						verifyPath := "/me"
						...
						case errors.As(authErr, &authAPIErr):
							...
							default:
								...
								// 404 additionally corrects the api row: it means the probe
								// never found this vendor's API at the configured base_url,
								// so leaving api green next to it reads as a clean bill of
								// health for a misconfigured install.
							}
						default:
...
func collectCacheReport(ctx context.Context, staleAfterSpec string) map[string]any {
	...
	if err != nil {
		if os.IsNotExist(err) {
			...
			report["hint"] = "Database not created yet; run 'cork-cli sync' to hydrate."
			return report
		}
		report["status"] = "error"
	...
	if qerr != nil {
		...
		report["status"] = "unknown"
		report["hint"] = "No sync state recorded; run 'cork-cli sync' to populate."
		return report
	}
	defer rows.Close()
	...
	case !haveAny && len(resources) == 0:
		...
		report["hint"] = "No sync recorded; run 'cork-cli sync' to hydrate API-backed resources. Rows written by other paths are not tracked in sync_state."
	case fresh:
		report["status"] = "fresh"
	default:
		report["status"] = "stale"
		report["oldest_age"] = oldest.Round(time.Minute).String()
		report["hint"] = "Some resources are older than stale_after; run 'cork-cli sync' to refresh."
	}
	return report
}
