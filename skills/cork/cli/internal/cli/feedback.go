...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("CORK_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("CORK_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "cork-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		// pp:patch generated-file hand-fix — the feedback parent shipped without
		// an Examples section, which Phase 5 dogfood flags. `list` leads so the
		// example is read-only. Retro candidate against the generator template.
		Example: `  cork-cli feedback list
  cork-cli feedback "coverage gaps needed a synced mirror before it was useful"`,
		// `feedback [text]` accepts any free text, so there is no such thing as an
		// invalid argument to reject; the error-path probe has nothing to assert.
...
When ` + "`CORK_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`CORK_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  cork-cli feedback list
  cork-cli feedback list --limit 5
  cork-cli feedback list --json`,
