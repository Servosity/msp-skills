...
type rootFlags struct {
	...
	// invocation. Mirrors the CORK_NO_LEARN env var.
	noLearn bool
...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "cork-cli",
		Short: `Cork CLI — Every Cork API operation, plus cross-client risk attribution, exploitability-first triage, and stale-connector…`,
		Long: `Cork CLI — Every Cork API operation, plus cross-client risk attribution, exploitability-first triage, and stale-connector detection that a stateless API mirror cannot do.
...
Health check: run 'cork-cli doctor' to verify auth and connectivity.
See README.md or the bundled SKILL.md for recipes.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("cork-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.selectFields, "select", "", "Comma-separated fields to include in output (e.g. --select associated_tenants,created_at,hidden)")
	...
	rootCmd.PersistentFlags().StringVar(&flags.runProfileName, "profile", "", "Apply values from a saved run profile; this does not select a client (see 'cork-cli profile list')")
	...
	rootCmd.AddCommand(newDistributorCmd(flags))
	...
	rootCmd.AddCommand(newWarrantiesPromotedCmd(flags))
