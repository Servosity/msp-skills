...
Run sync first to populate the local search index.`,
  ...
  cork-cli search "status"

  # Force local search explicitly
  cork-cli search "status" --data-source local
  # Search a specific resource type locally
  cork-cli search "status" --type clients --data-source local
  # JSON output for piping
  cork-cli search "status" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'cork-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
