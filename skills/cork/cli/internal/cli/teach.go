...
// cork-cli CLI: `teach` (fire-and-forget, silent on success,
...
import (
	...
	"cork-pp-cli/internal/learn/patterns"
...
const noLearnEnvVar = "CORK_NO_LEARN"
...
Disabling: pass --no-learn or set ` + noLearnEnvVar + `=true.`,
		Example: `  cork-cli teach --query "<question>" --resource-type <type> --resource <id> --resource <id> &`,
...
when learnings exist.`,
		Example: `  cork-cli recall "<question>" --agent
  cork-cli recall "<question>" --agent --min-confidence 2`,
...
func newLearningsListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  cork-cli learnings list --agent
  cork-cli learnings list --query "<substring>"
  cork-cli learnings list --warnings --agent`,
...
Requires at least one of --resource, --action, or --all.`,
		Example: `  cork-cli learnings forget "<question>" --resource <id>
  cork-cli learnings forget "<question>" --all`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if len(args) == 0 {
				return cmd.Help()
			}
...
a whole family.`,
		Example: `  cork-cli teach-pattern --query-template "items in {entity}" --resource-template "GROUP-{entity:category}" --resource-type "items" --entity-kind "category" --strategy substitute`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dryRunOK(flags) {
...
cannot be taught — they are derived from the canonical input.`,
		Example: `  cork-cli teach-lookup --kind country --canonical "United States" --value USA`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dryRunOK(flags) {
