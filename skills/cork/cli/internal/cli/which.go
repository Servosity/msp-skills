...
var whichIndex = []whichEntry{
	{Command: "score attribute", Description: "Explain why a client's Cork score moved, broken out by claims, compliance, coverage, and vulnerability impact.", Group: "Risk that compounds locally", WhyItMatters: "Reach for this when someone asks why a client got worse, instead of pulling raw score points and eyeballing a trend."},
	{Command: "score regressions", Description: "Rank every client by score change over a window, worst movers first.", Group: "Risk that compounds locally", WhyItMatters: "Use this to open a book-of-business review; it answers 'who moved backwards' in one call instead of one call per client."},
	{Command: "vulnerabilities triage", Description: "Rank software products by exploitability (known-exploited first, then EPSS, then CVSS) with a blast-radius device count and client names resolved.", Group: "Exploitability over CVSS", WhyItMatters: "Pick this over a raw vulnerability list when deciding what to patch first. Live-only: Cork's vulnerability rows carry no id so they cannot be mirrored locally, --limit counts products rather than individual findings, and a capped scan sets scan_cap_hit."},
	{Command: "vulnerabilities exposure", Description: "List every affected client, device, product, and version for a single CVE id.", Group: "Exploitability over CVSS", WhyItMatters: "Use this the moment a specific CVE is named in an advisory or a client email; it is the only way to answer 'are we exposed'. Exit 3 means the scan succeeded and found no exposure, which is a negative answer rather than a failure. Live-only: vulnerability rows carry no id so there is no local mirror to read, and a capped scan sets scan_cap_hit, which is explicitly not a clean bill of health."},
	{Command: "compliance overdue", Description: "Surface compliance events that have blown their event type's remediation window, bucketed by age.", Group: "Data you can actually trust", WhyItMatters: "Choose this over listing a client's events when you need the ones that are actively costing score, ordered by how late they are."},
	{Command: "integrations health", Description: "Flag connectors that are down, degraded, or reporting healthy while their last sync has gone stale, and name the clients they feed.", Group: "Data you can actually trust", WhyItMatters: "Run this before trusting any risk number; a silently stale connector makes every downstream score fiction."},
	{Command: "coverage gaps", Description: "Diff the devices a connector reports against the devices attributed to the client to expose endpoints one tool sees and another misses.", Group: "Data you can actually trust", WhyItMatters: "Use this during onboarding verification or when coverage impact is dragging a score and you need the specific unmonitored endpoints. Exit 3 means there were no connector devices to diff, a negative answer rather than a failure; the command refuses outright rather than reporting gaps against a device baseline it could not read."},
	{Command: "warranties exposure", Description: "Rank unwarranted or lapsed clients by current risk so coverage conversations start with the ones that need it most.", Group: "Commercial signal", WhyItMatters: "Reach for this when preparing commercial outreach, rather than listing warranties and cross-checking risk by hand."},
}
...
Exit codes:
  ...
  2  no confident match - the query did not score against any indexed capability; fall back to '--help' or 'search' if this CLI has one`,
		Example: `  cork-cli which "stale tickets"
  cork-cli which "bottleneck"
  cork-cli which --limit 1 "send message"
  cork-cli which                                # list the full capability index`,
