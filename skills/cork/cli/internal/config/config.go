...
type Config struct {
	...
	CorkApiKey       string          `toml:"api_key"`
}

func Load(configPath string) (*Config, error) {
	cfg := &Config{
		BaseURL: "https://api.corkinc.com/api/v1",
	}
	...
	// Env var overrides
	if v := os.Getenv("CORK_API_KEY"); v != "" {
		cfg.CorkApiKey = v
		cfg.markEnvOverride("CorkApiKey")
		cfg.AuthSource = "env:CORK_API_KEY"
		cfg.CredentialSource = "env:CORK_API_KEY"
	}
	...
	if v := os.Getenv("CORK_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	return cfg, nil
}
...
func resolveConfigPath(configPath string) (string, bool, error) {
	...
	if path := os.Getenv("CORK_CONFIG"); path != "" {
...
func LegacyConfigPath() (string, error) {
	...
	return filepath.Join(home, ".config", "cork-cli", "config.toml"), nil
}
...
func (c *Config) AuthHeader() string {
	...
	if c.CorkApiKey != "" {
		if c.AuthSource == "" {
			c.AuthSource = "env:CORK_API_KEY"
		}
		return "Bearer " + c.CorkApiKey
	}
	if c.AccessToken != "" {
...
func (c *Config) ClearTokens() error {
	...
	c.updateFileConfigField("ClientSecret")
	c.CorkApiKey = ""
	delete(c.envOverrides, "CorkApiKey")
	c.updateFileConfigField("CorkApiKey")
	if c.AgentcookieManagedByExternalStore() {
		c.markAgentcookieManaged()
...
func (c *Config) configForSave() Config {
	out := *c
	if c.fileConfig != nil {
		if c.envOverrides["CorkApiKey"] {
			out.CorkApiKey = c.fileConfig.CorkApiKey
		}
	}
...
func (c *Config) updateFileConfigField(field string) {
	...
	case "CorkApiKey":
		c.fileConfig.CorkApiKey = c.CorkApiKey
	}
}
