# install.ps1 - install cork-cli and cork-mcp on Windows.
#
# Pulls prebuilt binaries from this skill's latest GitHub Release (cork-v*).
...
$ReleaseBase = if ($env:MSP_SKILLS_RELEASE_BASE) {
  ...
  # Each skill is versioned/tagged independently (cork-vX.Y.Z),
...
Write-Host "  cork-cli --version"
Write-Host ""
Write-Host "Next:"
Write-Host "  First command + auth: https://github.com/$Owner/$Repo/tree/main/skills/cork#readme"
Write-Host "  Claude Desktop / ChatGPT wire-up: https://github.com/$Owner/$Repo/blob/main/skills/cork/mcp-install.md"
