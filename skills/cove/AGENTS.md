# Cove Data Protection Printed CLI Agent Guide

This directory is a generated `cove-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
cove-cli doctor --json
cove-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
cove-cli which "<capability>" --json
cove-cli <command> --help
```
...
```bash
cove-cli <command> --help
cove-cli <command> --dry-run --agent
```
