...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".local", "share", "cove-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("COVE_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("COVE_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "cove-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.local/share/cove-cli/feedback.jsonl.
When ` + "`COVE_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`COVE_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  cove-cli feedback list
  cove-cli feedback list --limit 5
  cove-cli feedback list --json`,
