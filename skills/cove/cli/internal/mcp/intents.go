...
func RegisterIntents(s *server.MCPServer) {
	s.AddTool(
		mcplib.NewTool("month_end_billing_export",
			mcplib.WithDescription("Per-device SKU, used storage, and M365 seats as CSV for the invoice model."),
			mcplib.WithBoolean("csv", mcplib.Description("Override the recipe's --csv value.")),
		),
		handleMonthEndBillingExport,
	)
}

// handleMonthEndBillingExport runs the month_end_billing_export recipe intent: Per-device SKU, used storage, and M365 seats as CSV for the invoice model.
func handleMonthEndBillingExport(ctx context.Context, req mcplib.CallToolRequest) (*mcplib.CallToolResult, error) {
