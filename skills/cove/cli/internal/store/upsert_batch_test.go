...
// TestUpsertBatch_SetsPartnersParentID verifies that dependent-resource
...
func TestUpsertBatch_SetsPartnersParentID(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	s, err := Open(dbPath)
	if err != nil {
		t.Fatalf("open: %v", err)
	...
	var matchedA int
	parentQuery := fmt.Sprintf(`SELECT COUNT(*) FROM "%s" WHERE parent_id = ?`, "partners")
...
// TestUpsertBatch_PartnersSearchableViaGenericFTS verifies that a
...
// non-FTS5 `--type partners` fallback), so populating it is the
...
func TestUpsertBatch_PartnersSearchableViaGenericFTS(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	s, err := Open(dbPath)
	if err != nil {
		t.Fatalf("open: %v", err)
	...
	// field is matchable regardless of which columns partners declares.
	const probe = "zqxjpp2629probe"
	items := []json.RawMessage{
		json.RawMessage(`{"id": "search-001", "pp_fts_probe": "` + probe + `", "parent_id": "parent-A"}`),
	}
	stored, _, err := s.UpsertBatch("partners", items)
	if err != nil {
		t.Fatalf("UpsertBatch: %v", err)
	}
	...
	if len(results) == 0 {
		t.Fatalf("dependent resource %q stored but not searchable via generic resources_fts (issue #2629)", "partners")
	}
}
