# CrowdStrike Falcon Printed CLI Agent Guide

This directory is a generated `crowdstrike-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
crowdstrike-cli doctor --json
crowdstrike-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
crowdstrike-cli which "<capability>" --json
crowdstrike-cli <command> --help
```
...
```bash
crowdstrike-cli <command> --help
crowdstrike-cli <command> --dry-run --agent
```
