...
func newAuthCmd(flags *rootFlags) *cobra.Command {
	cmd := &cobra.Command{
		Use:   "auth",
		Short: "Manage authentication for Crowdstrike Falcon Msp",
...
Mint an OAuth2 bearer token via POST https://api.crowdstrike.com/oauth2/token with
grant_type=client_credentials and cache it on disk. Subsequent commands
auto-refresh the token before expiry.

Credentials default to FALCON_CLIENT_ID (Client ID) and FALCON_CLIENT_SECRET (Client Secret) environment variables.
`),
  ...
  crowdstrike-cli auth login --client-id <id> --client-secret <secret>
`, "\n"),
		Annotations: map[string]string{"mcp:hidden": "true"},
		RunE: func(cmd *cobra.Command, args []string) error {
			// issue #270: CROWDSTRIKE_NO_CONFIG_WRITE makes the automatic token
			...
			if clientID == "" {
				clientID = strings.TrimSpace(os.Getenv("FALCON_CLIENT_ID"))
			}
			if clientSecret == "" {
				clientSecret = strings.TrimSpace(os.Getenv("FALCON_CLIENT_SECRET"))
			}
			if clientID == "" || clientSecret == "" {
				return authErr(fmt.Errorf("client ID and secret required (set --client-id/--client-secret or FALCON_CLIENT_ID/FALCON_CLIENT_SECRET)"))
			}
	...
	cmd.Flags().StringVar(&clientID, "client-id", "", "OAuth2 Client ID; defaults to $FALCON_CLIENT_ID")
	cmd.Flags().StringVar(&clientSecret, "client-secret", "", "OAuth2 Client Secret; defaults to $FALCON_CLIENT_SECRET")
...
func resolveClientCredentialsScope() string {
	if scope := os.Getenv("CROWDSTRIKE_OAUTH_SCOPE"); scope != "" {
		return scope
	}
	return ""
}
...
func newAuthStatusCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  crowdstrike-cli auth status",
		RunE: func(cmd *cobra.Command, args []string) error {
			cfg, err := config.Load(flags.configPath)
			if err != nil {
				return configErr(err)
			...
			if !authed {
				...
				fmt.Fprintln(w, "  export FALCON_CLIENT_ID=\"<client-id>\"")
				fmt.Fprintln(w, "  export FALCON_CLIENT_SECRET=\"<client-secret>\"")
				fmt.Fprintf(w, "  crowdstrike-cli auth login\n")
				return authErr(fmt.Errorf("no credentials configured"))
			}
...
func newAuthSetTokenCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  crowdstrike-cli auth set-token <bearer-jwt>",
...
func newAuthLogoutCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  crowdstrike-cli auth logout",
		RunE: func(cmd *cobra.Command, args []string) error {
			cfg, err := config.Load(flags.configPath)
			if err != nil {
				return configErr(err)
			...
			envStillSet := ""
			if envStillSet == "" && os.Getenv("FALCON_CLIENT_ID") != "" {
				envStillSet = "FALCON_CLIENT_ID"
			}
			if envStillSet == "" && os.Getenv("FALCON_CLIENT_SECRET") != "" {
				envStillSet = "FALCON_CLIENT_SECRET"
			}
