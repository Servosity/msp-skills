...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".local", "share", "crowdstrike-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("CROWDSTRIKE_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("CROWDSTRIKE_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "crowdstrike-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.local/share/crowdstrike-cli/feedback.jsonl.
When ` + "`CROWDSTRIKE_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`CROWDSTRIKE_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  crowdstrike-cli feedback list
  crowdstrike-cli feedback list --limit 5
  crowdstrike-cli feedback list --json`,
