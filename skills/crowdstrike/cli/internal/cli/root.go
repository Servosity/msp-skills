...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "crowdstrike-cli",
		Short: `Crowdstrike CLI — Every CrowdStrike Falcon MSP operation, plus a Flight-Control-aware local store that answers fleet-wide questions acros…`,
		Long: `Crowdstrike CLI — Every CrowdStrike Falcon MSP operation, plus a Flight-Control-aware local store that answers fleet-wide questions acros…

Highlights (not in the official API docs):
  • fleet sync   Pull hosts, alerts, vulnerabilities, policies, and the Flight Control fabric from every child tenant into one local store keyed by CID. Requires a parent-CID API client with Flight Control (MSSP) sco…
...
Health check: run 'crowdstrike-cli doctor' to verify auth and connectivity.
See README.md or the bundled SKILL.md for recipes.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("crowdstrike-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.profileName, "profile", "", "Apply values from a saved profile (see 'crowdstrike-cli profile list')")
	...
	rootCmd.AddCommand(newMsspCmd(flags))
	rootCmd.AddCommand(newPolicyCmd(flags))
	rootCmd.AddCommand(newSpotlightCmd(flags))
