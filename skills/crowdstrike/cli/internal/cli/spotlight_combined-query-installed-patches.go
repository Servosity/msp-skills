...
func newSpotlightCombinedQueryInstalledPatchesCmd(flags *rootFlags) *cobra.Command {
	...
	cmd.Flags().IntVar(&flagLimit, "limit", 0, "Maximum number of entities to return.")
	cmd.Flags().StringVar(&flagSort, "sort", "", "Sort installed patches by their properties.")
	cmd.Flags().StringVar(&flagFilter, "filter", "", "Filter items using a query in Falcon Query Language (FQL).")
