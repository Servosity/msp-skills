...
// touching disk. CROWDSTRIKE_NO_CONFIG_WRITE makes the automatic token cache a
...
const (
	ncwToken   = "minted-token-CROWDSTRIKE"
	ncwRefresh = "minted-refresh-CROWDSTRIKE"
	ncwSecret  = "minted-secret-CROWDSTRIKE"
)
...
func ncwMint(t *testing.T, value string, set bool) map[string]string {
	...
	home := t.TempDir()
	t.Setenv("HOME", home)
	t.Setenv("CROWDSTRIKE_CONFIG", filepath.Join(home, "config.toml"))
	if set {
...
func TestNoConfigWriteStillAllowsTheWipe(t *testing.T) {
	...
	t.Setenv("HOME", home)
	t.Setenv("CROWDSTRIKE_CONFIG", cfgPath)
...
func TestNoConfigWriteTruthiness(t *testing.T) {
	...
	if NoConfigWriteEnv != "CROWDSTRIKE_NO_CONFIG_WRITE" {
		t.Errorf("switch is named %q; the documented name is CROWDSTRIKE_NO_CONFIG_WRITE", NoConfigWriteEnv)
	}
}
