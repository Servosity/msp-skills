...
// resolver the CLI uses, which reads CROWDSTRIKE_CONFIG and applies
...
func TestMCPClientHonoursTheConfigEnvVar(t *testing.T) {
	...
	t.Setenv("HOME", dir)
	t.Setenv("CROWDSTRIKE_CONFIG", path)
	// Guard against a stray ambient override changing the assertion.
	os.Unsetenv("CROWDSTRIKE_BASE_URL")
	...
	if c.Config.Path != path {
		t.Errorf("MCP server read %q, want the CROWDSTRIKE_CONFIG path %q", c.Config.Path, path)
	}
