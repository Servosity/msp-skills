# install.ps1 - install crowdstrike-cli and crowdstrike-mcp on Windows.
#
# Pulls prebuilt binaries from this skill's latest GitHub Release (crowdstrike-v*).
...
$Skill   = "crowdstrike"
$CliBin  = "crowdstrike-cli.exe"
$McpBin  = "crowdstrike-mcp.exe"
...
$ReleaseBase = if ($env:MSP_SKILLS_RELEASE_BASE) {
  ...
  # Each skill is versioned/tagged independently (crowdstrike-vX.Y.Z),
...
Write-Host "  crowdstrike-cli --version"
Write-Host ""
Write-Host "Next:"
Write-Host "  First command + auth: https://github.com/$Owner/$Repo/tree/main/skills/crowdstrike#readme"
Write-Host "  Claude Desktop / ChatGPT wire-up: https://github.com/$Owner/$Repo/blob/main/skills/crowdstrike/mcp-install.md"
