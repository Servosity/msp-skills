...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  datagate-cli workflow archive

  # Full re-archive (ignore previous sync state)
  datagate-cli workflow archive --full

  # Archive without a wall-clock timeout
  datagate-cli workflow archive --timeout 0`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"account-managers", "agreements", "assignments", "customer-users", "customers", "delivery-methods", "invoices", "kit-templates", "product-templates", "product-transactions", "products", "rate-cards", "service-items", "sites"}
			if cliutil.IsDogfoodEnv() {
				if len(resources) > 3 {
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  datagate-cli workflow status

  # Show status as JSON
  datagate-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("datagate-cli")
			}
