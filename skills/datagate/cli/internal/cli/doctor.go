...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  datagate-cli doctor
  datagate-cli doctor --json
  datagate-cli doctor --fail-on warn
  datagate-cli doctor --fail-on stale`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if credentialRefused {
				...
				if cfg != nil {
					...
					if !configured {
						...
						report["auth_hint"] = "Set it with: echo \"$TOKEN\" | datagate-cli auth set-token or export DATAGATE_API_KEY=\"your-token-here\""
					} else {
						authConfigured = true
			...
			if os.Getenv("DATAGATE_API_KEY") != "" {
				authEnvSet = append(authEnvSet, "DATAGATE_API_KEY")
				...
				authEnvRequiredMissing = append(authEnvRequiredMissing, "DATAGATE_API_KEY")
			}
			switch {
...
func collectCacheReport(ctx context.Context, staleAfterSpec string) map[string]any {
	...
	if err != nil {
		if os.IsNotExist(err) {
			...
			report["hint"] = "Database not created yet; run 'datagate-cli sync' to hydrate."
			return report
		}
		report["status"] = "error"
	...
	if qerr != nil {
		...
		report["status"] = "unknown"
		report["hint"] = "No sync state recorded; run 'datagate-cli sync' to populate."
		return report
	}
	defer rows.Close()
	...
	case !haveAny:
		...
		report["hint"] = "No sync recorded; run 'datagate-cli sync' to hydrate API-backed resources. Rows written by other paths are not tracked in sync_state."
	case fresh:
		report["status"] = "fresh"
	default:
		report["status"] = "stale"
		report["oldest_age"] = oldest.Round(time.Minute).String()
		report["hint"] = "Some resources are older than stale_after; run 'datagate-cli sync' to refresh."
	}
	return report
}
