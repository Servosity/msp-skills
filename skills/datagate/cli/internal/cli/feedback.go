...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("DATAGATE_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("DATAGATE_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "datagate-cli/feedback")
...
When ` + "`DATAGATE_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`DATAGATE_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
maintainer sees it.`,
		Example: `  datagate-cli feedback "the --since flag is inclusive but docs say exclusive"
  datagate-cli feedback --stdin < notes.txt
  datagate-cli feedback list --limit 10`,
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  datagate-cli feedback list
  datagate-cli feedback list --limit 5
  datagate-cli feedback list --json`,
