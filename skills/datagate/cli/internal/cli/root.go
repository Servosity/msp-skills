...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "datagate-cli",
		Short: `Datagate CLI — Query DataGate telecom billing data from the terminal or an agent, with a local searchable mirror instead of raw API pagination…`,
		Long: `Datagate CLI — Query DataGate telecom billing data from the terminal or an agent, with a local searchable mirror instead of raw API pagination.

Highlights (not in the official API docs):
  • search   Search customers, agreements, and invoices offline from a local SQLite mirror instead of paging through the live API by hand.

Agent mode: add --agent to any command for JSON output + non-interactive mode.
Health check: run 'datagate-cli doctor' to verify auth and connectivity.
See README.md or the bundled SKILL.md for recipes.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("datagate-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.selectFields, "select", "", "Comma-separated fields to include in output (e.g. --select id,name,email)")
	...
	rootCmd.PersistentFlags().StringVar(&flags.runProfileName, "profile", "", "Apply values from a saved run profile; this does not select a client (see 'datagate-cli profile list')")
	...
	rootCmd.AddCommand(newCustomerUsersCmd(flags))
	rootCmd.AddCommand(newCustomersCmd(flags))
	rootCmd.AddCommand(newKitTemplatesCmd(flags))
	rootCmd.AddCommand(newProductTemplatesCmd(flags))
	rootCmd.AddCommand(newProductTransactionsCmd(flags))
	rootCmd.AddCommand(newProductsCmd(flags))
	rootCmd.AddCommand(newRateCardsCmd(flags))
	rootCmd.AddCommand(newServiceItemsCmd(flags))
	...
	rootCmd.AddCommand(newAccountManagersPromotedCmd(flags))
	rootCmd.AddCommand(newDeliveryMethodsPromotedCmd(flags))
