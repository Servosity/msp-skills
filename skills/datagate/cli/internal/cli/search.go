...
In local mode: searches locally synced data only.`,
  ...
  datagate-cli search "error timeout"

  # Force local search only
  datagate-cli search "status" --data-source local
  # Search a specific resource type locally
  datagate-cli search "status" --type account-managers --data-source local
  # JSON output for piping
  datagate-cli search "critical" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			query := args[0]
			// This API has a search endpoint: POST /invoices/search
			if flags.dataSource != "local" {
				c, err := flags.newClient()
				if err != nil {
					return err
				}
				data, _, getErr := c.Post(cmd.Context(), "/invoices/search", map[string]any{
					"q":           query,
					"periodStart": "",
					"periodEnd":   "",
					"status":      "",
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'datagate-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
