...
Resource scoping:
  ...
  datagate-cli sync --resources account-managers,agreements

  # Full resync (ignore previous checkpoint)
  datagate-cli sync --full

  # Incremental sync: only records from the last 7 days
  datagate-cli sync --since 7d

  # Parallel sync with 8 workers
  datagate-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  datagate-cli sync --latest-only`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if dbPath == "" {
				dbPath = defaultDBPath("datagate-cli")
			}
...
func upsertSingleObject(db *store.Store, resource string, data json.RawMessage) error {
	...
	switch resource {
	case "account-managers":
		return db.UpsertAccountManagers(data)
	case "agreements":
		return db.UpsertAgreements(data)
	case "assignments":
		return db.UpsertAssignments(data)
	case "customer-users":
		return db.UpsertCustomerUsers(data)
	case "customers":
		return db.UpsertCustomers(data)
	case "delivery-methods":
		return db.UpsertDeliveryMethods(data)
	case "invoices":
		return db.UpsertInvoices(data)
	case "kit-templates":
		return db.UpsertKitTemplates(data)
	case "product-templates":
		return db.UpsertProductTemplates(data)
	case "product-transactions":
		return db.UpsertProductTransactions(data)
	case "products":
		return db.UpsertProducts(data)
	case "rate-cards":
		return db.UpsertRateCards(data)
	case "service-items":
		return db.UpsertServiceItems(data)
	case "sites":
...
var reconcileTypedTables = map[string]string{
	"account-managers":     "account_managers",
	"agreements":           "agreements",
	"assignments":          "assignments",
	"customer-users":       "customer_users",
	"customers":            "customers",
	"delivery-methods":     "delivery_methods",
	"invoices":             "invoices",
	"kit-templates":        "kit_templates",
	"product-templates":    "product_templates",
	"product-transactions": "product_transactions",
	"products":             "products",
