...
var whichIndex = []whichEntry{
	{Command: "search", Description: "Search customers, agreements, and invoices offline from a local SQLite mirror instead of paging through the live API by hand.", Group: "Local state that compounds", WhyItMatters: "Use this to find a customer or agreement by name without knowing its GUID first."},
}
...
Exit codes:
  ...
  2  no confident match - the query did not score against any indexed capability; fall back to '--help' or 'search' if this CLI has one. Machine output (--json/--csv/--plain/--quiet, or a pipe) still exits 2 and writes {"matches":[]} (or the equivalent empty table) to stdout.`,
		SilenceUsage: true,
		Example: `  datagate-cli which "stale tickets"
  datagate-cli which "bottleneck"
  datagate-cli which --limit 1 "send message"
  datagate-cli which                                # list the full capability index`,
