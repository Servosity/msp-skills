...
type Config struct {
	...
	DatagateApiKey   string          `json:"api_key"`
}
...
func Load(configPath string) (*Config, error) {
	...
	// Env var overrides
	if v := os.Getenv("DATAGATE_API_KEY"); v != "" {
		cfg.DatagateApiKey = v
		cfg.markEnvOverride("DatagateApiKey")
		cfg.AuthSource = "env:DATAGATE_API_KEY"
		cfg.CredentialSource = "env:DATAGATE_API_KEY"
	}
	// DataGate requires a ClientId header (a GUID) on every request in addition
	// to the Bearer token; there is no first-class spec field for a second
	// required secret header, so it rides in on the generic Headers map instead.
	// NOTE: this block does not survive `generate --force` (config.go is a
	// generated file) - reapply after any future regeneration.
	if v := os.Getenv("DATAGATE_CLIENT_ID"); v != "" {
		if cfg.Headers == nil {
	...
	if v := os.Getenv("DATAGATE_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	return cfg, nil
}
...
func resolveConfigPath(configPath string) (string, bool, error) {
	...
	if path := os.Getenv("DATAGATE_CONFIG"); path != "" {
...
func LegacyConfigPath() (string, error) {
	...
	return filepath.Join(home, ".config", "datagate-cli", "config.json"), nil
}
...
func (c *Config) AuthHeader() string {
	...
	if c.DatagateApiKey != "" {
		if c.AuthSource == "" {
			c.AuthSource = "env:DATAGATE_API_KEY"
		}
		return applyAuthFormat("Bearer {token}", map[string]string{
			"api_key":          c.DatagateApiKey,
			"DATAGATE_API_KEY": c.DatagateApiKey,
			"token":            c.DatagateApiKey,
			"access_token":     c.DatagateApiKey,
		})
	}
	if c.AccessToken != "" {
...
func (c *Config) StoreScopeCredential() string {
	...
	if c.DatagateApiKey != "" {
		parts = append(parts, "api_key="+c.DatagateApiKey)
	}
	if len(parts) == 0 {
		return ""
	}
...
func (c *Config) credentials() *cliutil.Credentials {
	return &cliutil.Credentials{
		...
		DatagateApiKey: c.DatagateApiKey,
	}
}
...
func (c *Config) applyCredentials(creds *cliutil.Credentials) {
	...
	if c.DatagateApiKey == "" {
		c.DatagateApiKey = creds.DatagateApiKey
	}
}
...
func (c *Config) updateFileConfigField(field string) {
	...
	case "DatagateApiKey":
		c.fileConfig.DatagateApiKey = c.DatagateApiKey
	}
}
