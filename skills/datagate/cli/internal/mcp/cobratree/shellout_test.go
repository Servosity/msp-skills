...
// recorded in handfixes.json as mcp-argv-value-joined. blockedRootFlags and
// blockedDestinationFlags filter KEYS; a VALUE that begins with "--" used to
// be emitted as its own argv element, and because a Cobra bool flag does not
// consume the token after it, pflag parsed that value as a second flag - so
