...
func TestMCPPathResolutionMatchesCLIResolverWithPlatformDefaults(t *testing.T) {
	...
	if want := filepath.Join(home, ".config", "datagate-cli", "config.json"); cfg.Path != want {
	...
	if want := filepath.Join(home, ".local", "share", "datagate-cli", "data.db"); gotDB != want {
...
func TestMCPRegisterToolsPreservesTypedSpecialTools(t *testing.T) {
	s := server.NewMCPServer("datagate", "test")
	RegisterTools(s)
