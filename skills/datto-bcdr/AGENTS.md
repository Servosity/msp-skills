# Datto BCDR Printed CLI Agent Guide

This directory is a generated `datto-bcdr-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
datto-bcdr-cli doctor --json
datto-bcdr-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
datto-bcdr-cli which "<capability>" --json
datto-bcdr-cli <command> --help
```
...
```bash
datto-bcdr-cli <command> --help
datto-bcdr-cli <command> --dry-run --agent
```
