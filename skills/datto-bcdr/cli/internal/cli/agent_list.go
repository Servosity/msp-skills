...
func newAgentListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:         "list",
		Short:       "List every protected agent across all devices",
		Example:     "  datto-bcdr-cli agent list --agent --select agentName,os,lastScreenshotAttemptStatus,lastSnapshot",
		Annotations: map[string]string{"pp:endpoint": "agent.list", "pp:method": "GET", "pp:path": "/bcdr/agent", "mcp:read-only": "true"},
