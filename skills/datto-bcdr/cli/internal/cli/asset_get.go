...
func newAssetGetCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			path := "/bcdr/device/{serialNumber}/asset/{volumeName}"
			path = replacePathParam(path, "serialNumber", args[0])
			if len(args) < 2 {
				return usageErr(fmt.Errorf("volumeName is required\nUsage: %s <%s>", cmd.CommandPath(), "volumeName"))
			}
			path = replacePathParam(path, "volumeName", args[1])
