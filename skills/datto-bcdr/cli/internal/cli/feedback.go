...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".local", "share", "datto-bcdr-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("DATTO_BCDR_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("DATTO_BCDR_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "datto-bcdr-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.local/share/datto-bcdr-cli/feedback.jsonl.
When ` + "`DATTO_BCDR_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`DATTO_BCDR_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  datto-bcdr-cli feedback list
  datto-bcdr-cli feedback list --limit 5
  datto-bcdr-cli feedback list --json`,
