...
Resource scoping:
  ...
  datto-bcdr-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  datto-bcdr-cli sync --full

  # Incremental sync: only records from the last 7 days
  datto-bcdr-cli sync --since 7d

  # Parallel sync with 8 workers
  datto-bcdr-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  datto-bcdr-cli sync --latest-only`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if len(resources) == 0 {
				if humanFriendly {
					fmt.Fprintln(os.Stderr, "datto-bcdr-cli sync: no default sync resources in spec; use --resources for explicit sync targets or populate the store via single-fetch commands.")
				} else {
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/datto-bcdr-cli/data.db)")
...
func upsertSingleObject(db *store.Store, resource string, data json.RawMessage) error {
	...
	case "vm-restore":
		return db.UpsertVmRestore(data)
	default:
