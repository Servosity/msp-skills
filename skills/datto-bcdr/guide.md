...
For the short install path - a one-line installer for macOS / Linux / Windows, the
Claude Code plugin, and the `.mcpb` bundle for one-click Claude Desktop install - see
[README.md](./README.md). For every other agent (ChatGPT, Codex, Cursor, Copilot,
Gemini) see [mcp-install.md](./mcp-install.md). This file is the command reference.
