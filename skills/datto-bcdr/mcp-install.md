# Datto BCDR MCP - install for every agent that speaks MCP

This page wires the Datto BCDR MCP server into any MCP client. If you use Claude
...
launch `datto-bcdr-mcp` directly on your machine - no hosting. *Remote* agents (ChatGPT,
Microsoft 365 Copilot / Copilot Studio, the Gemini app) only talk to an HTTPS
endpoint, so you expose `datto-bcdr-mcp` over HTTPS first.

## Prerequisite: install the MCP binary

Run the install command from [README.md](./README.md). It drops both `datto-bcdr-cli`
and `datto-bcdr-mcp` on your PATH. `datto-bcdr-mcp` is what the agents talk to.
...
Restart Gemini CLI; the Datto BCDR tools appear in its tool list. (The **Gemini app /
...
All remote agents need `datto-bcdr-mcp` reachable as a public **HTTPS** endpoint. Run it
...
`datto-bcdr-mcp` you installed is not enough on its own. You also need a **Copilot Studio
license** and a **tenant admin** to enable it. This is a build-and-host task, not a
self-serve install.

Once `datto-bcdr-mcp` is hosted over HTTPS (above), the lowest-code route:
...
hermes mcp add datto-bcdr -- datto-bcdr-mcp

# OpenClaw
openclaw mcp set datto-bcdr '{"command":"datto-bcdr-mcp"}'
```
...
- `datto-bcdr-mcp: command not found`: the install dir is not on your PATH (the
