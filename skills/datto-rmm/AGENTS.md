# Datto RMM Printed CLI Agent Guide

This directory is a generated `datto-rmm-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
datto-rmm-cli doctor --json
datto-rmm-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
datto-rmm-cli which "<capability>" --json
datto-rmm-cli <command> --help
```
...
```bash
datto-rmm-cli <command> --help
datto-rmm-cli <command> --dry-run --agent
```
