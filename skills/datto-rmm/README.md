...
For ChatGPT, run `datto-rmm-mcp --transport http` and expose that behind HTTPS - no bridge package is needed. See [mcp-install.md](./mcp-install.md).
...
> **Interim note on `.mcpb`:** a bundle built before the [#287](https://github.com/Servosity/msp-skills/issues/287) fix does not launch. Its `manifest.json` runs `${__dirname}/bin/datto-rmm-mcp`, but the archive stores the binaries under platform-suffixed names (`bin/datto-rmm-mcp-darwin-arm64` and four siblings), so Claude Desktop finds nothing at that path. Check the bundle you downloaded with `unzip -l <file>.mcpb | grep bin/`: if `bin/datto-rmm-mcp` is not listed, use Path A or Path B below and wire Claude Desktop up through [mcp-install.md](./mcp-install.md).
...
The API key and secret key come from **Setup > Users** in your Datto RMM portal (the OAuth API user). A pre-minted bearer token is optional - set `DATTO_RMM_TOKEN` only if you want to skip the auto-mint. `doctor` confirms the credentials work before you run anything that touches data. It exits 0 even when the credential is rejected, so scripts should add `--fail-on error`.
