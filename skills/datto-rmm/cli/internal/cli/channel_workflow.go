...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  datto-rmm-cli workflow archive

  # Full re-archive (ignore previous sync state)
  datto-rmm-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"account", "account-alerts-open", "account-alerts-resolved", "account-components", "account-devices", "account-dnet-site-mappings", "account-users", "account-variables", "activity-logs", "filter", "filter-default-filters"}
			totalSynced := 0
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  datto-rmm-cli workflow status

  # Show status as JSON
  datto-rmm-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("datto-rmm-cli")
			}
