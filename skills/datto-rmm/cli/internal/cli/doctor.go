...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  datto-rmm-cli doctor
  datto-rmm-cli doctor --json
  datto-rmm-cli doctor --fail-on warn`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if os.Getenv("DATTO_RMM_API_KEY") != "" {
				authEnvSet = append(authEnvSet, "DATTO_RMM_API_KEY")
			} else {
				authEnvInfo = append(authEnvInfo, "DATTO_RMM_API_KEY set during auth login")
			}
			if os.Getenv("DATTO_RMM_API_SECRET_KEY") != "" {
				authEnvSet = append(authEnvSet, "DATTO_RMM_API_SECRET_KEY")
			} else {
				authEnvInfo = append(authEnvInfo, "DATTO_RMM_API_SECRET_KEY set during auth login")
			}
			if strings.Contains("Optional pre-minted OAuth bearer token (skips the auto-mint).", " OR ") {
				authEnvOptionalNames = append(authEnvOptionalNames, "DATTO_RMM_TOKEN")
				if os.Getenv("DATTO_RMM_TOKEN") != "" {
					authEnvSet = append(authEnvSet, "DATTO_RMM_TOKEN")
					authEnvOptionalSatisfied = true
				}
			} else if os.Getenv("DATTO_RMM_TOKEN") != "" {
				authEnvSet = append(authEnvSet, "DATTO_RMM_TOKEN")
			} else {
				authEnvInfo = append(authEnvInfo, "DATTO_RMM_TOKEN optional")
			}
			switch {
			...
			if cfg != nil && cfg.BaseURL != "" {
				...
				if clientErr != nil {
					...
					if authHeader == "" {
						...
						authHeaders["User-Agent"] = "datto-rmm-cli"
						verifyPath := "/v2/account"
...
func collectCacheReport(ctx context.Context, staleAfterSpec string) map[string]any {
	...
	if err != nil {
		if os.IsNotExist(err) {
			...
			report["hint"] = "Database not created yet; run 'datto-rmm-cli sync' to hydrate."
			return report
		}
		report["status"] = "error"
	...
	if qerr != nil {
		...
		report["status"] = "unknown"
		report["hint"] = "No sync state recorded; run 'datto-rmm-cli sync' to populate."
		return report
	}
	defer rows.Close()
	...
	case !haveAny && len(resources) == 0:
		report["status"] = "unknown"
		report["hint"] = "sync_state is empty; run 'datto-rmm-cli sync' to hydrate."
	case fresh:
		report["status"] = "fresh"
	default:
		report["status"] = "stale"
		report["oldest_age"] = oldest.Round(time.Minute).String()
		report["hint"] = "Some resources are older than stale_after; run 'datto-rmm-cli sync' to refresh."
	}
	return report
}
