...
In local mode: searches locally synced data only.`,
  ...
  datto-rmm-cli search "error timeout"

  # Force local search only
  datto-rmm-cli search "payment failed" --data-source local

  # Search a specific resource type locally
  datto-rmm-cli search "critical" --type transactions --data-source local

  # JSON output for piping
  datto-rmm-cli search "critical" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'datto-rmm-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
