...
Resource scoping:
  ...
  datto-rmm-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  datto-rmm-cli sync --full

  # Incremental sync: only records from the last 7 days
  datto-rmm-cli sync --since 7d

  # Parallel sync with 8 workers
  datto-rmm-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  datto-rmm-cli sync --latest-only`,
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/datto-rmm-cli/data.db)")
...
func resourceSupportsPagination(resource string) bool {
	...
	case "account":
		return true
	case "account-alerts-open":
		return true
	case "account-alerts-resolved":
		return true
	case "account-components":
		return true
	case "account-devices":
		return true
	case "account-dnet-site-mappings":
		return true
	case "account-users":
		return true
	case "account-variables":
		return true
	case "activity-logs":
		return true
	case "filter":
		return true
	case "filter-default-filters":
		return true
	}
	return false
}
...
func upsertSingleObject(db *store.Store, resource string, data json.RawMessage) error {
	...
	case "quickjob":
		return db.UpsertQuickjob(data)
	case "device_site":
		return db.UpsertDeviceSite(data)
	case "udf":
		return db.UpsertUdf(data)
	case "warranty":
		return db.UpsertWarranty(data)
	case "components":
		return db.UpsertComponents(data)
	case "results":
		return db.UpsertResults(data)
	case "site_alerts":
		return db.UpsertSiteAlerts(data)
	case "devices":
...
func syncResourcePath(resource string) (string, error) {
	paths := map[string]string{
		"account":                    "/v2/account/sites",
		"account-alerts-open":        "/v2/account/alerts/open",
		"account-alerts-resolved":    "/v2/account/alerts/resolved",
		"account-components":         "/v2/account/components",
		"account-devices":            "/v2/account/devices",
		"account-dnet-site-mappings": "/v2/account/dnet-site-mappings",
		"account-users":              "/v2/account/users",
		"account-variables":          "/v2/account/variables",
		"activity-logs":              "/v2/activity-logs",
		"filter":                     "/v2/filter/custom-filters",
		"filter-default-filters":     "/v2/filter/default-filters",
	}
