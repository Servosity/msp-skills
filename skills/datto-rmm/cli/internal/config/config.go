...
type Config struct {
	...
	DattoRmmApiKey       string            `toml:"rmm_api_key"`
	DattoRmmApiSecretKey string            `toml:"rmm_api_secret_key"`
	DattoRmmToken        string            `toml:"rmm_token"`
}

func Load(configPath string) (*Config, error) {
	cfg := &Config{
		BaseURL: "https://merlot-api.centrastage.net/api",
	}

	// Resolve config path
	path := configPath
	if path == "" {
		path = os.Getenv("DATTO_RMM_CONFIG")
	}
	if path == "" {
		home, _ := os.UserHomeDir()
		path = filepath.Join(home, ".config", "datto-rmm-cli", "config.toml")
	}
	cfg.Path = path
	...
	// Env var overrides
	if v := os.Getenv("DATTO_RMM_API_KEY"); v != "" {
		cfg.DattoRmmApiKey = v
		cfg.markEnvOverride("DattoRmmApiKey")
		cfg.AuthSource = "env:DATTO_RMM_API_KEY"
	}
	if v := os.Getenv("DATTO_RMM_API_SECRET_KEY"); v != "" {
		cfg.DattoRmmApiSecretKey = v
		cfg.markEnvOverride("DattoRmmApiSecretKey")
		cfg.AuthSource = "env:DATTO_RMM_API_SECRET_KEY"
	}
	if v := os.Getenv("DATTO_RMM_TOKEN"); v != "" {
		cfg.DattoRmmToken = v
		cfg.markEnvOverride("DattoRmmToken")
		cfg.AuthSource = "env:DATTO_RMM_TOKEN"
	}
	...
	if cfg.AuthSource == "" && cfg.DattoRmmApiKey != "" {
		cfg.AuthSource = "config"
	}
	if cfg.AuthSource == "" && cfg.DattoRmmApiSecretKey != "" {
		cfg.AuthSource = "config"
	}
	if cfg.AuthSource == "" && cfg.DattoRmmToken != "" {
		cfg.AuthSource = "config"
	}
	...
	if v := os.Getenv("DATTO_RMM_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	return cfg, nil
}
...
func (c *Config) AuthHeader() string {
	...
	if c.DattoRmmToken != "" {
		if c.AuthSource == "" {
			c.AuthSource = "env:DATTO_RMM_TOKEN"
		}
		return "Bearer " + c.DattoRmmToken
	}
	return ""
}
...
func (c *Config) ClearTokens() error {
	...
	c.updateFileConfigField("ClientSecret")
	c.DattoRmmApiKey = ""
	delete(c.envOverrides, "DattoRmmApiKey")
	c.DattoRmmApiSecretKey = ""
	delete(c.envOverrides, "DattoRmmApiSecretKey")
	c.DattoRmmToken = ""
	delete(c.envOverrides, "DattoRmmToken")
	return c.save()
}
...
func (c *Config) configForSave() Config {
	out := *c
	if c.fileConfig != nil {
		if c.envOverrides["DattoRmmApiKey"] {
			out.DattoRmmApiKey = c.fileConfig.DattoRmmApiKey
		}
		if c.envOverrides["DattoRmmApiSecretKey"] {
			out.DattoRmmApiSecretKey = c.fileConfig.DattoRmmApiSecretKey
		}
		if c.envOverrides["DattoRmmToken"] {
			out.DattoRmmToken = c.fileConfig.DattoRmmToken
		}
	}
...
func (c *Config) updateFileConfigField(field string) {
	...
	case "DattoRmmApiKey":
		c.fileConfig.DattoRmmApiKey = c.DattoRmmApiKey
	case "DattoRmmApiSecretKey":
		c.fileConfig.DattoRmmApiSecretKey = c.DattoRmmApiSecretKey
	case "DattoRmmToken":
		c.fileConfig.DattoRmmToken = c.DattoRmmToken
	}
}
...
//   - EnsureDattoToken's best-effort SaveTokens (the OAuth mint path) is
//     gated too: the minted bearer stays in memory for this process only.
const NoConfigWriteEnv = "DATTO_RMM_NO_CONFIG_WRITE"
...
// material. It is the ONLY save path DATTO_RMM_NO_CONFIG_WRITE suppresses;
