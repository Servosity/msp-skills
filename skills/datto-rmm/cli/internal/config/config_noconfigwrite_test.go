...
// touching disk. DATTO_RMM_NO_CONFIG_WRITE makes the automatic token cache a
...
// ncwHome points every config-path resolution at a throwaway HOME.
func ncwHome(t *testing.T) string {
	t.Helper()
	home := t.TempDir()
	t.Setenv("HOME", home)
...
// An existing file is never erased by the switch: a later credential write
// under the switch leaves the on-disk bytes exactly as they were.
func TestNoConfigWriteLeavesAnExistingFileByteIdentical(t *testing.T) {
	home := ncwHome(t)
	os.Unsetenv(NoConfigWriteEnv)
	seed, err := Load("")
	if err != nil {
	...
	if err := seed.SaveTokens("minted-cid", ncwSecret, "older-token", ncwRefresh, time.Unix(1767225600, 0).UTC()); err != nil {
		t.Fatalf("SaveTokens (seed): %v", err)
	}
	before := ncwSnapshot(t, home)
	if len(before) == 0 {
		t.Fatal("seed wrote nothing, so this test proves nothing")
	}
	...
	if err := cfg.SaveTokens("minted-cid", ncwSecret, ncwToken, ncwRefresh, time.Unix(1767312000, 0).UTC()); err != nil {
		t.Fatalf("SaveTokens under the switch: %v", err)
	}
	if cfg.AccessToken != ncwToken {
		t.Fatalf("in-memory token not updated under the switch: %q", cfg.AccessToken)
	}
	if after := ncwSnapshot(t, home); !reflect.DeepEqual(after, before) {
		t.Errorf("the switch changed an existing file.\nbefore: %v\nafter:  %v", before, after)
	}
}
...
func TestNoConfigWriteTruthiness(t *testing.T) {
	...
	if NoConfigWriteEnv != "DATTO_RMM_NO_CONFIG_WRITE" {
		t.Errorf("switch is named %q; the documented name is DATTO_RMM_NO_CONFIG_WRITE", NoConfigWriteEnv)
	}
}

// The real persistence route on this connector: EnsureDattoToken mints a
// bearer from the API key + secret and calls SaveTokens best-effort
// (datto_oauth.go). Under the switch the mint must still succeed and land in
// memory, and the config file must be absent; without it the file must carry
// the minted token.
func ncwMintViaOAuth(t *testing.T, set bool) (*Config, map[string]string) {
	t.Helper()
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		_, _ = w.Write([]byte(`{"access_token":"` + ncwToken + `","token_type":"bearer","expires_in":3600}`))
	}))
	t.Cleanup(srv.Close)

	home := ncwHome(t)
	t.Setenv("DATTO_RMM_BASE_URL", srv.URL+"/api")
	t.Setenv("DATTO_RMM_API_KEY", "not-a-real-key")
	t.Setenv("DATTO_RMM_API_SECRET_KEY", ncwSecret)
	os.Unsetenv("DATTO_RMM_TOKEN")
	if set {
		t.Setenv(NoConfigWriteEnv, "1")
	} else {
		os.Unsetenv(NoConfigWriteEnv)
	...
	if err := EnsureDattoToken(context.Background(), cfg); err != nil {
		t.Fatalf("EnsureDattoToken: %v", err)
	}
	if cfg.AccessToken != ncwToken {
		t.Fatalf("minted token not in memory: %q", cfg.AccessToken)
	}
	if cfg.AuthHeader() != "Bearer "+ncwToken {
		t.Fatalf("AuthHeader() = %q, want the minted bearer", cfg.AuthHeader())
	}
	return cfg, ncwSnapshot(t, home)
}

func TestNoConfigWriteGatesTheOAuthMint(t *testing.T) {
	_, on := ncwMintViaOAuth(t, true)
	if len(on) != 0 {
		t.Errorf("%s=1: the OAuth mint still wrote file(s): %v", NoConfigWriteEnv, on)
	}
	_, off := ncwMintViaOAuth(t, false)
	if hits := ncwFilesMentioning(off, ncwToken); len(hits) == 0 {
		t.Fatalf("without the switch the mint cached nothing, so the ON case proves nothing; wrote: %v", off)
	}
}
