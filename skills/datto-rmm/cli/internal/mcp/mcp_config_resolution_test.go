...
// resolver the CLI uses, which reads DATTO_RMM_CONFIG and applies
...
func TestMCPClientHonoursTheConfigEnvVar(t *testing.T) {
	...
	if cfg.Path != path {
		t.Errorf("MCP server read %q, want the DATTO_RMM_CONFIG path %q", cfg.Path, path)
	}
