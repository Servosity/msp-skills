# Domotz Printed CLI Agent Guide

This directory is a generated `domotz-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
domotz-cli doctor --json
domotz-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
domotz-cli which "<capability>" --json
domotz-cli <command> --help
```
...
```bash
domotz-cli <command> --help
domotz-cli <command> --dry-run --agent
```
