...
func newAgentDeviceSetInventoryFieldValueCmd(flags *rootFlags) *cobra.Command {
	var stdinBody bool
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if len(args) < 3 {
				return usageErr(fmt.Errorf("inventory_field is required\nUsage: %s <%s>", cmd.CommandPath(), "inventory_field"))
			}
			path = replacePathParam(path, "inventory_field", args[2])
