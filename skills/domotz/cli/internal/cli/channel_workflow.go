...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  domotz-cli workflow archive

  # Full re-archive (ignore previous sync state)
  domotz-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"agent", "agent-uptime", "alert-profile", "area", "custom-driver", "custom-tag", "device-profile", "inventory", "rbac", "rbac-member", "rbac-user-group", "type", "type-device-detected"}
			totalSynced := 0
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  domotz-cli workflow status

  # Show status as JSON
  domotz-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("domotz-cli")
			}
