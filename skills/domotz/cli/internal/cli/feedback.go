...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".local", "share", "domotz-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("DOMOTZ_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("DOMOTZ_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "domotz-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.local/share/domotz-cli/feedback.jsonl.
When ` + "`DOMOTZ_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`DOMOTZ_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  domotz-cli feedback list
  domotz-cli feedback list --limit 5
  domotz-cli feedback list --json`,
