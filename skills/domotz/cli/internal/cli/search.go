...
In local mode: searches locally synced data only.`,
  ...
  domotz-cli search "error timeout"

  # Force local search only
  domotz-cli search "payment failed" --data-source local

  # Search a specific resource type locally
  domotz-cli search "critical" --type transactions --data-source local

  # JSON output for piping
  domotz-cli search "critical" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'domotz-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
			...
			switch resourceType {
			case "alert-profile":
				results, err = db.SearchAlertProfile(query, limit)
			case "custom-driver":
				results, err = db.SearchCustomDriver(query, limit)
			case "device-profile":
				results, err = db.SearchDeviceProfile(query, limit)
			case "rbac":
				results, err = db.SearchRbac(query, limit)
			case "":
				...
				{
					partial, searchErr := db.SearchAlertProfile(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search alert_profile failed: %w", searchErr)
					}
				...
				{
					partial, searchErr := db.SearchCustomDriver(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search custom_driver failed: %w", searchErr)
					}
					for _, r := range partial {
						key := string(r)
						if !seen[key] {
				...
				{
					partial, searchErr := db.SearchDeviceProfile(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search device_profile failed: %w", searchErr)
					}
					for _, r := range partial {
						key := string(r)
						if !seen[key] {
				...
				{
					partial, searchErr := db.SearchRbac(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search rbac failed: %w", searchErr)
					}
					for _, r := range partial {
						key := string(r)
						if !seen[key] {
