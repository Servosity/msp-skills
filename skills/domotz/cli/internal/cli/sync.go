...
Resource scoping:
  ...
  domotz-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  domotz-cli sync --full

  # Incremental sync: only records from the last 7 days
  domotz-cli sync --since 7d

  # Parallel sync with 8 workers
  domotz-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  domotz-cli sync --latest-only`,
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/domotz-cli/data.db)")
...
func upsertSingleObject(db *store.Store, resource string, data json.RawMessage) error {
	...
	case "agent_custom_tag":
		return db.UpsertAgentCustomTag(data)
	case "device":
		return db.UpsertDevice(data)
	case "eye_statistics":
		return db.UpsertEyeStatistics(data)
	case "history":
		return db.UpsertHistory(data)
	case "ip_conflict":
		return db.UpsertIpConflict(data)
	case "metric_statistics":
		return db.UpsertMetricStatistics(data)
	case "network":
		return db.UpsertNetwork(data)
	case "network_topology":
		return db.UpsertNetworkTopology(data)
	case "ownership":
		return db.UpsertOwnership(data)
	case "uptime":
		return db.UpsertUptime(data)
	case "variable":
		return db.UpsertVariable(data)
	case "alert-profile":
		return db.UpsertAlertProfile(data)
	case "binding":
		return db.UpsertBinding(data)
	case "area":
		return db.UpsertArea(data)
	case "team":
		return db.UpsertTeam(data)
	case "custom-driver":
		return db.UpsertCustomDriver(data)
	case "custom_driver_agent":
		return db.UpsertCustomDriverAgent(data)
	case "association":
		return db.UpsertAssociation(data)
	case "custom-tag":
		return db.UpsertCustomTag(data)
	case "device-profile":
		return db.UpsertDeviceProfile(data)
	case "apply":
	...
	case "user_alert_profile":
		return db.UpsertUserAlertProfile(data)
	default:
...
func syncResourcePath(resource string) (string, error) {
	paths := map[string]string{
		...
		"inventory":            "/inventory",
		"rbac":                 "/rbac/role",
		"rbac-member":          "/rbac/member",
		"rbac-user-group":      "/rbac/user-group",
		"type":                 "/type/device/base",
		"type-device-detected": "/type/device/detected",
	}
...
func dependentResourceDefs() []dependentResourceDef {
	return []dependentResourceDef{
		{Name: "device", ParentTable: "agent", ParentIDParam: "agent_id", PathTemplate: "/agent/{agent_id}/device/application", KeyField: "", PathParams: []dependentPathParamDef{
			{Param: "agent_id", Field: "id"},
		}},
		{Name: "device", ParentTable: "agent", ParentIDParam: "agent_id", PathTemplate: "/agent/{agent_id}/device/variable", KeyField: "", PathParams: []dependentPathParamDef{
			{Param: "agent_id", Field: "id"},
		}},
		{Name: "variable", ParentTable: "agent", ParentIDParam: "agent_id", PathTemplate: "/agent/{agent_id}/variable", KeyField: "", PathParams: []dependentPathParamDef{
			{Param: "agent_id", Field: "id"},
		}},
		{Name: "device_application", ParentTable: "device", ParentIDParam: "device_id", PathTemplate: "/agent/{agent_id}/device/{device_id}/application", KeyField: "", PathParams: []dependentPathParamDef{
			{Param: "agent_id", Field: "agent_id"},
			{Param: "device_id", Field: "id"},
		}},
		{Name: "device_variable", ParentTable: "device", ParentIDParam: "device_id", PathTemplate: "/agent/{agent_id}/device/{device_id}/variable", KeyField: "", PathParams: []dependentPathParamDef{
			{Param: "agent_id", Field: "agent_id"},
			{Param: "device_id", Field: "id"},
		}},
	}
