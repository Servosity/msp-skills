...
type Config struct {
	...
	Path               string            `toml:"-"`
	DomotzApiKey       string            `toml:"api_key"`
	DomotzPublicApiKey string            `toml:"public_api_key"`
...
func Load(configPath string) (*Config, error) {
	cfg := &Config{
		BaseURL: "https://api-{region}.domotz.com/public-api/v1",
	}

	// Resolve config path
	path := configPath
	if path == "" {
		path = os.Getenv("DOMOTZ_CONFIG")
	}
	if path == "" {
		home, _ := os.UserHomeDir()
		path = filepath.Join(home, ".config", "domotz-cli", "config.toml")
	}
	cfg.Path = path
	...
	if v := os.Getenv("DOMOTZ_API_KEY"); v != "" {
		cfg.DomotzApiKey = v
		cfg.AuthSource = "env:DOMOTZ_API_KEY"
	}
	if v := os.Getenv("DOMOTZ_PUBLIC_API_KEY"); v != "" {
		cfg.DomotzPublicApiKey = v
		cfg.AuthSource = "env:DOMOTZ_PUBLIC_API_KEY"
	}
	...
	if cfg.AuthSource == "" && cfg.DomotzApiKey != "" {
		cfg.AuthSource = "config"
	}
	if cfg.AuthSource == "" && cfg.DomotzPublicApiKey != "" {
		cfg.AuthSource = "config"
	}
	...
	if v := os.Getenv("DOMOTZ_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	...
	if v := strings.TrimSpace(os.Getenv("DOMOTZ_REGION")); v != "" {
		cfg.TemplateVars["region"] = normalizeEndpointTemplateValue(v)
	} else {
		cfg.TemplateVars["region"] = "us-east-1-cell-1"
	}
	return cfg, nil
}
...
func (c *Config) AuthHeader() string {
	...
	if c.DomotzApiKey != "" {
		return c.DomotzApiKey
	}
	if c.DomotzPublicApiKey != "" {
		return c.DomotzPublicApiKey
	}
	return ""
}
