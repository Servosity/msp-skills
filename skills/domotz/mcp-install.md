# Domotz MCP - install for every agent that speaks MCP

This page wires the Domotz MCP server into any MCP client. If you use Claude
...
launch `domotz-mcp` directly on your machine - no hosting. *Remote* agents (ChatGPT,
Microsoft 365 Copilot / Copilot Studio, the Gemini app) only talk to an HTTPS
endpoint, so you expose `domotz-mcp` over HTTPS first.

## Prerequisite: install the MCP binary

Run the install command from [README.md](./README.md). It drops both `domotz-cli`
and `domotz-mcp` on your PATH. `domotz-mcp` is what the agents talk to.
  ...
  "mcpServers": {
    "domotz": {
      ...
      "env": {
        ...
        "DOMOTZ_PUBLIC_API_KEY": "<your-domotz_public_api_key>",
...
Restart Gemini CLI; the Domotz tools appear in its tool list. (The **Gemini app /
...
All remote agents need `domotz-mcp` reachable as a public **HTTPS** endpoint. Run it
in HTTP mode with your credentials in the environment:

```bash
DOMOTZ_API_KEY=<value> DOMOTZ_PUBLIC_API_KEY=<value> DOMOTZ_REGION=<value> domotz-mcp --transport http --addr :7777
```
...
`domotz-mcp` you installed is not enough on its own. You also need a **Copilot Studio
license** and a **tenant admin** to enable it. This is a build-and-host task, not a
self-serve install.

Once `domotz-mcp` is hosted over HTTPS (above), the lowest-code route:
...
hermes mcp add domotz -- domotz-mcp

# OpenClaw
openclaw mcp set domotz '{"command":"domotz-mcp"}'
```
...
- `domotz-mcp: command not found`: the install dir is not on your PATH (the
