# Gradient MSP Printed CLI Agent Guide

This directory is a generated `gradient-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
gradient-cli doctor --json
gradient-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
gradient-cli which "<capability>" --json
gradient-cli <command> --help
```
...
```bash
gradient-cli <command> --help
gradient-cli <command> --dry-run --agent
```
