...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".local", "share", "gradient-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("GRADIENT_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("GRADIENT_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "gradient-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.local/share/gradient-cli/feedback.jsonl.
When ` + "`GRADIENT_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`GRADIENT_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  gradient-cli feedback list
  gradient-cli feedback list --limit 5
  gradient-cli feedback list --json`,
