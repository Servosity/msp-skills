...
In local mode: searches locally synced data only.`,
  ...
  gradient-cli search "error timeout"

  # Force local search only
  gradient-cli search "payment failed" --data-source local

  # Search a specific resource type locally
  gradient-cli search "critical" --type transactions --data-source local

  # JSON output for piping
  gradient-cli search "critical" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'gradient-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
			...
			case "mappings":
				results, err = db.SearchMappings(query, limit)
			case "":
				...
				{
					partial, searchErr := db.SearchMappings(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search mappings failed: %w", searchErr)
					}
					for _, r := range partial {
						key := string(r)
						if !seen[key] {
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/gradient-cli/data.db)")
