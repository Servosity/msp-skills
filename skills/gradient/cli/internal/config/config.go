...
type Config struct {
	...
	GradientToken string            `toml:"token"`
}

func Load(configPath string) (*Config, error) {
	cfg := &Config{
		BaseURL: "https://app.usegradient.com/api",
	}

	// Resolve config path
	path := configPath
	if path == "" {
		path = os.Getenv("GRADIENT_CONFIG")
	}
	if path == "" {
		home, _ := os.UserHomeDir()
		path = filepath.Join(home, ".config", "gradient-cli", "config.toml")
	}
	cfg.Path = path
	...
	// Env var overrides
	if v := os.Getenv("GRADIENT_TOKEN"); v != "" {
		cfg.GradientToken = v
		cfg.markEnvOverride("GradientToken")
		cfg.AuthSource = "env:GRADIENT_TOKEN"
	}

	// Synthesize issues a Vendor + Partner API key pair; the wire header is
	// base64("vendor:partner"). Derive it when GRADIENT_TOKEN itself is unset
	// so users can export the two keys exactly as the vendor hands them out.
	// Env (pair) beats any token persisted in the config file, matching the
	// usual env-over-file precedence. (Hand-wired composed auth — gradient_keys.go.)
	if cfg.AuthSource != "env:GRADIENT_TOKEN" {
		if t, src := DeriveGradientToken(); t != "" {
			cfg.GradientToken = t
			cfg.AuthSource = src
		}
	}
	...
	if cfg.AuthSource == "" && cfg.GradientToken != "" {
		cfg.AuthSource = "config"
	}
	...
	if v := os.Getenv("GRADIENT_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	return cfg, nil
}
...
func (c *Config) SaveCredential(token string) error {
	...
	c.updateFileConfigField("AccessToken")
	c.GradientToken = token
	delete(c.envOverrides, "GradientToken")
	c.updateFileConfigField("GradientToken")
	return c.save()
}
...
func (c *Config) configForSave() Config {
	out := *c
	if c.fileConfig != nil {
		if c.envOverrides["GradientToken"] {
			out.GradientToken = c.fileConfig.GradientToken
		}
	}
...
func (c *Config) updateFileConfigField(field string) {
	...
	case "GradientToken":
		c.fileConfig.GradientToken = c.GradientToken
	}
}
