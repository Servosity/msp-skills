# install.ps1 - install gradient-cli and gradient-mcp on Windows.
#
# Pulls prebuilt binaries from this skill's latest GitHub Release (gradient-v*).
...
$Skill   = "gradient"
$CliBin  = "gradient-cli.exe"
$McpBin  = "gradient-mcp.exe"
...
$ReleaseBase = if ($env:MSP_SKILLS_RELEASE_BASE) {
  ...
  # Each skill is versioned/tagged independently (gradient-vX.Y.Z),
...
Write-Host "  gradient-cli --version"
Write-Host ""
Write-Host "Next:"
Write-Host "  First command + auth: https://github.com/$Owner/$Repo/tree/main/skills/gradient#readme"
Write-Host "  Claude Desktop / ChatGPT wire-up: https://github.com/$Owner/$Repo/blob/main/skills/gradient/mcp-install.md"
