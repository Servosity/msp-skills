...
func newNovelSlaBreachingCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Reads each ticket's resolution deadline locally (fixbydate, falling back to
targetdate); joins agent + client + current status.
...
`, "\n"),
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			where := []string{
				...
				"datetime(" + slaResolutionTargetExpr("") + ") BETWEEN datetime('now') AND datetime('now', '+' || ? || ' hours')",
			}
			argsSQL := []any{hours}
			if team != "" {
			...
			q := `SELECT id,
                ...
                ` + slaResolutionTargetExpr("") + ` AS targetdate,
                CAST((julianday(` + slaResolutionTargetExpr("") + `) - julianday('now')) * 24 * 60 AS INTEGER) AS minutes_to_breach
