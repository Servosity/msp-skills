...
Resource scoping:
  ...
  halopsa-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  halopsa-cli sync --full

  # Incremental sync: only records from the last 7 days
  halopsa-cli sync --since 7d

  # Parallel sync with 8 workers
  halopsa-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  halopsa-cli sync --latest-only`,
...
func syncResource(ctx context.Context, c interface {
	...
	if storedCount, err := db.Count(resource); err == nil {
		if totalCount > storedCount {
			if humanFriendly {
...
// page_no 1/2/3 returns three disjoint 100-row pages and reports
// record_count as the full result-set total rather than the returned-row
// count. The same call without page_no returns the agent default page size
// with record_count equal to that short count.
...
func defaultSyncResources() []string {
	return []string{
		...
		"feed",
		"feedback_items",
		"feedback_items-feedback-message",
		"field",
		"field-group",
		"field-info",
		"forecast-details",
...
func syncResourcePath(resource string) (string, error) {
	paths := map[string]string{
		...
		"feedback_items-feedback-message": "/Feedback/FeedbackMessage",
		"field":                           "/Field",
