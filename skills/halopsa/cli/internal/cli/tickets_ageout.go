...
`, "\n"),
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			where = append(where, "(julianday('now') - julianday(" + haloTicketActivityExpr("") + ")) >= ?")
			argsSQL = append(argsSQL, staleDays)
			q := `SELECT id, COALESCE(client_name,'?'), ` + haloAgentLabelExpr("", "?") + `,
                COALESCE(json_extract(data,'$.status_name'), json_extract(data,'$.status'), '?'),
                ` + haloTicketActivityExpr("") + `,
                COALESCE(summary,'')
...
`, "\n"),
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			defer db.Close()
			where := []string{"datetime(" + haloTicketActivityExpr("") + ") >= datetime(?)"}
			...
			q := `SELECT id, COALESCE(client_name,'?') AS client,
                ...
                ` + haloTicketActivityExpr("") + ` AS last_action
