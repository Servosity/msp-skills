...
//
// Two Halo realities shape this query, both reported live in #203:
//
...
func buildTriageQuery(team, agent string, staleDays, breachHrs, limit int) (string, []any) {
	...
	q := `WITH scoped AS (
            ...
            SELECT
                ...
                SUM(CASE WHEN datetime(` + slaResolutionTargetExpr("") + `) BETWEEN datetime('now') AND datetime('now', '+' || ? || ' hours') THEN 1 ELSE 0 END) AS breach_count,
