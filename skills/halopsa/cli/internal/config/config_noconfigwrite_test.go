...
// touching disk. HALOPSA_NO_CONFIG_WRITE makes the automatic token cache a
...
const (
	ncwToken   = "minted-token-HALOPSA"
	ncwRefresh = "minted-refresh-HALOPSA"
	ncwSecret  = "minted-secret-HALOPSA"
)
...
func ncwMint(t *testing.T, value string, set bool) map[string]string {
	...
	home := t.TempDir()
	t.Setenv("HOME", home)
	t.Setenv("HALOPSA_CONFIG", filepath.Join(home, "config.toml"))
	if set {
...
func TestNoConfigWriteStillAllowsTheWipe(t *testing.T) {
	...
	t.Setenv("HOME", home)
	t.Setenv("HALOPSA_CONFIG", cfgPath)
...
func TestNoConfigWriteTruthiness(t *testing.T) {
	...
	if NoConfigWriteEnv != "HALOPSA_NO_CONFIG_WRITE" {
		t.Errorf("switch is named %q; the documented name is HALOPSA_NO_CONFIG_WRITE", NoConfigWriteEnv)
	}
}
