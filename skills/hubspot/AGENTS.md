...
This directory is a generated `hubspot-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
hubspot-cli doctor --json
hubspot-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
hubspot-cli which "<capability>" --json
hubspot-cli <command> --help
```
...
```bash
hubspot-cli <command> --help
hubspot-cli <command> --dry-run --agent
```
