...
func classifyAPIError(err error, flags *rootFlags) error {
	...
	case strings.Contains(msg, "HTTP 400") && cliutil.LooksLikeAuthError(msg):
		return authErr(fmt.Errorf("%w\nhint: the API rejected the request — this usually means auth is missing or invalid."+
			...
			"\n      Run 'hubspot-cli doctor' to check auth status."+
			"\n      Response: "+cliutil.SanitizeErrorBody(msg), err))
	case strings.Contains(msg, "HTTP 401"):
		return authErr(fmt.Errorf("%w\nhint: check your token. Set it with: hubspot-cli auth set-token <token>"+
			"\n      or: export HUBSPOT_ACCESS_TOKEN=<your-token>"+
			"\n      Get a key at: https://app.hubspot.com/private-apps"+
			"\n      Create a private app, grant CRM scopes, and copy the access token."+
			"\n      Run 'hubspot-cli doctor' to check auth status.", err))
