# Hudu Printed CLI Agent Guide

This directory is a generated `hudu-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
hudu-cli doctor --json
hudu-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
hudu-cli which "<capability>" --json
hudu-cli <command> --help
```
...
```bash
hudu-cli <command> --help
hudu-cli <command> --dry-run --agent
```
