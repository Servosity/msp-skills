...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  hudu-cli workflow archive

  # Full re-archive (ignore previous sync state)
  hudu-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"activity-logs", "articles", "asset-layouts", "asset-passwords", "assets", "companies", "expirations", "exports", "flag-types", "flags", "folders", "groups", "ip-addresses", "lists", "magic-dash", "matchers", "networks", "procedure-tasks", "procedures", "public-photos", "rack-storage-items", "rack-storages", "relations", "uploads", "users", "vlan-zones", "vlans", "websites"}
			totalSynced := 0
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  hudu-cli workflow status

  # Show status as JSON
  hudu-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("hudu-cli")
			}
