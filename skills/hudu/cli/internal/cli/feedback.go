...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".local", "share", "hudu-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("HUDU_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("HUDU_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "hudu-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.local/share/hudu-cli/feedback.jsonl.
When ` + "`HUDU_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`HUDU_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  hudu-cli feedback list
  hudu-cli feedback list --limit 5
  hudu-cli feedback list --json`,
