...
In local mode: searches locally synced data only.`,
  ...
  hudu-cli search "error timeout"

  # Force local search only
  hudu-cli search "payment failed" --data-source local

  # Search a specific resource type locally
  hudu-cli search "critical" --type transactions --data-source local

  # JSON output for piping
  hudu-cli search "critical" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'hudu-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
			...
			case "magic-dash":
				results, err = db.SearchMagicDash(query, limit)
			case "procedure-tasks":
				results, err = db.SearchProcedureTasks(query, limit)
			case "procedures":
				results, err = db.SearchProcedures(query, limit)
			case "":
				...
				{
					partial, searchErr := db.SearchMagicDash(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search magic_dash failed: %w", searchErr)
					}
					for _, r := range partial {
						key := string(r)
						if !seen[key] {
				...
				{
					partial, searchErr := db.SearchProcedureTasks(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search procedure_tasks failed: %w", searchErr)
					}
					for _, r := range partial {
						key := string(r)
						if !seen[key] {
				...
				{
					partial, searchErr := db.SearchProcedures(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search procedures failed: %w", searchErr)
					}
					for _, r := range partial {
						key := string(r)
						if !seen[key] {
