...
type Config struct {
	...
	HuduApiKey    string            `toml:"api_key"`
}

func Load(configPath string) (*Config, error) {
	cfg := &Config{
		BaseURL: "https://your-domain.huducloud.com/api/v1",
	}

	// Resolve config path
	path := configPath
	if path == "" {
		path = os.Getenv("HUDU_CONFIG")
	}
	if path == "" {
		home, _ := os.UserHomeDir()
		path = filepath.Join(home, ".config", "hudu-cli", "config.toml")
	}
	cfg.Path = path
	...
	// Env var overrides
	if v := os.Getenv("HUDU_API_KEY"); v != "" {
		cfg.HuduApiKey = v
		cfg.markEnvOverride("HuduApiKey")
		cfg.AuthSource = "env:HUDU_API_KEY"
	}
	...
	if cfg.AuthSource == "" && cfg.HuduApiKey != "" {
		cfg.AuthSource = "config"
	}
...
func (c *Config) AuthHeader() string {
	...
	replacements := map[string]string{
		"token":        token,
		"api_key":      c.HuduApiKey,
		"HUDU_API_KEY": c.HuduApiKey,
	}
...
func (c *Config) SaveCredential(token string) error {
	...
	c.updateFileConfigField("AccessToken")
	c.HuduApiKey = token
	delete(c.envOverrides, "HuduApiKey")
	c.updateFileConfigField("HuduApiKey")
	return c.save()
}
...
func (c *Config) configForSave() Config {
	out := *c
	if c.fileConfig != nil {
		if c.envOverrides["HuduApiKey"] {
			out.HuduApiKey = c.fileConfig.HuduApiKey
		}
	}
...
func (c *Config) updateFileConfigField(field string) {
	...
	case "HuduApiKey":
		c.fileConfig.HuduApiKey = c.HuduApiKey
	}
}
