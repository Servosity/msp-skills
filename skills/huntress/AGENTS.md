# Huntress Printed CLI Agent Guide

This directory is a generated `huntress-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
huntress-cli doctor --json
huntress-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
huntress-cli which "<capability>" --json
huntress-cli <command> --help
```
...
```bash
huntress-cli <command> --help
huntress-cli <command> --dry-run --agent
```
