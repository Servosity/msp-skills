...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  huntress-cli workflow archive

  # Full re-archive (ignore previous sync state)
  huntress-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"accounts", "agents", "escalations", "external-ports", "identities", "incident-reports", "invoices", "known-vpns", "memberships", "organizations", "reports", "reseller", "reseller-subscriptions", "signals", "unwanted-access-rules"}
			totalSynced := 0
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  huntress-cli workflow status

  # Show status as JSON
  huntress-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("huntress-cli")
			}
