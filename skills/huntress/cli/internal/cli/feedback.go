...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".local", "share", "huntress-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("HUNTRESS_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("HUNTRESS_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "huntress-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.local/share/huntress-cli/feedback.jsonl.
When ` + "`HUNTRESS_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`HUNTRESS_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  huntress-cli feedback list
  huntress-cli feedback list --limit 5
  huntress-cli feedback list --json`,
