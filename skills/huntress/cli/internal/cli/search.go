...
In local mode: searches locally synced data only.`,
  ...
  huntress-cli search "error timeout"

  # Force local search only
  huntress-cli search "payment failed" --data-source local

  # Search a specific resource type locally
  huntress-cli search "critical" --type transactions --data-source local

  # JSON output for piping
  huntress-cli search "critical" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'huntress-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
			...
			case "incident-reports":
				results, err = db.SearchIncidentReports(query, limit)
			case "unwanted-access-rules":
				results, err = db.SearchUnwantedAccessRules(query, limit)
			case "":
				...
				{
					partial, searchErr := db.SearchIncidentReports(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search incident_reports failed: %w", searchErr)
					}
				...
				{
					partial, searchErr := db.SearchUnwantedAccessRules(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search unwanted_access_rules failed: %w", searchErr)
					}
					for _, r := range partial {
						key := string(r)
						if !seen[key] {
