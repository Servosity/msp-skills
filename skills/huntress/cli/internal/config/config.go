...
type Config struct {
	...
	HuntressApiKey    string            `toml:"api_key"`
	HuntressApiSecret string            `toml:"api_secret"`
}
...
func Load(configPath string) (*Config, error) {
	...
	path := configPath
	if path == "" {
		path = os.Getenv("HUNTRESS_CONFIG")
	}
	if path == "" {
		home, _ := os.UserHomeDir()
		path = filepath.Join(home, ".config", "huntress-cli", "config.toml")
	}
	cfg.Path = path
	...
	// Env var overrides
	if v := os.Getenv("HUNTRESS_API_KEY"); v != "" {
		cfg.HuntressApiKey = v
		cfg.markEnvOverride("HuntressApiKey")
		cfg.AuthSource = "env:HUNTRESS_API_KEY"
	}
	if v := os.Getenv("HUNTRESS_API_SECRET"); v != "" {
		cfg.HuntressApiSecret = v
		cfg.markEnvOverride("HuntressApiSecret")
		cfg.AuthSource = "env:HUNTRESS_API_SECRET"
	}
	...
	if cfg.AuthSource == "" && cfg.HuntressApiKey != "" {
		cfg.AuthSource = "config"
	}
	if cfg.AuthSource == "" && cfg.HuntressApiSecret != "" {
		cfg.AuthSource = "config"
	}
	...
	if v := os.Getenv("HUNTRESS_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	return cfg, nil
}
...
func (c *Config) AuthHeader() string {
	...
	if c.HuntressApiKey == "" || c.HuntressApiSecret == "" {
		return ""
	}
	credentials := c.HuntressApiKey + ":" + c.HuntressApiSecret
...
func (c *Config) SaveCredential(token string) error {
	...
	c.updateFileConfigField("AccessToken")
	c.HuntressApiKey = token
	delete(c.envOverrides, "HuntressApiKey")
	c.updateFileConfigField("HuntressApiKey")
	return c.save()
}
...
func (c *Config) ClearTokens() error {
	...
	delete(c.envOverrides, "HuntressApiKey")
	c.HuntressApiSecret = ""
	delete(c.envOverrides, "HuntressApiSecret")
	return c.save()
}
...
func (c *Config) configForSave() Config {
	out := *c
	if c.fileConfig != nil {
		if c.envOverrides["HuntressApiKey"] {
			out.HuntressApiKey = c.fileConfig.HuntressApiKey
		}
		if c.envOverrides["HuntressApiSecret"] {
			out.HuntressApiSecret = c.fileConfig.HuntressApiSecret
		}
	}
...
func (c *Config) updateFileConfigField(field string) {
	...
	case "HuntressApiKey":
		c.fileConfig.HuntressApiKey = c.HuntressApiKey
	case "HuntressApiSecret":
		c.fileConfig.HuntressApiSecret = c.HuntressApiSecret
	}
}
