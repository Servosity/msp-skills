# ImmyBot Printed CLI Agent Guide

This directory is a generated `immybot-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
immybot-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
immybot-cli which "<capability>" --json
immybot-cli <command> --help
```

Add `--agent` to command invocations for JSON, compact output, non-interactive defaults, and no color:

```bash
immybot-cli <command> --agent
```
...
```bash
immybot-cli <command> --help
immybot-cli <command> --dry-run --agent
```
...
Use exactly one of `auto`, `local`, `live`, or `computed`. Keep `auto` when the command honors `--data-source auto|local|live` by preferring live data with a local fallback; use `local` for local-only reads, `live` for remote-only reads, and `computed` for pure computation from embedded rules. Change a generated scaffold's `auto` default deliberately when its implementation has a narrower source, and reject incompatible `--data-source` requests with a clear error. Placeholder stubs still fail dogfood even when annotated.
...
3. After answering, always fire `immybot-cli teach --query "<question>" --resource <id> --resource-type <type> &` in the background - teaching is unconditional and is the anchor that triggers playbook synthesis. Teach the structural question with identifiers stripped (no names, emails, phone numbers, account ids); the CLI warns on obvious PII shapes but does not block.
...
Annotations: `teach`, `teach-playbook`, `playbook amend`, `playbook list`, `recall`, `learnings confirm`, `teach-pattern`, and `teach-lookup` carry `mcp:local-write=true` (writes land only in the CLI's own local store); `learnings list`, `learnings candidates`, and `learnings stats` carry `mcp:read-only=true`, which is narrower than it sounds: each one still opens the learn store read-write and creates the database file when it is absent, but none records a learning row; `learnings forget` and `learnings reject` keep the may-write/destructive defaults.
...
Disable the loop with `--no-learn` per-invocation or `IMMYBOT_NO_LEARN=true` for the whole session - useful for deterministic agent flows that don't want a learning row to silently change subsequent query results.
