...
func newAccessGetMePermissionsByPermissionTypeTenantsCmd(flags *rootFlags) *cobra.Command {
			...
			if len(args) == 0 {
				...
				if flags.asJSON {
					if printErr := printJSONFiltered(cmd.OutOrStdout(), map[string]any{
						...
						"usage": fmt.Sprintf("%s%s", cmd.CommandPath(), " <permissionType>"),
					}, flags); printErr != nil {
						return printErr
					}
				}
				return usageErr(fmt.Errorf("missing required argument\nUsage: %s%s", cmd.CommandPath(), " <permissionType>"))
			}
			path := "/api/v1/auth/me/permissions/{permissionType}/tenants"
			if len(args) < 1 || args[0] == "" {
				return usageErr(fmt.Errorf("permissionType is required\nUsage: %s <%s>", cmd.CommandPath(), "permissionType"))
			}
			path = replacePathParam(path, "permissionType", args[0])
			c, err := flags.newClient()
			if err != nil {
				return err
			}
			params := map[string]string{}
			data, prov, err := resolveReadWithStrategyAndResponsePath(cmd.Context(), c, flags, "live", "access", false, path, params, nil, "", cmd.ErrOrStderr())
			if err != nil {
