...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  immybot-cli workflow archive --timeout 0`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"application-locks", "audits", "audits-local-dx", "azure-errors", "brandings", "change-requests", "chocolatey", "computers", "computers-agent-status", "computers-dx", "computers-inventory", "computers-my", "computers-onboarding", "computers-paged", "computers-user-affinities", "dynamic-provider-types", "groups", "inventory-tasks", "licenses", "licenses-dx", "maintenance-actions", "maintenance-actions-maintenance-item", "maintenance-actions-version", "maintenance-sessions", "maintenance-tasks", "maintenance-tasks-local", "media", "media-local", "notifications", "notifications-dx", "notifications-unacknowledged", "persons", "persons-dx", "persons-requesting-access", "provider-agents", "provider-links", "provider-types", "rmm-links", "roles", "roles-permissions", "schedules", "schedules-running-schedule-ids", "scripts", "scripts-dx", "scripts-functions", "scripts-global", "scripts-global-names", "scripts-local", "scripts-local-names", "smtp-configs", "software", "software-local", "system", "tags", "target-assignments", "target-assignments-change-requests", "target-assignments-global", "target-assignments-maintenance-item-orders", "tenants", "tenants-computer-counts", "tenants-excluded-from-cross-tenant-deployments", "tenants-software-from-inventory-dx", "user-role-assignments", "users"}
			if cliutil.IsDogfoodEnv() {
				if len(resources) > 3 {
