...
func newComputersCreateSkipOnboardingCmd(flags *rootFlags) *cobra.Command {
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if flags.asJSON || (!isTerminal(cmd.OutOrStdout()) && !flags.csv && !flags.quiet && !flags.plain) {
				...
				if flags.selectFields != "" {
					...
					filtered = compactFields(filtered, map[string]bool{"computerId": true, "message": true, "success": true})
				}
				if len(filtered) > 0 {
					var parsed any
