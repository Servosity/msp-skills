...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
  ...
  immybot-cli doctor --fail-on stale`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if os.Getenv("IMMYBOT_TENANT_ID") != "" {
				authEnvSet = append(authEnvSet, "IMMYBOT_TENANT_ID")
			} else {
				authEnvInfo = append(authEnvInfo, "IMMYBOT_TENANT_ID set during auth login")
			}
			if os.Getenv("IMMYBOT_CLIENT_ID") != "" {
				authEnvSet = append(authEnvSet, "IMMYBOT_CLIENT_ID")
			} else {
				authEnvInfo = append(authEnvInfo, "IMMYBOT_CLIENT_ID set during auth login")
			}
			if os.Getenv("IMMYBOT_CLIENT_SECRET") != "" {
				authEnvSet = append(authEnvSet, "IMMYBOT_CLIENT_SECRET")
			} else {
				authEnvInfo = append(authEnvInfo, "IMMYBOT_CLIENT_SECRET set during auth login")
			}
			switch {
