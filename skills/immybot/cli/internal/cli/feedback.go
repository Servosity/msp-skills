...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("IMMYBOT_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("IMMYBOT_FEEDBACK_AUTO_SEND")))
...
When ` + "`IMMYBOT_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`IMMYBOT_FEEDBACK_AUTO_SEND=true`" + `, the entry is
