...
func newInventoryTasksDeleteLocalByTaskIdScriptsByInventoryKeyCmd(flags *rootFlags) *cobra.Command {
			...
			if len(args) == 0 {
				...
				if flags.asJSON {
					if printErr := printJSONFiltered(cmd.OutOrStdout(), map[string]any{
						...
						"usage": fmt.Sprintf("%s%s", cmd.CommandPath(), " <taskId> <inventoryKey>"),
					}, flags); printErr != nil {
						return printErr
					}
				}
				return usageErr(fmt.Errorf("missing required argument\nUsage: %s%s", cmd.CommandPath(), " <taskId> <inventoryKey>"))
			}
