...
func newMaintenanceActionsLogsCmd(flags *rootFlags) *cobra.Command {
	cmd := &cobra.Command{
		Use:         "logs",
		Short:       "Get logs for maintenance actions",
		Annotations: map[string]string{"mcp:read-only": "true", "pp:parent-group": "true", "pp:typed-exit-codes": "0,2"},
		RunE:        parentNoSubcommandRunE(flags),
	}

	cmd.AddCommand(newMaintenanceActionsLogsGetCmd(flags))
	return cmd
}
