...
func newMaintenanceSessionsPhasesCmd(flags *rootFlags) *cobra.Command {
	cmd := &cobra.Command{
		Use:         "phases",
		Short:       "Get phases for maintenance sessions",
		Annotations: map[string]string{"mcp:read-only": "true", "pp:parent-group": "true", "pp:typed-exit-codes": "0,2"},
		RunE:        parentNoSubcommandRunE(flags),
	}

	cmd.AddCommand(newMaintenanceSessionsPhasesGetCmd(flags))
	return cmd
}
