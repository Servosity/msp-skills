...
func newPluginsCmd(flags *rootFlags) *cobra.Command {
	cmd := &cobra.Command{
		Use:         "plugins",
		Short:       "Get and create plugins",
		Annotations: map[string]string{"mcp:read-only": "true", "pp:parent-group": "true", "pp:api-resource": "true", "pp:typed-exit-codes": "0,2"},
		RunE:        parentNoSubcommandRunE(flags),
	}

	cmd.AddCommand(newPluginsCreateApiV1ByProviderLinkIdByCatchAllCmd(flags))
	cmd.AddCommand(newPluginsGetApiV1ByProviderLinkIdByCatchAllCmd(flags))
	cmd.AddCommand(newPluginsGetApiV1ByProviderLinkIdByCatchAllV2Cmd(flags))
	return cmd
}
