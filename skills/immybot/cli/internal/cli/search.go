...
In local mode: searches locally synced data only.`,
  ...
  immybot-cli search "critical" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			query := args[0]
			// This API has a search endpoint: GET /api/v1/media/search
			if flags.dataSource != "local" {
				c, err := flags.newClient()
				if err != nil {
					return err
				}
				data, getErr := c.Get(cmd.Context(), "/api/v1/media/search", map[string]string{
					"q": query,
				})
				if getErr == nil {
			...
			case "":
				...
				{
					partial, searchErr := db.SearchComputers(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search computers failed: %w", searchErr)
					}
				...
				{
					partial, searchErr := db.SearchDynamicProviderTypes(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search dynamic_provider_types failed: %w", searchErr)
					}
					for _, r := range partial {
						key := string(r)
						if !seen[key] {
				...
				{
					partial, searchErr := db.SearchMaintenanceTasks(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search maintenance_tasks failed: %w", searchErr)
					}
					for _, r := range partial {
						key := string(r)
						if !seen[key] {
				...
				{
					partial, searchErr := db.SearchProviderTypes(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search provider_types failed: %w", searchErr)
					}
					for _, r := range partial {
						key := string(r)
						if !seen[key] {
				...
				{
					partial, searchErr := db.SearchSoftware(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search software failed: %w", searchErr)
					}
					for _, r := range partial {
						key := string(r)
						if !seen[key] {
				...
				{
					partial, searchErr := db.SearchTargetAssignments(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search target_assignments failed: %w", searchErr)
					}
					for _, r := range partial {
						key := string(r)
						if !seen[key] {
