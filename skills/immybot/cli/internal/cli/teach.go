...
// immybot-cli CLI: `teach` (fire-and-forget, silent on success,
...
Disabling: pass --no-learn or set ` + noLearnEnvVar + `=true.`,
		Example: `  immybot-cli teach --query "<question>" --resource-type <type> --resource <id> --resource <id> &`,
...
when learnings exist.`,
		Example: `  immybot-cli recall "<question>" --agent
  immybot-cli recall "<question>" --agent --min-confidence 2`,
...
func newLearningsListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  immybot-cli learnings list --agent
  immybot-cli learnings list --query "<substring>"
  immybot-cli learnings list --warnings --agent`,
...
Requires at least one of --resource, --action, or --all.`,
		Example: `  immybot-cli learnings forget "<question>" --resource <id>
  immybot-cli learnings forget "<question>" --all`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if len(args) == 0 {
				return cmd.Help()
			}
...
a whole family.`,
		Example: `  immybot-cli teach-pattern --query-template "items in {entity}" --resource-template "GROUP-{entity:category}" --resource-type "items" --entity-kind "category" --strategy substitute`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dryRunOK(flags) {
...
cannot be taught — they are derived from the canonical input.`,
		Example: `  immybot-cli teach-lookup --kind country --canonical "United States" --value USA`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dryRunOK(flags) {
