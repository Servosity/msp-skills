...
// immybot-pp-cli CLI: `teach` (fire-and-forget, silent on success,
...
import (
	...
	"immybot-pp-cli/internal/learn/entities"
	"immybot-pp-cli/internal/learn/lookups"
	"immybot-pp-cli/internal/learn/patterns"
...
const noLearnEnvVar = "IMMYBOT_NO_LEARN"
...
Disabling: pass --no-learn or set ` + noLearnEnvVar + `=true.`,
		Example: `  immybot-pp-cli teach --query "<question>" --resource-type <type> --resource <id> --resource <id> &`,
...
when learnings exist.`,
		Example: `  immybot-pp-cli recall "<question>" --agent
  immybot-pp-cli recall "<question>" --agent --min-confidence 2`,
...
func newLearningsListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  immybot-pp-cli learnings list --agent
  immybot-pp-cli learnings list --query "<substring>"
  immybot-pp-cli learnings list --warnings --agent`,
		Annotations: map[string]string{"mcp:read-only": "true"},
		RunE: func(cmd *cobra.Command, args []string) error {
			if dryRunOK(flags) {
...
Requires at least one of --resource, --action, or --all.`,
		Example: `  immybot-pp-cli learnings forget "<question>" --resource <id>
  immybot-pp-cli learnings forget "<question>" --all`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if len(args) == 0 {
				return cmd.Help()
			}
...
a whole family.`,
		Example: `  immybot-pp-cli teach-pattern --query-template "items in {entity}" --resource-template "GROUP-{entity:category}" --resource-type "items" --entity-kind "category" --strategy substitute`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dryRunOK(flags) {
...
cannot be taught — they are derived from the canonical input.`,
		Example: `  immybot-pp-cli teach-lookup --kind country --canonical "United States" --value USA`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dryRunOK(flags) {
