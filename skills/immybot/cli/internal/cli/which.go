...
var whichIndex = []whichEntry{
	{Command: "session-triage", Description: "Group last night's failed maintenance actions by root cause instead of reading the same error on forty machines.", Group: "Local state that compounds", WhyItMatters: "Reach for this first after any maintenance window: it turns N red machines into the handful of distinct problems actually worth a ticket."},
	{Command: "assignment-explain", Description: "Show every target assignment that resolves onto one computer, which scope matched, and which rules are shadowed.", Group: "Deployment resolution", WhyItMatters: "Use this for any \"why didn't this machine get X\" question; it answers what a computer receives and why, which no single endpoint does."},
	{Command: "version-spread", Description: "Semver-ordered distribution of one software title across every tenant, flagging everything below a floor.", Group: "Local state that compounds", WhyItMatters: "This is the CVE-response command: one call answers which clients are still exposed on a given title."},
	{Command: "fleet-diff", Description: "What actually changed between two syncs: computers added or removed, software versions moved, assignments modified.", Group: "Local state that compounds", WhyItMatters: "Use this to answer \"what changed since last night\" without diffing two exports by hand."},
	{Command: "script-blast-radius", Description: "Every maintenance task, software package, and computer that a script reaches before you edit it.", Group: "Deployment resolution", WhyItMatters: "Run this before editing any shared script; it is the only way to see downstream reach across tenants."},
	{Command: "onboarding-stalled", Description: "Computers stuck waiting to onboard, bucketed by age and annotated with whether onboarding was ever attempted.", Group: "Local state that compounds", WhyItMatters: "Surfaces machines that silently never finished onboarding, which is the failure mode clients notice first."},
	{Command: "psa-reconcile", Description: "Diff the ImmyBot roster against a linked PSA or RMM asset roster to find unlinked computers and orphaned assets.", Group: "Integration hygiene", WhyItMatters: "Run after each week of onboards and decommissions; mapping gaps otherwise surface as a wrong invoice or a machine that stopped getting maintenance."},
}
...
Exit codes:
  ...
  2  no confident match - the query did not score against any indexed capability; fall back to '--help' or 'search' if this CLI has one`,
		Example: `  immybot-pp-cli which "stale tickets"
  immybot-pp-cli which "bottleneck"
  immybot-pp-cli which --limit 1 "send message"
  immybot-pp-cli which                                # list the full capability index`,
