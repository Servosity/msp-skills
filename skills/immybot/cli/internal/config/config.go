...
type Config struct {
	...
	ImmybotTenantId string          `toml:"tenant_id"`
...
func Load(configPath string) (*Config, error) {
	cfg := &Config{
		BaseURL: "https://{subdomain}.immy.bot",
	}
	...
	// Env var overrides
	if v := os.Getenv("IMMYBOT_TENANT_ID"); v != "" {
		cfg.ImmybotTenantId = v
		cfg.markEnvOverride("ImmybotTenantId")
		cfg.AuthSource = "env:IMMYBOT_TENANT_ID"
		cfg.CredentialSource = "env:IMMYBOT_TENANT_ID"
	}
	if v := os.Getenv("IMMYBOT_CLIENT_ID"); v != "" {
		cfg.ClientID = v
		cfg.markEnvOverride("ClientID")
		cfg.AuthSource = "env:IMMYBOT_CLIENT_ID"
		cfg.CredentialSource = "env:IMMYBOT_CLIENT_ID"
	}
	if v := os.Getenv("IMMYBOT_CLIENT_SECRET"); v != "" {
		cfg.ClientSecret = v
		cfg.markEnvOverride("ClientSecret")
		cfg.AuthSource = "env:IMMYBOT_CLIENT_SECRET"
		cfg.CredentialSource = "env:IMMYBOT_CLIENT_SECRET"
	}
	...
	if v := os.Getenv("IMMYBOT_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	if v := os.Getenv("IMMYBOT_TOKEN_URL"); v != "" {
		cfg.TokenURL = v
	}
	...
	if v := strings.TrimSpace(os.Getenv("IMMYBOT_SUBDOMAIN")); v != "" {
		cfg.TemplateVars["subdomain"] = normalizeEndpointTemplateValue(v)
	} else {
		cfg.TemplateVars["subdomain"] = "your-instance"
	}
	return cfg, nil
}
...
func resolveConfigPath(configPath string) (string, bool, error) {
	...
	if path := os.Getenv("IMMYBOT_CONFIG"); path != "" {
...
func (c *Config) StoreScopeCredential() string {
	...
	if c.ImmybotTenantId != "" {
		parts = append(parts, "tenant_id="+c.ImmybotTenantId)
	}
	if len(parts) == 0 {
		return ""
	}
...
func (c *Config) credentials() *cliutil.Credentials {
	return &cliutil.Credentials{
		...
		ImmybotTenantId: c.ImmybotTenantId,
	}
}
...
func (c *Config) applyCredentials(creds *cliutil.Credentials) {
	...
	if c.ImmybotTenantId == "" {
		c.ImmybotTenantId = creds.ImmybotTenantId
	}
}
...
func (c *Config) ClearTokens() error {
	...
	c.updateFileConfigField("ClientSecret")
	c.ImmybotTenantId = ""
	delete(c.envOverrides, "ImmybotTenantId")
	c.updateFileConfigField("ImmybotTenantId")
	if c.AgentcookieManagedByExternalStore() {
		c.markAgentcookieManaged()
...
func (c *Config) configForSave() Config {
	out := *c
	if c.fileConfig != nil {
		if c.envOverrides["ImmybotTenantId"] {
			out.ImmybotTenantId = c.fileConfig.ImmybotTenantId
		}
