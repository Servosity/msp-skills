...
// touching disk. IMMYBOT_NO_CONFIG_WRITE makes the automatic token cache a
...
const (
	ncwToken   = "minted-token-IMMYBOT"
	ncwRefresh = "minted-refresh-IMMYBOT"
	ncwSecret  = "minted-secret-IMMYBOT"
)
...
func ncwMint(t *testing.T, value string, set bool) map[string]string {
	...
	home := t.TempDir()
	t.Setenv("HOME", home)
	t.Setenv("IMMYBOT_CONFIG", filepath.Join(home, "config.toml"))
	if set {
...
func TestNoConfigWriteTruthiness(t *testing.T) {
	...
	if NoConfigWriteEnv != "IMMYBOT_NO_CONFIG_WRITE" {
		t.Errorf("switch is named %q; the documented name is IMMYBOT_NO_CONFIG_WRITE", NoConfigWriteEnv)
	}
}
