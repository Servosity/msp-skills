...
func AdoptVerifiedLegacyDatabase(ctx context.Context, session *Session, state *LegacyMigrationState) error {
	...
	// SQLite takes VACUUM INTO's destination as an expression, so the path
	// binds as a parameter. Hand-quoting it instead put a caller-supplied
	// path inside the SQL text, which only stayed safe as long as the
	// doubling of every embedded quote was exactly right.
