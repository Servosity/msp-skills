# IT Glue Printed CLI Agent Guide

This directory is a generated `itglue-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
itglue-cli doctor --json
itglue-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
itglue-cli which "<capability>" --json
itglue-cli <command> --help
```
...
```bash
itglue-cli <command> --help
itglue-cli <command> --dry-run --agent
```
