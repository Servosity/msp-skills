...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  itglue-cli workflow archive

  # Full re-archive (ignore previous sync state)
  itglue-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"configurations", "contacts", "documents", "organizations", "passwords"}
			totalSynced := 0
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  itglue-cli workflow status

  # Show status as JSON
  itglue-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("itglue-cli")
			}
