...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".local", "share", "itglue-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("ITGLUE_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("ITGLUE_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "itglue-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.local/share/itglue-cli/feedback.jsonl.
When ` + "`ITGLUE_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`ITGLUE_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  itglue-cli feedback list
  itglue-cli feedback list --limit 5
  itglue-cli feedback list --json`,
