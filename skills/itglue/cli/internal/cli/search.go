...
In local mode: searches locally synced data only.`,
  ...
  itglue-cli search "error timeout"

  # Force local search only
  itglue-cli search "payment failed" --data-source local

  # Search a specific resource type locally
  itglue-cli search "critical" --type transactions --data-source local

  # JSON output for piping
  itglue-cli search "critical" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			query := args[0]
			// Sanitize into FTS5 quoted phrases so special characters (hyphens
			// in serial numbers, colons, operator barewords) match literally
			// instead of crashing the MATCH parser.
			ftsQuery := ftsQuote(query)
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'itglue-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
			...
			case "":
				...
				{
					partial, searchErr := db.Search(ftsQuery, limit)
					if searchErr != nil {
			...
			default:
				...
				results, err = db.Search(ftsQuery, limit)
			}
			if err != nil {
				return fmt.Errorf("search failed: %w", err)
			}

			// Project raw JSON:API records into clean, org-annotated summaries so
			// the fleet-wide search surfaces resource type + owning org per hit
			// (the differentiator over per-resource, per-org wrappers/MCPs).
			projected := make([]json.RawMessage, 0, len(results))
			for _, r := range results {
				rec, ok := parseITGRecord(r)
				if !ok {
					projected = append(projected, r)
					continue
				}
				if b, mErr := json.Marshal(rec.summary()); mErr == nil {
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/itglue-cli/data.db)")
