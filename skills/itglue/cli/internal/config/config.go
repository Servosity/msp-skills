...
type Config struct {
	...
	ItglueApiKey  string            `toml:"api_key"`
}
...
func Load(configPath string) (*Config, error) {
	...
	path := configPath
	if path == "" {
		path = os.Getenv("ITGLUE_CONFIG")
	}
	if path == "" {
		home, _ := os.UserHomeDir()
		path = filepath.Join(home, ".config", "itglue-cli", "config.toml")
	}
	cfg.Path = path
	...
	// Env var overrides
	if v := os.Getenv("ITGLUE_API_KEY"); v != "" {
		cfg.ItglueApiKey = v
		cfg.markEnvOverride("ItglueApiKey")
		cfg.AuthSource = "env:ITGLUE_API_KEY"
	}
	...
	if cfg.AuthSource == "" && cfg.ItglueApiKey != "" {
		cfg.AuthSource = "config"
	}
	...
	if v := os.Getenv("ITGLUE_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	return cfg, nil
}
...
func (c *Config) SaveCredential(token string) error {
	...
	c.updateFileConfigField("AccessToken")
	c.ItglueApiKey = token
	delete(c.envOverrides, "ItglueApiKey")
	c.updateFileConfigField("ItglueApiKey")
	return c.save()
}
...
func (c *Config) configForSave() Config {
	out := *c
	if c.fileConfig != nil {
		if c.envOverrides["ItglueApiKey"] {
			out.ItglueApiKey = c.fileConfig.ItglueApiKey
		}
	}
...
func (c *Config) updateFileConfigField(field string) {
	...
	case "ItglueApiKey":
		c.fileConfig.ItglueApiKey = c.ItglueApiKey
	}
}
