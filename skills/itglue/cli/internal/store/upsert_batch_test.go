...
func TestUpsertBatch_TypedFailureDoesNotStrandRelationshipsGeneric(t *testing.T) {
	...
	if typed != 0 {
		t.Fatalf("relationships count = %d, want 0 (typed insert violated NOT NULL on %q)", typed, "organizations_id")
	}
}
...
func TestUpsertBatch_KeysRelationshipsByChildAndParent(t *testing.T) {
	...
	parentQuery := fmt.Sprintf(`SELECT COUNT(*) FROM "%s" WHERE "organizations_id" = ?`, "relationships")
...
// TestUpsertBatch_SetsRelationshipsParentID verifies that dependent-resource
...
func TestUpsertBatch_SetsRelationshipsParentID(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	s, err := Open(dbPath)
	if err != nil {
		t.Fatalf("open: %v", err)
	...
	var matchedA int
	parentQuery := fmt.Sprintf(`SELECT COUNT(*) FROM "%s" WHERE parent_id = ?`, "relationships")
...
// TestUpsertBatch_RelationshipsSearchableViaGenericFTS verifies that a
...
// non-FTS5 `--type relationships` fallback), so populating it is the
...
func TestUpsertBatch_RelationshipsSearchableViaGenericFTS(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	s, err := Open(dbPath)
	if err != nil {
		t.Fatalf("open: %v", err)
	...
	if len(results) == 0 {
		t.Fatalf("dependent resource %q stored but not searchable via generic resources_fts (issue #2629)", "relationships")
	}
}
