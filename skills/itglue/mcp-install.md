# IT Glue MCP - install for every agent that speaks MCP

This page wires the IT Glue MCP server into any MCP client. If you use Claude
...
launch `itglue-mcp` directly on your machine - no hosting. *Remote* agents (ChatGPT,
Microsoft 365 Copilot / Copilot Studio, the Gemini app) only talk to an HTTPS
endpoint, so you expose `itglue-mcp` over HTTPS first.

## Prerequisite: install the MCP binary

Run the install command from [README.md](./README.md). It drops both `itglue-cli`
and `itglue-mcp` on your PATH. `itglue-mcp` is what the agents talk to.
...
Restart Gemini CLI; the IT Glue tools appear in its tool list. (The **Gemini app /
...
All remote agents need `itglue-mcp` reachable as a public **HTTPS** endpoint. Run it
in HTTP mode with your credentials in the environment:

```bash
ITGLUE_API_KEY=<value> ITGLUE_BASE_URL=<value> itglue-mcp --transport http --addr :7777
```
...
`itglue-mcp` you installed is not enough on its own. You also need a **Copilot Studio
license** and a **tenant admin** to enable it. This is a build-and-host task, not a
self-serve install.

Once `itglue-mcp` is hosted over HTTPS (above), the lowest-code route:
...
hermes mcp add itglue -- itglue-mcp

# OpenClaw
openclaw mcp set itglue '{"command":"itglue-mcp"}'
```
...
- `itglue-mcp: command not found`: the install dir is not on your PATH (the
