# Kaseya BMS Printed CLI Agent Guide

This directory is a generated `kaseya-bms-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
kaseya-bms-cli doctor --json
kaseya-bms-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
kaseya-bms-cli which "<capability>" --json
kaseya-bms-cli <command> --help
```
...
```bash
kaseya-bms-cli <command> --help
kaseya-bms-cli <command> --dry-run --agent
```
