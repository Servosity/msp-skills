...
func newAuthCmd(flags *rootFlags) *cobra.Command {
	cmd := &cobra.Command{
		Use:   "auth",
		Short: "Manage authentication for Bms",
...
func newAuthSetupCmd(_ *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: "  kaseya-bms-cli auth setup\n  kaseya-bms-cli auth setup --launch",
		RunE: func(cmd *cobra.Command, args []string) error {
			w := cmd.OutOrStdout()
			fmt.Fprintln(w, "No setup URL is configured for this CLI; check the API's docs.")
			fmt.Fprintln(w, "")
			fmt.Fprintln(w, "Then set:")
			fmt.Fprintln(w, "  export KASEYA_BMS_TOKEN=\"<your-token>\"")
			fmt.Fprintln(w, "  export KASEYA_BMS_BEARER_AUTH=\"<your-token>\"")
			fmt.Fprintln(w, "  kaseya-bms-cli auth set-token <token>")
			if !launch {
				return nil
			}
...
func newAuthStatusCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  kaseya-bms-cli auth status",
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if !authed {
				...
				fmt.Fprintln(w, "  export KASEYA_BMS_TOKEN=\"your-token-here\"")
				fmt.Fprintln(w, "  export KASEYA_BMS_BEARER_AUTH=\"your-token-here\"")
				fmt.Fprintf(w, "  kaseya-bms-cli auth set-token <token>\n")
				return authErr(fmt.Errorf("no credentials configured"))
			}
...
func newAuthSetTokenCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  kaseya-bms-cli auth set-token YOUR_TOKEN_HERE",
...
func newAuthLogoutCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  kaseya-bms-cli auth logout",
		RunE: func(cmd *cobra.Command, args []string) error {
			cfg, err := config.Load(flags.configPath)
			if err != nil {
				return configErr(err)
			...
			envStillSet := ""
			if envStillSet == "" && os.Getenv("KASEYA_BMS_TOKEN") != "" {
				envStillSet = "KASEYA_BMS_TOKEN"
			}
			if envStillSet == "" && os.Getenv("KASEYA_BMS_BEARER_AUTH") != "" {
				envStillSet = "KASEYA_BMS_BEARER_AUTH"
			}
