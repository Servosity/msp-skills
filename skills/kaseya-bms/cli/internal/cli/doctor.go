...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  kaseya-bms-cli doctor
  kaseya-bms-cli doctor --json
  kaseya-bms-cli doctor --fail-on warn`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if strings.Contains("", " OR ") {
				authEnvOptionalNames = append(authEnvOptionalNames, "KASEYA_BMS_TOKEN")
				if os.Getenv("KASEYA_BMS_TOKEN") != "" {
					authEnvSet = append(authEnvSet, "KASEYA_BMS_TOKEN")
					authEnvOptionalSatisfied = true
				}
			} else if os.Getenv("KASEYA_BMS_TOKEN") != "" {
				authEnvSet = append(authEnvSet, "KASEYA_BMS_TOKEN")
			} else {
				authEnvInfo = append(authEnvInfo, "KASEYA_BMS_TOKEN optional")
			}
			if strings.Contains("", " OR ") {
				authEnvOptionalNames = append(authEnvOptionalNames, "KASEYA_BMS_BEARER_AUTH")
				if os.Getenv("KASEYA_BMS_BEARER_AUTH") != "" {
					authEnvSet = append(authEnvSet, "KASEYA_BMS_BEARER_AUTH")
					authEnvOptionalSatisfied = true
				}
			} else if os.Getenv("KASEYA_BMS_BEARER_AUTH") != "" {
				authEnvSet = append(authEnvSet, "KASEYA_BMS_BEARER_AUTH")
			} else {
				authEnvInfo = append(authEnvInfo, "KASEYA_BMS_BEARER_AUTH optional")
			}
			switch {
			...
			if cfg != nil && cfg.BaseURL != "" {
				...
				if clientErr != nil {
					...
					if authHeader == "" {
						...
						if suggestion != "" {
							report["credentials"] = fmt.Sprintf("present, not verified. Run `%s %s` to confirm the token works end-to-end.", "kaseya-bms-cli", suggestion)
						} else {
...
func collectCacheReport(ctx context.Context, staleAfterSpec string) map[string]any {
	...
	if err != nil {
		if os.IsNotExist(err) {
			...
			report["hint"] = "Database not created yet; run 'kaseya-bms-cli sync' to hydrate."
			return report
		}
		report["status"] = "error"
	...
	if qerr != nil {
		...
		report["status"] = "unknown"
		report["hint"] = "No sync state recorded; run 'kaseya-bms-cli sync' to populate."
		return report
	}
	defer rows.Close()
	...
	case !haveAny && len(resources) == 0:
		report["status"] = "unknown"
		report["hint"] = "sync_state is empty; run 'kaseya-bms-cli sync' to hydrate."
	case fresh:
		report["status"] = "fresh"
	default:
		report["status"] = "stale"
		report["oldest_age"] = oldest.Round(time.Minute).String()
		report["hint"] = "Some resources are older than stale_after; run 'kaseya-bms-cli sync' to refresh."
	}
	return report
}
