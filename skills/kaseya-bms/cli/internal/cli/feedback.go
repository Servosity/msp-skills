...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".local", "share", "kaseya-bms-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("KASEYA_BMS_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("KASEYA_BMS_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "kaseya-bms-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.local/share/kaseya-bms-cli/feedback.jsonl.
When ` + "`KASEYA_BMS_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`KASEYA_BMS_FEEDBACK_AUTO_SEND=true`" + `, the entry is
