...
func newFinanceGetContractLookupsCmd(flags *rootFlags) *cobra.Command {
	var flagIds string
	var flagAccountId string
	var flagName string
	var flagExcludeExpiredContracts bool

	cmd := &cobra.Command{
		Use:         "get-contract-lookups",
		Aliases:     []string{"list"},
		Short:       "Get contract lookups",
		Example:     "  kaseya-bms-cli finance get-contract-lookups",
		Annotations: map[string]string{"pp:endpoint": "finance.get-contract-lookups", "pp:method": "GET", "pp:path": "/v2/finance/contracts/lookup", "mcp:read-only": "true"},
	...
	cmd.Flags().BoolVar(&flagExcludeExpiredContracts, "exclude-expired-contracts", false, "Exclude expired contracts")
