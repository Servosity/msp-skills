...
func newIntegrationsGetItgsuggestedResourcesCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			path = replacePathParam(path, "subjectId", args[0])
			if len(args) < 2 {
				return usageErr(fmt.Errorf("subjectType is required\nUsage: %s <%s>", cmd.CommandPath(), "subjectType"))
			}
	...
	cmd.Flags().IntVar(&flagResourceType, "resource-type", 0, "Resource type (one of: 0, 1)")
