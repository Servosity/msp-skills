...
func newServicedeskGetTicketTasksCmd(flags *rootFlags) *cobra.Command {
	var flagWait bool
	var flagWaitTimeout time.Duration
	var flagWaitInterval time.Duration

	cmd := &cobra.Command{
		Use:         "get-ticket-tasks <ticketId>",
		Short:       "Get ticket tasks",
		Example:     "  kaseya-bms-cli servicedesk get-ticket-tasks 42",
		Annotations: map[string]string{"pp:endpoint": "servicedesk.get-ticket-tasks", "pp:method": "GET", "pp:path": "/v2/servicedesk/tickets/{ticketId}/relatedtasks", "mcp:read-only": "true"},
