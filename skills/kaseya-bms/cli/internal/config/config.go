...
type Config struct {
	...
	KaseyaBmsToken      string            `toml:"bms_token"`
	KaseyaBmsBearerAuth string            `toml:"bms_bearer_auth"`
}
...
func Load(configPath string) (*Config, error) {
	...
	path := configPath
	if path == "" {
		path = os.Getenv("KASEYA_BMS_CONFIG")
	}
	if path == "" {
		home, _ := os.UserHomeDir()
		path = filepath.Join(home, ".config", "kaseya-bms-cli", "config.toml")
	}
	cfg.Path = path
	...
	// Env var overrides
	if v := os.Getenv("KASEYA_BMS_TOKEN"); v != "" {
		cfg.KaseyaBmsToken = v
		cfg.markEnvOverride("KaseyaBmsToken")
		cfg.AuthSource = "env:KASEYA_BMS_TOKEN"
	}
	if v := os.Getenv("KASEYA_BMS_BEARER_AUTH"); v != "" {
		cfg.KaseyaBmsBearerAuth = v
		cfg.markEnvOverride("KaseyaBmsBearerAuth")
		cfg.AuthSource = "env:KASEYA_BMS_BEARER_AUTH"
	}
	...
	if cfg.AuthSource == "" && cfg.KaseyaBmsToken != "" {
		cfg.AuthSource = "config"
	}
	if cfg.AuthSource == "" && cfg.KaseyaBmsBearerAuth != "" {
		cfg.AuthSource = "config"
	}
	...
	if v := os.Getenv("KASEYA_BMS_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	return cfg, nil
}
...
func (c *Config) AuthHeader() string {
	...
	if c.KaseyaBmsToken != "" {
		if c.AuthSource == "" {
			c.AuthSource = "env:KASEYA_BMS_TOKEN"
		}
		return "Bearer " + c.KaseyaBmsToken
	}
	if c.KaseyaBmsBearerAuth != "" {
		if c.AuthSource == "" {
			c.AuthSource = "env:KASEYA_BMS_BEARER_AUTH"
		}
		return "Bearer " + c.KaseyaBmsBearerAuth
	}
	if c.AccessToken != "" {
...
func (c *Config) ClearTokens() error {
	...
	c.updateFileConfigField("ClientSecret")
	c.KaseyaBmsToken = ""
	delete(c.envOverrides, "KaseyaBmsToken")
	c.KaseyaBmsBearerAuth = ""
	delete(c.envOverrides, "KaseyaBmsBearerAuth")
	return c.save()
}
...
func (c *Config) configForSave() Config {
	out := *c
	if c.fileConfig != nil {
		if c.envOverrides["KaseyaBmsToken"] {
			out.KaseyaBmsToken = c.fileConfig.KaseyaBmsToken
		}
		if c.envOverrides["KaseyaBmsBearerAuth"] {
			out.KaseyaBmsBearerAuth = c.fileConfig.KaseyaBmsBearerAuth
		}
	}
...
func (c *Config) updateFileConfigField(field string) {
	...
	case "KaseyaBmsToken":
		c.fileConfig.KaseyaBmsToken = c.KaseyaBmsToken
	case "KaseyaBmsBearerAuth":
		c.fileConfig.KaseyaBmsBearerAuth = c.KaseyaBmsBearerAuth
	}
}
