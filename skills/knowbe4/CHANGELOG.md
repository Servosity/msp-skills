...
- **`doctor` reported health it had not established.** It treated any HTTP response to `GET /` as
  ...
  It also dialled the shipped placeholder base URL (`https://{region}.api.knowbe4.com/v1`) and rendered the resulting failure
  as `FAIL API: unreachable`, telling an operator who had supplied every credential the install asked
  for that they were broken. It now refuses to dial a placeholder and names `KNOWBE4_BASE_URL`,
...
- **The install prompted for the wrong credentials.** The binary reads environment variables that the
  ...
  authenticate. Now declared on every install channel: `KNOWBE4_BASE_URL`, `KNOWBE4_USER_EVENT_API_KEY`.
