...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  knowbe4-cli workflow archive

  # Full re-archive (ignore previous sync state)
  knowbe4-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"account", "groups", "phishing-campaigns", "phishing-tests", "policies", "store-purchases", "training-campaigns", "training-enrollments", "users"}
			totalSynced := 0
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  knowbe4-cli workflow status

  # Show status as JSON
  knowbe4-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("knowbe4-cli")
			}
