...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".local", "share", "knowbe4-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("KNOWBE4_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("KNOWBE4_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "knowbe4-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.local/share/knowbe4-cli/feedback.jsonl.
When ` + "`KNOWBE4_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`KNOWBE4_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  knowbe4-cli feedback list
  knowbe4-cli feedback list --limit 5
  knowbe4-cli feedback list --json`,
