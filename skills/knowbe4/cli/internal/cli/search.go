...
In local mode: searches locally synced data only.`,
  ...
  knowbe4-cli search "error timeout"

  # Force local search only
  knowbe4-cli search "payment failed" --data-source local

  # Search a specific resource type locally
  knowbe4-cli search "critical" --type transactions --data-source local

  # JSON output for piping
  knowbe4-cli search "critical" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'knowbe4-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
			...
			case "store-purchases":
				results, err = db.SearchStorePurchases(query, limit)
			case "":
				...
				{
					partial, searchErr := db.SearchStorePurchases(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search store_purchases failed: %w", searchErr)
					}
