...
Resource scoping:
  ...
  knowbe4-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  knowbe4-cli sync --full

  # Incremental sync: only records from the last 7 days
  knowbe4-cli sync --since 7d

  # Parallel sync with 8 workers
  knowbe4-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  knowbe4-cli sync --latest-only`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			// Walk per-PST recipients and snapshot risk scores into the
			// insights-owned tables (pst_recipients, risk_snapshots) so the
			// transcendence commands (repeat-clickers, untrained-clickers,
			// risk-drift, group-risk-contribution, report-rate, qbr) have
			// data to read. Best-effort: a failure here never fails the
			// primary sync.
			if c != nil && db != nil && !flags.dryRun {
				if rc, sc, ierr := insights.SyncExtras(cmd.Context(), c, db.DB(), 100); ierr != nil {
					if humanFriendly {
						fmt.Fprintf(os.Stderr, "warning: insights sync incomplete: %v\n", ierr)
					} else {
						fmt.Fprintf(syncEventWriter, `{"event":"sync_warning","reason":"insights_sync_incomplete","message":%q}`+"\n", ierr.Error())
					}
				} else if !humanFriendly {
					fmt.Fprintf(syncEventWriter, `{"event":"insights_sync","recipients":%d,"risk_snapshots":%d}`+"\n", rc, sc)
				}
			}
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/knowbe4-cli/data.db)")
...
func upsertSingleObject(db *store.Store, resource string, data json.RawMessage) error {
	...
	case "phishing-campaigns":
		return db.UpsertPhishingCampaigns(data)
	case "phishing-tests":
		return db.UpsertPhishingTests(data)
	case "policies":
		return db.UpsertPolicies(data)
	case "store-purchases":
		return db.UpsertStorePurchases(data)
	case "training-campaigns":
		return db.UpsertTrainingCampaigns(data)
	case "training-enrollments":
		return db.UpsertTrainingEnrollments(data)
	case "users":
...
func syncResourcePath(resource string) (string, error) {
	paths := map[string]string{
		"account":              "/account/risk_score_history",
		"groups":               "/groups",
		"phishing-campaigns":   "/phishing/campaigns",
		"phishing-tests":       "/phishing/security_tests",
		"policies":             "/policies",
		"store-purchases":      "/training/store_purchases",
...
func dependentResourceDefs() []dependentResourceDef {
	return []dependentResourceDef{
		{Name: "groups", ParentTable: "groups", ParentIDParam: "group_id", PathTemplate: "/groups/{group_id}/members", KeyField: "", PathParams: []dependentPathParamDef{
			{Param: "group_id", Field: "id"},
		}},
	}
}
