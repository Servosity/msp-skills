...
func newUsersListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:         "list",
		Short:       "List all users with risk score and phish-prone percentage",
		Example:     "  knowbe4-cli users list --per-page 100 --agent",
	...
	cmd.Flags().IntVar(&flagPerPage, "per-page", 100, "Items per page (default 100, max 500)")
	cmd.Flags().StringVar(&flagStatus, "status", "", "Filter by status: active or archived")
	cmd.Flags().StringVar(&flagGroupId, "group-id", "", "Only users in this group id")
