...
type Config struct {
	...
	Knowbe4ApiKey string            `toml:"api_key"`
...
func Load(configPath string) (*Config, error) {
	...
	path := configPath
	if path == "" {
		path = os.Getenv("KNOWBE4_CONFIG")
	}
	if path == "" {
		home, _ := os.UserHomeDir()
		path = filepath.Join(home, ".config", "knowbe4-cli", "config.toml")
	}
	cfg.Path = path
	...
	// Env var overrides
	if v := os.Getenv("KNOWBE4_API_KEY"); v != "" {
		cfg.Knowbe4ApiKey = v
		cfg.markEnvOverride("Knowbe4ApiKey")
		cfg.AuthSource = "env:KNOWBE4_API_KEY"
	}
	...
	if cfg.AuthSource == "" && cfg.Knowbe4ApiKey != "" {
		cfg.AuthSource = "config"
	}
	...
	if v := os.Getenv("KNOWBE4_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	...
	if v := strings.TrimSpace(os.Getenv("KNOWBE4_REGION")); v != "" {
		cfg.TemplateVars["region"] = normalizeEndpointTemplateValue(v)
	} else {
		cfg.TemplateVars["region"] = "us"
	}
	return cfg, nil
}
...
func (c *Config) AuthHeader() string {
	...
	if c.Knowbe4ApiKey != "" {
		if c.AuthSource == "" {
			c.AuthSource = "env:KNOWBE4_API_KEY"
		}
		return applyAuthFormat("Bearer {token}", map[string]string{
			"api_key":         c.Knowbe4ApiKey,
			"KNOWBE4_API_KEY": c.Knowbe4ApiKey,
			"token":           c.Knowbe4ApiKey,
		})
	}
	if c.AccessToken != "" {
...
func (c *Config) ClearTokens() error {
	...
	c.updateFileConfigField("ClientSecret")
	c.Knowbe4ApiKey = ""
	delete(c.envOverrides, "Knowbe4ApiKey")
	return c.save()
}
...
func (c *Config) configForSave() Config {
	out := *c
	if c.fileConfig != nil {
		if c.envOverrides["Knowbe4ApiKey"] {
			out.Knowbe4ApiKey = c.fileConfig.Knowbe4ApiKey
		}
	}
...
func (c *Config) updateFileConfigField(field string) {
	...
	case "Knowbe4ApiKey":
		c.fileConfig.Knowbe4ApiKey = c.Knowbe4ApiKey
	}
}
