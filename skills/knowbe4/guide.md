...
- **Read-only against the Reporting API** - reporting commands never create, update, delete, or mutate remote resources; the one write surface is the `events` group (`events create` / `events delete`) against the separate User Event API, both `--dry-run`-previewable
