# KnowBe4 MCP - install for every agent that speaks MCP

This page wires the KnowBe4 MCP server into any MCP client. If you use Claude
...
launch `knowbe4-mcp` directly on your machine - no hosting. *Remote* agents (ChatGPT,
Microsoft 365 Copilot / Copilot Studio, the Gemini app) only talk to an HTTPS
endpoint, so you expose `knowbe4-mcp` over HTTPS first.

## Prerequisite: install the MCP binary

Run the install command from [README.md](./README.md). It drops both `knowbe4-cli`
and `knowbe4-mcp` on your PATH. `knowbe4-mcp` is what the agents talk to.
...
Restart Gemini CLI; the KnowBe4 tools appear in its tool list. (The **Gemini app /
...
All remote agents need `knowbe4-mcp` reachable as a public **HTTPS** endpoint. Run it
...
`knowbe4-mcp` you installed is not enough on its own. You also need a **Copilot Studio
license** and a **tenant admin** to enable it. This is a build-and-host task, not a
self-serve install.

Once `knowbe4-mcp` is hosted over HTTPS (above), the lowest-code route:
...
hermes mcp add knowbe4 -- knowbe4-mcp

# OpenClaw
openclaw mcp set knowbe4 '{"command":"knowbe4-mcp"}'
```
...
- `knowbe4-mcp: command not found`: the install dir is not on your PATH (the
