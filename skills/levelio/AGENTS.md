# Level Printed CLI Agent Guide

This directory is a generated `levelio-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
levelio-cli doctor --json
levelio-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
levelio-cli which "<capability>" --json
...
```bash
levelio-cli <command> --help
levelio-cli <command> --dry-run --agent
```
