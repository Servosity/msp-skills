...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  levelio-cli workflow archive

  # Full re-archive (ignore previous sync state)
  levelio-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"alerts", "custom-field-values", "custom-fields", "devices", "groups", "tags", "updates"}
			totalSynced := 0
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  levelio-cli workflow status

  # Show status as JSON
  levelio-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("levelio-cli")
			}
