...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".local", "share", "levelio-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("LEVELIO_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("LEVELIO_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "levelio-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.local/share/levelio-cli/feedback.jsonl.
When ` + "`LEVELIO_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`LEVELIO_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  levelio-cli feedback list
  levelio-cli feedback list --limit 5
  levelio-cli feedback list --json`,
