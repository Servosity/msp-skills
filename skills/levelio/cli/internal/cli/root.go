...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "levelio-cli",
		Short: `Levelio CLI — Every Level RMM endpoint, plus a local SQLite fleet store and offline cross-entity rollups no Level tool has: at-risk r…`,
		Long: `Levelio CLI — Every Level RMM endpoint, plus a local SQLite fleet store and offline cross-entity rollups no Level tool has: at-risk r…
...
Health check: run 'levelio-cli doctor' to verify auth and connectivity.
See README.md or the bundled SKILL.md for recipes.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("levelio-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.profileName, "profile", "", "Apply values from a saved profile (see 'levelio-cli profile list')")
	...
	rootCmd.AddCommand(newCustomFieldValuesCmd(flags))
	...
	rootCmd.AddCommand(newNovelAlertRecurrenceCmd(flags))
	rootCmd.AddCommand(newNovelAlertTriageCmd(flags))
	rootCmd.AddCommand(newNovelAtRiskCmd(flags))
	rootCmd.AddCommand(newNovelCfCoverageCmd(flags))
	rootCmd.AddCommand(newNovelClientScorecardCmd(flags))
	rootCmd.AddCommand(newNovelFleetCmd(flags))
	rootCmd.AddCommand(newNovelGroupTreeCmd(flags))
	rootCmd.AddCommand(newNovelPatchPostureCmd(flags))
	rootCmd.AddCommand(newNovelRebootDueCmd(flags))
	rootCmd.AddCommand(newNovelSecurityPostureCmd(flags))
	rootCmd.AddCommand(newNovelSinceCmd(flags))
	rootCmd.AddCommand(newNovelStaleCmd(flags))
	rootCmd.AddCommand(newNovelTagAuditCmd(flags))
	rootCmd.AddCommand(newAPICmd(flags))
	rootCmd.AddCommand(newAutomationsPromotedCmd(flags))
