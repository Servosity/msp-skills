...
Resource scoping:
  ...
  levelio-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  levelio-cli sync --full

  # Incremental sync: only records from the last 7 days
  levelio-cli sync --since 7d

  # Parallel sync with 8 workers
  levelio-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  levelio-cli sync --latest-only`,
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/levelio-cli/data.db)")
...
func syncResourceFieldSelector(resource string) (string, string) {
	switch resource {
	case "custom-field-values":
		return "assigned_to_id", "assigned_to_id,custom_field_id,custom_field_name,value"
	}
	return "", ""
}
...
func upsertSingleObject(db *store.Store, resource string, data json.RawMessage) error {
	...
	case "groups_devices":
		return db.UpsertGroupsDevices(data)
	case "tags_devices":
		return db.UpsertTagsDevices(data)
	default:
...
func syncResourcePath(resource string) (string, error) {
	paths := map[string]string{
		"alerts":              "/v2/alerts",
		"custom-field-values": "/v2/custom_field_values",
		"custom-fields":       "/v2/custom_fields",
		"devices":             "/v2/devices",
		"groups":              "/v2/groups",
		"tags":                "/v2/tags",
		"updates":             "/v2/updates",
