...
func RegisterIntents(s *server.MCPServer) {
	s.AddTool(
		mcplib.NewTool("data_hygiene_audit",
			mcplib.WithDescription("Every assignee missing a custom-field value, so required asset/warranty data can be backfilled."),
			mcplib.WithBoolean("missing", mcplib.Description("Override the recipe's --missing value.")),
		),
		handleDataHygieneAudit,
	)
}

// handleDataHygieneAudit runs the data_hygiene_audit recipe intent: Every assignee missing a custom-field value, so required asset/warranty data can be backfilled.
func handleDataHygieneAudit(ctx context.Context, req mcplib.CallToolRequest) (*mcplib.CallToolResult, error) {
	...
	args = appendRecipeBoolFlag(args, "missing", input["missing"], true)
