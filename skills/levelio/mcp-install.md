# Level MCP - install for every agent that speaks MCP

This page wires the Level MCP server into any MCP client. If you use Claude
...
launch `levelio-mcp` directly on your machine - no hosting. *Remote* agents (ChatGPT,
Microsoft 365 Copilot / Copilot Studio, the Gemini app) only talk to an HTTPS
endpoint, so you expose `levelio-mcp` over HTTPS first.

## Prerequisite: install the MCP binary

Run the install command from [README.md](./README.md). It drops both `levelio-cli`
and `levelio-mcp` on your PATH. `levelio-mcp` is what the agents talk to.
...
Restart Gemini CLI; the Level tools appear in its tool list. (The **Gemini app /
...
`levelio-mcp` you installed is not enough on its own. You also need a **Copilot Studio
license** and a **tenant admin** to enable it. This is a build-and-host task, not a
self-serve install.

Once `levelio-mcp` is hosted over HTTPS (above), the lowest-code route:
...
hermes mcp add levelio -- levelio-mcp

# OpenClaw
openclaw mcp set levelio '{"command":"levelio-mcp"}'
```
...
- `levelio-mcp: command not found`: the install dir is not on your PATH (the
