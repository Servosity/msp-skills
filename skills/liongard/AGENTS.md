# Liongard Printed CLI Agent Guide

This directory is a generated `liongard-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
liongard-cli doctor --json
liongard-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
liongard-cli which "<capability>" --json
liongard-cli <command> --help
```
...
```bash
liongard-cli <command> --help
liongard-cli <command> --dry-run --agent
```
