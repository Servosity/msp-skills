...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  liongard-cli workflow archive

  # Full re-archive (ignore previous sync state)
  liongard-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"access-keys", "agents", "detections", "environments", "groups", "launchpoints", "systems", "tasks", "timeline", "users"}
			totalSynced := 0
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  liongard-cli workflow status

  # Show status as JSON
  liongard-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("liongard-cli")
			}
