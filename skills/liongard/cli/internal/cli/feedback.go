...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".local", "share", "liongard-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("LIONGARD_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("LIONGARD_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "liongard-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.local/share/liongard-cli/feedback.jsonl.
When ` + "`LIONGARD_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`LIONGARD_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  liongard-cli feedback list
  liongard-cli feedback list --limit 5
  liongard-cli feedback list --json`,
