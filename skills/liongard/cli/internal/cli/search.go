...
In local mode: searches locally synced data only.`,
  ...
  liongard-cli search "error timeout"

  # Force local search only
  liongard-cli search "payment failed" --data-source local

  # Search a specific resource type locally
  liongard-cli search "critical" --type transactions --data-source local

  # JSON output for piping
  liongard-cli search "critical" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'liongard-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
			...
			case "detections":
				results, err = db.SearchDetections(query, limit)
			case "environments":
				results, err = db.SearchEnvironments(query, limit)
			case "groups":
				results, err = db.SearchGroups(query, limit)
			case "inspectors":
				results, err = db.SearchInspectors(query, limit)
			case "metrics":
				results, err = db.SearchMetrics(query, limit)
			case "systems":
				results, err = db.SearchSystems(query, limit)
			case "tasks":
			...
			case "":
				...
				{
					partial, searchErr := db.SearchDetections(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search detections failed: %w", searchErr)
					}
					for _, r := range partial {
						key := string(r)
						if !seen[key] {
				...
				{
					partial, searchErr := db.SearchInspectors(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search inspectors failed: %w", searchErr)
					}
					for _, r := range partial {
						key := string(r)
						if !seen[key] {
				...
				{
					partial, searchErr := db.SearchSystems(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search systems failed: %w", searchErr)
					}
					for _, r := range partial {
						key := string(r)
						if !seen[key] {
