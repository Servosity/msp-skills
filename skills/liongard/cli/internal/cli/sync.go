...
Resource scoping:
  ...
  liongard-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  liongard-cli sync --full

  # Incremental sync: only records from the last 7 days
  liongard-cli sync --since 7d

  # Parallel sync with 8 workers
  liongard-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  liongard-cli sync --latest-only`,
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/liongard-cli/data.db)")
...
func syncResourceFieldSelector(resource string) (string, string) {
	switch resource {
	case "agents":
		return "fields[]", "Bucket,CreatedOn,Description,Hostname,ID,IP,JobQueue,LastHeartbeat,Managed,Name,Platform,SAAS,Status,Suspended,Type,UID,UpdatedOn,UpdatesCheckedOn"
	case "detections":
		return "fields[]", "CreatedOn,Description,Detection,ID,Keywords,Name,TimelineID"
	case "environments":
		return "fields[]", "CreatedOn,Description,ExpiresOn,ID,KeyRotation,Name,ServiceProviderID,ShortName,Static,Status,UpdatedOn,Visible,Website"
	case "groups":
		return "fields[]", "Description,ID,Name"
	case "launchpoints":
		return "fields[]", "ActiveEnd,ActiveStart,Alias,Config,CreatedOn,Enabled,EnvironmentID,FreqInterval,FreqRecurrenceFactor,FreqRelativeInterval,FreqSubdayInterval,FreqSubdayType,FreqType,ID,LastAdHocRun,LastScheduledRun,NextScheduledFor,ParentID,Status,TotalAdHocRuns,TotalScheduledRuns,UpdatedOn,Version,Visible"
	case "systems":
		return "fields[]", "CreatedBy,CreatedOn,Description,ID,LastVerifyTime,Name,Status,Tags,UniqueCompositeKey,UpdatedBy,UpdatedOn,UserDefined,Visible"
	case "tasks":
		return "fields[]", "AutoClose,Category,CommonID,CreatedBy,CreatedOn,Description,DueDate,ExternalID,ID,Keywords,Name,Owner,PercentComplete,RoarStatus,Source,StartDate,UpdatedBy,UpdatedOn,Visible"
	case "timeline":
		return "fields[]", "AdHoc,Alerts,ChangeDetections,CreatedOn,ExitCode,ExitMessage,Expired,FinishedAt,ID,Latest,PayloadSizeBytes,ProcessingAt,RunningAt,ScheduledAt,Status,UpdatedOn"
	case "users":
		return "fields[]", "CreatedOn,Department,Email,EnvironmentName,FirstName,ID,LastLogin,LastName,Locked,MfaFactorAuth,Owner,ReceiveTechUpdates,Status,UpdatedOn,Username,Visible"
	}
	return "", ""
}
...
func upsertSingleObject(db *store.Store, resource string, data json.RawMessage) error {
	...
	switch resource {
	case "access-keys":
		return db.UpsertAccessKeys(data)
	case "agents":
		return db.UpsertAgents(data)
	case "flush":
		return db.UpsertFlush(data)
	case "authentication":
		return db.UpsertAuthentication(data)
	case "detections":
		return db.UpsertDetections(data)
	case "environments":
		return db.UpsertEnvironments(data)
	case "related_entities":
		return db.UpsertRelatedEntities(data)
	case "groups":
	...
	case "inspectors":
		return db.UpsertInspectors(data)
	case "launchpoints":
		return db.UpsertLaunchpoints(data)
	case "run":
		return db.UpsertRun(data)
	case "metrics":
		return db.UpsertMetrics(data)
	case "systems":
		return db.UpsertSystems(data)
	case "view":
