...
var whichIndex = []whichEntry{
	{Command: "drift", Description: "Every change detected across all your client environments within a time window, joined to the owning environment and system.", Group: "Whole-estate visibility", WhyItMatters: "Reach for this when an agent needs the whole-estate 'what changed overnight' answer instead of paging the per-environment detections endpoint."},
	{Command: "environments overview", Description: "One client's complete picture in a single command: its systems, each system's launchpoints, agent, latest inspection, open detections, and key metrics.", Group: "Whole-estate visibility", WhyItMatters: "Use when an agent is asked about a specific client and needs the joined picture rather than five separate endpoint calls."},
	{Command: "launchpoints stale", Description: "Every launchpoint whose newest inspection is older than a threshold, with the owning environment and system named.", Group: "Inspection health", WhyItMatters: "Use to find collectors that silently stopped reporting before a client notices their data is old."},
	{Command: "agents offline", Description: "Every offline agent across the estate, joined to the environment it serves.", Group: "Inspection health", WhyItMatters: "Use for the daily 'is everything still collecting' health sweep across all clients at once."},
	{Command: "coverage", Description: "Monitoring gaps: systems with no launchpoint bound, and environments with no systems at all.", Group: "Inspection health", WhyItMatters: "Use during onboarding QA to catch clients you provisioned but never pointed an inspector at."},
...
Exit codes:
  ...
  2  no confident match - the query did not score against any indexed capability; fall back to '--help' or 'search' if this CLI has one`,
		Example: `  liongard-cli which "stale tickets"
  liongard-cli which "bottleneck"
  liongard-cli which --limit 1 "send message"
  liongard-cli which                                # list the full capability index`,
