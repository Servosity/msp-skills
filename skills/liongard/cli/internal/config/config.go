...
type Config struct {
	...
	Path                    string            `toml:"-"`
	LiongardApiKey          string            `toml:"api_key"`
	LiongardEndpointsApiKey string            `toml:"endpoints_api_key"`
...
func Load(configPath string) (*Config, error) {
	...
	path := configPath
	if path == "" {
		path = os.Getenv("LIONGARD_CONFIG")
	}
	if path == "" {
		home, _ := os.UserHomeDir()
		path = filepath.Join(home, ".config", "liongard-cli", "config.toml")
	}
	cfg.Path = path
	...
	if v := os.Getenv("LIONGARD_ENDPOINTS_API_KEY"); v != "" {
		cfg.LiongardEndpointsApiKey = v
		cfg.AuthSource = "env:LIONGARD_ENDPOINTS_API_KEY"
	}
	...
	if cfg.AuthSource == "" && cfg.LiongardApiKey != "" {
		cfg.AuthSource = "config"
	}
	if cfg.AuthSource == "" && cfg.LiongardEndpointsApiKey != "" {
		cfg.AuthSource = "config"
	}
	...
	if v := os.Getenv("LIONGARD_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	...
	if v := strings.TrimSpace(os.Getenv("LIONGARD_INSTANCE")); v != "" {
		cfg.TemplateVars["instance"] = normalizeEndpointTemplateValue(v)
	} else {
		cfg.TemplateVars["instance"] = "example"
	}
	return cfg, nil
}
...
func (c *Config) AuthHeader() string {
	...
	if c.LiongardApiKey != "" {
		return c.LiongardApiKey
	}
	if c.LiongardEndpointsApiKey != "" {
		return c.LiongardEndpointsApiKey
	}
	return ""
}
