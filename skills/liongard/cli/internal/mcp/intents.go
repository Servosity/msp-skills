...
func RegisterIntents(s *server.MCPServer) {
	s.AddTool(
		mcplib.NewTool("pivot_one_metric_across_all_systems",
			mcplib.WithDescription("A system-by-value table for a single RoarPath metric across the estate."),
			mcplib.WithBoolean("csv", mcplib.Description("Override the recipe's --csv value.")),
		),
		handlePivotOneMetricAcrossAllSystems,
	)
}

// handlePivotOneMetricAcrossAllSystems runs the pivot_one_metric_across_all_systems recipe intent: A system-by-value table for a single RoarPath metric across the estate.
func handlePivotOneMetricAcrossAllSystems(ctx context.Context, req mcplib.CallToolRequest) (*mcplib.CallToolResult, error) {
