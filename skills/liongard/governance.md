# liongard skill - governance and safety model

> Unofficial. Community-built skill for the Liongard API. Not affiliated with,
> endorsed by, or sponsored by the vendor.
> This page tells an MSP owner exactly what the liongard skill can touch and how to
> scope it, so you can decide what to let an AI agent do.

## What it authenticates as

The skill drives the `liongard-cli` binary (and `liongard-mcp`),
authenticating with `LIONGARD_API_KEY`, `LIONGARD_ENDPOINTS_API_KEY`, `LIONGARD_INSTANCE`. Credentials are read from the environment only -
never written to disk, never logged, never sent anywhere except the Liongard API.
...
| **Write (routine)** | Day-to-day mutations and inspection triggers. | `environments create` / `create-bulk` / `update`, `launchpoints launchpoint` (create) / `update` / `update-bulk`, `metrics create` / `update`, `users create` / `update`, `agents update`, `import` (bulk create/upsert), and the inspection triggers `launchpoints run` / `bulk-run` / `run-stale` and `agents flush job-queue` | Preview with `--dry-run`, then an approved write (where a command documents its own confirm gate, use it too) |
| **Credential / security** | Touches tokens, keys, MFA. | `access-keys create` (mints an API key), `authentication log-in`, `authentication verify-token`, `auth set-token`, `auth setup` | Human-in-the-loop only |
| **Destructive** | Irreversible data or config loss. | `access-keys delete`, `agents delete`, `environments delete`, `launchpoints bulk-delete`, `launchpoints delete`, `metrics delete`, `users delete` | Human-in-the-loop only, explicit confirmation |
...
the Liongard API, and you can read every line of how it does so. The skill is
