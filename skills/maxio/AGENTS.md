# Maxio Printed CLI Agent Guide

This directory is a generated `maxio-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
maxio-cli doctor --json
maxio-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
maxio-cli which "<capability>" --json
maxio-cli <command> --help
```
...
```bash
maxio-cli <command> --help
maxio-cli <command> --dry-run --agent
```
