...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  maxio-cli workflow archive

  # Full re-archive (ignore previous sync state)
  maxio-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"endpoints-json", "subscriptions-components-json", "subscriptions-mrr-json"}
			totalSynced := 0
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  maxio-cli workflow status

  # Show status as JSON
  maxio-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("maxio-cli")
			}
