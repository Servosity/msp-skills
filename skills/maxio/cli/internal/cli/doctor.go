...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  maxio-cli doctor
  maxio-cli doctor --json
  maxio-cli doctor --fail-on warn`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if cfg != nil {
				...
				if header == "" {
					...
					report["auth_hint"] = "Set Basic credentials with: export MAXIO_USERNAME=\"your-username\" MAXIO_PASSWORD=\"your-password\""
					report["auth_key_url"] = "https://app.chargify.com/signup/maxio-billing-sandbox"
				} else {
					authConfigured = true
			...
			if os.Getenv("MAXIO_USERNAME") != "" {
				authEnvSet = append(authEnvSet, "MAXIO_USERNAME")
				...
				authEnvRequiredMissing = append(authEnvRequiredMissing, "MAXIO_USERNAME")
			}
			if os.Getenv("MAXIO_PASSWORD") != "" {
				authEnvSet = append(authEnvSet, "MAXIO_PASSWORD")
			} else if authConfigured {
				authSource, _ := report["auth_source"].(string)
				if authSource == "" {
					authSource = "config"
				...
				authEnvRequiredMissing = append(authEnvRequiredMissing, "MAXIO_PASSWORD")
			}
			switch {
			...
			if cfg != nil && cfg.BaseURL != "" {
				...
				if clientErr != nil {
					...
					if authHeader == "" {
						...
						if suggestion != "" {
							report["credentials"] = fmt.Sprintf("present, not verified. Run `%s %s` to confirm the token works end-to-end.", "maxio-cli", suggestion)
						} else {
...
func collectCacheReport(ctx context.Context, staleAfterSpec string) map[string]any {
	...
	if err != nil {
		if os.IsNotExist(err) {
			...
			report["hint"] = "Database not created yet; run 'maxio-cli sync' to hydrate."
			return report
		}
		report["status"] = "error"
	...
	if qerr != nil {
		...
		report["status"] = "unknown"
		report["hint"] = "No sync state recorded; run 'maxio-cli sync' to populate."
		return report
	}
	defer rows.Close()
	...
	case !haveAny && len(resources) == 0:
		report["status"] = "unknown"
		report["hint"] = "sync_state is empty; run 'maxio-cli sync' to hydrate."
	case fresh:
		report["status"] = "fresh"
	default:
		report["status"] = "stale"
		report["oldest_age"] = oldest.Round(time.Minute).String()
		report["hint"] = "Some resources are older than stale_after; run 'maxio-cli sync' to refresh."
	}
	return report
}
