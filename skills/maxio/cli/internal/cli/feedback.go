...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".local", "share", "maxio-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("MAXIO_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("MAXIO_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "maxio-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.local/share/maxio-cli/feedback.jsonl.
When ` + "`MAXIO_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`MAXIO_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  maxio-cli feedback list
  maxio-cli feedback list --limit 5
  maxio-cli feedback list --json`,
