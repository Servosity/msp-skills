...
type Config struct {
	...
	MaxioUsername string            `toml:"username"`
	MaxioPassword string            `toml:"password"`
...
func Load(configPath string) (*Config, error) {
	cfg := &Config{
		BaseURL: "https://{site}.chargify.com",
	}

	// Resolve config path
	path := configPath
	if path == "" {
		path = os.Getenv("MAXIO_CONFIG")
	}
	if path == "" {
		home, _ := os.UserHomeDir()
		path = filepath.Join(home, ".config", "maxio-cli", "config.toml")
	}
	cfg.Path = path
	...
	// Env var overrides
	if v := os.Getenv("MAXIO_USERNAME"); v != "" {
		cfg.MaxioUsername = v
		cfg.markEnvOverride("MaxioUsername")
		cfg.AuthSource = "env:MAXIO_USERNAME"
	}
	// Advanced Billing HTTP Basic auth uses the API key as the username and the
	// constant "x" as the password. MAXIO_API_KEY is the canonical single-secret
	// form: accept it as a username alias so the user supplies one secret instead
	// of a username/password pair.
	if v := os.Getenv("MAXIO_API_KEY"); v != "" {
		cfg.MaxioUsername = v
		cfg.markEnvOverride("MaxioUsername")
		cfg.AuthSource = "env:MAXIO_API_KEY"
	}
	if v := os.Getenv("MAXIO_PASSWORD"); v != "" {
		cfg.MaxioPassword = v
		cfg.markEnvOverride("MaxioPassword")
		cfg.AuthSource = "env:MAXIO_PASSWORD"
	}
	// Default the Basic password to the constant "x" the API requires, so a user
	// who provides only the key authenticates without also setting MAXIO_PASSWORD.
	// Mark it as an env override so this synthetic value (which the user never
	// set) is not persisted back into config.toml by a save flow.
	if cfg.MaxioUsername != "" && cfg.MaxioPassword == "" {
		cfg.MaxioPassword = "x"
		cfg.markEnvOverride("MaxioPassword")
	}
	...
	if cfg.AuthSource == "" && cfg.MaxioUsername != "" {
		cfg.AuthSource = "config"
	}
	if cfg.AuthSource == "" && cfg.MaxioPassword != "" {
		cfg.AuthSource = "config"
	}
	...
	if v := os.Getenv("MAXIO_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	...
	if v := strings.TrimSpace(os.Getenv("MAXIO_SITE")); v != "" {
		cfg.TemplateVars["site"] = normalizeEndpointTemplateValue(v)
	} else {
		cfg.TemplateVars["site"] = "subdomain"
	}
	return cfg, nil
}
...
func (c *Config) AuthHeader() string {
	...
	if c.MaxioUsername == "" || c.MaxioPassword == "" {
		return ""
	}
	credentials := c.MaxioUsername + ":" + c.MaxioPassword
...
func (c *Config) SaveCredential(token string) error {
	...
	c.updateFileConfigField("AccessToken")
	c.MaxioUsername = token
	delete(c.envOverrides, "MaxioUsername")
	c.updateFileConfigField("MaxioUsername")
	return c.save()
}
...
func (c *Config) ClearTokens() error {
	...
	delete(c.envOverrides, "MaxioUsername")
	c.MaxioPassword = ""
	delete(c.envOverrides, "MaxioPassword")
	return c.save()
}
...
func (c *Config) configForSave() Config {
	out := *c
	if c.fileConfig != nil {
		if c.envOverrides["MaxioUsername"] {
			out.MaxioUsername = c.fileConfig.MaxioUsername
		}
		if c.envOverrides["MaxioPassword"] {
			out.MaxioPassword = c.fileConfig.MaxioPassword
		}
	}
...
func (c *Config) updateFileConfigField(field string) {
	...
	case "MaxioUsername":
		c.fileConfig.MaxioUsername = c.MaxioUsername
	case "MaxioPassword":
		c.fileConfig.MaxioPassword = c.MaxioPassword
	}
}
