# maxio skill - governance and safety model

> Unofficial. Community-built skill for the Maxio API. Not affiliated with,
> endorsed by, or sponsored by the vendor.
> This page tells an MSP owner exactly what the maxio skill can touch and how to
> scope it, so you can decide what to let an AI agent do.

## What it authenticates as

The skill drives the `maxio-cli` binary (and `maxio-mcp`),
authenticating with `MAXIO_PASSWORD`, `MAXIO_SITE`, `MAXIO_USERNAME`. Credentials are read from the environment only -
never written to disk, never logged, never sent anywhere except the Maxio API.
...
the Maxio API, and you can read every line of how it does so. The skill is
