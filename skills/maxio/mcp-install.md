# Maxio MCP - install for every agent that speaks MCP

This page wires the Maxio MCP server into any MCP client. If you use Claude
...
launch `maxio-mcp` directly on your machine - no hosting. *Remote* agents (ChatGPT,
Microsoft 365 Copilot / Copilot Studio, the Gemini app) only talk to an HTTPS
endpoint, so you expose `maxio-mcp` over HTTPS first.

## Prerequisite: install the MCP binary

Run the install command from [README.md](./README.md). It drops both `maxio-cli`
and `maxio-mcp` on your PATH. `maxio-mcp` is what the agents talk to.
...
Restart Gemini CLI; the Maxio tools appear in its tool list. (The **Gemini app /
...
All remote agents need `maxio-mcp` reachable as a public **HTTPS** endpoint. Run it
...
`maxio-mcp` you installed is not enough on its own. You also need a **Copilot Studio
license** and a **tenant admin** to enable it. This is a build-and-host task, not a
self-serve install.

Once `maxio-mcp` is hosted over HTTPS (above), the lowest-code route:
...
hermes mcp add maxio -- maxio-mcp

# OpenClaw
openclaw mcp set maxio '{"command":"maxio-mcp"}'
```
...
- `maxio-mcp: command not found`: the install dir is not on your PATH (the
