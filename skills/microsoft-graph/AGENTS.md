# Microsoft Graph Printed CLI Agent Guide

This directory is a generated `microsoft-graph-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
microsoft-graph-cli doctor --json
microsoft-graph-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
microsoft-graph-cli which "<capability>" --json
microsoft-graph-cli <command> --help
```
...
```bash
microsoft-graph-cli <command> --agent
```
...
```bash
microsoft-graph-cli <command> --help
microsoft-graph-cli <command> --dry-run --agent
```
