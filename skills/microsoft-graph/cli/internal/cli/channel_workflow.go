...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  microsoft-graph-cli workflow archive

  # Full re-archive (ignore previous sync state)
  microsoft-graph-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"devices", "directory-roles", "groups", "licenses", "managed-devices", "security", "security-incidents", "users"}
			totalSynced := 0
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  microsoft-graph-cli workflow status

  # Show status as JSON
  microsoft-graph-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("microsoft-graph-cli")
			}
