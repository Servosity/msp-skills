...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".local", "share", "microsoft-graph-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("MICROSOFT_GRAPH_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("MICROSOFT_GRAPH_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "microsoft-graph-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.local/share/microsoft-graph-cli/feedback.jsonl.
When ` + "`MICROSOFT_GRAPH_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`MICROSOFT_GRAPH_FEEDBACK_AUTO_SEND=true`" + `, the entry is
