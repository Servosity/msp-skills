...
In local mode: searches locally synced data only.`,
  ...
  microsoft-graph-cli search "error timeout"

  # Force local search only
  microsoft-graph-cli search "payment failed" --data-source local

  # Search a specific resource type locally
  microsoft-graph-cli search "critical" --type transactions --data-source local

  # JSON output for piping
  microsoft-graph-cli search "critical" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'microsoft-graph-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
			...
			switch resourceType {
			case "security":
				results, err = db.SearchSecurity(query, limit)
			case "":
				...
				{
					partial, searchErr := db.SearchSecurity(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search security failed: %w", searchErr)
					}
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/microsoft-graph-cli/data.db)")
