...
Resource scoping:
  ...
  microsoft-graph-cli sync

  # Sync specific resources only
  microsoft-graph-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  microsoft-graph-cli sync --full

  # Incremental sync: only records from the last 7 days
  microsoft-graph-cli sync --since 7d

  # Parallel sync with 8 workers
  microsoft-graph-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  microsoft-graph-cli sync --latest-only`,
