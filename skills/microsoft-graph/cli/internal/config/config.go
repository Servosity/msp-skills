...
type Config struct {
	...
	MicrosoftGraphToken string            `toml:"graph_token"`
}
...
func Load(configPath string) (*Config, error) {
	...
	if path == "" {
		...
		path = filepath.Join(home, ".config", "microsoft-graph-cli", "config.toml")
	}
	cfg.Path = path
	...
	// Env var overrides
	if v := os.Getenv("MICROSOFT_GRAPH_TOKEN"); v != "" {
		cfg.MicrosoftGraphToken = v
		cfg.markEnvOverride("MicrosoftGraphToken")
		cfg.AuthSource = "env:MICROSOFT_GRAPH_TOKEN"
	}
	...
	if cfg.AuthSource == "" && cfg.MicrosoftGraphToken != "" {
		cfg.AuthSource = "config"
	}
	...
	if v := os.Getenv("MICROSOFT_GRAPH_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	return cfg, nil
}
...
func (c *Config) AuthHeader() string {
	...
	if c.MicrosoftGraphToken != "" {
		if c.AuthSource == "" {
			c.AuthSource = "env:MICROSOFT_GRAPH_TOKEN"
		}
		return applyAuthFormat("Bearer {token}", map[string]string{
			"graph_token":           c.MicrosoftGraphToken,
			"MICROSOFT_GRAPH_TOKEN": c.MicrosoftGraphToken,
			"token":                 c.MicrosoftGraphToken,
		})
	}
	if c.AccessToken != "" {
...
func (c *Config) ClearTokens() error {
	...
	c.updateFileConfigField("ClientID")
	c.updateFileConfigField("ClientSecret")
	c.MicrosoftGraphToken = ""
	delete(c.envOverrides, "MicrosoftGraphToken")
	return c.save()
}
...
func (c *Config) configForSave() Config {
	out := *c
	if c.fileConfig != nil {
		if c.envOverrides["MicrosoftGraphToken"] {
			out.MicrosoftGraphToken = c.fileConfig.MicrosoftGraphToken
		}
	}
...
func (c *Config) updateFileConfigField(field string) {
	...
	case "MicrosoftGraphToken":
		c.fileConfig.MicrosoftGraphToken = c.MicrosoftGraphToken
	}
}
