# install.ps1 - install microsoft-graph-cli and microsoft-graph-mcp on Windows.
#
# Pulls prebuilt binaries from this skill's latest GitHub Release (microsoft-graph-v*).
...
$CliBin  = "microsoft-graph-cli.exe"
$McpBin  = "microsoft-graph-mcp.exe"
...
$ReleaseBase = if ($env:MSP_SKILLS_RELEASE_BASE) {
  ...
  # Each skill is versioned/tagged independently (microsoft-graph-vX.Y.Z),
...
Write-Host "  microsoft-graph-cli --version"
Write-Host ""
Write-Host "Next:"
Write-Host "  First command + auth: https://github.com/$Owner/$Repo/tree/main/skills/microsoft-graph#readme"
Write-Host "  Claude Desktop / ChatGPT wire-up: https://github.com/$Owner/$Repo/blob/main/skills/microsoft-graph/mcp-install.md"
