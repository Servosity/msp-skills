# MSPbots Printed CLI Agent Guide

This directory is a generated `mspbots-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
mspbots-cli doctor --json
mspbots-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
mspbots-cli which "<capability>" --json
mspbots-cli <command> --help
```
...
```bash
mspbots-cli <command> --help
mspbots-cli <command> --dry-run --agent
```
