...
Resource scoping:
  ...
  mspbots-cli sync --latest-only`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if len(resources) == 0 {
				if humanFriendly {
					fmt.Fprintln(os.Stderr, "mspbots-cli sync: no bulk-list endpoints in spec; populate the store via single-fetch commands instead.")
				} else {
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/mspbots-pp-cli/data.db)")
