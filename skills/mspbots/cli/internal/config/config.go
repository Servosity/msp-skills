...
type Config struct {
	...
	MspbotsApiKey string            `toml:"api_key"`
}

func Load(configPath string) (*Config, error) {
	cfg := &Config{
		BaseURL: "https://api.mspbots.ai",
	}

	// Resolve config path
	path := configPath
	if path == "" {
		path = os.Getenv("MSPBOTS_CONFIG")
	}
	if path == "" {
		home, _ := os.UserHomeDir()
		path = filepath.Join(home, ".config", "mspbots-cli", "config.toml")
	}
	cfg.Path = path
	...
	// Env var overrides
	if v := os.Getenv("MSPBOTS_API_KEY"); v != "" {
		cfg.MspbotsApiKey = v
		cfg.markEnvOverride("MspbotsApiKey")
		cfg.AuthSource = "env:MSPBOTS_API_KEY"
	}
	...
	if cfg.AuthSource == "" && cfg.MspbotsApiKey != "" {
		cfg.AuthSource = "config"
	}
	...
	if v := os.Getenv("MSPBOTS_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	return cfg, nil
}
...
func (c *Config) AuthHeader() string {
	...
	replacements := map[string]string{
		"token":           token,
		"api_key":         c.MspbotsApiKey,
		"MSPBOTS_API_KEY": c.MspbotsApiKey,
	}
	return applyAuthFormat("{api_key}", replacements)
}
...
func (c *Config) SaveCredential(token string) error {
	...
	c.updateFileConfigField("AccessToken")
	c.MspbotsApiKey = token
	delete(c.envOverrides, "MspbotsApiKey")
	c.updateFileConfigField("MspbotsApiKey")
	return c.save()
}
...
func (c *Config) configForSave() Config {
	out := *c
	if c.fileConfig != nil {
		if c.envOverrides["MspbotsApiKey"] {
			out.MspbotsApiKey = c.fileConfig.MspbotsApiKey
		}
	}
...
func (c *Config) updateFileConfigField(field string) {
	...
	case "MspbotsApiKey":
		c.fileConfig.MspbotsApiKey = c.MspbotsApiKey
	}
}
