...
The skill drives the `mspbots-cli` binary (and `mspbots-mcp`),
authenticating with `MSPBOTS_API_KEY`. Credentials are read from the environment only -
never written to disk, never logged, never sent anywhere except the MSPbots API.
...
| **Write (routine)** | Day-to-day mutations. | `promoted dataset` | Preview with `--dry-run`, then an approved write (where a command documents its own confirm gate, use it too) |
...
the MSPbots API, and you can read every line of how it does so. The skill is
