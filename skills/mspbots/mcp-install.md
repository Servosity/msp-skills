# MSPbots MCP - install for every agent that speaks MCP

This page wires the MSPbots MCP server into any MCP client. If you use Claude
...
launch `mspbots-mcp` directly on your machine - no hosting. *Remote* agents (ChatGPT,
Microsoft 365 Copilot / Copilot Studio, the Gemini app) only talk to an HTTPS
endpoint, so you expose `mspbots-mcp` over HTTPS first.

## Prerequisite: install the MCP binary

Run the install command from [README.md](./README.md). It drops both `mspbots-cli`
and `mspbots-mcp` on your PATH. `mspbots-mcp` is what the agents talk to.
...
```json
{
  "mcpServers": {
    "mspbots": {
      "command": "mspbots-mcp",
      "env": {
        "MSPBOTS_API_KEY": "<your-mspbots_api_key>",
        "MSPBOTS_BASE_URL": "https://api.mspbots.ai"
      }
    }
  }
}
...
Restart Gemini CLI; the MSPbots tools appear in its tool list. (The **Gemini app /
...
All remote agents need `mspbots-mcp` reachable as a public **HTTPS** endpoint. Run it
in HTTP mode with your credentials in the environment:

```bash
MSPBOTS_API_KEY=<value> MSPBOTS_BASE_URL=<value> mspbots-mcp --transport http --addr :7777
```
...
`mspbots-mcp` you installed is not enough on its own. You also need a **Copilot Studio
license** and a **tenant admin** to enable it. This is a build-and-host task, not a
self-serve install.

Once `mspbots-mcp` is hosted over HTTPS (above), the lowest-code route:
...
hermes mcp add mspbots -- mspbots-mcp

# OpenClaw
openclaw mcp set mspbots '{"command":"mspbots-mcp"}'
```
...
- `mspbots-mcp: command not found`: the install dir is not on your PATH (the
