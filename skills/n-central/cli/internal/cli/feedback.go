...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".local", "share", "n-central-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("N_CENTRAL_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("N_CENTRAL_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "n-central-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.local/share/n-central-pp-cli/feedback.jsonl.
When ` + "`N_CENTRAL_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`N_CENTRAL_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  n-central-cli feedback list
  n-central-cli feedback list --limit 5
  n-central-cli feedback list --json`,
