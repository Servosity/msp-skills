...
Resource scoping:
  ...
  n-central-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  n-central-cli sync --full

  # Incremental sync: only records from the last 7 days
  n-central-cli sync --since 7d

  # Parallel sync with 8 workers
  n-central-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  n-central-cli sync --latest-only`,
...
func upsertSingleObject(db *store.Store, resource string, data json.RawMessage) error {
	...
	case "org-units":
		return db.UpsertOrgUnits(data)
	case "scheduled-tasks":
		return db.UpsertScheduledTasks(data)
	case "server":
		return db.UpsertServer(data)
	case "service-orgs":
		return db.UpsertServiceOrgs(data)
	case "sites":
