...
func (c *Client) doInternal(ctx context.Context, method, path string, params map[string]string, body any, headerOverrides map[string]string, readOnlyIntent bool) (json.RawMessage, int, error) {
	...
	// N-central two-step auth: exchange the long-lived User-API JWT for a
	// short-lived access token before live data calls. Auth endpoints
	// (/auth/authenticate, /auth/refresh, /auth/validate) manage their own
	// tokens via the ncauth package and must not recurse here. This runs only
	// on the live path (after the dry-run return) so --dry-run never dials.
	if c.Config != nil && c.Config.NcentralJwt != "" && !strings.HasPrefix(path, "/auth") {
		if err := ncauth.Ensure(ctx, c.Config); err != nil {
			return nil, 0, err
		}
		if h, herr := c.authHeader(ctx); herr == nil {
			authHeader = h
		}
	}
...
func (c *Client) maskCredentialText(text string, extraCredentials ...string) string {
	...
	if c != nil && c.Config != nil {
		...
		addCredential(c.Config.NcentralJwt)
	}
