...
func Load(configPath string) (*Config, error) {
	...
	if path == "" {
		...
		path = filepath.Join(home, ".config", "n-central-cli", "config.toml")
	}
	cfg.Path = path
	...
	if v := os.Getenv("NCENTRAL_JWT"); v != "" {
		cfg.NcentralJwt = v
		cfg.markEnvOverride("NcentralJwt")
...
func (c *Config) ClearTokens() error {
	...
	c.updateFileConfigField("ClientSecret")
	c.NcentralJwt = ""
	delete(c.envOverrides, "NcentralJwt")
	return c.save()
}
...
func (c *Config) configForSave() Config {
	out := *c
	if c.fileConfig != nil {
		if c.envOverrides["NcentralJwt"] {
			out.NcentralJwt = c.fileConfig.NcentralJwt
		}
	}
...
func (c *Config) updateFileConfigField(field string) {
	...
	case "NcentralJwt":
		c.fileConfig.NcentralJwt = c.NcentralJwt
	}
}
...
//   - ncauth.applyTokens' best-effort SaveTokens (the JWT exchange and the
//     refresh) is gated too: the exchanged access token stays in memory.
const NoConfigWriteEnv = "N_CENTRAL_NO_CONFIG_WRITE"
...
// material. It is the ONLY save path N_CENTRAL_NO_CONFIG_WRITE suppresses;
