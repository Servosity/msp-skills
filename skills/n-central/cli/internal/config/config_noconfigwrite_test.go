...
// touching disk. N_CENTRAL_NO_CONFIG_WRITE makes the automatic token cache a
...
func TestNoConfigWriteTruthiness(t *testing.T) {
	...
	if NoConfigWriteEnv != "N_CENTRAL_NO_CONFIG_WRITE" {
		t.Errorf("switch is named %q; the documented name is N_CENTRAL_NO_CONFIG_WRITE", NoConfigWriteEnv)
	}
}
