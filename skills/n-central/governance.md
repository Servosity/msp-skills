...
authenticating with `NCENTRAL_JWT`. Credentials are read from the environment only -
never written to disk, never logged, never sent anywhere except the N-able N-central API.
...
| **Write (routine)** | Day-to-day mutations. | `devices assets`, `triage` | Preview with `--dry-run`, then an approved write (where a command documents its own confirm gate, use it too) |
| **Credential / security** | Touches tokens, keys, MFA. | `customers registration-token`, `org-units registration-token` | Human-in-the-loop only |
...
the N-able N-central API, and you can read every line of how it does so. The skill is
