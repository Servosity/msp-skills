# N-able N-central MCP - install for every agent that speaks MCP

This page wires the N-able N-central MCP server into any MCP client. If you use Claude
...
launch `n-central-mcp` directly on your machine - no hosting. *Remote* agents (ChatGPT,
Microsoft 365 Copilot / Copilot Studio, the Gemini app) only talk to an HTTPS
endpoint, so you expose `n-central-mcp` over HTTPS first.

## Prerequisite: install the MCP binary

Run the install command from [README.md](./README.md). It drops both `n-central-cli`
and `n-central-mcp` on your PATH. `n-central-mcp` is what the agents talk to.
...
Restart Gemini CLI; the N-able N-central tools appear in its tool list. (The **Gemini app /
...
All remote agents need `n-central-mcp` reachable as a public **HTTPS** endpoint.
`n-central-mcp` speaks **stdio only** - it has no HTTP mode and opens no listener of
its own - so put a stdio-to-HTTP bridge in front of it:

```bash
# ChatGPT and anything else that accepts SSE:
NCENTRAL_JWT=<value> N_CENTRAL_BASE_URL=<value> npx -y supergateway --stdio "n-central-mcp" --port 7777
...
`n-central-mcp` you installed is not enough on its own. You also need a **Copilot Studio
license** and a **tenant admin** to enable it. This is a build-and-host task, not a
self-serve install.

Once `n-central-mcp` is hosted over HTTPS (above), the lowest-code route:
...
hermes mcp add n-central -- n-central-mcp

# OpenClaw
openclaw mcp set n-central '{"command":"n-central-mcp"}'
```
...
- `n-central-mcp: command not found`: the install dir is not on your PATH (the
