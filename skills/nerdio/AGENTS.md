# Nerdio Manager Printed CLI Agent Guide

This directory is a generated `nerdio-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
nerdio-cli doctor --json
nerdio-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
nerdio-cli which "<capability>" --json
nerdio-cli <command> --help
```
...
```bash
nerdio-cli <command> --help
nerdio-cli <command> --dry-run --agent
```
