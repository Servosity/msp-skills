...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  nerdio-cli workflow archive

  # Full re-archive (ignore previous sync state)
  nerdio-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"accounts", "autoscale-profiles", "cost-estimator", "directories", "environment-variables", "resource-groups", "schedules", "scripted-actions", "secure-variables"}
			totalSynced := 0
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  nerdio-cli workflow status

  # Show status as JSON
  nerdio-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("nerdio-cli")
			}
