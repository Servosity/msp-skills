...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".local", "share", "nerdio-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("NERDIO_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("NERDIO_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "nerdio-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.local/share/nerdio-cli/feedback.jsonl.
When ` + "`NERDIO_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`NERDIO_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  nerdio-cli feedback list
  nerdio-cli feedback list --limit 5
  nerdio-cli feedback list --json`,
