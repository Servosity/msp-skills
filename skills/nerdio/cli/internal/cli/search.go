...
In local mode: searches locally synced data only.`,
  ...
  nerdio-cli search "error timeout"

  # Force local search only
  nerdio-cli search "payment failed" --data-source local

  # Search a specific resource type locally
  nerdio-cli search "critical" --type transactions --data-source local

  # JSON output for piping
  nerdio-cli search "critical" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			query := args[0]
			// This API has a search endpoint: POST /rest-api/v1/accounts/{account_id}/users
			if flags.dataSource != "local" {
				c, err := flags.newClient()
				if err != nil {
					return err
				}
				data, _, getErr := c.Post(cmd.Context(), "/rest-api/v1/accounts/{account_id}/users", map[string]any{
					"searchTerm": query,
					"pageSize":   100,
					"pageNum":    0,
				})
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'nerdio-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
			...
			switch resourceType {
			case "host-pools":
				results, err = db.SearchHostPools(query, limit)
			case "scripted-actions":
				results, err = db.SearchScriptedActions(query, limit)
			case "":
				...
				{
					partial, searchErr := db.SearchHostPools(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search host_pools failed: %w", searchErr)
					}
				...
				{
					partial, searchErr := db.SearchScriptedActions(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search scripted_actions failed: %w", searchErr)
					}
					for _, r := range partial {
						key := string(r)
						if !seen[key] {
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/nerdio-cli/data.db)")
