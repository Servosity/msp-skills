...
Resource scoping:
  ...
  nerdio-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  nerdio-cli sync --full

  # Incremental sync: only records from the last 7 days
  nerdio-cli sync --since 7d

  # Parallel sync with 8 workers
  nerdio-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  nerdio-cli sync --latest-only`,
...
func upsertSingleObject(db *store.Store, resource string, data json.RawMessage) error {
	...
	case "autoscale-profiles":
		return db.UpsertAutoscaleProfiles(data)
	case "desktop-images":
		return db.UpsertDesktopImages(data)
	case "devices":
		return db.UpsertDevices(data)
	case "directories":
		return db.UpsertDirectories(data)
	case "host-pools":
		return db.UpsertHostPools(data)
	case "hosts":
		return db.UpsertHosts(data)
	case "job":
	...
	case "scripted-actions":
		return db.UpsertScriptedActions(data)
	case "workspaces":
...
func syncResourcePath(resource string) (string, error) {
	paths := map[string]string{
		...
		"autoscale-profiles":    "/rest-api/v1/autoscale-profiles",
		"cost-estimator":        "/rest-api/v1/costestimator/list",
		"directories":           "/rest-api/v1/directories",
		"environment-variables": "/rest-api/v1/environment-variables",
		"resource-groups":       "/rest-api/v1/resource-group",
		"schedules":             "/rest-api/v1/schedules",
		"scripted-actions":      "/rest-api/v1/scripted-actions",
		"secure-variables":      "/rest-api/v1/secure-variables",
	}
