...
func Load(configPath string) (*Config, error) {
	cfg := &Config{
		BaseURL: "https://nmm.example.com",
	}

	// Resolve config path
	path := configPath
	if path == "" {
		path = os.Getenv("NERDIO_CONFIG")
	}
	if path == "" {
		home, _ := os.UserHomeDir()
		path = filepath.Join(home, ".config", "nerdio-cli", "config.toml")
	}
	cfg.Path = path
	...
	// Env var overrides
	if v := os.Getenv("NERDIO_CLIENT_ID"); v != "" {
		cfg.ClientID = v
		cfg.markEnvOverride("ClientID")
		cfg.AuthSource = "env:NERDIO_CLIENT_ID"
	}
	if v := os.Getenv("NERDIO_CLIENT_SECRET"); v != "" {
		cfg.ClientSecret = v
		cfg.markEnvOverride("ClientSecret")
		cfg.AuthSource = "env:NERDIO_CLIENT_SECRET"
	}
	...
	if v := os.Getenv("NERDIO_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	if v := os.Getenv("NERDIO_TOKEN_URL"); v != "" {
		cfg.TokenURL = v
	}
	return cfg, nil
}
...
const NoConfigWriteEnv = "NERDIO_NO_CONFIG_WRITE"
...
// material. It is the ONLY save path NERDIO_NO_CONFIG_WRITE suppresses;
