...
// touching disk. NERDIO_NO_CONFIG_WRITE makes the automatic token cache a
...
const (
	ncwToken   = "minted-token-NERDIO"
	ncwRefresh = "minted-refresh-NERDIO"
	ncwSecret  = "minted-secret-NERDIO"
)
...
func ncwMint(t *testing.T, value string, set bool) map[string]string {
	...
	home := t.TempDir()
	t.Setenv("HOME", home)
	t.Setenv("NERDIO_CONFIG", filepath.Join(home, "config.toml"))
	if set {
...
func TestNoConfigWriteStillAllowsTheWipe(t *testing.T) {
	...
	t.Setenv("HOME", home)
	t.Setenv("NERDIO_CONFIG", cfgPath)
...
func TestNoConfigWriteTruthiness(t *testing.T) {
	...
	if NoConfigWriteEnv != "NERDIO_NO_CONFIG_WRITE" {
		t.Errorf("switch is named %q; the documented name is NERDIO_NO_CONFIG_WRITE", NoConfigWriteEnv)
	}
}
