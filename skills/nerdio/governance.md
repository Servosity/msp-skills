# nerdio skill - governance and safety model

> Unofficial. Community-built skill for the Nerdio Manager API. Not affiliated with,
> endorsed by, or sponsored by the vendor.
> This page tells an MSP owner exactly what the nerdio skill can touch and how to
> scope it, so you can decide what to let an AI agent do.

## What it authenticates as

The skill drives the `nerdio-cli` binary (and `nerdio-mcp`),
authenticating with `NERDIO_BASE_URL`, `NERDIO_CLIENT_ID`, `NERDIO_CLIENT_SECRET`, `NERDIO_OAUTH_SCOPE`, `NERDIO_TOKEN_URL`. Credentials are read from the environment only -
never written to disk, never logged, never sent anywhere except the Nerdio Manager API.
...
the Nerdio Manager API, and you can read every line of how it does so. The skill is
