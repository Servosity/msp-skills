...
For the short install path - the one-line installer, the `.mcpb` one-click for
Claude Desktop, and per-agent MCP wire-up (Hermes, OpenClaw, GitHub Copilot, the
Microsoft and Google stacks) - see [README.md](./README.md) and
...
| --- | --- | --- | --- |
| `NERDIO_BASE_URL` | per_call | Yes | Your NMM instance root URL (per-MSP install), e.g. `https://nmm.contoso.com`. There is no vendor-global endpoint. |
| `NERDIO_TOKEN_URL` | per_call | Yes | Entra ID token endpoint for your tenant: `https://login.microsoftonline.com/<TENANT_ID>/oauth2/v2.0/token` |
| `NERDIO_CLIENT_ID` | per_call | Yes | OAuth2 client ID of the API client created in NMM (Settings -> Integrations -> REST API). |
| `NERDIO_CLIENT_SECRET` | per_call | Yes | OAuth2 client secret for the API client. |
| `NERDIO_OAUTH_SCOPE` | per_call | Yes | Bare scope `<app-id>/.default` - do NOT prefix with `api://` (triggers AADSTS500011). |
