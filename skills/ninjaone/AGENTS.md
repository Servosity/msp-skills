...
This directory is a generated `ninjaone-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
ninjaone-cli doctor --json
ninjaone-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
ninjaone-cli which "<capability>" --json
ninjaone-cli <command> --help
```
...
```bash
ninjaone-cli <command> --help
ninjaone-cli <command> --dry-run --agent
```
