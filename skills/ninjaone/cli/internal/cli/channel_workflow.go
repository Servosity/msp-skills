...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  ninjaone-cli workflow archive

  # Full re-archive (ignore previous sync state)
  ninjaone-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"activities", "checklist", "contacts", "devices", "devices-detailed", "document-templates", "knowledgebase", "knowledgebase-global-articles", "knowledgebase-organization-articles", "locations", "organization", "organization-checklists", "organizations", "organizations-detailed", "queries", "queries-antivirus-status", "queries-antivirus-threats", "queries-backup-usage", "queries-computer-systems", "queries-custom-fields", "queries-custom-fields-detailed", "queries-device-health", "queries-logged-on-users", "queries-network-interfaces", "queries-operating-systems", "queries-os-patch-installs", "queries-os-patches", "queries-policy-overrides", "queries-processors", "queries-raid-controllers", "queries-raid-drives", "queries-scoped-custom-fields", "queries-scoped-custom-fields-detailed", "queries-software", "queries-software-patch-installs", "queries-software-patches", "queries-volumes", "queries-windows-services", "related-items", "tag", "ticketing", "ticketing-app-user-contact", "user", "user-end-users", "user-technicians", "vulnerability"}
			totalSynced := 0
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/ninjaone-cli/data.db)")
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  ninjaone-cli workflow status

  # Show status as JSON
  ninjaone-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("ninjaone-cli")
			}
