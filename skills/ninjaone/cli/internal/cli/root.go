...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "ninjaone-cli",
		Short: `Ninjaone CLI — Every NinjaOne report, plus a local store that answers fleet-wide questions no single API call can: patch compliance, b…`,
		Long: `Ninjaone CLI — Every NinjaOne report, plus a local store that answers fleet-wide questions no single API call can: patch compliance, b…
...
Health check: run 'ninjaone-cli doctor' to verify auth and connectivity.
See README.md or the bundled SKILL.md for recipes.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("ninjaone-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.profileName, "profile", "", "Apply values from a saved profile (see 'ninjaone-cli profile list')")
	...
	rootCmd.AddCommand(newNovelAvSweepCmd(flags))
	rootCmd.AddCommand(newNovelBackupCoverageCmd(flags))
	rootCmd.AddCommand(newNovelDriftCmd(flags))
	rootCmd.AddCommand(newNovelFleetHealthCmd(flags))
	rootCmd.AddCommand(newNovelOsEolCmd(flags))
	rootCmd.AddCommand(newNovelPatchComplianceCmd(flags))
	rootCmd.AddCommand(newNovelSoftwareAuditCmd(flags))
