...
// resolver the CLI uses, which reads NINJAONE_CONFIG and applies
...
func TestMCPClientHonoursTheConfigEnvVar(t *testing.T) {
	...
	if c.Config.Path != path {
		t.Errorf("MCP server read %q, want the NINJAONE_CONFIG path %q", c.Config.Path, path)
	}
