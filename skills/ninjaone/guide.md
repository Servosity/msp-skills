...
For the short install path  -  a one-line installer, or one prompt pasted into your agent  -  see [README.md](./README.md); for per-agent MCP wire-up (Claude Desktop, ChatGPT, GitHub Copilot, Gemini, Microsoft 365 Copilot) see [mcp-install.md](./mcp-install.md). This file is the command reference.
...
Config file: `~/.config/ninjaone-cli/config.toml`
...
| `NINJAONE_BASE_URL` | optional | No | Override the API base URL for non-US instances (us2/eu/ca/oc.ninjarmm.com). |
| `NINJAONE_TOKEN_URL` | optional | No | Override the OAuth2 token endpoint (defaults to the base instance's /ws/oauth/token). |
| `NINJAONE_OAUTH_SCOPE` | optional | No | Space-separated OAuth scopes to request (default: monitoring management control). |
