# NinjaOne MCP - install for every agent that speaks MCP

This page wires the NinjaOne MCP server into any MCP client. If you use Claude
...
launch `ninjaone-mcp` directly on your machine - no hosting. *Remote* agents (ChatGPT,
Microsoft 365 Copilot / Copilot Studio, the Gemini app) only talk to an HTTPS
endpoint, so you expose `ninjaone-mcp` over HTTPS first.

## Prerequisite: install the MCP binary

Run the install command from [README.md](./README.md). It drops both `ninjaone-cli`
and `ninjaone-mcp` on your PATH. `ninjaone-mcp` is what the agents talk to.
...
Restart Gemini CLI; the NinjaOne tools appear in its tool list. (The **Gemini app /
...
All remote agents need `ninjaone-mcp` reachable as a public **HTTPS** endpoint. Run it
...
`ninjaone-mcp` you installed is not enough on its own. You also need a **Copilot Studio
license** and a **tenant admin** to enable it. This is a build-and-host task, not a
self-serve install.

Once `ninjaone-mcp` is hosted over HTTPS (above), the lowest-code route:
...
hermes mcp add ninjaone -- ninjaone-mcp

# OpenClaw
openclaw mcp set ninjaone '{"command":"ninjaone-mcp"}'
```
...
- `ninjaone-mcp: command not found`: the install dir is not on your PATH (the
