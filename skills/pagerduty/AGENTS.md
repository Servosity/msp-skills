# PagerDuty Printed CLI Agent Guide

This directory is a generated `pagerduty-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
pagerduty-cli doctor --json
pagerduty-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
pagerduty-cli which "<capability>" --json
pagerduty-cli <command> --help
```
...
```bash
pagerduty-cli <command> --help
pagerduty-cli <command> --dry-run --agent
```
