...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".local", "share", "pagerduty-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("PAGERDUTY_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("PAGERDUTY_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "pagerduty-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.local/share/pagerduty-cli/feedback.jsonl.
When ` + "`PAGERDUTY_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`PAGERDUTY_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  pagerduty-cli feedback list
  pagerduty-cli feedback list --limit 5
  pagerduty-cli feedback list --json`,
