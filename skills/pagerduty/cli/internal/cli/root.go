...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "pagerduty-cli",
		Short: `Pagerduty CLI — Every PagerDuty incident, on-call and service operation from the terminal, plus a local SQLite mirror that answers cros…`,
		Long: `Pagerduty CLI — Every PagerDuty incident, on-call and service operation from the terminal, plus a local SQLite mirror that answers cros…
...
Health check: run 'pagerduty-cli doctor' to verify auth and connectivity.
See README.md or the bundled SKILL.md for recipes.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("pagerduty-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.profileName, "profile", "", "Apply values from a saved profile (see 'pagerduty-cli profile list')")
	...
	rootCmd.AddCommand(newAlertGroupingSettingsCmd(flags))
	rootCmd.AddCommand(newAutomationActionsCmd(flags))
	rootCmd.AddCommand(newBusinessServicesCmd(flags))
	rootCmd.AddCommand(newChangeEventsCmd(flags))
	rootCmd.AddCommand(newEscalationPoliciesCmd(flags))
	rootCmd.AddCommand(newEventOrchestrationsCmd(flags))
	rootCmd.AddCommand(newExtensionSchemasCmd(flags))
	rootCmd.AddCommand(newExtensionsCmd(flags))
	rootCmd.AddCommand(newIncidentWorkflowsCmd(flags))
	rootCmd.AddCommand(newIncidentsCmd(flags))
	rootCmd.AddCommand(newIpAllowListsCmd(flags))
	rootCmd.AddCommand(newLogEntriesCmd(flags))
	rootCmd.AddCommand(newMaintenanceWindowsCmd(flags))
	rootCmd.AddCommand(newOauthDelegationsCmd(flags))
	rootCmd.AddCommand(newPagerdutyAnalyticsCmd(flags))
	rootCmd.AddCommand(newPausedIncidentReportsCmd(flags))
	rootCmd.AddCommand(newRulesetsCmd(flags))
	rootCmd.AddCommand(newSchedulesCmd(flags))
	rootCmd.AddCommand(newServiceDependenciesCmd(flags))
	rootCmd.AddCommand(newServicesCmd(flags))
	rootCmd.AddCommand(newSessionConfigurationsCmd(flags))
	rootCmd.AddCommand(newSreAgentCmd(flags))
	rootCmd.AddCommand(newStandardsCmd(flags))
	rootCmd.AddCommand(newStatusDashboardsCmd(flags))
	...
	rootCmd.AddCommand(newNovelInsightsCmd(flags))
	...
	rootCmd.AddCommand(newStatusPagesPromotedCmd(flags))
