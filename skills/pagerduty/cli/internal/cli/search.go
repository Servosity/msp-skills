...
In local mode: searches locally synced data only.`,
  ...
  pagerduty-cli search "error timeout"

  # Force local search only
  pagerduty-cli search "payment failed" --data-source local

  # Search a specific resource type locally
  pagerduty-cli search "critical" --type transactions --data-source local

  # JSON output for piping
  pagerduty-cli search "critical" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'pagerduty-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
			...
			switch resourceType {
			case "incidents":
				results, err = db.SearchIncidents(query, limit)
			case "licenses":
				results, err = db.SearchLicenses(query, limit)
			case "services":
				results, err = db.SearchServices(query, limit)
			case "standards":
				results, err = db.SearchStandards(query, limit)
			case "templates":
			...
			case "":
				...
				{
					partial, searchErr := db.SearchIncidents(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search incidents failed: %w", searchErr)
					}
				...
				{
					partial, searchErr := db.SearchStandards(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search standards failed: %w", searchErr)
					}
					for _, r := range partial {
						key := string(r)
						if !seen[key] {
