...
type Config struct {
	...
	PagerdutyApiKey string            `toml:"api_key"`
}
...
func Load(configPath string) (*Config, error) {
	...
	path := configPath
	if path == "" {
		path = os.Getenv("PAGERDUTY_CONFIG")
	}
	if path == "" {
		home, _ := os.UserHomeDir()
		path = filepath.Join(home, ".config", "pagerduty-cli", "config.toml")
	}
	cfg.Path = path
	...
	// Env var overrides
	if v := os.Getenv("PAGERDUTY_API_KEY"); v != "" {
		cfg.PagerdutyApiKey = v
		cfg.markEnvOverride("PagerdutyApiKey")
		cfg.AuthSource = "env:PAGERDUTY_API_KEY"
	}
	...
	if cfg.AuthSource == "" && cfg.PagerdutyApiKey != "" {
		cfg.AuthSource = "config"
	}
	...
	if v := os.Getenv("PAGERDUTY_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	return cfg, nil
}
...
func (c *Config) AuthHeader() string {
	...
	token := c.PagerdutyApiKey
	if token == "" {
		return ""
	}
	// PagerDuty's REST API requires the literal "Token token=<key>" prefix in
	// the Authorization header. The OpenAPI apiKey scheme only carries the bare
	// key, so the generator emits the raw token here; prepend the required
	// prefix. Guard against double-wrapping when a value already carries it
	// (e.g. a user pasted the full header into PAGERDUTY_API_KEY).
...
func (c *Config) SaveCredential(token string) error {
	...
	c.updateFileConfigField("AccessToken")
	c.PagerdutyApiKey = token
	delete(c.envOverrides, "PagerdutyApiKey")
	c.updateFileConfigField("PagerdutyApiKey")
	return c.save()
}
...
func (c *Config) configForSave() Config {
	out := *c
	if c.fileConfig != nil {
		if c.envOverrides["PagerdutyApiKey"] {
			out.PagerdutyApiKey = c.fileConfig.PagerdutyApiKey
		}
	}
...
func (c *Config) updateFileConfigField(field string) {
	...
	case "PagerdutyApiKey":
		c.fileConfig.PagerdutyApiKey = c.PagerdutyApiKey
	}
}
