# PagerDuty MCP - install for every agent that speaks MCP

This page wires the PagerDuty MCP server into any MCP client. If you use Claude
...
launch `pagerduty-mcp` directly on your machine - no hosting. *Remote* agents (ChatGPT,
Microsoft 365 Copilot / Copilot Studio, the Gemini app) only talk to an HTTPS
endpoint, so you expose `pagerduty-mcp` over HTTPS first.

## Prerequisite: install the MCP binary

Run the install command from [README.md](./README.md). It drops both `pagerduty-cli`
and `pagerduty-mcp` on your PATH. `pagerduty-mcp` is what the agents talk to.
...
```json
{
  "mcpServers": {
    "pagerduty": {
      ...
      "env": {
        "PAGERDUTY_API_KEY": "<your-pagerduty_api_key>",
...
Restart Gemini CLI; the PagerDuty tools appear in its tool list. (The **Gemini app /
...
All remote agents need `pagerduty-mcp` reachable as a public **HTTPS** endpoint. Run it
in HTTP mode with your credentials in the environment:

```bash
PAGERDUTY_API_KEY=<value> PAGERDUTY_BASE_URL=<value> pagerduty-mcp --transport http --addr :7777
```
...
`pagerduty-mcp` you installed is not enough on its own. You also need a **Copilot Studio
license** and a **tenant admin** to enable it. This is a build-and-host task, not a
self-serve install.

Once `pagerduty-mcp` is hosted over HTTPS (above), the lowest-code route:
...
hermes mcp add pagerduty -- pagerduty-mcp

# OpenClaw
openclaw mcp set pagerduty '{"command":"pagerduty-mcp"}'
```
...
- `pagerduty-mcp: command not found`: the install dir is not on your PATH (the
