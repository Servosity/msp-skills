# PandaDoc Printed CLI Agent Guide

This directory is a generated `pandadoc-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
pandadoc-cli doctor --json
pandadoc-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
pandadoc-cli which "<capability>" --json
pandadoc-cli <command> --help
```
...
```bash
pandadoc-cli <command> --help
pandadoc-cli <command> --dry-run --agent
```
