...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  pandadoc-cli workflow archive

  # Full re-archive (ignore previous sync state)
  pandadoc-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"contacts", "content-library-items", "documents", "documents-folders", "forms", "logs", "members", "public", "public-v2-notary-notaries", "public-v2-notary-notarization-requests", "sms-opt-outs", "templates", "templates-folders", "users", "webhook-events", "webhook-subscriptions", "workspaces"}
			totalSynced := 0
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  pandadoc-cli workflow status

  # Show status as JSON
  pandadoc-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("pandadoc-cli")
			}
