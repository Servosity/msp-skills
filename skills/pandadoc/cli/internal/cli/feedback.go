...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".local", "share", "pandadoc-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("PANDADOC_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("PANDADOC_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "pandadoc-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.local/share/pandadoc-cli/feedback.jsonl.
When ` + "`PANDADOC_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`PANDADOC_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  pandadoc-cli feedback list
  pandadoc-cli feedback list --limit 5
  pandadoc-cli feedback list --json`,
