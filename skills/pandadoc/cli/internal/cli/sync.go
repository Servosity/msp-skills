...
Resource scoping:
  ...
  pandadoc-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  pandadoc-cli sync --full

  # Incremental sync: only records from the last 7 days
  pandadoc-cli sync --since 7d

  # Parallel sync with 8 workers
  pandadoc-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  pandadoc-cli sync --latest-only`,
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/pandadoc-cli/data.db)")
...
func resourceSupportsPagination(resource string) bool {
	...
	case "public":
		return true
	case "public-v2-notary-notaries":
		return true
	case "public-v2-notary-notarization-requests":
		return true
	case "templates":
		return true
	case "templates-folders":
		return true
	case "users":
		return true
	case "webhook-events":
...
func syncResourceFieldSelector(resource string) (string, string) {
	switch resource {
	case "templates":
		return "fields", "id,content_date_modified,date_created,date_modified,name,version"
	case "public":
		return "limit", "id,field_type,key"
	}
	return "", ""
}
...
func upsertSingleObject(db *store.Store, resource string, data json.RawMessage) error {
	...
	case "content-library-items":
		return db.UpsertContentLibraryItems(data)
	case "content_library_items_details":
		return db.UpsertContentLibraryItemsDetails(data)
	case "content-library-items-upload":
		return db.UpsertContentLibraryItemsUpload(data)
	case "documents":
		return db.UpsertDocuments(data)
	case "append_content_library_item":
		return db.UpsertAppendContentLibraryItem(data)
	case "attachments":
		return db.UpsertAttachments(data)
	case "attachments_upload":
		return db.UpsertAttachmentsUpload(data)
	case "auto_reminders":
		return db.UpsertAutoReminders(data)
	case "documents_details":
		return db.UpsertDocumentsDetails(data)
	case "download":
		return db.UpsertDownload(data)
	case "download_protected":
		return db.UpsertDownloadProtected(data)
	case "draft":
		return db.UpsertDraft(data)
	case "documents_editing_sessions":
		return db.UpsertDocumentsEditingSessions(data)
	case "esign_disclosure":
		return db.UpsertEsignDisclosure(data)
	case "fields":
		return db.UpsertFields(data)
	case "linked_objects":
		return db.UpsertLinkedObjects(data)
	case "move_to_folder":
		return db.UpsertMoveToFolder(data)
	case "ownership":
	...
	case "send_reminder":
		return db.UpsertSendReminder(data)
	case "session":
		return db.UpsertSession(data)
	case "status":
		return db.UpsertStatus(data)
	case "status_upload":
		return db.UpsertStatusUpload(data)
	case "forms":
	...
	case "sms-opt-outs":
		return db.UpsertSmsOptOuts(data)
	case "templates":
		return db.UpsertTemplates(data)
	case "templates_details":
		return db.UpsertTemplatesDetails(data)
	case "templates_editing_sessions":
		return db.UpsertTemplatesEditingSessions(data)
	case "templates-upload":
		return db.UpsertTemplatesUpload(data)
	case "users":
	...
	case "shared_key":
		return db.UpsertSharedKey(data)
	case "workspaces":
	...
	case "workspaces_members":
		return db.UpsertWorkspacesMembers(data)
	default:
...
func syncResourcePath(resource string) (string, error) {
	paths := map[string]string{
		"contacts":                               "/contacts",
		"content-library-items":                  "/content-library-items",
		"documents":                              "/documents",
		"documents-folders":                      "/documents/folders",
		"forms":                                  "/forms",
		"logs":                                   "/logs",
		"members":                                "/members",
		"public":                                 "/public/v2/logs",
		"public-v2-notary-notaries":              "/public/v2/notary/notaries",
		"public-v2-notary-notarization-requests": "/public/v2/notary/notarization-requests",
		"sms-opt-outs":                           "/sms-opt-outs",
		"templates":                              "/templates",
		"templates-folders":                      "/templates/folders",
		"users":                                  "/users",
...
func dependentResourceDefs() []dependentResourceDef {
	return []dependentResourceDef{
		{Name: "public", ParentTable: "documents", ParentIDParam: "document_id", PathTemplate: "/public/beta/documents/{document_id}/ai-metadata", KeyField: "", PathParams: []dependentPathParamDef{
			{Param: "document_id", Field: "id"},
		}},
		{Name: "public", ParentTable: "documents", ParentIDParam: "document_id", PathTemplate: "/public/v2/documents/{document_id}/audit-trail", KeyField: "", PathParams: []dependentPathParamDef{
			{Param: "document_id", Field: "id"},
		}},
	}
}
