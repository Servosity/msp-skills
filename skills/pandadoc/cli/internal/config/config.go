...
type Config struct {
	...
	PandadocApiKey string            `toml:"api_key"`
}
...
func Load(configPath string) (*Config, error) {
	...
	path := configPath
	if path == "" {
		path = os.Getenv("PANDADOC_CONFIG")
	}
	if path == "" {
		home, _ := os.UserHomeDir()
		path = filepath.Join(home, ".config", "pandadoc-cli", "config.toml")
	}
	cfg.Path = path
	...
	// Env var overrides
	if v := os.Getenv("PANDADOC_API_KEY"); v != "" {
		cfg.PandadocApiKey = v
		cfg.markEnvOverride("PandadocApiKey")
		cfg.AuthSource = "env:PANDADOC_API_KEY"
	}
	...
	if cfg.AuthSource == "" && cfg.PandadocApiKey != "" {
		cfg.AuthSource = "config"
	}
	...
	if v := os.Getenv("PANDADOC_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	return cfg, nil
}
...
func (c *Config) AuthHeader() string {
	...
	token := c.PandadocApiKey
	if token == "" {
		return ""
	}
	// PandaDoc expects `Authorization: API-Key <key>` (spec x-auth-value-prefix).
	// Accept an already-prefixed value defensively so users who paste the full
	// header value (or an OAuth bearer token) don't get a doubled prefix.
	if strings.HasPrefix(token, "API-Key ") || strings.HasPrefix(token, "Bearer ") {
		return token
	}
...
func (c *Config) SaveCredential(token string) error {
	...
	c.updateFileConfigField("AccessToken")
	c.PandadocApiKey = token
	delete(c.envOverrides, "PandadocApiKey")
	c.updateFileConfigField("PandadocApiKey")
	return c.save()
}
...
func (c *Config) configForSave() Config {
	out := *c
	if c.fileConfig != nil {
		if c.envOverrides["PandadocApiKey"] {
			out.PandadocApiKey = c.fileConfig.PandadocApiKey
		}
	}
...
func (c *Config) updateFileConfigField(field string) {
	...
	case "PandadocApiKey":
		c.fileConfig.PandadocApiKey = c.PandadocApiKey
	}
}
