# pandadoc skill - governance and safety model

> Unofficial. Community-built skill for the PandaDoc API. Not affiliated with,
> endorsed by, or sponsored by the vendor.
> This page tells an MSP owner exactly what the pandadoc skill can touch and how to
> scope it, so you can decide what to let an AI agent do.

## What it authenticates as

The skill drives the `pandadoc-cli` binary (and `pandadoc-mcp`),
authenticating with `PANDADOC_API_KEY`. Credentials are read from the environment only -
never written to disk, never logged, never sent anywhere except the PandaDoc API.
...
the PandaDoc API, and you can read every line of how it does so. The skill is
