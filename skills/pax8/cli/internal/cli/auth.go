...
func newAuthCmd(flags *rootFlags) *cobra.Command {
	cmd := &cobra.Command{
		Use:   "auth",
		Short: "Manage authentication for Partner Endpoints",
...
Mint an OAuth2 bearer token via POST https://token-manager.pax8.com/oauth/token with
grant_type=client_credentials and cache it on disk. Subsequent commands
auto-refresh the token before expiry.

Credentials default to PAX8_CLIENT_ID (Client ID) and PAX8_CLIENT_SECRET (Client Secret) environment variables.
`),
  ...
  pax8-cli auth login --client-id <id> --client-secret <secret>
`, "\n"),
		Annotations: map[string]string{"mcp:hidden": "true"},
		RunE: func(cmd *cobra.Command, args []string) error {
			// issue #270: PAX8_NO_CONFIG_WRITE makes the automatic token
			...
			if clientID == "" {
				clientID = strings.TrimSpace(os.Getenv("PAX8_CLIENT_ID"))
			}
			if clientSecret == "" {
				clientSecret = strings.TrimSpace(os.Getenv("PAX8_CLIENT_SECRET"))
			}
			if clientID == "" || clientSecret == "" {
				return authErr(fmt.Errorf("client ID and secret required (set --client-id/--client-secret or PAX8_CLIENT_ID/PAX8_CLIENT_SECRET)"))
			}
	...
	cmd.Flags().StringVar(&clientID, "client-id", "", "OAuth2 Client ID; defaults to $PAX8_CLIENT_ID")
	cmd.Flags().StringVar(&clientSecret, "client-secret", "", "OAuth2 Client Secret; defaults to $PAX8_CLIENT_SECRET")
...
func resolveClientCredentialsScope() string {
	if scope := os.Getenv("PAX8_OAUTH_SCOPE"); scope != "" {
		return scope
	}
	return ""
}
	...
	return &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if !authed {
				...
				fmt.Fprintln(w, "  export PAX8_CLIENT_ID=\"<client-id>\"")
				fmt.Fprintln(w, "  export PAX8_CLIENT_SECRET=\"<client-secret>\"")
				fmt.Fprintf(w, "  pax8-cli auth login\n")
				return authErr(fmt.Errorf("no credentials configured"))
			}
...
func newAuthSetTokenCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  pax8-cli auth set-token <bearer-jwt>",
...
func newAuthLogoutCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			envStillSet := ""
			if envStillSet == "" && os.Getenv("PAX8_CLIENT_ID") != "" {
				envStillSet = "PAX8_CLIENT_ID"
			}
			if envStillSet == "" && os.Getenv("PAX8_CLIENT_SECRET") != "" {
				envStillSet = "PAX8_CLIENT_SECRET"
			}
