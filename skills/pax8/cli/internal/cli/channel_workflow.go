...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  pax8-cli workflow archive

  # Full re-archive (ignore previous sync state)
  pax8-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"companies", "invoices", "invoices-draft-items", "orders", "products", "subscriptions"}
			totalSynced := 0
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  pax8-cli workflow status

  # Show status as JSON
  pax8-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("pax8-cli")
			}
