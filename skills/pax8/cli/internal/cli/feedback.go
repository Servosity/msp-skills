...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".local", "share", "pax8-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("PAX8_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("PAX8_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "pax8-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.local/share/pax8-pp-cli/feedback.jsonl.
When ` + "`PAX8_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`PAX8_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  pax8-cli feedback list
  pax8-cli feedback list --limit 5
  pax8-cli feedback list --json`,
