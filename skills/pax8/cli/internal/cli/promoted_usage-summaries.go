...
func newUsageSummariesPromotedCmd(flags *rootFlags) *cobra.Command {
			...
			if len(args) < 1 {
				...
				if flags.asJSON {
					if printErr := printJSONFiltered(cmd.OutOrStdout(), map[string]any{
						"error": "usageSummaryId is required",
						"usage": fmt.Sprintf("%s <%s>", cmd.CommandPath(), "usageSummaryId"),
					}, flags); printErr != nil {
						return printErr
					}
				}
				return usageErr(fmt.Errorf("usageSummaryId is required\nUsage: %s <%s>", cmd.CommandPath(), "usageSummaryId"))
			}
			path = replacePathParam(path, "usageSummaryId", args[0])
			params := map[string]string{}
			data, prov, err := resolveReadWithStrategy(cmd.Context(), c, flags, "auto", "usage-summaries", false, path, params, nil, cmd.ErrOrStderr())
			if err != nil {
	...
	{
		sub := newUsageSummariesUsageLinesCmd(flags)
