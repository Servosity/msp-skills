...
Resource scoping:
  ...
  pax8-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  pax8-cli sync --full

  # Incremental sync: only records from the last 7 days
  pax8-cli sync --since 7d

  # Parallel sync with 8 workers
  pax8-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  pax8-cli sync --latest-only`,
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/pax8-pp-cli/data.db)")
...
func syncResourceFieldSelector(resource string) (string, string) {
	switch resource {
	case "companies":
		return "sort", "id,billOnBehalfOfEnabled,contacts.id,externalId,name,orderApprovalRequired,phone,selfServiceAllowed,status,updatedDate,website"
	case "invoices":
		return "sort", "id,balance,carriedBalance,companyId,currencyCode,dueDate,externalId,invoiceDate,partnerName,total"
	case "products":
		return "sort", "id,altVendorSku,name,requiresCommitment,shortDescription,sku,vendorName,vendorSku"
	case "subscriptions":
		return "sort", "id,billingStart,billingTerm,commitmentTerm.id,companyId,createdDate,currencyCode,endDate,parentSubscriptionId,partnerCost,price,productId,quantity,startDate,status,updatedDate,vendorSubscriptionId"
	case "subscriptions_usage_summaries":
		return "sort", "id,companyId,currencyCode,currentCharges,isTrial,partnerTotal,productId,resourceGroup,vendorName"
	}
	return "", ""
}
...
func upsertSingleObject(db *store.Store, resource string, data json.RawMessage) error {
	...
	case "provision_details":
		return db.UpsertProvisionDetails(data)
	case "subscriptions":
		return db.UpsertSubscriptions(data)
	case "history":
		return db.UpsertHistory(data)
	case "subscriptions_usage_summaries":
		return db.UpsertSubscriptionsUsageSummaries(data)
	case "usage-summaries":
		return db.UpsertUsageSummaries(data)
	case "usage_lines":
		return db.UpsertUsageLines(data)
	default:
...
func syncResourcePath(resource string) (string, error) {
	paths := map[string]string{
		"companies":            "/companies",
		"invoices":             "/invoices",
		"invoices-draft-items": "/invoices/draftItems",
		"orders":               "/orders",
		"products":             "/products",
...
func dependentResourceDefs() []dependentResourceDef {
	return []dependentResourceDef{
		{Name: "contacts", ParentTable: "companies", ParentIDParam: "companyId", PathTemplate: "/companies/{companyId}/contacts", KeyField: "", PathParams: []dependentPathParamDef{
			{Param: "companyId", Field: "id"},
		}},
		{Name: "items", ParentTable: "invoices", ParentIDParam: "invoiceId", PathTemplate: "/invoices/{invoiceId}/items", KeyField: "", PathParams: []dependentPathParamDef{
			{Param: "invoiceId", Field: "id"},
		}},
		{Name: "subscriptions_usage_summaries", ParentTable: "subscriptions", ParentIDParam: "subscriptionId", PathTemplate: "/subscriptions/{subscriptionId}/usage-summaries", KeyField: "", PathParams: []dependentPathParamDef{
			{Param: "subscriptionId", Field: "id"},
		}},
	}
}
