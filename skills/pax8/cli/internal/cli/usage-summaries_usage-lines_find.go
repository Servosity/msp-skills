...
func newUsageSummariesUsageLinesFindCmd(flags *rootFlags) *cobra.Command {
	var flagUsageDate string
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if !cmd.Flags().Changed("usage-date") && !flags.dryRun {
				return fmt.Errorf("required flag \"%s\" not set", "usage-date")
			}
	...
	cmd.Flags().StringVar(&flagUsageDate, "usage-date", "", "The date usage was recorded")
	cmd.Flags().StringVar(&flagProductId, "product-id", "", "Return only usage summaries with this ```productId``` value")
