...
func New(cfg *config.Config, timeout time.Duration, rateLimit float64) *Client {
	...
	cacheDir := filepath.Join(homeDir, ".cache", "pax8-pp-cli", "http")
...
func (c *Client) doInternal(ctx context.Context, method, path string, params map[string]string, body any, headerOverrides map[string]string, readOnlyIntent bool) (json.RawMessage, int, error) {
	...
	for attempt := 0; attempt <= maxRetries; attempt++ {
		...
		if req.Header.Get("User-Agent") == "" {
			req.Header.Set("User-Agent", "pax8-pp-cli/1.0.0")
		}
...
func authPlaceholderCredentialError(cfg *config.Config) error {
	return authPlaceholderCredentialErrorWithSetup(cfg, "pax8-pp-cli auth login or pax8-pp-cli auth set-token <token>")
}
...
func resolveClientCredentials(cfg *config.Config) (string, string) {
	...
	if id == "" {
		id = os.Getenv("PAX8_CLIENT_ID")
	}
	if secret == "" {
		secret = os.Getenv("PAX8_CLIENT_SECRET")
	}
	return id, secret
}
...
// resolveClientCredentialsAudience returns the OAuth2 audience Pax8 requires on
// the client_credentials token request. Pax8's token-manager rejects (or mints
// an unusable token) when the audience is absent, so it defaults to the known
// partner-API audience and stays overridable via PAX8_AUDIENCE.
func resolveClientCredentialsAudience() string {
	if aud := os.Getenv("PAX8_AUDIENCE"); aud != "" {
		return aud
	}
	return "api://p8p.client"
}
...
func (c *Client) mintClientCredentials(ctx context.Context, clientID, clientSecret string) error {
	...
	if aud := resolveClientCredentialsAudience(); aud != "" {
