...
// touching disk. PAX8_NO_CONFIG_WRITE makes the automatic token cache a
...
func ncwMint(t *testing.T, value string, set bool) map[string]string {
	...
	home := t.TempDir()
	t.Setenv("HOME", home)
	t.Setenv("PAX8_CONFIG", filepath.Join(home, "config.toml"))
	if set {
...
func TestNoConfigWriteTruthiness(t *testing.T) {
	...
	if NoConfigWriteEnv != "PAX8_NO_CONFIG_WRITE" {
		t.Errorf("switch is named %q; the documented name is PAX8_NO_CONFIG_WRITE", NoConfigWriteEnv)
	}
}
