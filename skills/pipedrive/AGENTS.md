...
This directory is a generated `pipedrive-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
pipedrive-cli doctor --json
pipedrive-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
pipedrive-cli which "<capability>" --json
pipedrive-cli <command> --help
```
...
```bash
pipedrive-cli <command> --help
pipedrive-cli <command> --dry-run --agent
```
