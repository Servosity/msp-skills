...
In local mode: searches locally synced data only.`,
  ...
  pipedrive-cli search "error timeout"

  # Force local search only
  pipedrive-cli search "payment failed" --data-source local

  # Search a specific resource type locally
  pipedrive-cli search "critical" --type transactions --data-source local

  # JSON output for piping
  pipedrive-cli search "critical" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'pipedrive-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
			...
			case "persons":
				results, err = db.SearchPersons(query, limit)
			case "products":
			...
			case "":
				...
				{
					partial, searchErr := db.SearchPersons(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search persons failed: %w", searchErr)
					}
					for _, r := range partial {
						key := string(r)
						if !seen[key] {
