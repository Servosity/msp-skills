# Proofpoint TAP Printed CLI Agent Guide

This directory is a generated `proofpoint-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
proofpoint-cli doctor --json
proofpoint-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
proofpoint-cli which "<capability>" --json
...
```bash
proofpoint-cli <command> --help
proofpoint-cli <command> --dry-run --agent
```
