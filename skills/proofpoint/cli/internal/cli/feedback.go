...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".local", "share", "proofpoint-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("PROOFPOINT_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("PROOFPOINT_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "proofpoint-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.local/share/proofpoint-cli/feedback.jsonl.
When ` + "`PROOFPOINT_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`PROOFPOINT_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  proofpoint-cli feedback list
  proofpoint-cli feedback list --limit 5
  proofpoint-cli feedback list --json`,
