...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "proofpoint-cli",
		Short: `Proofpoint CLI — Every TAP Threat Insight endpoint, plus a local threat store that answers the cross-endpoint questions — who is both at…`,
		Long: `Proofpoint CLI — Every TAP Threat Insight endpoint, plus a local threat store that answers the cross-endpoint questions — who is both at…
...
Health check: run 'proofpoint-cli doctor' to verify auth and connectivity.
See README.md or the bundled SKILL.md for recipes.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("proofpoint-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.profileName, "profile", "", "Apply values from a saved profile (see 'proofpoint-cli profile list')")
	...
	rootCmd.AddCommand(newSiemCmd(flags))
	...
	rootCmd.AddCommand(newNovelBackfillCmd(flags))
	rootCmd.AddCommand(newNovelCampaignThreatsCmd(flags))
	rootCmd.AddCommand(newNovelIncidentCmd(flags))
	rootCmd.AddCommand(newNovelIocsCmd(flags))
	rootCmd.AddCommand(newNovelRiskOverlapCmd(flags))
	rootCmd.AddCommand(newNovelUserCmd(flags))
	rootCmd.AddCommand(newAPICmd(flags))
	rootCmd.AddCommand(newForensicsPromotedCmd(flags))
	rootCmd.AddCommand(newThreatPromotedCmd(flags))
	rootCmd.AddCommand(newUrlPromotedCmd(flags))
