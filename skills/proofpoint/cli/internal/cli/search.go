...
In local mode: searches locally synced data only.`,
  ...
  proofpoint-cli search "error timeout"

  # Force local search only
  proofpoint-cli search "payment failed" --data-source local

  # Search a specific resource type locally
  proofpoint-cli search "critical" --type transactions --data-source local

  # JSON output for piping
  proofpoint-cli search "critical" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'proofpoint-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
			...
			switch resourceType {
			case "campaign":
				results, err = db.SearchCampaign(query, limit)
			case "threat":
				results, err = db.SearchThreat(query, limit)
			case "":
				...
				{
					partial, searchErr := db.SearchCampaign(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search campaign failed: %w", searchErr)
					}
				...
				{
					partial, searchErr := db.SearchThreat(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search threat failed: %w", searchErr)
					}
					for _, r := range partial {
						key := string(r)
						if !seen[key] {
