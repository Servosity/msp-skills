...
Resource scoping:
  ...
  proofpoint-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  proofpoint-cli sync --full

  # Incremental sync: only records from the last 7 days
  proofpoint-cli sync --since 7d

  # Parallel sync with 8 workers
  proofpoint-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  proofpoint-cli sync --latest-only`,
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/proofpoint-cli/data.db)")
...
func syncResourceSinceParam(resource string) string {
	...
	case "siem":
		return "sinceSeconds"
	case "siem-clicks-blocked":
		return "sinceSeconds"
	case "siem-clicks-permitted":
		return "sinceSeconds"
	case "siem-issues":
		return "sinceSeconds"
	case "siem-messages-blocked":
		return "sinceSeconds"
	case "siem-messages-delivered":
		return "sinceSeconds"
	}
	return ""
}
...
func upsertSingleObject(db *store.Store, resource string, data json.RawMessage) error {
	...
	case "siem":
		return db.UpsertSiem(data)
	case "threat":
		return db.UpsertThreat(data)
	case "url":
...
func syncResourcePath(resource string) (string, error) {
	paths := map[string]string{
		"forensics":               "/forensics",
		"people":                  "/people/vap",
		"people-top-clickers":     "/people/top-clickers",
		"siem":                    "/siem/all",
		"siem-clicks-blocked":     "/siem/clicks/blocked",
		"siem-clicks-permitted":   "/siem/clicks/permitted",
		"siem-issues":             "/siem/issues",
		"siem-messages-blocked":   "/siem/messages/blocked",
		"siem-messages-delivered": "/siem/messages/delivered",
	}
