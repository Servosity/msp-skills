...
type Config struct {
	...
	ProofpointServicePrincipal string            `toml:"service_principal"`
	ProofpointApiSecret        string            `toml:"api_secret"`
}
...
func Load(configPath string) (*Config, error) {
	...
	path := configPath
	if path == "" {
		path = os.Getenv("PROOFPOINT_CONFIG")
	}
	if path == "" {
		home, _ := os.UserHomeDir()
		path = filepath.Join(home, ".config", "proofpoint-cli", "config.toml")
	}
	cfg.Path = path
	...
	// Env var overrides
	if v := os.Getenv("PROOFPOINT_SERVICE_PRINCIPAL"); v != "" {
		cfg.ProofpointServicePrincipal = v
		cfg.markEnvOverride("ProofpointServicePrincipal")
		cfg.AuthSource = "env:PROOFPOINT_SERVICE_PRINCIPAL"
	}
	if v := os.Getenv("PROOFPOINT_API_SECRET"); v != "" {
		cfg.ProofpointApiSecret = v
		cfg.markEnvOverride("ProofpointApiSecret")
		cfg.AuthSource = "env:PROOFPOINT_API_SECRET"
	}
	...
	if cfg.AuthSource == "" && cfg.ProofpointServicePrincipal != "" {
		cfg.AuthSource = "config"
	}
	if cfg.AuthSource == "" && cfg.ProofpointApiSecret != "" {
		cfg.AuthSource = "config"
	}
	...
	if v := os.Getenv("PROOFPOINT_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	return cfg, nil
}
...
func (c *Config) AuthHeader() string {
	...
	if c.ProofpointServicePrincipal == "" || c.ProofpointApiSecret == "" {
		return ""
	}
	credentials := c.ProofpointServicePrincipal + ":" + c.ProofpointApiSecret
...
func (c *Config) SaveCredential(token string) error {
	...
	c.updateFileConfigField("AccessToken")
	c.ProofpointServicePrincipal = token
	delete(c.envOverrides, "ProofpointServicePrincipal")
	c.updateFileConfigField("ProofpointServicePrincipal")
	return c.save()
}
...
func (c *Config) ClearTokens() error {
	...
	c.updateFileConfigField("ClientID")
	c.updateFileConfigField("ClientSecret")
	c.ProofpointServicePrincipal = ""
	delete(c.envOverrides, "ProofpointServicePrincipal")
	c.ProofpointApiSecret = ""
	delete(c.envOverrides, "ProofpointApiSecret")
	return c.save()
}
...
func (c *Config) configForSave() Config {
	out := *c
	if c.fileConfig != nil {
		if c.envOverrides["ProofpointServicePrincipal"] {
			out.ProofpointServicePrincipal = c.fileConfig.ProofpointServicePrincipal
		}
		if c.envOverrides["ProofpointApiSecret"] {
			out.ProofpointApiSecret = c.fileConfig.ProofpointApiSecret
		}
	}
...
func (c *Config) updateFileConfigField(field string) {
	...
	case "ProofpointServicePrincipal":
		c.fileConfig.ProofpointServicePrincipal = c.ProofpointServicePrincipal
	case "ProofpointApiSecret":
		c.fileConfig.ProofpointApiSecret = c.ProofpointApiSecret
	}
}
