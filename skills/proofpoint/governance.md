...
authenticating with `PROOFPOINT_API_SECRET`, `PROOFPOINT_SERVICE_PRINCIPAL`. Credentials are read from the environment only -
never written to disk, never logged, never sent anywhere except the Proofpoint API.
...
| **Write (routine)** | Day-to-day mutations. | (none detected) | Preview with `--dry-run`, then an approved write (where a command documents its own confirm gate, use it too) |
...
the Proofpoint API, and you can read every line of how it does so. The skill is
