...
```bash
proofpoint-cli backfill --since 12h && proofpoint-cli search "delivered" --limit 20
```

Backfill loops the 1-hour windows for you, then the issues data is queryable offline.
...
proofpoint-cli campaign get "campaign-xyz789"

# JSON for scripting and agents
proofpoint-cli campaign get "campaign-xyz789" --json

# Filter to specific fields
proofpoint-cli campaign get "campaign-xyz789" --json --select id,name,status

# Dry run - show the request without sending
proofpoint-cli campaign get "campaign-xyz789" --dry-run

# Agent mode - JSON + compact + no prompts in one flag
proofpoint-cli campaign get "campaign-xyz789" --agent
```
...
- **Read-only surface** - every command retrieves data; the only POST is `url` (decoding rewritten links), so no create/update/delete semantics apply
- **Offline-friendly** - sync/search commands can use the local SQLite store when available
- **Agent-safe by default** - no colors or formatting unless `--human-friendly` is set

Exit codes: `0` success, `2` usage error, `3` not found, `4` auth error, `5` API error, `6` partial failure (some backfill windows or incident sources failed; pass `--allow-partial-failure` to tolerate), `7` rate limited, `10` config error.
