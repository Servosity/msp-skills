# Proofpoint MCP - install for every agent that speaks MCP

This page wires the Proofpoint MCP server into any MCP client. If you use Claude
...
launch `proofpoint-mcp` directly on your machine - no hosting. *Remote* agents (ChatGPT,
Microsoft 365 Copilot / Copilot Studio, the Gemini app) only talk to an HTTPS
endpoint, so you expose `proofpoint-mcp` over HTTPS first.

## Prerequisite: install the MCP binary

Run the install command from [README.md](./README.md). It drops both `proofpoint-cli`
and `proofpoint-mcp` on your PATH. `proofpoint-mcp` is what the agents talk to.
...
Restart Gemini CLI; the Proofpoint tools appear in its tool list. (The **Gemini app /
...
All remote agents need `proofpoint-mcp` reachable as a public **HTTPS** endpoint. Run it
...
`proofpoint-mcp` you installed is not enough on its own. You also need a **Copilot Studio
license** and a **tenant admin** to enable it. This is a build-and-host task, not a
self-serve install.

Once `proofpoint-mcp` is hosted over HTTPS (above), the lowest-code route:
...
hermes mcp add proofpoint -- proofpoint-mcp

# OpenClaw
openclaw mcp set proofpoint '{"command":"proofpoint-mcp"}'
```
...
- `proofpoint-mcp: command not found`: the install dir is not on your PATH (the
