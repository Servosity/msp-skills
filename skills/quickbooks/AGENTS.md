# QuickBooks Online Printed CLI Agent Guide

This directory is a generated `quickbooks-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
quickbooks-cli doctor --json
quickbooks-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
quickbooks-cli which "<capability>" --json
...
```bash
quickbooks-cli <command> --help
quickbooks-cli <command> --dry-run --agent
```
