...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  quickbooks-cli workflow archive

  # Full re-archive (ignore previous sync state)
  quickbooks-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"accounts", "bills", "customers", "invoices", "items", "journal-entries", "payments", "vendors"}
			totalSynced := 0
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  quickbooks-cli workflow status

  # Show status as JSON
  quickbooks-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("quickbooks-cli")
			}
