...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".local", "share", "quickbooks-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("QUICKBOOKS_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("QUICKBOOKS_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "quickbooks-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.local/share/quickbooks-cli/feedback.jsonl.
When ` + "`QUICKBOOKS_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`QUICKBOOKS_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  quickbooks-cli feedback list
  quickbooks-cli feedback list --limit 5
  quickbooks-cli feedback list --json`,
