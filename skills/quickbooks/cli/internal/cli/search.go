...
In local mode: searches locally synced data only.`,
  ...
  quickbooks-cli search "error timeout"

  # Force local search only
  quickbooks-cli search "payment failed" --data-source local

  # Search a specific resource type locally
  quickbooks-cli search "critical" --type transactions --data-source local

  # JSON output for piping
  quickbooks-cli search "critical" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'quickbooks-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/quickbooks-cli/data.db)")
