# Resource Guru Printed CLI Agent Guide

This directory is a generated `resourceguru-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
resourceguru-cli doctor --json
resourceguru-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
resourceguru-cli which "<capability>" --json
resourceguru-cli <command> --help
```
...
```bash
resourceguru-cli <command> --agent
```
...
```bash
resourceguru-cli <command> --help
resourceguru-cli <command> --dry-run --agent
```
