...
func newAuthCmd(flags *rootFlags) *cobra.Command {
	cmd := &cobra.Command{
		Use:   "auth",
		Short: "Manage authentication for Resource Guru",
...
func newAuthSetupCmd(_ *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: "  resourceguru-cli auth setup\n  resourceguru-cli auth setup --launch",
		RunE: func(cmd *cobra.Command, args []string) error {
			w := cmd.OutOrStdout()
			fmt.Fprintln(w, "Get a key at: https://api.resourceguruapp.com/`")
			fmt.Fprintln(w, "")
			fmt.Fprintln(w, "Then set:")
			fmt.Fprintln(w, "  export RESOURCEGURU_EMAIL=\"your-token-here\"")
			fmt.Fprintln(w, "  export RESOURCEGURU_PASSWORD=\"your-password\"")
			fmt.Fprintln(w, "  resourceguru-cli auth set-credentials your-token-here your-password")
			if !launch {
				return nil
			}
			launchURL := "https://api.resourceguruapp.com/`"
...
func newAuthStatusCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  resourceguru-cli auth status",
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if !authed {
				...
				fmt.Fprintln(w, "  export RESOURCEGURU_EMAIL=\"your-token-here\"")
				fmt.Fprintln(w, "  export RESOURCEGURU_PASSWORD=\"your-password\"")
				fmt.Fprintf(w, "  resourceguru-cli auth set-credentials your-token-here your-password\n")
				return authErr(fmt.Errorf("no credentials configured"))
			}
...
func newAuthSetTokenCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  resourceguru-cli auth set-token YOUR_TOKEN_HERE",
...
func newAuthSetCredentialsCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		Use:     "set-credentials <email> <password>",
		Short:   "Save Basic auth credentials to the credentials file",
		Example: "  resourceguru-cli auth set-credentials your-token-here your-password",
		Args: func(cmd *cobra.Command, args []string) error {
			if len(args) != 2 {
				return fmt.Errorf("requires exactly 2 args: <email> <password>")
			}
			if args[0] == "" {
				return fmt.Errorf("RESOURCEGURU_EMAIL is required")
			}
			if args[1] == "" {
				return fmt.Errorf("RESOURCEGURU_PASSWORD is required")
			}
			return nil
		},
		RunE: func(cmd *cobra.Command, args []string) error {
...
func newAuthLogoutCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  resourceguru-cli auth logout",
		RunE: func(cmd *cobra.Command, args []string) error {
			cfg, err := config.Load(flags.configPath)
			if err != nil {
				return configErr(err)
			...
			envStillSet := ""
			if envStillSet == "" && os.Getenv("RESOURCEGURU_EMAIL") != "" {
				envStillSet = "RESOURCEGURU_EMAIL"
			}
			if envStillSet == "" && os.Getenv("RESOURCEGURU_PASSWORD") != "" {
				envStillSet = "RESOURCEGURU_PASSWORD"
			}
