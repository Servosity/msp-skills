...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  resourceguru-cli doctor
  resourceguru-cli doctor --json
  resourceguru-cli doctor --fail-on warn
  resourceguru-cli doctor --fail-on stale`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if cfg != nil {
				...
				if header == "" {
					...
					report["auth_hint"] = "Set Basic credentials with: export RESOURCEGURU_EMAIL=\"your-token-here\" RESOURCEGURU_PASSWORD=\"your-password\""
					report["auth_key_url"] = "https://api.resourceguruapp.com/`"
				} else {
					authConfigured = true
			...
			if os.Getenv("RESOURCEGURU_EMAIL") != "" {
				authEnvSet = append(authEnvSet, "RESOURCEGURU_EMAIL")
				...
				authEnvRequiredMissing = append(authEnvRequiredMissing, "RESOURCEGURU_EMAIL")
			}
			if os.Getenv("RESOURCEGURU_PASSWORD") != "" {
				authEnvSet = append(authEnvSet, "RESOURCEGURU_PASSWORD")
			} else if authConfigured {
				authSource, _ := report["auth_source"].(string)
				if authSource == "" {
					authSource = "config"
				...
				authEnvRequiredMissing = append(authEnvRequiredMissing, "RESOURCEGURU_PASSWORD")
			}
			switch {
			...
			if cfg != nil && doctorBaseURLIsPlaceholder(cfg.BaseURL) {
				...
				if clientErr != nil {
					...
					if authHeader == "" {
						...
						authHeaders["User-Agent"] = "resourceguru-cli"
						verifyPath := "/v1/me"
...
func collectCacheReport(ctx context.Context, staleAfterSpec string) map[string]any {
	...
	if err != nil {
		if os.IsNotExist(err) {
			...
			report["hint"] = "Database not created yet; run 'resourceguru-cli sync' to hydrate."
			return report
		}
		report["status"] = "error"
	...
	if qerr != nil {
		...
		report["status"] = "unknown"
		report["hint"] = "No sync state recorded; run 'resourceguru-cli sync' to populate."
		return report
	}
	defer rows.Close()
	...
	case !haveAny && len(resources) == 0:
		report["status"] = "empty"
		report["hint"] = "Cache is empty; run 'resourceguru-cli sync' to hydrate."
	case fresh:
		report["status"] = "fresh"
	default:
		report["status"] = "stale"
		report["oldest_age"] = oldest.Round(time.Minute).String()
		report["hint"] = "Some resources are older than stale_after; run 'resourceguru-cli sync' to refresh."
	}
	return report
}
