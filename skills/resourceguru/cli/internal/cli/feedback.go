...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("RESOURCEGURU_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("RESOURCEGURU_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "resourceguru-cli/feedback")
...
When ` + "`RESOURCEGURU_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`RESOURCEGURU_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  resourceguru-cli feedback list
  resourceguru-cli feedback list --limit 5
  resourceguru-cli feedback list --json`,
