...
func classifyAPIError(err error, flags *rootFlags) error {
	...
	case strings.Contains(msg, "HTTP 400") && cliutil.LooksLikeAuthError(msg):
		return authErr(fmt.Errorf("%w\nhint: the API rejected the request — this usually means auth is missing or invalid."+
			...
			"\n      Run 'resourceguru-cli doctor' to check auth status."+
			"\n      Response: "+cliutil.SanitizeErrorBody(msg), err))
	case strings.Contains(msg, "HTTP 401"):
		return authErr(fmt.Errorf("%w\nhint: check your Basic credentials."+
			" Set Basic credentials with: export RESOURCEGURU_EMAIL=\"your-token-here\" RESOURCEGURU_PASSWORD=\"your-password\""+
			"\n      Get a key at: https://api.resourceguruapp.com/`"+
			"\n      Run 'resourceguru-cli doctor' to check auth status.", err))
...
func nonJSONPayloadError(data json.RawMessage) error {
	...
	if len(trimmed) > 0 && trimmed[0] == '<' {
		return authErr(fmt.Errorf("not authenticated or session expired; API returned HTML instead of JSON. " + "Set Basic credentials with: export RESOURCEGURU_EMAIL=\"your-token-here\" RESOURCEGURU_PASSWORD=\"your-password\""))
	}
	if len(trimmed) == 0 {
