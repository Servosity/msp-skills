...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "resourceguru-cli",
		Short: `Resourceguru CLI — Every Resource Guru endpoint as a typed command, plus a local database and per-resource per-day utilization no Resource…`,
		Long: `Resourceguru CLI — Every Resource Guru endpoint as a typed command, plus a local database and per-resource per-day utilization no Resource…
...
Health check: run 'resourceguru-cli doctor' to verify auth and connectivity.
See README.md or the bundled SKILL.md for recipes.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("resourceguru-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.profileName, "profile", "", "Apply values from a saved profile (see 'resourceguru-cli profile list')")
	...
	rootCmd.AddCommand(newCustomAvailablePeriodsCmd(flags))
	rootCmd.AddCommand(newCustomFieldOptionsCmd(flags))
	rootCmd.AddCommand(newCustomFieldsCmd(flags))
	rootCmd.AddCommand(newDowntimesCmd(flags))
	...
	rootCmd.AddCommand(newTimesheetsCmd(flags))
	...
	rootCmd.AddCommand(newNovelBenchCmd(flags))
	rootCmd.AddCommand(newNovelCapacityCmd(flags))
	rootCmd.AddCommand(newNovelOverbookedCmd(flags))
	rootCmd.AddCommand(newNovelSinceCmd(flags))
	rootCmd.AddCommand(newNovelUtilizationCmd(flags))
	rootCmd.AddCommand(newAPICmd(flags))
	rootCmd.AddCommand(newDowntimeTypesPromotedCmd(flags))
	rootCmd.AddCommand(newMePromotedCmd(flags))
	rootCmd.AddCommand(newOvertimesPromotedCmd(flags))
