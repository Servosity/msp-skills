...
type Config struct {
	...
	ResourceguruEmail    string          `toml:"email"`
	ResourceguruPassword string          `toml:"password"`
}
...
func Load(configPath string) (*Config, error) {
	...
	// Env var overrides
	if v := os.Getenv("RESOURCEGURU_EMAIL"); v != "" {
		cfg.ResourceguruEmail = v
		cfg.markEnvOverride("ResourceguruEmail")
		cfg.AuthSource = "env:RESOURCEGURU_EMAIL"
		cfg.CredentialSource = "env:RESOURCEGURU_EMAIL"
	}
	if v := os.Getenv("RESOURCEGURU_PASSWORD"); v != "" {
		cfg.ResourceguruPassword = v
		cfg.markEnvOverride("ResourceguruPassword")
		cfg.AuthSource = "env:RESOURCEGURU_PASSWORD"
		cfg.CredentialSource = "env:RESOURCEGURU_PASSWORD"
	}
	...
	if v := os.Getenv("RESOURCEGURU_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	return cfg, nil
}
...
func resolveConfigPath(configPath string) (string, bool, error) {
	...
	if path := os.Getenv("RESOURCEGURU_CONFIG"); path != "" {
...
func LegacyConfigPath() (string, error) {
	...
	return filepath.Join(home, ".config", "resourceguru-cli", "config.toml"), nil
}
...
func (c *Config) AuthHeader() string {
	...
	if c.ResourceguruEmail == "" {
		return ""
	}
	if c.ResourceguruPassword == "" {
		return ""
	}
	credentials := basicAuthPayload("Basic {username}:{password}", map[string]string{
		"email":                 c.ResourceguruEmail,
		"RESOURCEGURU_EMAIL":    c.ResourceguruEmail,
		"username":              c.ResourceguruEmail,
		"user":                  c.ResourceguruEmail,
		"password":              c.ResourceguruPassword,
		"RESOURCEGURU_PASSWORD": c.ResourceguruPassword,
		"secret":                c.ResourceguruPassword,
		"token":                 c.ResourceguruPassword,
		"access_token":          c.ResourceguruPassword,
	}, false)
	if credentials == "" {
		return ""
	}
...
func (c *Config) credentials() *cliutil.Credentials {
	return &cliutil.Credentials{
		...
		ResourceguruEmail:    c.ResourceguruEmail,
		ResourceguruPassword: c.ResourceguruPassword,
	}
}
...
func (c *Config) applyCredentials(creds *cliutil.Credentials) {
	...
	c.ResourceguruEmail = creds.ResourceguruEmail
	c.ResourceguruPassword = creds.ResourceguruPassword
}
...
func (c *Config) SaveCredentials(value0, value1 string) error {
	...
	c.updateFileConfigField("AccessToken")
	c.ResourceguruEmail = value0
	delete(c.envOverrides, "ResourceguruEmail")
	c.updateFileConfigField("ResourceguruEmail")
	c.ResourceguruPassword = value1
	delete(c.envOverrides, "ResourceguruPassword")
	c.updateFileConfigField("ResourceguruPassword")
	if err := c.saveCredentialsFirst(); err != nil {
		return err
	}
	return c.save()
...
func (c *Config) configForSave() Config {
	out := *c
	if c.fileConfig != nil {
		if c.envOverrides["ResourceguruEmail"] {
			out.ResourceguruEmail = c.fileConfig.ResourceguruEmail
		}
		if c.envOverrides["ResourceguruPassword"] {
			out.ResourceguruPassword = c.fileConfig.ResourceguruPassword
		}
	}
...
func (c *Config) updateFileConfigField(field string) {
	...
	case "ResourceguruEmail":
		c.fileConfig.ResourceguruEmail = c.ResourceguruEmail
	case "ResourceguruPassword":
		c.fileConfig.ResourceguruPassword = c.ResourceguruPassword
	}
}
