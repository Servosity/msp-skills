# resourceguru skill - governance and safety model

> Unofficial. Community-built skill for the Resource Guru API. Not affiliated with,
> endorsed by, or sponsored by the vendor.
> This page tells an MSP owner exactly what the resourceguru skill can touch and how to
> scope it, so you can decide what to let an AI agent do.

## What it authenticates as

The skill drives the `resourceguru-cli` binary (and `resourceguru-mcp`),
authenticating with the credentials documented in mcp-install.md. Credentials are read from the environment only -
never written to disk, never logged, never sent anywhere except the Resource Guru API.
...
| **Write (routine)** | Day-to-day mutations. | `accounts update`, `activity-types create`, `activity-types update`, `activity-types update-activitytypes`, `bookings create`, `bookings remind create`, `bookings resolve create`, `bookings split create`, ... (39 total) | Preview with `--dry-run`, then an approved write (where a command documents its own confirm gate, use it too) |
| **Credential / security** | Touches tokens, keys, MFA. | (none detected) | Human-in-the-loop only |
| **Destructive** | Irreversible data or config loss. | `activity-types delete`, `bookings delete`, `calendars delete`, `clients delete`, `custom-available-periods delete`, `custom-field-options delete`, `custom-fields delete`, `downtimes delete`, ... (12 total) | Human-in-the-loop only, explicit confirmation |
...
the Resource Guru API, and you can read every line of how it does so. The skill is
