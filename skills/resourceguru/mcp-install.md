# Resource Guru MCP - install for every agent that speaks MCP

This page wires the Resource Guru MCP server into any MCP client. If you use Claude
...
launch `resourceguru-mcp` directly on your machine - no hosting. *Remote* agents (ChatGPT,
Microsoft 365 Copilot / Copilot Studio, the Gemini app) only talk to an HTTPS
endpoint, so you expose `resourceguru-mcp` over HTTPS first.

## Prerequisite: install the MCP binary

Run the install command from [README.md](./README.md). It drops both `resourceguru-cli`
and `resourceguru-mcp` on your PATH. `resourceguru-mcp` is what the agents talk to.

```bash
resourceguru-mcp --help
```
...
Restart Gemini CLI; the Resource Guru tools appear in its tool list. (The **Gemini app /
...
All remote agents need `resourceguru-mcp` reachable as a public **HTTPS** endpoint. Run it
...
`resourceguru-mcp` you installed is not enough on its own. You also need a **Copilot Studio
license** and a **tenant admin** to enable it. This is a build-and-host task, not a
self-serve install.

Once `resourceguru-mcp` is hosted over HTTPS (above), the lowest-code route:
...
hermes mcp add resourceguru -- resourceguru-mcp

# OpenClaw
openclaw mcp set resourceguru '{"command":"resourceguru-mcp"}'
```
...
- `resourceguru-mcp: command not found`: the install dir is not on your PATH (the
