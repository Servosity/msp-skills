# Rewst Printed CLI Agent Guide

This directory is a generated `rewst-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
rewst-cli doctor --json
rewst-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
rewst-cli which "<capability>" --json
rewst-cli <command> --help
```
...
```bash
rewst-cli <command> --help
rewst-cli <command> --dry-run --agent
```
