...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".local", "share", "rewst-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("REWST_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("REWST_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "rewst-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.local/share/rewst-cli/feedback.jsonl.
When ` + "`REWST_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`REWST_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  rewst-cli feedback list
  rewst-cli feedback list --limit 5
  rewst-cli feedback list --json`,
