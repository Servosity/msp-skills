...
type Config struct {
	...
	RewstApiToken string            `toml:"api_token"`
}

func Load(configPath string) (*Config, error) {
	cfg := &Config{
		BaseURL: "https://api.rewst.io",
	}

	// Resolve config path
	path := configPath
	if path == "" {
		path = os.Getenv("REWST_CONFIG")
	}
	if path == "" {
		home, _ := os.UserHomeDir()
		path = filepath.Join(home, ".config", "rewst-cli", "config.toml")
	}
	cfg.Path = path
	...
	// Env var overrides
	if v := os.Getenv("REWST_API_TOKEN"); v != "" {
		cfg.RewstApiToken = v
		cfg.markEnvOverride("RewstApiToken")
		cfg.AuthSource = "env:REWST_API_TOKEN"
	}
	...
	if cfg.AuthSource == "" && cfg.RewstApiToken != "" {
		cfg.AuthSource = "config"
	}
	...
	if v := os.Getenv("REWST_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	return cfg, nil
}
...
func (c *Config) AuthHeader() string {
	...
	if c.RewstApiToken != "" {
		if c.AuthSource == "" {
			c.AuthSource = "env:REWST_API_TOKEN"
		}
		return "Bearer " + c.RewstApiToken
	}
	if c.AccessToken != "" {
...
func (c *Config) ClearTokens() error {
	...
	c.updateFileConfigField("ClientSecret")
	c.RewstApiToken = ""
	delete(c.envOverrides, "RewstApiToken")
	return c.save()
}
...
func (c *Config) configForSave() Config {
	out := *c
	if c.fileConfig != nil {
		if c.envOverrides["RewstApiToken"] {
			out.RewstApiToken = c.fileConfig.RewstApiToken
		}
	}
...
func (c *Config) updateFileConfigField(field string) {
	...
	case "RewstApiToken":
		c.fileConfig.RewstApiToken = c.RewstApiToken
	}
}
