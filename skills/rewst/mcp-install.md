# Rewst MCP - install for every agent that speaks MCP

This page wires the Rewst MCP server into any MCP client. If you use Claude
...
launch `rewst-mcp` directly on your machine - no hosting. *Remote* agents (ChatGPT,
Microsoft 365 Copilot / Copilot Studio, the Gemini app) only talk to an HTTPS
endpoint, so you expose `rewst-mcp` over HTTPS first.

## Prerequisite: install the MCP binary

Run the install command from [README.md](./README.md). It drops both `rewst-cli`
and `rewst-mcp` on your PATH. `rewst-mcp` is what the agents talk to.
...
```json
{
  "mcpServers": {
    "rewst": {
      "command": "rewst-mcp",
      "env": {
        "REWST_BASE_URL": "<your-rewst_base_url>",
        "REWST_API_TOKEN": "<your-rewst_api_token>"
      }
    }
  }
}
...
Restart Gemini CLI; the Rewst tools appear in its tool list. (The **Gemini app /
...
All remote agents need `rewst-mcp` reachable as a public **HTTPS** endpoint. Run it
...
`rewst-mcp` you installed is not enough on its own. You also need a **Copilot Studio
license** and a **tenant admin** to enable it. This is a build-and-host task, not a
self-serve install.

Once `rewst-mcp` is hosted over HTTPS (above), the lowest-code route:
...
hermes mcp add rewst -- rewst-mcp

# OpenClaw
openclaw mcp set rewst '{"command":"rewst-mcp"}'
```
...
- `rewst-mcp: command not found`: the install dir is not on your PATH (the
