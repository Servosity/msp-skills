...
// touching disk. RIVERSIDE_FM_NO_CONFIG_WRITE makes the automatic token cache a
...
func ncwHome(t *testing.T) string {
	...
	home := t.TempDir()
	t.Setenv("HOME", home)
	t.Setenv("RIVERSIDE_FM_CONFIG", filepath.Join(home, "config.toml"))
	return home
}
...
func TestNoConfigWriteStillAllowsTheWipe(t *testing.T) {
	...
	// Seed through the connector's own writer, with exactly what its
	// `auth login --chrome` saves: the cookie string in AccessToken and
	// nothing else (riverside-fm has no client id/secret; its ClearTokens
	// wipes the cookie, refresh and expiry fields it actually uses).
	os.Unsetenv(NoConfigWriteEnv)
	seed, err := Load("")
	if err != nil {
		t.Fatalf("Load (seed): %v", err)
	}
	if err := seed.SaveTokens("", "", ncwToken, ncwRefresh, time.Time{}); err != nil {
		t.Fatalf("SaveTokens (seed): %v", err)
	}
	...
	for _, secret := range []string{ncwToken, ncwRefresh} {
...
func TestNoConfigWriteTruthiness(t *testing.T) {
	...
	if NoConfigWriteEnv != "RIVERSIDE_FM_NO_CONFIG_WRITE" {
		t.Errorf("switch is named %q; the documented name is RIVERSIDE_FM_NO_CONFIG_WRITE", NoConfigWriteEnv)
	}
}
