...
// resolver the CLI uses, which reads RIVERSIDE_FM_CONFIG and applies
...
func TestMCPClientHonoursTheConfigEnvVar(t *testing.T) {
	...
	t.Setenv("HOME", dir)
	t.Setenv("RIVERSIDE_FM_CONFIG", path)
	// Guard against a stray ambient override changing the assertion.
	os.Unsetenv("RIVERSIDE_FM_BASE_URL")
	...
	if cfg.Path != path {
		t.Errorf("MCP server read %q, want the RIVERSIDE_FM_CONFIG path %q", cfg.Path, path)
	}
