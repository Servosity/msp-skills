...
var readCommandResources = map[string][]string{
	"rocketcyber-cli agents":             {"agents"},
	"rocketcyber-cli agents list":        {"agents"},
	"rocketcyber-cli agents get":         {"agents"},
	"rocketcyber-cli agents search":      {"agents"},
	"rocketcyber-cli apps":               {"apps"},
	"rocketcyber-cli apps list":          {"apps"},
	"rocketcyber-cli apps get":           {"apps"},
	"rocketcyber-cli apps search":        {"apps"},
	"rocketcyber-cli firewalls":          {"firewalls"},
	"rocketcyber-cli firewalls list":     {"firewalls"},
	"rocketcyber-cli firewalls get":      {"firewalls"},
	"rocketcyber-cli firewalls search":   {"firewalls"},
	"rocketcyber-cli incidents":          {"incidents"},
	"rocketcyber-cli incidents list":     {"incidents"},
	"rocketcyber-cli incidents get":      {"incidents"},
	"rocketcyber-cli incidents search":   {"incidents"},
	"rocketcyber-cli suppression":        {"suppression"},
	"rocketcyber-cli suppression list":   {"suppression"},
	"rocketcyber-cli suppression get":    {"suppression"},
	"rocketcyber-cli suppression search": {"suppression"},
}
...
func cachePolicy() cliutil.Policy {
	...
	envOptOut := "ROCKETCYBER_NO_AUTO_REFRESH"
...
func autoRefreshIfStale(ctx context.Context, flags *rootFlags, resources []string) (meta cliutil.FreshnessMeta) {
	...
	dbPath := defaultDBPath("rocketcyber-cli")
	...
	if err := runAutoRefresh(refreshCtx, flags, db, resources); err != nil {
		fmt.Fprintf(os.Stderr, "warning: using stale rocketcyber-cli cache (refresh failed: %v)\n", err)
