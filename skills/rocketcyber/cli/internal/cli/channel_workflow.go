...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  rocketcyber-cli workflow archive

  # Full re-archive (ignore previous sync state)
  rocketcyber-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"agents", "apps", "firewalls", "incidents", "suppression"}
			totalSynced := 0
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  rocketcyber-cli workflow status

  # Show status as JSON
  rocketcyber-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("rocketcyber-cli")
			}
