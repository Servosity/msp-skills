...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".local", "share", "rocketcyber-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("ROCKETCYBER_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("ROCKETCYBER_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "rocketcyber-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.local/share/rocketcyber-cli/feedback.jsonl.
When ` + "`ROCKETCYBER_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`ROCKETCYBER_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  rocketcyber-cli feedback list
  rocketcyber-cli feedback list --limit 5
  rocketcyber-cli feedback list --json`,
