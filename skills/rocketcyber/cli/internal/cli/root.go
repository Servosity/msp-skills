...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "rocketcyber-cli",
		Short: `Rocketcyber CLI — The first CLI and MCP server for RocketCyber Managed SOC, with triage and posture analytics no console page or API call…`,
		Long: `Rocketcyber CLI — The first CLI and MCP server for RocketCyber Managed SOC, with triage and posture analytics no console page or API call…
...
Health check: run 'rocketcyber-cli doctor' to verify auth and connectivity.
See README.md or the bundled SKILL.md for recipes.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("rocketcyber-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.profileName, "profile", "", "Apply values from a saved profile (see 'rocketcyber-cli profile list')")
	...
	// Novel analytics leaves attach to the promoted resource parents so the
	// promoted endpoint commands and the novel commands share one tree.
	agentsCmd := newAgentsPromotedCmd(flags)
	agentsCmd.AddCommand(newNovelAgentsStaleCmd(flags))
	rootCmd.AddCommand(agentsCmd)
	rootCmd.AddCommand(newAppsPromotedCmd(flags))
	defenderCmd := newDefenderPromotedCmd(flags)
	defenderCmd.AddCommand(newNovelDefenderRiskiestCmd(flags))
	rootCmd.AddCommand(defenderCmd)
	rootCmd.AddCommand(newFirewallsPromotedCmd(flags))
	incidentsCmd := newIncidentsPromotedCmd(flags)
	incidentsCmd.AddCommand(newNovelIncidentsMttrCmd(flags))
	rootCmd.AddCommand(incidentsCmd)
	officeCmd := newOfficePromotedCmd(flags)
	officeCmd.AddCommand(newNovelOfficeTrendCmd(flags))
