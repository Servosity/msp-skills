...
Resource scoping:
  ...
  rocketcyber-cli sync

  # Sync specific resources only
  rocketcyber-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  rocketcyber-cli sync --full

  # Incremental sync: only records from the last 7 days
  rocketcyber-cli sync --since 7d

  # Parallel sync with 8 workers
  rocketcyber-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  rocketcyber-cli sync --latest-only

  # Sync historical data for a date range
  rocketcyber-cli sync --dates 20250101-20250131`,
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/rocketcyber-cli/data.db)")
	...
	cmd.Flags().StringVar(&dates, "dates", "", "Date or date range to sync (passed as dates query parameter)")
