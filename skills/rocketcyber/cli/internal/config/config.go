...
type Config struct {
	...
	RocketcyberApiToken string            `toml:"api_token"`
}
...
func Load(configPath string) (*Config, error) {
	...
	path := configPath
	if path == "" {
		path = os.Getenv("ROCKETCYBER_CONFIG")
	}
	if path == "" {
		home, _ := os.UserHomeDir()
		path = filepath.Join(home, ".config", "rocketcyber-cli", "config.toml")
	}
	cfg.Path = path
	...
	// Env var overrides
	if v := os.Getenv("ROCKETCYBER_API_TOKEN"); v != "" {
		cfg.RocketcyberApiToken = v
		cfg.markEnvOverride("RocketcyberApiToken")
		cfg.AuthSource = "env:ROCKETCYBER_API_TOKEN"
	}
	...
	if cfg.AuthSource == "" && cfg.RocketcyberApiToken != "" {
		cfg.AuthSource = "config"
	}
	...
	if v := os.Getenv("ROCKETCYBER_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	return cfg, nil
}
...
func (c *Config) AuthHeader() string {
	...
	if c.RocketcyberApiToken != "" {
		if c.AuthSource == "" {
			c.AuthSource = "env:ROCKETCYBER_API_TOKEN"
		}
		return applyAuthFormat("Bearer {token}", map[string]string{
			"api_token":             c.RocketcyberApiToken,
			"ROCKETCYBER_API_TOKEN": c.RocketcyberApiToken,
			"token":                 c.RocketcyberApiToken,
		})
	}
	if c.AccessToken != "" {
...
func (c *Config) ClearTokens() error {
	...
	c.updateFileConfigField("ClientID")
	c.updateFileConfigField("ClientSecret")
	c.RocketcyberApiToken = ""
	delete(c.envOverrides, "RocketcyberApiToken")
	return c.save()
}
...
func (c *Config) configForSave() Config {
	out := *c
	if c.fileConfig != nil {
		if c.envOverrides["RocketcyberApiToken"] {
			out.RocketcyberApiToken = c.fileConfig.RocketcyberApiToken
		}
	}
...
func (c *Config) updateFileConfigField(field string) {
	...
	case "RocketcyberApiToken":
		c.fileConfig.RocketcyberApiToken = c.RocketcyberApiToken
	}
}
