# install.ps1 - install rocketcyber-cli and rocketcyber-mcp on Windows.
#
# Pulls prebuilt binaries from this skill's latest GitHub Release (rocketcyber-v*).
...
$Skill   = "rocketcyber"
$CliBin  = "rocketcyber-cli.exe"
$McpBin  = "rocketcyber-mcp.exe"
...
$ReleaseBase = if ($env:MSP_SKILLS_RELEASE_BASE) {
  ...
  # Each skill is versioned/tagged independently (rocketcyber-vX.Y.Z),
...
Write-Host "  rocketcyber-cli --version"
Write-Host ""
Write-Host "Next:"
Write-Host "  First command + auth: https://github.com/$Owner/$Repo/tree/main/skills/rocketcyber#readme"
Write-Host "  Claude Desktop / ChatGPT wire-up: https://github.com/$Owner/$Repo/blob/main/skills/rocketcyber/mcp-install.md"
