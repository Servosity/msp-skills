# Rootly Printed CLI Agent Guide

This directory is a generated `rootly-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
rootly-cli doctor --json
rootly-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
rootly-cli which "<capability>" --json
rootly-cli <command> --help
```
...
```bash
rootly-cli <command> --help
rootly-cli <command> --dry-run --agent
```
