...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  rootly-cli workflow archive

  # Full re-archive (ignore previous sync state)
  rootly-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"action-items", "alert-fields", "alert-groups", "alert-routes", "alert-routing-rules", "alert-sources", "alert-urgencies", "alerts", "api-keys", "audits", "authorizations", "catalog-checklist-templates", "catalog-entity-checklists", "catalogs", "causes", "causes-properties", "communications", "communications-groups", "communications-stages", "communications-templates", "custom-fields", "custom-forms", "dashboards", "edge-connectors", "environments", "environments-properties", "escalation-policies", "form-fields", "form-sets", "functionalities", "functionalities-properties", "heartbeats", "incident-permission-sets", "incident-roles", "incident-types", "incident-types-properties", "incidents", "ip-ranges", "live-call-routers", "on-call-pay-reports", "on-call-roles", "oncalls", "playbooks", "post-mortem-templates", "post-mortems", "pulses", "retrospective-configurations", "retrospective-processes", "roles", "schedules", "secrets", "services", "services-properties", "severities", "shifts", "slas", "status-pages", "statuses", "sub-statuses", "teams", "teams-properties", "users", "users-me", "webhooks", "workflow-groups", "workflows"}
			totalSynced := 0
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  rootly-cli workflow status

  # Show status as JSON
  rootly-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("rootly-cli")
			}
