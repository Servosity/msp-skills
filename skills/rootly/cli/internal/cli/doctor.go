...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  rootly-cli doctor
  rootly-cli doctor --json
  rootly-cli doctor --fail-on warn`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if os.Getenv("ROOTLY_API_KEY") != "" {
				authEnvSet = append(authEnvSet, "ROOTLY_API_KEY")
				...
				authEnvRequiredMissing = append(authEnvRequiredMissing, "ROOTLY_API_KEY")
			}
			if strings.Contains("Accepted as an alias for ROOTLY_API_KEY (the variable the Rootly MCP server uses).", " OR ") {
				authEnvOptionalNames = append(authEnvOptionalNames, "ROOTLY_API_TOKEN")
				if os.Getenv("ROOTLY_API_TOKEN") != "" {
					authEnvSet = append(authEnvSet, "ROOTLY_API_TOKEN")
					authEnvOptionalSatisfied = true
				}
			} else if os.Getenv("ROOTLY_API_TOKEN") != "" {
				authEnvSet = append(authEnvSet, "ROOTLY_API_TOKEN")
			} else {
				authEnvInfo = append(authEnvInfo, "ROOTLY_API_TOKEN optional")
			}
			switch {
			...
			if cfg != nil && cfg.BaseURL != "" {
				...
				if clientErr != nil {
					...
					if authHeader == "" {
						...
						authHeaders["User-Agent"] = "rootly-cli"
...
func collectCacheReport(ctx context.Context, staleAfterSpec string) map[string]any {
	...
	if err != nil {
		if os.IsNotExist(err) {
			...
			report["hint"] = "Database not created yet; run 'rootly-cli sync' to hydrate."
			return report
		}
		report["status"] = "error"
	...
	if qerr != nil {
		...
		report["status"] = "unknown"
		report["hint"] = "No sync state recorded; run 'rootly-cli sync' to populate."
		return report
	}
	defer rows.Close()
	...
	case !haveAny && len(resources) == 0:
		report["status"] = "unknown"
		report["hint"] = "sync_state is empty; run 'rootly-cli sync' to hydrate."
	case fresh:
		report["status"] = "fresh"
	default:
		report["status"] = "stale"
		report["oldest_age"] = oldest.Round(time.Minute).String()
		report["hint"] = "Some resources are older than stale_after; run 'rootly-cli sync' to refresh."
	}
	return report
}
