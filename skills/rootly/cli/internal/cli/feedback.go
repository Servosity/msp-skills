...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".local", "share", "rootly-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("ROOTLY_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("ROOTLY_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "rootly-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.local/share/rootly-cli/feedback.jsonl.
When ` + "`ROOTLY_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`ROOTLY_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  rootly-cli feedback list
  rootly-cli feedback list --limit 5
  rootly-cli feedback list --json`,
