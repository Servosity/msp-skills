...
type Config struct {
	...
	RootlyApiKey   string            `toml:"api_key"`
	RootlyApiToken string            `toml:"api_token"`
}
...
func Load(configPath string) (*Config, error) {
	...
	path := configPath
	if path == "" {
		path = os.Getenv("ROOTLY_CONFIG")
	}
	if path == "" {
		home, _ := os.UserHomeDir()
		path = filepath.Join(home, ".config", "rootly-cli", "config.toml")
	}
	cfg.Path = path
	...
	// Env var overrides
	if v := os.Getenv("ROOTLY_API_KEY"); v != "" {
		cfg.RootlyApiKey = v
		cfg.markEnvOverride("RootlyApiKey")
		cfg.AuthSource = "env:ROOTLY_API_KEY"
	}
	if v := os.Getenv("ROOTLY_API_TOKEN"); v != "" {
		cfg.RootlyApiToken = v
		cfg.markEnvOverride("RootlyApiToken")
		cfg.AuthSource = "env:ROOTLY_API_TOKEN"
	}
	...
	if cfg.AuthSource == "" && cfg.RootlyApiKey != "" {
		cfg.AuthSource = "config"
	}
	if cfg.AuthSource == "" && cfg.RootlyApiToken != "" {
		cfg.AuthSource = "config"
	}
	...
	if v := os.Getenv("ROOTLY_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	return cfg, nil
}
...
func (c *Config) AuthHeader() string {
	...
	if c.RootlyApiKey != "" {
		if c.AuthSource == "" {
			c.AuthSource = "env:ROOTLY_API_KEY"
		}
		return "Bearer " + c.RootlyApiKey
	}
	if c.RootlyApiToken != "" {
		if c.AuthSource == "" {
			c.AuthSource = "env:ROOTLY_API_TOKEN"
		}
		return "Bearer " + c.RootlyApiToken
	}
	if c.AccessToken != "" {
...
func (c *Config) ClearTokens() error {
	...
	c.updateFileConfigField("ClientSecret")
	c.RootlyApiKey = ""
	delete(c.envOverrides, "RootlyApiKey")
	c.RootlyApiToken = ""
	delete(c.envOverrides, "RootlyApiToken")
	return c.save()
}
...
func (c *Config) configForSave() Config {
	out := *c
	if c.fileConfig != nil {
		if c.envOverrides["RootlyApiKey"] {
			out.RootlyApiKey = c.fileConfig.RootlyApiKey
		}
		if c.envOverrides["RootlyApiToken"] {
			out.RootlyApiToken = c.fileConfig.RootlyApiToken
		}
	}
...
func (c *Config) updateFileConfigField(field string) {
	...
	case "RootlyApiKey":
		c.fileConfig.RootlyApiKey = c.RootlyApiKey
	case "RootlyApiToken":
		c.fileConfig.RootlyApiToken = c.RootlyApiToken
	}
}
