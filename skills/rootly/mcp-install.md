# Rootly MCP - install for every agent that speaks MCP

This page wires the Rootly MCP server into any MCP client. If you use Claude
...
launch `rootly-mcp` directly on your machine - no hosting. *Remote* agents (ChatGPT,
Microsoft 365 Copilot / Copilot Studio, the Gemini app) only talk to an HTTPS
endpoint, so you expose `rootly-mcp` over HTTPS first.

## Prerequisite: install the MCP binary

Run the install command from [README.md](./README.md). It drops both `rootly-cli`
and `rootly-mcp` on your PATH. `rootly-mcp` is what the agents talk to.
...
Restart Gemini CLI; the Rootly tools appear in its tool list. (The **Gemini app /
...
All remote agents need `rootly-mcp` reachable as a public **HTTPS** endpoint. Run it
...
`rootly-mcp` you installed is not enough on its own. You also need a **Copilot Studio
license** and a **tenant admin** to enable it. This is a build-and-host task, not a
self-serve install.

Once `rootly-mcp` is hosted over HTTPS (above), the lowest-code route:
...
hermes mcp add rootly -- rootly-mcp

# OpenClaw
openclaw mcp set rootly '{"command":"rootly-mcp"}'
```
...
- `rootly-mcp: command not found`: the install dir is not on your PATH (the
