# runZero Printed CLI Agent Guide

This directory is a generated `runzero-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
runzero-cli doctor --json
runzero-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
runzero-cli which "<capability>" --json
runzero-cli <command> --help
```
...
```bash
runzero-cli <command> --help
runzero-cli <command> --dry-run --agent
```
