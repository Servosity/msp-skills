...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".local", "share", "runzero-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("RUNZERO_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("RUNZERO_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "runzero-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.local/share/runzero-cli/feedback.jsonl.
When ` + "`RUNZERO_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`RUNZERO_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  runzero-cli feedback list
  runzero-cli feedback list --limit 5
  runzero-cli feedback list --json`,
