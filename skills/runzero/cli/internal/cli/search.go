...
In local mode: searches locally synced data only.`,
  ...
  runzero-cli search "error timeout"

  # Force local search only
  runzero-cli search "payment failed" --data-source local

  # Search a specific resource type locally
  runzero-cli search "critical" --type transactions --data-source local

  # JSON output for piping
  runzero-cli search "critical" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'runzero-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
			...
			switch resourceType {
			case "account":
				results, err = db.SearchAccount(query, limit)
			case "org":
				results, err = db.SearchOrg(query, limit)
			case "runzero-export":
				results, err = db.SearchRunzeroExport(query, limit)
			case "runzero-import":
				results, err = db.SearchRunzeroImport(query, limit)
			case "":
				...
				{
					partial, searchErr := db.SearchAccount(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search account failed: %w", searchErr)
					}
				...
				{
					partial, searchErr := db.SearchOrg(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search org failed: %w", searchErr)
					}
					for _, r := range partial {
						key := string(r)
						if !seen[key] {
				...
				{
					partial, searchErr := db.SearchRunzeroExport(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search runzero_export failed: %w", searchErr)
					}
					for _, r := range partial {
						key := string(r)
						if !seen[key] {
				...
				{
					partial, searchErr := db.SearchRunzeroImport(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search runzero_import failed: %w", searchErr)
					}
					for _, r := range partial {
						key := string(r)
						if !seen[key] {
