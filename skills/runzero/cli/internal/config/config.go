...
type Config struct {
	...
	RunzeroApiKey string            `toml:"api_key"`
}
...
func Load(configPath string) (*Config, error) {
	...
	path := configPath
	if path == "" {
		path = os.Getenv("RUNZERO_CONFIG")
	}
	if path == "" {
		home, _ := os.UserHomeDir()
		path = filepath.Join(home, ".config", "runzero-cli", "config.toml")
	}
	cfg.Path = path
	...
	// Env var overrides
	if v := os.Getenv("RUNZERO_API_KEY"); v != "" {
		cfg.RunzeroApiKey = v
		cfg.markEnvOverride("RunzeroApiKey")
		cfg.AuthSource = "env:RUNZERO_API_KEY"
	}
	...
	if cfg.AuthSource == "" && cfg.RunzeroApiKey != "" {
		cfg.AuthSource = "config"
	}
	...
	if v := os.Getenv("RUNZERO_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	return cfg, nil
}
...
func (c *Config) AuthHeader() string {
	...
	if c.RunzeroApiKey != "" {
		if c.AuthSource == "" {
			c.AuthSource = "env:RUNZERO_API_KEY"
		}
		return "Bearer " + c.RunzeroApiKey
	}
	if c.AccessToken != "" {
...
func (c *Config) ClearTokens() error {
	...
	c.updateFileConfigField("ClientSecret")
	c.RunzeroApiKey = ""
	delete(c.envOverrides, "RunzeroApiKey")
	return c.save()
}
...
func (c *Config) configForSave() Config {
	out := *c
	if c.fileConfig != nil {
		if c.envOverrides["RunzeroApiKey"] {
			out.RunzeroApiKey = c.fileConfig.RunzeroApiKey
		}
	}
...
func (c *Config) updateFileConfigField(field string) {
	...
	case "RunzeroApiKey":
		c.fileConfig.RunzeroApiKey = c.RunzeroApiKey
	}
}
