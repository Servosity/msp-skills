...
func newMCPClient() (*client.Client, error) {
	...
	cfgPath := filepath.Join(home, ".config", "runzero-cli", "config.toml")
...
func dbPath() string {
	home, _ := os.UserHomeDir()
	return filepath.Join(home, ".local", "share", "runzero-cli", "data.db")
}
...
func handleContext(_ context.Context, _ mcplib.CallToolRequest) (*mcplib.CallToolResult, error) {
	ctx := map[string]any{
		...
		"tool_surface": "MCP exposes typed endpoint tools plus a runtime mirror of user-facing CLI commands. Endpoint tools keep typed schemas; command-mirror tools shell out to the companion runzero-cli binary.",
		"auth": map[string]any{
			"type": "bearer_token",
			"env_vars": []map[string]any{
				{
					"name":        "RUNZERO_API_KEY",
					"kind":        "per_call",
					"required":    true,
					"sensitive":   true,
		...
		"resources": []map[string]any{
			{
				...
				"endpoints":   []string{"create-asset-ownership-types", "create-credential", "create-custom-integration", "create-custom-integration-and-id", "create-group", "create-group-mapping", "create-key", "create-organization", "create-organization-export-token", "create-scan-template", "create-user", "create-user-invite", "delete-asset-ownership-type", "delete-asset-ownership-types", "delete-custom-integration", "delete-organization-export-token", "delete-organization-export-token-deprecated", "export-events-json", "export-events-jsonl", "get-agents", "get-apitoken", "get-asset-ownership-types", "get-credential", "get-credentials", "get-custom-integration", "get-custom-integrations", "get-group", "get-group-mapping", "get-group-mappings", "get-groups", "get-key", "get-keys", "get-license", "get-organization", "get-organization-export-token", "get-organization-export-tokens", "get-organizations", "get-scan-template", "get-scan-templates", "get-sites", "get-tasks", "get-user", "get-users", "remove-credential", "remove-group", "remove-group-mapping", "remove-key", "remove-organization", "remove-scan-template", "remove-user", "reset-user-lockout", "reset-user-mfa", "reset-user-password", "rotate-apitoken", "rotate-key", "rotate-organization-export-token", "rotate-organization-export-token-deprecated", "update-asset-ownership-type", "update-asset-ownership-types", "update-custom-integration", "update-group", "update-group-mapping", "update-organization", "update-scan-template", "update-user"},
				"syncable":    true,
				"searchable":  true,
			},
			{
				...
				"endpoints":   []string{"get-check"},
			},
			{
				"name":        "org",
				"description": "Manage org",
				"endpoints":   []string{"bulk-remove-custom-integration", "clear-bulk-asset-owners", "clear-bulk-asset-tags", "create-sample", "create-scan", "create-site", "export-asset-metrics-json", "export-asset-top-hwcsv", "export-asset-top-oscsv", "export-asset-top-tags-csv", "export-asset-top-types-csv", "export-services-top-products-csv", "export-services-top-protocols-csv", "export-services-top-tcpcsv", "export-services-top-udpcsv", "get-agent", "get-agents", "get-asset", "get-assets", "get-custom-integration", "get-custom-integrations", "get-explorer", "get-explorers", "get-hosted-zone", "get-hosted-zones", "get-key", "get-organization", "get-service", "get-services", "get-site", "get-sites", "get-task", "get-task-change-report", "get-task-log", "get-task-scan-data", "get-tasks", "get-wireless-lan", "get-wireless-lans", "hide-task", "import-nessus-scan-data", "import-packet-data", "import-scan-data", "merge-assets", "remove-agent", "remove-asset", "remove-asset-source", "remove-bulk-assets", "remove-custom-integration", "remove-explorer", "remove-key", "remove-service", "remove-site", "remove-wireless-lan", "rotate-key", "stop-task", "update-agent-settings", "update-asset-comments", "update-asset-criticality", "update-asset-owners", "update-asset-tags", "update-bulk-asset-criticality", "update-bulk-asset-owners", "update-bulk-asset-tags", "update-explorer-settings", "update-organization", "update-site", "update-task", "upgrade-agent", "upgrade-explorer"},
				"syncable":    true,
				"searchable":  true,
			},
			{
				...
				"endpoints":   []string{"get-latest-agent-version", "get-latest-platform-version", "get-latest-scanner-version"},
			},
			{
				"name":        "runzero-export",
				"description": "Manage runzero export",
				"endpoints":   []string{"assets-cisco-csv", "assets-csv", "assets-json", "assets-jsonl", "assets-nmap-xml", "certificates-csv", "certificates-json", "certificates-jsonl", "directory-groups-csv", "directory-groups-json", "directory-groups-jsonl", "directory-users-csv", "directory-users-json", "directory-users-jsonl", "findings-csv", "findings-json", "findings-jsonl", "services-csv", "services-json", "services-jsonl", "sites-csv", "sites-json", "sites-jsonl", "snmparpcache-csv", "snow-assets-csv", "snow-assets-json", "snow-service-graph-assets-json", "snow-services-csv", "software-csv", "software-json", "software-jsonl", "splunk-asset-sync-created-json", "splunk-asset-sync-updated-json", "subnet-utilization-stats-csv", "tasks-json", "tasks-jsonl", "vulnerabilities-csv", "vulnerabilities-json", "vulnerabilities-jsonl", "wireless-csv", "wireless-json", "wireless-jsonl"},
				"syncable":    true,
				"searchable":  true,
			},
			{
				"name":        "runzero-import",
				"description": "Manage runzero import",
				"endpoints":   []string{"custom-integration-assets"},
				"searchable":  true,
			},
		},
		"query_tips": []string{
		...
		"playbook": []map[string]string{
			...
			{"topic": "Exposure triage (criticality x vulnerability)", "insight": "Requires joining assets, services, and vulnerabilities and sorting by asset criticality in one pass — the live API answers one entity at a time and cannot order across them."},
			{"topic": "CVE-to-fleet blast radius", "insight": "Pivots from a single CVE to the affected fleet via a local join plus full-text search over CVE ids — the console makes you query the vuln, then re-query each asset."},
			{"topic": "Software/version fleet rollup", "insight": "GROUP BY product+version with per-version asset counts — the API returns rows, never the rollup, so today it is a hand-built spreadsheet."},
			{"topic": "Stale / EOL / unowned asset hygiene", "insight": "Local-store query over last_seen, os_eol, and null tag/owner; the console shows the rows but will not roll them up or feed a pipeline."},
			{"topic": "Service exposure by subnet", "insight": "Joins services to assets filtered by address range and groups by protocol/port — a cross-entity aggregation the one-query API cannot return."},
			{"topic": "Scan-watch (kick + follow to completion)", "insight": "Composes the scan trigger and task polling endpoints into one poll-to-completion loop with streamed status — every other tool makes you hand-write the polling loop."},
			{"topic": "Exposure delta since last sync", "insight": "Joins the two most-recent local snapshots to isolate security-relevant change — the console keeps no queryable history, so this question is a manual two-export diff today."},
			{"topic": "Certificate-expiry / weak-crypto watch", "insight": "Filters the synced certificates table and joins to the presenting service and asset with criticality ranking — the live API returns cert rows but never the cert-to-asset exposure context."},
