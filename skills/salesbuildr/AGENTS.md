# Salesbuildr Printed CLI Agent Guide

This directory is a generated `salesbuildr-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
salesbuildr-cli doctor --json
salesbuildr-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
salesbuildr-cli which "<capability>" --json
salesbuildr-cli <command> --help
```
...
```bash
salesbuildr-cli <command> --help
salesbuildr-cli <command> --dry-run --agent
```
