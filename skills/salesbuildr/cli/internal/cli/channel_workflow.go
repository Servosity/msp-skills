...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  salesbuildr-cli workflow archive

  # Full re-archive (ignore previous sync state)
  salesbuildr-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"category", "category-tree", "company", "contact", "opportunity", "pricing-book", "product", "quote", "quote-approved", "quote-discount-group", "quote-template", "quote-widget-template"}
			totalSynced := 0
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  salesbuildr-cli workflow status

  # Show status as JSON
  salesbuildr-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("salesbuildr-cli")
			}
