...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".local", "share", "salesbuildr-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("SALESBUILDR_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("SALESBUILDR_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "salesbuildr-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.local/share/salesbuildr-cli/feedback.jsonl.
When ` + "`SALESBUILDR_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`SALESBUILDR_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  salesbuildr-cli feedback list
  salesbuildr-cli feedback list --limit 5
  salesbuildr-cli feedback list --json`,
