...
In local mode: searches locally synced data only.`,
  ...
  salesbuildr-cli search "error timeout"

  # Force local search only
  salesbuildr-cli search "payment failed" --data-source local

  # Search a specific resource type locally
  salesbuildr-cli search "critical" --type transactions --data-source local

  # JSON output for piping
  salesbuildr-cli search "critical" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'salesbuildr-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
			...
			case "opportunity":
				results, err = db.SearchOpportunity(query, limit)
			case "pricing-book":
				results, err = db.SearchPricingBook(query, limit)
			case "product":
				results, err = db.SearchProduct(query, limit)
			case "quote":
				results, err = db.SearchQuote(query, limit)
			case "":
				...
				{
					partial, searchErr := db.SearchOpportunity(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search opportunity failed: %w", searchErr)
					}
					for _, r := range partial {
						key := string(r)
						if !seen[key] {
				...
				{
					partial, searchErr := db.SearchPricingBook(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search pricing_book failed: %w", searchErr)
					}
					for _, r := range partial {
						key := string(r)
						if !seen[key] {
				...
				{
					partial, searchErr := db.SearchProduct(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search product failed: %w", searchErr)
					}
					for _, r := range partial {
						key := string(r)
						if !seen[key] {
				...
				{
					partial, searchErr := db.SearchQuote(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search quote failed: %w", searchErr)
					}
					for _, r := range partial {
						key := string(r)
						if !seen[key] {
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/salesbuildr-cli/data.db)")
