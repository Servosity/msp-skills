...
Resource scoping:
  ...
  salesbuildr-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  salesbuildr-cli sync --full

  # Incremental sync: only records from the last 7 days
  salesbuildr-cli sync --since 7d

  # Parallel sync with 8 workers
  salesbuildr-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  salesbuildr-cli sync --latest-only`,
...
func upsertSingleObject(db *store.Store, resource string, data json.RawMessage) error {
	...
	case "undelete":
		return db.UpsertUndelete(data)
	case "field":
		return db.UpsertField(data)
	case "opportunity":
		return db.UpsertOpportunity(data)
	case "pricing-book":
		return db.UpsertPricingBook(data)
	case "product":
	...
	case "quote-discount-group":
		return db.UpsertQuoteDiscountGroup(data)
	case "companies":
		return db.UpsertCompanies(data)
	case "quote-template":
		return db.UpsertQuoteTemplate(data)
	case "quote-widget-template":
		return db.UpsertQuoteWidgetTemplate(data)
	default:
...
func syncResourcePath(resource string) (string, error) {
	paths := map[string]string{
		...
		"pricing-book":          "/pricing-book",
		"product":               "/product",
		"quote":                 "/quote",
		"quote-approved":        "/quote/approved",
		"quote-discount-group":  "/quote-discount-group",
		"quote-template":        "/quote-template",
		"quote-widget-template": "/quote-widget-template",
	}
