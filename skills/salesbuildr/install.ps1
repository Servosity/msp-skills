# install.ps1 - install salesbuildr-cli and salesbuildr-mcp on Windows.
#
# Pulls prebuilt binaries from this skill's latest GitHub Release (salesbuildr-v*).
...
$Skill   = "salesbuildr"
$CliBin  = "salesbuildr-cli.exe"
$McpBin  = "salesbuildr-mcp.exe"
...
$ReleaseBase = if ($env:MSP_SKILLS_RELEASE_BASE) {
  ...
  # Each skill is versioned/tagged independently (salesbuildr-vX.Y.Z),
...
Write-Host "  salesbuildr-cli --version"
Write-Host ""
Write-Host "Next:"
Write-Host "  First command + auth: https://github.com/$Owner/$Repo/tree/main/skills/salesbuildr#readme"
Write-Host "  Claude Desktop / ChatGPT wire-up: https://github.com/$Owner/$Repo/blob/main/skills/salesbuildr/mcp-install.md"
