# Salesbuildr MCP - install for every agent that speaks MCP

This page wires the Salesbuildr MCP server into any MCP client. If you use Claude
...
launch `salesbuildr-mcp` directly on your machine - no hosting. *Remote* agents (ChatGPT,
Microsoft 365 Copilot / Copilot Studio, the Gemini app) only talk to an HTTPS
endpoint, so you expose `salesbuildr-mcp` over HTTPS first.

## Prerequisite: install the MCP binary

Run the install command from [README.md](./README.md). It drops both `salesbuildr-cli`
and `salesbuildr-mcp` on your PATH. `salesbuildr-mcp` is what the agents talk to.

```bash
salesbuildr-mcp --help
```
...
Restart Gemini CLI; the Salesbuildr tools appear in its tool list. (The **Gemini app /
...
All remote agents need `salesbuildr-mcp` reachable as a public **HTTPS** endpoint. Run it
...
`salesbuildr-mcp` you installed is not enough on its own. You also need a **Copilot Studio
license** and a **tenant admin** to enable it. This is a build-and-host task, not a
self-serve install.

Once `salesbuildr-mcp` is hosted over HTTPS (above), the lowest-code route:
...
hermes mcp add salesbuildr -- salesbuildr-mcp

# OpenClaw
openclaw mcp set salesbuildr '{"command":"salesbuildr-mcp"}'
```
...
- `salesbuildr-mcp: command not found`: the install dir is not on your PATH (the
