...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  sentinelone-cli workflow archive

  # Full re-archive (ignore previous sync state)
  sentinelone-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"accounts", "activities", "agents", "agents-passphrases", "application-inventory", "application-inventory-counts", "cloud-detection", "cloud-detection-alerts", "config-override", "device-control", "device-control-events", "exclusions", "filters", "filters-dv", "firewall-control", "firewall-control-protocols", "groups", "installed-applications", "installed-applications-cves", "last-activity-as-syslog", "locations", "ranger", "ranger-cred-groups", "ranger-cred-groups-details", "ranger-table-view", "rbac", "remote-scripts", "report-tasks", "reports", "restrictions", "rogues", "sentinelone-export", "singularity-marketplace", "sites", "threats", "update", "users"}
			totalSynced := 0
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  sentinelone-cli workflow status

  # Show status as JSON
  sentinelone-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("sentinelone-cli")
			}
