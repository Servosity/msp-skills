  ...
  sentinelone-cli doctor --fail-on warn`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if os.Getenv("SENTINELONE_API_TOKEN") != "" {
				authEnvSet = append(authEnvSet, "SENTINELONE_API_TOKEN")
				...
				authEnvRequiredMissing = append(authEnvRequiredMissing, "SENTINELONE_API_TOKEN")
			}
			switch {
...
func collectCacheReport(ctx context.Context, staleAfterSpec string) map[string]any {
	...
	if err != nil {
		if os.IsNotExist(err) {
			...
			report["hint"] = "Database not created yet; run 'sentinelone-cli sync' to hydrate."
			return report
		}
		report["status"] = "error"
	...
	if qerr != nil {
		...
		report["status"] = "unknown"
		report["hint"] = "No sync state recorded; run 'sentinelone-cli sync' to populate."
		return report
	}
	defer rows.Close()
	...
	default:
		...
		report["hint"] = "Some resources are older than stale_after; run 'sentinelone-cli sync' to refresh."
	}
	return report
}
