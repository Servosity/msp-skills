...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".local", "share", "sentinelone-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("SENTINELONE_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("SENTINELONE_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "sentinelone-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.local/share/sentinelone-cli/feedback.jsonl.
When ` + "`SENTINELONE_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`SENTINELONE_FEEDBACK_AUTO_SEND=true`" + `, the entry is
