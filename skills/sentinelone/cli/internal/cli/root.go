...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "sentinelone-cli",
		Short: `Sentinelone CLI — Every SentinelOne v2.1 management endpoint, plus an offline SQLite store and cross-entity analytics — fleet health, thr…`,
		Long: `Sentinelone CLI — Every SentinelOne v2.1 management endpoint, plus an offline SQLite store and cross-entity analytics — fleet health, thr…

Highlights (not in the official API docs):
  • whatchanged   One answer to 'what changed across all my tenants since I logged off?' — new threats, newly-offline or newly-unhealthy agents, version regressions, and protection-mode flips. Needs at least 2 syncs o…
  • threats recurrence   Surfaces threats whose same hash or name re-appears across endpoints — or returns on an endpoint after a prior mitigation — the signal of an unkilled root cause. Needs at least 2 syncs of local histo…
  ...
  • threats verdicts   Flags threats whose analyst verdict, confidence level, or incident status changed since the last sync — suspicious to malicious, or an auto-mitigated threat re-opened — so nothing flips silently. Nee…
...
Health check: run 'sentinelone-cli doctor' to verify auth and connectivity.
See README.md or the bundled SKILL.md for recipes.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("sentinelone-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.profileName, "profile", "", "Apply values from a saved profile (see 'sentinelone-cli profile list')")
	...
	rootCmd.AddCommand(newCloudDetectionCmd(flags))
	rootCmd.AddCommand(newConfigOverrideCmd(flags))
	rootCmd.AddCommand(newDeviceControlCmd(flags))
	rootCmd.AddCommand(newDvCmd(flags))
	rootCmd.AddCommand(newExclusionsCmd(flags))
	rootCmd.AddCommand(newFiltersCmd(flags))
	rootCmd.AddCommand(newFirewallControlCmd(flags))
	rootCmd.AddCommand(newGroupsCmd(flags))
	rootCmd.AddCommand(newHashesCmd(flags))
	rootCmd.AddCommand(newInstalledApplicationsCmd(flags))
	rootCmd.AddCommand(newLocationsCmd(flags))
	rootCmd.AddCommand(newRangerCmd(flags))
	rootCmd.AddCommand(newRbacCmd(flags))
	rootCmd.AddCommand(newRemoteScriptsCmd(flags))
	rootCmd.AddCommand(newReportTasksCmd(flags))
	rootCmd.AddCommand(newReportsCmd(flags))
	rootCmd.AddCommand(newRestrictionsCmd(flags))
	rootCmd.AddCommand(newRoguesCmd(flags))
	rootCmd.AddCommand(newSentineloneExportCmd(flags))
	rootCmd.AddCommand(newSettingsCmd(flags))
	rootCmd.AddCommand(newSingularityMarketplaceCmd(flags))
	rootCmd.AddCommand(newSitesCmd(flags))
	rootCmd.AddCommand(newSystemCmd(flags))
	rootCmd.AddCommand(newTagsCmd(flags))
	rootCmd.AddCommand(newTasksConfigurationCmd(flags))
	rootCmd.AddCommand(newTenantCmd(flags))
	rootCmd.AddCommand(newThreatIntelligenceCmd(flags))
	rootCmd.AddCommand(newThreatsCmd(flags))
	...
	rootCmd.AddCommand(newNovelVersionsCmd(flags))
	rootCmd.AddCommand(newNovelWhatchangedCmd(flags))
	rootCmd.AddCommand(newAPICmd(flags))
	rootCmd.AddCommand(newApplicationInventoryPromotedCmd(flags))
	rootCmd.AddCommand(newApplicationInventoryCountsPromotedCmd(flags))
	rootCmd.AddCommand(newLastActivityAsSyslogPromotedCmd(flags))
	rootCmd.AddCommand(newSentinelonerssPromotedCmd(flags))
	rootCmd.AddCommand(newSiteWithAdminPromotedCmd(flags))
	rootCmd.AddCommand(newTestsPromotedCmd(flags))
