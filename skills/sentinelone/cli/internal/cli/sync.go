...
Resource scoping:
  ...
  sentinelone-cli sync

  # Sync specific resources only
  sentinelone-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  sentinelone-cli sync --full

  # Incremental sync: only records from the last 7 days
  sentinelone-cli sync --since 7d

  # Parallel sync with 8 workers
  sentinelone-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  sentinelone-cli sync --latest-only`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			// Append a point-in-time history snapshot of agents/threats so the
			// time-travel novel features (whatchanged, versions rollout, threats
			// mttr / verdicts --changed) have a baseline to diff against.
			// Best-effort: a snapshot failure never fails the sync.
			if snapCounts, snapErr := db.CaptureFleetSnapshot(started); snapErr != nil {
				if humanFriendly {
					fmt.Fprintf(os.Stderr, "  history snapshot skipped: %v\n", snapErr)
				}
			} else if humanFriendly && (snapCounts["agents"] > 0 || snapCounts["threats"] > 0) {
				fmt.Fprintf(os.Stderr, "  history snapshot: %d agents, %d threats captured\n", snapCounts["agents"], snapCounts["threats"])
			}
...
func resourceSupportsPagination(resource string) bool {
	...
	case "agents":
		return true
	case "agents-passphrases":
		return true
	case "application-inventory":
		return true
	case "application-inventory-counts":
		return true
	case "cloud-detection":
		return true
	case "cloud-detection-alerts":
		return true
	case "config-override":
		return true
	case "device-control":
		return true
	case "device-control-events":
		return true
	case "exclusions":
		return true
	case "explore":
	...
	case "firewall-control":
		return true
	case "firewall-control-protocols":
		return true
	case "groups":
		return true
	case "installed-applications":
		return true
	case "installed-applications-cves":
		return true
	case "last-activity-as-syslog":
		return true
	case "locations":
		return true
	case "notes":
	...
	case "ranger":
		return true
	case "ranger-cred-groups":
		return true
	case "ranger-cred-groups-details":
		return true
	case "ranger-table-view":
		return true
	case "rbac":
		return true
	case "remote-scripts":
...
func upsertSingleObject(db *store.Store, resource string, data json.RawMessage) error {
	...
	case "accounts_expire_now":
		return db.UpsertAccountsExpireNow(data)
	case "accounts_policy":
		return db.UpsertAccountsPolicy(data)
	case "accounts_reactivate":
		return db.UpsertAccountsReactivate(data)
	case "accounts_revert_policy":
		return db.UpsertAccountsRevertPolicy(data)
	case "uninstall_password":
		return db.UpsertUninstallPassword(data)
	case "actions":
		return db.UpsertActions(data)
	case "uploads":
		return db.UpsertUploads(data)
	case "forensics":
		return db.UpsertForensics(data)
	case "add_tags":
		return db.UpsertAddTags(data)
	case "configuration":
		return db.UpsertConfiguration(data)
	case "copy_rules":
		return db.UpsertCopyRules(data)
	case "enable":
	...
	case "move_rules":
		return db.UpsertMoveRules(data)
	case "protocols":
		return db.UpsertProtocols(data)
	case "remove_tags":
		return db.UpsertRemoveTags(data)
	case "reorder":
		return db.UpsertReorder(data)
	case "set_location":
		return db.UpsertSetLocation(data)
	case "move_agents":
		return db.UpsertMoveAgents(data)
	case "groups_policy":
		return db.UpsertGroupsPolicy(data)
	case "groups_regenerate_key":
		return db.UpsertGroupsRegenerateKey(data)
	case "groups_revert_policy":
		return db.UpsertGroupsRevertPolicy(data)
	case "classification":
		return db.UpsertClassification(data)
	case "reputation":
		return db.UpsertReputation(data)
	case "json":
		return db.UpsertJson(data)
	case "sites_expire_now":
		return db.UpsertSitesExpireNow(data)
	case "sites_policy":
		return db.UpsertSitesPolicy(data)
	case "sites_reactivate":
		return db.UpsertSitesReactivate(data)
	case "sites_regenerate_key":
		return db.UpsertSitesRegenerateKey(data)
	case "sites_revert_policy":
		return db.UpsertSitesRevertPolicy(data)
	case "download_from_cloud":
		return db.UpsertDownloadFromCloud(data)
	case "explore":
	...
	case "whitening_options":
		return db.UpsertWhiteningOptions(data)
	case "api_token_details":
		return db.UpsertApiTokenDetails(data)
	default:
...
func defaultSyncResources() []string {
	return []string{
		...
		"installed-applications-cves",
		"last-activity-as-syslog",
		"locations",
		"ranger",
		"ranger-cred-groups",
		"ranger-cred-groups-details",
		"ranger-table-view",
		"rbac",
		"remote-scripts",
		"report-tasks",
...
func syncResourcePath(resource string) (string, error) {
	paths := map[string]string{
		...
		"agents":                       "/agents",
		"agents-passphrases":           "/agents/passphrases",
		"application-inventory":        "/application-inventory",
		"application-inventory-counts": "/application-inventory-counts",
		"cloud-detection":              "/cloud-detection/rules",
		...
		"filters":                      "/filters",
		"filters-dv":                   "/filters/dv",
		"firewall-control":             "/firewall-control",
		"firewall-control-protocols":   "/firewall-control/protocols",
		"groups":                       "/groups",
		"installed-applications":       "/installed-applications",
		"installed-applications-cves":  "/installed-applications/cves",
		"last-activity-as-syslog":      "/last-activity-as-syslog",
		"locations":                    "/locations",
		"ranger":                       "/ranger/gateways",
		"ranger-cred-groups":           "/ranger/cred-groups",
		"ranger-cred-groups-details":   "/ranger/cred-groups/details",
		"ranger-table-view":            "/ranger/table-view",
		"rbac":                         "/rbac/roles",
		"remote-scripts":               "/remote-scripts",
		"report-tasks":                 "/report-tasks",
		"reports":                      "/reports",
		"restrictions":                 "/restrictions",
		"rogues":                       "/rogues/table-view",
		"sentinelone-export":           "/export/installed-applications",
		"singularity-marketplace":      "/singularity-marketplace/applications",
		"sites":                        "/sites",
		"threats":                      "/threats",
		"update":                       "/update/agent/packages",
		"users":                        "/users",
	}
...
func dependentResourceDefs() []dependentResourceDef {
	return []dependentResourceDef{
		{Name: "explore", ParentTable: "threats", ParentIDParam: "threat_id", PathTemplate: "/threats/{threat_id}/explore/events", KeyField: "", PathParams: []dependentPathParamDef{
			{Param: "threat_id", Field: "id"},
		}},
		{Name: "notes", ParentTable: "threats", ParentIDParam: "threat_id", PathTemplate: "/threats/{threat_id}/notes", KeyField: "", PathParams: []dependentPathParamDef{
			{Param: "threat_id", Field: "id"},
		}},
		{Name: "protocols", ParentTable: "firewall-control", ParentIDParam: "firewall_rule_category", PathTemplate: "/firewall-control/{firewall_rule_category}/protocols", KeyField: "", PathParams: []dependentPathParamDef{
			{Param: "firewall_rule_category", Field: "firewall_rule_category"},
		}},
		{Name: "timeline", ParentTable: "threats", ParentIDParam: "threat_id", PathTemplate: "/threats/{threat_id}/timeline", KeyField: "", PathParams: []dependentPathParamDef{
			{Param: "threat_id", Field: "id"},
		}},
	}
}
