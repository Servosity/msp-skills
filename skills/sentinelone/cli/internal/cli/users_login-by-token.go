...
func newUsersLoginByTokenCmd(flags *rootFlags) *cobra.Command {
	var flagRemovedSavedScope string
	var flagToken string

	cmd := &cobra.Command{
		Use:         "login-by-token",
		Short:       "Log in with user token.",
		Example:     "  sentinelone-cli users login-by-token --token your-token-here",
		Annotations: map[string]string{"pp:endpoint": "users.login-by-token", "pp:method": "GET", "pp:path": "/users/login/by-token", "mcp:read-only": "true"},
