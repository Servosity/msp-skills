...
type Config struct {
	...
	SentineloneApiToken string            `toml:"api_token"`
}

func Load(configPath string) (*Config, error) {
	cfg := &Config{
		BaseURL: "https://your-console.sentinelone.net/web/api/v2.1",
	}

	// Resolve config path
	path := configPath
	if path == "" {
		path = os.Getenv("SENTINELONE_CONFIG")
	}
	if path == "" {
		home, _ := os.UserHomeDir()
		path = filepath.Join(home, ".config", "sentinelone-cli", "config.toml")
	}
	cfg.Path = path
	...
	// Env var overrides
	if v := os.Getenv("SENTINELONE_API_TOKEN"); v != "" {
		cfg.SentineloneApiToken = v
		cfg.markEnvOverride("SentineloneApiToken")
		cfg.AuthSource = "env:SENTINELONE_API_TOKEN"
	}
	...
	if cfg.AuthSource == "" && cfg.SentineloneApiToken != "" {
		cfg.AuthSource = "config"
	}
	...
	if v := os.Getenv("SENTINELONE_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	return cfg, nil
}
...
func (c *Config) AuthHeader() string {
	...
	token := strings.TrimSpace(c.SentineloneApiToken)
	if token == "" {
		return ""
	}
	// SentinelOne expects the token sent as "Authorization: ApiToken <token>".
	// Spec declares x-auth-value-prefix: "ApiToken "; the generator does not
	// consume it yet — hand-carried from the prior print. Guard double-prefix.
	if strings.HasPrefix(strings.ToLower(token), "apitoken ") {
		return token
	}
...
func (c *Config) SaveCredential(token string) error {
	...
	c.updateFileConfigField("AccessToken")
	c.SentineloneApiToken = token
	delete(c.envOverrides, "SentineloneApiToken")
	c.updateFileConfigField("SentineloneApiToken")
	return c.save()
}
...
func (c *Config) configForSave() Config {
	out := *c
	if c.fileConfig != nil {
		if c.envOverrides["SentineloneApiToken"] {
			out.SentineloneApiToken = c.fileConfig.SentineloneApiToken
		}
	}
...
func (c *Config) updateFileConfigField(field string) {
	...
	case "SentineloneApiToken":
		c.fileConfig.SentineloneApiToken = c.SentineloneApiToken
	}
}
