...
| Credential / security | `credentials rotate/delete`, `current-user api-token-list --reveal-token`, `current-user *-mfa-*`, `resellers agent-install-token`, `*-backups encryption-key update`, `auth set-token`/`logout` | Human-in-the-loop only; secret-returning reads are excluded from MCP and sync |
