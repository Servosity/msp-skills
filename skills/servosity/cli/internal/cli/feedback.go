...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".local", "share", "servosity-cli")
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "servosity-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.local/share/servosity-cli/feedback.jsonl.
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  servosity-cli feedback list
  servosity-cli feedback list --limit 5
  servosity-cli feedback list --json`,
