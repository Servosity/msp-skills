...
In local mode: searches locally synced data only.`,
  ...
  servosity-cli search "error timeout"

  # Force local search only
  servosity-cli search "payment failed" --data-source local

  # Search a specific resource type locally
  servosity-cli search "critical" --type transactions --data-source local

  # JSON output for piping
  servosity-cli search "critical" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if flags.dataSource != "local" {
				...
				data, getErr := c.Get(cmd.Context(), "/backup-search/", map[string]string{
					"search": query,
				})
				if getErr == nil {
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'servosity-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
