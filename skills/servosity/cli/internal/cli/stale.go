...
default 30s timeout, e.g. 'servosity-cli --timeout 180s stale-backups
--refresh'.`,
		Example: `  # Refresh the snapshot from the live API, then show >7-day stale sets
  servosity-cli stale-backups --refresh
  servosity-cli stale-backups --days 7 --json

  # Filter to one reseller's restic backups
  servosity-cli stale-backups --reseller 12 --engine restic --json`,
