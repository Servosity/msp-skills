...
// servosity-cli CLI: `teach` (fire-and-forget, silent on success,
...
import (
	...
	"servosity-msp-pp-cli/internal/learn/entities"
	"servosity-msp-pp-cli/internal/learn/lookups"
	"servosity-msp-pp-cli/internal/learn/patterns"
...
const noLearnEnvVar = "SERVOSITY_MSP_NO_LEARN"
...
func learnDBPath(explicit string) string {
	...
	return defaultDBPath("servosity-cli")
}
...
Disabling: pass --no-learn or set ` + noLearnEnvVar + `=true.`,
		Example: `  servosity-cli teach --query "<question>" --resource-type <type> --resource <id> --resource <id> &`,
...
when learnings exist.`,
		Example: `  servosity-cli recall "<question>" --agent
  servosity-cli recall "<question>" --agent --min-confidence 2`,
...
func newLearningsListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  servosity-cli learnings list --agent
  servosity-cli learnings list --query "<substring>"
  servosity-cli learnings list --warnings --agent`,
		Annotations: map[string]string{"mcp:read-only": "true"},
		RunE: func(cmd *cobra.Command, args []string) error {
			if dryRunOK(flags) {
...
Requires at least one of --resource, --action, or --all.`,
		Example: `  servosity-cli learnings forget "<question>" --resource <id>
  servosity-cli learnings forget "<question>" --all`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if len(args) == 0 {
				return cmd.Help()
			}
...
a whole family.`,
		Example: `  servosity-cli teach-pattern --query-template "items in {entity}" --resource-template "GROUP-{entity:category}" --resource-type "items" --entity-kind "category" --strategy substitute`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dryRunOK(flags) {
...
cannot be taught — they are derived from the canonical input.`,
		Example: `  servosity-cli teach-lookup --kind country --canonical "United States" --value USA`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dryRunOK(flags) {
