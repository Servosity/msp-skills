# Sherweb Printed CLI Agent Guide

This directory is a generated `sherweb-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
sherweb-cli doctor --json
sherweb-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
sherweb-cli which "<capability>" --json
...
```bash
sherweb-cli <command> --help
sherweb-cli <command> --dry-run --agent
```
