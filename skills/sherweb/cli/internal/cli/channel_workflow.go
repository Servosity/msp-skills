...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  sherweb-cli workflow archive

  # Full re-archive (ignore previous sync state)
  sherweb-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"distributor", "service-provider", "service-provider-v1-platforms"}
			totalSynced := 0
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  sherweb-cli workflow status

  # Show status as JSON
  sherweb-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("sherweb-cli")
			}
