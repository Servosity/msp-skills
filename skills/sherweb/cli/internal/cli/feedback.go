...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".local", "share", "sherweb-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("SHERWEB_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("SHERWEB_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "sherweb-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.local/share/sherweb-cli/feedback.jsonl.
When ` + "`SHERWEB_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`SHERWEB_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  sherweb-cli feedback list
  sherweb-cli feedback list --limit 5
  sherweb-cli feedback list --json`,
