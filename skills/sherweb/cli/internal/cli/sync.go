...
Resource scoping:
  ...
  sherweb-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  sherweb-cli sync --full

  # Incremental sync: only records from the last 7 days
  sherweb-cli sync --since 7d

  # Parallel sync with 8 workers
  sherweb-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  sherweb-cli sync --latest-only`,
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/sherweb-cli/data.db)")
...
func upsertSingleObject(db *store.Store, resource string, data json.RawMessage) error {
	...
	case "service-provider":
		return db.UpsertServiceProvider(data)
	default:
...
func syncResourcePath(resource string) (string, error) {
	paths := map[string]string{
		"distributor":                   "/distributor/v1/billing/payable-charges",
		"service-provider":              "/service-provider/v1/customers",
		"service-provider-v1-platforms": "/service-provider/v1/platforms",
	}
