...
func New(cfg *config.Config, timeout time.Duration, rateLimit float64) *Client {
	...
	cacheDir := filepath.Join(homeDir, ".cache", "sherweb-cli", "http")
...
func (c *Client) doInternal(ctx context.Context, method, path string, params map[string]string, body any, headerOverrides map[string]string, readOnlyIntent bool) (json.RawMessage, int, error) {
	...
	for attempt := 0; attempt <= maxRetries; attempt++ {
		...
		if c.Config != nil {
			if v := c.Config.SherwebSubscriptionKey; v != "" {
				if authHeaderLooksLikePlaceholderCredential(v) {
					return nil, 0, authPlaceholderCredentialErrorWithSetup(c.Config, "export SHERWEB_SUBSCRIPTION_KEY=<your-token>")
				}
				req.Header.Set("Ocp-Apim-Subscription-Key", v)
			}
		}
		if c.Config != nil {
		...
		if req.Header.Get("User-Agent") == "" {
			req.Header.Set("User-Agent", "sherweb-cli/1.0")
		}
...
func authPlaceholderCredentialError(cfg *config.Config) error {
	return authPlaceholderCredentialErrorWithSetup(cfg, "sherweb-cli auth login or sherweb-cli auth set-token <token>")
}
...
func resolveClientCredentials(cfg *config.Config) (string, string) {
	...
	if id == "" {
		id = os.Getenv("SHERWEB_CLIENT_ID")
	}
	if secret == "" {
		secret = os.Getenv("SHERWEB_CLIENT_SECRET")
	}
	return id, secret
}
...
func (c *Client) maskCredentialText(text string, extraCredentials ...string) string {
	...
	if c != nil && c.Config != nil {
		...
		addCredential(c.Config.ClientSecret)
		addCredential(c.Config.SherwebSubscriptionKey)
	}
