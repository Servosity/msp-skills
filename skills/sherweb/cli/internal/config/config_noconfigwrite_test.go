...
// touching disk. SHERWEB_NO_CONFIG_WRITE makes the automatic token cache a
...
const (
	ncwToken   = "minted-token-SHERWEB"
	ncwRefresh = "minted-refresh-SHERWEB"
	ncwSecret  = "minted-secret-SHERWEB"
)
...
func ncwMint(t *testing.T, value string, set bool) map[string]string {
	...
	home := t.TempDir()
	t.Setenv("HOME", home)
	t.Setenv("SHERWEB_CONFIG", filepath.Join(home, "config.toml"))
	if set {
...
func TestNoConfigWriteStillAllowsTheWipe(t *testing.T) {
	...
	t.Setenv("HOME", home)
	t.Setenv("SHERWEB_CONFIG", cfgPath)
...
func TestNoConfigWriteTruthiness(t *testing.T) {
	...
	if NoConfigWriteEnv != "SHERWEB_NO_CONFIG_WRITE" {
		t.Errorf("switch is named %q; the documented name is SHERWEB_NO_CONFIG_WRITE", NoConfigWriteEnv)
	}
}
