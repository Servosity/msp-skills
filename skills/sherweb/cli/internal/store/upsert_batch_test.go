...
// TestUpsertBatch_PopulatesServiceProviderTable verifies that UpsertBatch
// dispatches paginated items into both the generic resources table AND the
// typed service_provider table. Regression for issue #268: before the fix, paginated
// syncs only filled the generic resources table, so domain commands that
// query the typed table saw zero rows.
func TestUpsertBatch_PopulatesServiceProviderTable(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	s, err := Open(dbPath)
	if err != nil {
		t.Fatalf("open: %v", err)
	...
	if _, _, err := s.UpsertBatch("service-provider", items); err != nil {
		t.Fatalf("UpsertBatch: %v", err)
	}
	...
	var generic int
	if err := db.QueryRow(`SELECT COUNT(*) FROM resources WHERE resource_type = ?`, "service-provider").Scan(&generic); err != nil {
		t.Fatalf("count resources: %v", err)
	}
	if generic != len(items) {
		t.Fatalf("resources count = %d, want %d", generic, len(items))
	...
	var typed int
	typedQuery := fmt.Sprintf(`SELECT COUNT(*) FROM "%s"`, "service_provider")
	if err := db.QueryRow(typedQuery).Scan(&typed); err != nil {
		t.Fatalf("count service_provider: %v", err)
	}
	if typed != len(items) {
		t.Fatalf("service_provider count = %d, want %d (typed table not populated by UpsertBatch)", typed, len(items))
	}
}
