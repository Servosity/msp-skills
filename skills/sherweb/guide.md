...
For the short install path - including Claude Desktop, Hermes, and OpenClaw - see [README.md](./README.md) and [mcp-install.md](./mcp-install.md). This file is the command reference.
...
Config file: `~/.config/sherweb-cli/config.toml`
...
| `SHERWEB_SUBSCRIPTION_KEY` | per_call | Yes | APIM gateway subscription key from cumulus.sherweb.com Security > APIs; sent as `Ocp-Apim-Subscription-Key` on every call. |
