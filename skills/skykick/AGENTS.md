# SkyKick Printed CLI Agent Guide

This directory is a generated `skykick-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
skykick-cli doctor --json
skykick-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
skykick-cli which "<capability>" --json
...
```bash
skykick-cli <command> --help
skykick-cli <command> --dry-run --agent
```
