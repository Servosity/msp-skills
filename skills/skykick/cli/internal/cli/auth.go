...
func newAuthCmd(flags *rootFlags) *cobra.Command {
	cmd := &cobra.Command{
		Use:   "auth",
		Short: "Manage authentication for Skykick",
...
Mint an OAuth2 bearer token via POST https://apis.cloudservices.connectwise.com/auth/token with
grant_type=client_credentials and cache it on disk. Subsequent commands
auto-refresh the token before expiry.

Credentials default to SKYKICK_CLIENT_ID (Client ID) and SKYKICK_CLIENT_SECRET (Client Secret) environment variables.
`),
  ...
  skykick-cli auth login --client-id <id> --client-secret <secret>
`, "\n"),
		Annotations: map[string]string{"mcp:hidden": "true"},
		RunE: func(cmd *cobra.Command, args []string) error {
			// issue #270: SKYKICK_NO_CONFIG_WRITE makes the automatic token
			...
			if clientSecret == "" {
				clientSecret = strings.TrimSpace(os.Getenv("SKYKICK_CLIENT_SECRET"))
			}
			if clientID == "" || clientSecret == "" {
				return authErr(fmt.Errorf("client ID and secret required (set --client-id/--client-secret or SKYKICK_CLIENT_ID/SKYKICK_CLIENT_SECRET)"))
			}
			...
			if tokenURL == "" {
				tokenURL = "https://apis.cloudservices.connectwise.com/auth/token"
			}
	...
	cmd.Flags().StringVar(&clientID, "client-id", "", "OAuth2 Client ID; defaults to $SKYKICK_CLIENT_ID")
	cmd.Flags().StringVar(&clientSecret, "client-secret", "", "OAuth2 Client Secret; defaults to $SKYKICK_CLIENT_SECRET")
...
func resolveClientCredentialsScope() string {
	if scope := os.Getenv("SKYKICK_OAUTH_SCOPE"); scope != "" {
		return scope
	}
	return "Partner"
}
...
func newAuthStatusCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  skykick-cli auth status",
		RunE: func(cmd *cobra.Command, args []string) error {
			cfg, err := config.Load(flags.configPath)
			if err != nil {
				return configErr(err)
			...
			if !authed {
				...
				fmt.Fprintln(w, "  export SKYKICK_CLIENT_ID=\"<client-id>\"")
				fmt.Fprintln(w, "  export SKYKICK_CLIENT_SECRET=\"<client-secret>\"")
				fmt.Fprintf(w, "  skykick-cli auth login\n")
				return authErr(fmt.Errorf("no credentials configured"))
			}
...
func newAuthSetTokenCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  skykick-cli auth set-token <bearer-jwt>",
...
func newAuthLogoutCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  skykick-cli auth logout",
		RunE: func(cmd *cobra.Command, args []string) error {
			cfg, err := config.Load(flags.configPath)
			if err != nil {
				return configErr(err)
			...
			envStillSet := ""
			if envStillSet == "" && os.Getenv("SKYKICK_CLIENT_ID") != "" {
				envStillSet = "SKYKICK_CLIENT_ID"
			}
			if envStillSet == "" && os.Getenv("SKYKICK_CLIENT_SECRET") != "" {
				envStillSet = "SKYKICK_CLIENT_SECRET"
			}
