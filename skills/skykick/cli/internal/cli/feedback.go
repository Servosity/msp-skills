...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".local", "share", "skykick-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("SKYKICK_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("SKYKICK_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "skykick-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.local/share/skykick-cli/feedback.jsonl.
When ` + "`SKYKICK_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`SKYKICK_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  skykick-cli feedback list
  skykick-cli feedback list --limit 5
  skykick-cli feedback list --json`,
