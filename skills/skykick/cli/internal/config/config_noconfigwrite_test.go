...
// touching disk. SKYKICK_NO_CONFIG_WRITE makes the automatic token cache a
...
func TestNoConfigWriteTruthiness(t *testing.T) {
	...
	if NoConfigWriteEnv != "SKYKICK_NO_CONFIG_WRITE" {
		t.Errorf("switch is named %q; the documented name is SKYKICK_NO_CONFIG_WRITE", NoConfigWriteEnv)
	}
}
