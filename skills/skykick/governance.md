...
The skill drives the `skykick-cli` binary (and `skykick-mcp`),
authenticating with `SKYKICK_CLIENT_ID`, `SKYKICK_CLIENT_SECRET`, `SKYKICK_OAUTH_SCOPE`. Credentials are read from the environment only -
never written to disk, never logged, never sent anywhere except the SkyKick API.
...
| **Write (routine)** | Day-to-day mutations. | `backup storage-settings`, `backup subscription-settings` | Preview with `--dry-run`, then an approved write (where a command documents its own confirm gate, use it too) |
