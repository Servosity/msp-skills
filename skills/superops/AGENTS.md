# SuperOps Printed CLI Agent Guide

This directory is a generated `superops-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
superops-cli doctor --json
superops-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
superops-cli which "<capability>" --json
superops-cli <command> --help
```
...
```bash
superops-cli <command> --help
superops-cli <command> --dry-run --agent
```
