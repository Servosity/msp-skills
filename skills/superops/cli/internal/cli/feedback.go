...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".local", "share", "superops-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("SUPEROPS_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("SUPEROPS_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "superops-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.local/share/superops-pp-cli/feedback.jsonl.
When ` + "`SUPEROPS_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`SUPEROPS_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  superops-cli feedback list
  superops-cli feedback list --limit 5
  superops-cli feedback list --json`,
