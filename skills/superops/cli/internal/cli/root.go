...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "superops-cli",
		Short: `Superops CLI — Every SuperOps PSA+RMM entity in your terminal, plus a local SQLite mirror that answers cross-entity questions the web …`,
		Long: `Superops CLI — Every SuperOps PSA+RMM entity in your terminal, plus a local SQLite mirror that answers cross-entity questions the web …
...
Health check: run 'superops-cli doctor' to verify auth and connectivity.
See README.md or the bundled SKILL.md for recipes.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("superops-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.profileName, "profile", "", "Apply values from a saved profile (see 'superops-cli profile list')")
	...
	rootCmd.AddCommand(newNovelAlertCoverageCmd(flags))
	rootCmd.AddCommand(newNovelAtRiskAssetsCmd(flags))
	rootCmd.AddCommand(newNovelClient360Cmd(flags))
	rootCmd.AddCommand(newNovelContextTicketCmd(flags))
	...
	rootCmd.AddCommand(newItDocsPromotedCmd(flags))
	rootCmd.AddCommand(newKbPromotedCmd(flags))
	rootCmd.AddCommand(newServiceItemsPromotedCmd(flags))
	rootCmd.AddCommand(newSitesPromotedCmd(flags))
	rootCmd.AddCommand(newTasksPromotedCmd(flags))
	rootCmd.AddCommand(newTechniciansPromotedCmd(flags))
