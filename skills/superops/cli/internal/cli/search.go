...
In local mode: searches locally synced data only.`,
  ...
  superops-cli search "error timeout"

  # Force local search only
  superops-cli search "payment failed" --data-source local

  # Search a specific resource type locally
  superops-cli search "critical" --type transactions --data-source local

  # JSON output for piping
  superops-cli search "critical" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'superops-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
			...
			switch resourceType {
			case "alerts":
				results, err = db.SearchAlerts(query, limit)
			case "it-docs":
				results, err = db.SearchItDocs(query, limit)
			case "kb":
				results, err = db.SearchKb(query, limit)
			case "service-items":
				results, err = db.SearchServiceItems(query, limit)
			case "tasks":
			...
			case "":
				...
				{
					partial, searchErr := db.SearchAlerts(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search alerts failed: %w", searchErr)
					}
				...
				{
					partial, searchErr := db.SearchItDocs(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search it_docs failed: %w", searchErr)
					}
					for _, r := range partial {
						key := string(r)
						if !seen[key] {
				...
				{
					partial, searchErr := db.SearchKb(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search kb failed: %w", searchErr)
					}
					for _, r := range partial {
						key := string(r)
						if !seen[key] {
				...
				{
					partial, searchErr := db.SearchServiceItems(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search service_items failed: %w", searchErr)
					}
					for _, r := range partial {
						key := string(r)
						if !seen[key] {
