...
func newTicketsListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if flags.dataSource == "local" || "auto" == "local" {
				data, prov, err = resolveLocal(cmd.Context(), flags, cmd.ErrOrStderr(), "tickets", true, path, nil, "user_requested")
			} else {
				variables := map[string]any{}
				variables["first"] = flagFirst
				data, err = c.Query(cmd.Context(), client.TicketsListQuery, variables)
				if err == nil && !flags.dryRun {
					data, err = extractGraphQLConnection(data, "getTicketList")
				}
				if err != nil && flags.dataSource == "auto" && "auto" == "auto" && isNetworkError(err) {
					data, prov, err = resolveLocal(cmd.Context(), flags, cmd.ErrOrStderr(), "tickets", true, path, nil, networkFallbackReason)
				} else if err == nil {
	...
	cmd.Flags().IntVar(&flagFirst, "first", 50, "Max tickets per page")
