...
The skill drives the `superops-cli` binary (and `superops-mcp`),
authenticating with `SUPEROPS_API_TOKEN`. Credentials are read from the environment only -
never written to disk, never logged, never sent anywhere except the SuperOps API.
...
| **Write (routine)** | Day-to-day mutations. | `assets list`, `at risk assets`, `promoted assets` | Preview with `--dry-run`, then an approved write (where a command documents its own confirm gate, use it too) |
...
the SuperOps API, and you can read every line of how it does so. The skill is
