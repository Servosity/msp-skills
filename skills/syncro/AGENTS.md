# Syncro Printed CLI Agent Guide

This directory is a generated `syncro-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
syncro-cli doctor --json
syncro-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
syncro-cli which "<capability>" --json
...
```bash
syncro-cli <command> --help
syncro-cli <command> --dry-run --agent
```
