...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  syncro-cli workflow archive

  # Full re-archive (ignore previous sync state)
  syncro-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"appointment-types", "appointments", "canned-responses", "canned-responses-settings", "contacts", "contracts", "customer-assets", "customers", "customers-autocomplete", "customers-latest", "estimates", "invoices", "items", "leads", "line-items", "me", "new-ticket-forms", "payment-methods", "payments", "portal-users", "products", "products-barcode", "products-categories", "purchase-orders", "remote_search", "rmm-alerts", "schedules", "settings", "settings-printing", "settings-tabs", "ticket-blueprints", "ticket-comments", "ticket-timers", "tickets", "tickets-settings", "timelogs", "timelogs-last", "users", "vendors", "wiki-pages"}
			totalSynced := 0
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  syncro-cli workflow status

  # Show status as JSON
  syncro-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("syncro-cli")
			}
