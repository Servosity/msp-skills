...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".local", "share", "syncro-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("SYNCRO_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("SYNCRO_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "syncro-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.local/share/syncro-cli/feedback.jsonl.
When ` + "`SYNCRO_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`SYNCRO_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  syncro-cli feedback list
  syncro-cli feedback list --limit 5
  syncro-cli feedback list --json`,
