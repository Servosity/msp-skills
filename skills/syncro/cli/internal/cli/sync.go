...
Resource scoping:
  ...
  syncro-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  syncro-cli sync --full

  # Incremental sync: only records from the last 7 days
  syncro-cli sync --since 7d

  # Parallel sync with 8 workers
  syncro-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  syncro-cli sync --latest-only`,
...
func upsertSingleObject(db *store.Store, resource string, data json.RawMessage) error {
	...
	case "customer-assets":
		return db.UpsertCustomerAssets(data)
	case "installed_applications":
		return db.UpsertInstalledApplications(data)
	case "patches":
		return db.UpsertPatches(data)
	case "payment_profiles":
		return db.UpsertPaymentProfiles(data)
	case "phones":
		return db.UpsertPhones(data)
	case "convert_to_invoice":
		return db.UpsertConvertToInvoice(data)
	case "estimates_email":
		return db.UpsertEstimatesEmail(data)
	case "estimates_line_items":
		return db.UpsertEstimatesLineItems(data)
	case "estimates_print":
		return db.UpsertEstimatesPrint(data)
	case "invoices":
		return db.UpsertInvoices(data)
	case "invoices_email":
		return db.UpsertInvoicesEmail(data)
	case "invoices_line_items":
		return db.UpsertInvoicesLineItems(data)
	case "invoices_print":
		return db.UpsertInvoicesPrint(data)
	case "ticket":
		return db.UpsertTicket(data)
	case "process_form":
		return db.UpsertProcessForm(data)
	case "add_images":
		return db.UpsertAddImages(data)
	case "delete_image":
		return db.UpsertDeleteImage(data)
	case "location_quantities":
		return db.UpsertLocationQuantities(data)
	case "product_serials":
		return db.UpsertProductSerials(data)
	case "product_skus":
		return db.UpsertProductSkus(data)
	case "create_po_line_item":
		return db.UpsertCreatePoLineItem(data)
	case "receive":
		return db.UpsertReceive(data)
	case "mute":
		return db.UpsertMute(data)
	case "schedules_add_line_item":
		return db.UpsertSchedulesAddLineItem(data)
	case "schedules_line_items":
		return db.UpsertSchedulesLineItems(data)
	case "schedules_remove_line_item":
		return db.UpsertSchedulesRemoveLineItem(data)
	case "apply":
		return db.UpsertApply(data)
	case "tickets_add_line_item":
		return db.UpsertTicketsAddLineItem(data)
	case "attach_file_url":
		return db.UpsertAttachFileUrl(data)
	case "charge_timer_entry":
		return db.UpsertChargeTimerEntry(data)
	case "comment":
		return db.UpsertComment(data)
	case "comments":
		return db.UpsertComments(data)
	case "delete_attachment":
		return db.UpsertDeleteAttachment(data)
	case "delete_timer_entry":
		return db.UpsertDeleteTimerEntry(data)
	case "tickets_print":
		return db.UpsertTicketsPrint(data)
	case "tickets_remove_line_item":
		return db.UpsertTicketsRemoveLineItem(data)
	case "timer_entry":
		return db.UpsertTimerEntry(data)
	case "update_line_item":
		return db.UpsertUpdateLineItem(data)
	case "update_timer_entry":
		return db.UpsertUpdateTimerEntry(data)
	case "worksheet_results":
		return db.UpsertWorksheetResults(data)
	default:
...
func syncResourcePath(resource string) (string, error) {
	paths := map[string]string{
		...
		"canned-responses-settings": "/canned_responses/settings",
		"contacts":                  "/contacts",
		"contracts":                 "/contracts",
		"customer-assets":           "/customer_assets",
		"customers":                 "/customers",
		"customers-autocomplete":    "/customers/autocomplete",
		"customers-latest":          "/customers/latest",
		"estimates":                 "/estimates",
		"invoices":                  "/invoices",
		"items":                     "/items",
		"leads":                     "/leads",
		...
		"me":                        "/me",
		"new-ticket-forms":          "/new_ticket_forms",
		...
		"rmm-alerts":                "/rmm_alerts",
		"schedules":                 "/schedules",
		"settings":                  "/settings",
		"settings-printing":         "/settings/printing",
		"settings-tabs":             "/settings/tabs",
		"ticket-blueprints":         "/ticket_blueprints",
		"ticket-comments":           "/ticket_comments",
		"ticket-timers":             "/ticket_timers",
		"tickets":                   "/tickets",
		"tickets-settings":          "/tickets/settings",
		"timelogs":                  "/timelogs",
		"timelogs-last":             "/timelogs/last",
		"users":                     "/users",
		"vendors":                   "/vendors",
...
func dependentResourceDefs() []dependentResourceDef {
	return []dependentResourceDef{
		{Name: "comments", ParentTable: "tickets", ParentIDParam: "id", PathTemplate: "/tickets/{id}/comments", KeyField: "", PathParams: []dependentPathParamDef{
			{Param: "id", Field: "id"},
		}},
		{Name: "installed_applications", ParentTable: "customer-assets", ParentIDParam: "id", PathTemplate: "/customer_assets/{id}/installed_applications", KeyField: "", PathParams: []dependentPathParamDef{
			{Param: "id", Field: "id"},
		}},
		{Name: "product_serials", ParentTable: "products", ParentIDParam: "product_id", PathTemplate: "/products/{product_id}/product_serials", KeyField: "", PathParams: []dependentPathParamDef{
			{Param: "product_id", Field: "id"},
		}},
		{Name: "worksheet_results", ParentTable: "tickets", ParentIDParam: "ticket_id", PathTemplate: "/tickets/{ticket_id}/worksheet_results", KeyField: "", PathParams: []dependentPathParamDef{
			{Param: "ticket_id", Field: "id"},
		}},
	}
}
