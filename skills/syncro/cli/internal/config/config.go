...
func Load(configPath string) (*Config, error) {
	...
	path := configPath
	if path == "" {
		path = os.Getenv("SYNCRO_CONFIG")
	}
	if path == "" {
		home, _ := os.UserHomeDir()
		path = filepath.Join(home, ".config", "syncro-cli", "config.toml")
	}
	cfg.Path = path
	...
	if cfg.AuthSource == "" && cfg.SyncroApiKey != "" {
		cfg.AuthSource = "config"
	}
	...
	if v := os.Getenv("SYNCRO_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	...
	if v := strings.TrimSpace(os.Getenv("SYNCRO_SUBDOMAIN")); v != "" {
		cfg.TemplateVars["subdomain"] = normalizeEndpointTemplateValue(v)
	} else {
		cfg.TemplateVars["subdomain"] = "your-subdomain"
	}
	return cfg, nil
}
...
func (c *Config) AuthHeader() string {
	...
	token := c.SyncroApiKey
	if token == "" {
		return ""
	}
	// Syncro requires the API token as a Bearer credential
	// ("Authorization: Bearer <token>"). Guard against double-prefixing when
	// the operator already supplied the full header value.
