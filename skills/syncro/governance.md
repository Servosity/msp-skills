# syncro skill - governance and safety model

> Unofficial. Community-built skill for the Syncro API. Not affiliated with,
> endorsed by, or sponsored by the vendor.
> This page tells an MSP owner exactly what the syncro skill can touch and how to
> scope it, so you can decide what to let an AI agent do.

## What it authenticates as

The skill drives the `syncro-cli` binary (and `syncro-mcp`),
authenticating with `SYNCRO_API_KEY`, `SYNCRO_SUBDOMAIN`. Credentials are read from the environment only -
never written to disk, never logged, never sent anywhere except the Syncro API.
...
| --- | --- | --- | --- |
| **Read** | Reports, rollups, search. No change. | `billing uninvoiced`, `billing ar-aging`, `customers margin`, `tickets aging`, `assets patch-gaps`, `alerts noise`, `customers profile`, and every `list` / `get` / `search` / `export` command | Allow |
| **Write (routine)** | Day-to-day mutations. | `tickets create`, `tickets comment create`, `tickets update`, `invoices create`, `customers update`, `appointments create`, `canned-responses create`, ... (~100 total) | Preview with `--dry-run`, then an approved write (where a command documents its own confirm gate, use it too) |
| **Credential / security** | Touches tokens, keys, MFA. | (none detected) | Human-in-the-loop only |
| **Destructive** | Irreversible data or config loss. | `appointment-types delete`, `appointments delete`, `canned-responses delete`, `contacts delete`, `contracts delete`, `customers delete`, `customers payment-profiles delete`, `customers phones delete`, ... (~20 total) | Human-in-the-loop only, explicit confirmation |
...
the Syncro API, and you can read every line of how it does so. The skill is
