# Tactical RMM Printed CLI Agent Guide

This directory is a generated `tactical-rmm-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
tactical-rmm-cli doctor --json
tactical-rmm-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
tactical-rmm-cli which "<capability>" --json
tactical-rmm-cli <command> --help
```
...
```bash
tactical-rmm-cli <command> --agent
```
...
```bash
tactical-rmm-cli <command> --help
tactical-rmm-cli <command> --dry-run --agent
```
