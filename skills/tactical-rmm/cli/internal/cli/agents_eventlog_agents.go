...
func newAgentsEventlogAgentsCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if len(args) < 2 {
				return usageErr(fmt.Errorf("logtype is required\nUsage: %s <%s>", cmd.CommandPath(), "logtype"))
			}
			path = replacePathParam(path, "logtype", args[1])
			if len(args) < 3 {
				return usageErr(fmt.Errorf("days is required\nUsage: %s <%s>", cmd.CommandPath(), "days"))
			}
			path = replacePathParam(path, "days", args[2])
			params := map[string]string{}
			data, prov, err := resolveReadWithStrategy(cmd.Context(), c, flags, "auto", "eventlog", false, path, params, nil, cmd.ErrOrStderr())
			if err != nil {
