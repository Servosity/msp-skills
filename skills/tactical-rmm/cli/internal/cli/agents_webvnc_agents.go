...
func newAgentsWebvncAgentsCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if len(args) < 2 {
				return usageErr(fmt.Errorf("port is required\nUsage: %s <%s>", cmd.CommandPath(), "port"))
			}
			path = replacePathParam(path, "port", args[1])
			params := map[string]string{}
			data, prov, err := resolveReadWithStrategy(cmd.Context(), c, flags, "auto", "webvnc", false, path, params, nil, cmd.ErrOrStderr())
			if err != nil {
