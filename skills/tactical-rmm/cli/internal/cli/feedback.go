...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".local", "share", "tactical-rmm-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("TACTICAL_RMM_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("TACTICAL_RMM_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "tactical-rmm-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.local/share/tactical-rmm-cli/feedback.jsonl.
When ` + "`TACTICAL_RMM_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`TACTICAL_RMM_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  tactical-rmm-cli feedback list
  tactical-rmm-cli feedback list --limit 5
  tactical-rmm-cli feedback list --json`,
