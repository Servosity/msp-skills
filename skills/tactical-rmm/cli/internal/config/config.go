...
type Config struct {
	...
	TrmmApiKey    string            `toml:"api_key"`
}
...
func Load(configPath string) (*Config, error) {
	...
	path := configPath
	if path == "" {
		path = os.Getenv("TACTICAL_RMM_CONFIG")
	}
	if path == "" {
		home, _ := os.UserHomeDir()
		path = filepath.Join(home, ".config", "tactical-rmm-cli", "config.toml")
	}
	cfg.Path = path
	...
	// Env var overrides
	if v := os.Getenv("TRMM_API_KEY"); v != "" {
		cfg.TrmmApiKey = v
		cfg.markEnvOverride("TrmmApiKey")
		cfg.AuthSource = "env:TRMM_API_KEY"
	}
	...
	if cfg.AuthSource == "" && cfg.TrmmApiKey != "" {
		cfg.AuthSource = "config"
	}
	...
	if v := os.Getenv("TACTICAL_RMM_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	return cfg, nil
}
...
func (c *Config) SaveCredential(token string) error {
	...
	c.updateFileConfigField("AccessToken")
	c.TrmmApiKey = token
	delete(c.envOverrides, "TrmmApiKey")
	c.updateFileConfigField("TrmmApiKey")
	return c.save()
}
...
func (c *Config) configForSave() Config {
	out := *c
	if c.fileConfig != nil {
		if c.envOverrides["TrmmApiKey"] {
			out.TrmmApiKey = c.fileConfig.TrmmApiKey
		}
	}
...
func (c *Config) updateFileConfigField(field string) {
	...
	case "TrmmApiKey":
		c.fileConfig.TrmmApiKey = c.TrmmApiKey
	}
}
