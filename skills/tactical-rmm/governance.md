...
authenticating with `TRMM_API_KEY`. Credentials are read from the environment only -
never written to disk, never logged, never sent anywhere except the Tactical RMM API.
...
| **Write (routine)** | Day-to-day mutations. | `accounts accounts-reset2fa`, `accounts accounts-resetpw`, `agents agents-update`, `automation automation-patchpolicy`, `automation automation-patchpolicy-2`, `automation automation-patchpolicy-3`, `automation automation-patchpolicy-4`, `automation automation-patchpolicy-5`, ... (26 total) | Preview with `--dry-run`, then an approved write (where a command documents its own confirm gate, use it too) |
...
the Tactical RMM API, and you can read every line of how it does so. The skill is
