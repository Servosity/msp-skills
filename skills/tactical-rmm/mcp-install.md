# Tactical RMM MCP - install for every agent that speaks MCP

This page wires the Tactical RMM MCP server into any MCP client. If you use Claude
...
launch `tactical-rmm-mcp` directly on your machine - no hosting. *Remote* agents (ChatGPT,
Microsoft 365 Copilot / Copilot Studio, the Gemini app) only talk to an HTTPS
endpoint, so you expose `tactical-rmm-mcp` over HTTPS first.

## Prerequisite: install the MCP binary

Run the install command from [README.md](./README.md). It drops both `tactical-rmm-cli`
and `tactical-rmm-mcp` on your PATH. `tactical-rmm-mcp` is what the agents talk to.

```bash
tactical-rmm-mcp --help
```
...
```json
{
  "mcpServers": {
    "tactical-rmm": {
      ...
      "env": {
        "TACTICAL_RMM_BASE_URL": "<your-tactical_rmm_base_url>",
...
Restart Gemini CLI; the Tactical RMM tools appear in its tool list. (The **Gemini app /
...
All remote agents need `tactical-rmm-mcp` reachable as a public **HTTPS** endpoint. Run it
in HTTP mode with your credentials in the environment:

```bash
TACTICAL_RMM_BASE_URL=<value> TRMM_API_KEY=<value> tactical-rmm-mcp --transport http --addr :7777
```
...
`tactical-rmm-mcp` you installed is not enough on its own. You also need a **Copilot Studio
license** and a **tenant admin** to enable it. This is a build-and-host task, not a
self-serve install.

Once `tactical-rmm-mcp` is hosted over HTTPS (above), the lowest-code route:
...
hermes mcp add tactical-rmm -- tactical-rmm-mcp

# OpenClaw
openclaw mcp set tactical-rmm '{"command":"tactical-rmm-mcp"}'
```
...
- `tactical-rmm-mcp: command not found`: the install dir is not on your PATH (the
