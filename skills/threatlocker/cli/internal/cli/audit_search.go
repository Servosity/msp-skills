...
func newAuditSearchCmd(flags *rootFlags) *cobra.Command {
	...
	var bodyShowChildOrganizations bool
	var bodyOnlyTrueDenies bool
	var stdinBody bool

	cmd := &cobra.Command{
		Use:         "search",
		Short:       "Search the Unified Audit log. actionId 1=Permit,2=Deny,99=AnyDeny. Requires startDate/endDate.",
		Example:     "  threatlocker-cli audit search --start 2026-01-15",
		Annotations: map[string]string{"pp:endpoint": "audit.search", "pp:method": "POST", "pp:path": "/ActionLog/ActionLogGetByParametersV2", "mcp:read-only": "true"},
	...
	cmd.Flags().BoolVar(&bodyShowChildOrganizations, "child-orgs", false, "Span all managed orgs")
	cmd.Flags().BoolVar(&bodyOnlyTrueDenies, "only-denies", false, "Only true denies")
