...
func newComputersListCmd(flags *rootFlags) *cobra.Command {
	...
	var bodySearchBy int
	var bodyAction string
	var bodyKindOfAction string
	var bodyComputerGroup string
	var bodyOrderBy string
	var bodyIsAscending bool
	var bodyChildOrganizations bool
	var stdinBody bool

	cmd := &cobra.Command{
		Use:         "list",
		Short:       "List/search computers (paginated). searchBy 1-5; orderBy e.g. computername.",
		Example:     "  threatlocker-cli computers list",
		Annotations: map[string]string{"pp:endpoint": "computers.list", "pp:method": "POST", "pp:path": "/Computer/ComputerGetByAllParameters", "mcp:read-only": "true"},
	...
	cmd.Flags().IntVar(&bodyPageSize, "page-size", 100, "Results per page (1-1000)")
	cmd.Flags().StringVar(&bodySearchText, "search-text", "", "Free-text search")
	cmd.Flags().IntVar(&bodySearchBy, "search-by", 1, "Search field index (1-5)")
	cmd.Flags().StringVar(&bodyAction, "action", "", "Filter by last action")
	cmd.Flags().StringVar(&bodyKindOfAction, "kind-of-action", "", "Filter by action kind")
