...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  threatlocker-cli doctor
  threatlocker-cli doctor --json
  threatlocker-cli doctor --fail-on warn
  threatlocker-cli doctor --fail-on stale`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if cfg != nil {
				...
				if !configured {
					...
					report["auth_hint"] = "Set your API key with: export THREATLOCKER_API_KEY=\"your-token-here\""
				} else {
					authConfigured = true
			...
			// ThreatLocker-specific diagnosis: the recurring community failure
			// is a 401 from a malformed/expired raw token or a missing
			// ManagedOrganizationId header. Surface both proactively.
			// which.go and internal/mcp/tools.go both advertise doctor as
			// validating the 64-hex token format and the org header, so this
			// block is what makes that claim true.
			tlTokenOK := false
			if cfg != nil {
				if h := cfg.AuthHeader(); h != "" {
					ok, reason := isTLTokenWellFormed(h)
					tlTokenOK = ok
					if ok {
						report["token_format"] = "OK " + reason
					} else {
						report["token_format"] = "WARN " + reason
					}
				}
			}
			...
			tlOrg := flags.orgID
			if tlOrg == "" {
				tlOrg = os.Getenv("THREATLOCKER_ORG_ID")
			}
			if tlOrg == "" {
				report["managed_org"] = "WARN not set — most Portal API data calls need a ManagedOrganizationId; set THREATLOCKER_ORG_ID or pass --org <tenant-guid>"
			} else {
				report["managed_org"] = "OK " + tlOrg
			}
			...
			if os.Getenv("THREATLOCKER_API_KEY") != "" {
				authEnvSet = append(authEnvSet, "THREATLOCKER_API_KEY")
				...
				authEnvRequiredMissing = append(authEnvRequiredMissing, "THREATLOCKER_API_KEY")
			}
			switch {
			...
			if cfg != nil && cfg.BaseURL != "" {
				...
				if clientErr != nil {
					...
					healthPath := "/ApprovalRequest/ApprovalRequestGetCount"
					...
					case errors.As(reachErr, &reachAPIErr):
						...
						// Maps a 401/403 to its most likely ThreatLocker cause
						// (bad token format, Bearer prefix, missing org header,
						// idle-expired token). which.go/mcp advertise this.
						if status == 401 || status == 403 {
							report["auth_diagnosis"] = tlAuthDiagnosis(authConfigured, tlTokenOK, tlOrg != "")
						}
					default:
...
func collectCacheReport(ctx context.Context, staleAfterSpec string) map[string]any {
	...
	if err != nil {
		if os.IsNotExist(err) {
			...
			report["hint"] = "Database not created yet; run 'threatlocker-cli sync' to hydrate."
			return report
		}
		report["status"] = "error"
	...
	if qerr != nil {
		...
		report["status"] = "unknown"
		report["hint"] = "No sync state recorded; run 'threatlocker-cli sync' to populate."
		return report
	}
	defer rows.Close()
	...
	case !haveAny && len(resources) == 0:
		...
		report["hint"] = "No sync recorded; run 'threatlocker-cli sync' to hydrate API-backed resources. Rows written by other paths are not tracked in sync_state."
	case fresh:
		report["status"] = "fresh"
	default:
		report["status"] = "stale"
		report["oldest_age"] = oldest.Round(time.Minute).String()
		report["hint"] = "Some resources are older than stale_after; run 'threatlocker-cli sync' to refresh."
	}
	return report
}
