...
large datasets as it has no memory pressure.`,
  ...
  threatlocker-cli export <resource> --format jsonl --output data.jsonl

  # Export with limit
  threatlocker-cli export <resource> --format jsonl --limit 1000

  # Pipe to another tool
  threatlocker-cli export <resource> --format jsonl | jq '.id'`,
		Args: cobra.RangeArgs(1, 2),
		RunE: func(cmd *cobra.Command, args []string) (err error) {
			validResources := map[string]bool{
				"application-files": true,
				"audit":             true,
				"computer-groups":   true,
				"maintenance":       true,
				"online-devices":    true,
				"organizations":     true,
				"reports":           true,
				"scheduled-actions": true,
				"tags":              true,
				"versions":          true,
			}
