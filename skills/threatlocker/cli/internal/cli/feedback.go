...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".local", "share", "threatlocker-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("THREATLOCKER_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("THREATLOCKER_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "threatlocker-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.local/share/threatlocker-cli/feedback.jsonl.
When ` + "`THREATLOCKER_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`THREATLOCKER_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  threatlocker-cli feedback list
  threatlocker-cli feedback list --limit 5
  threatlocker-cli feedback list --json`,
