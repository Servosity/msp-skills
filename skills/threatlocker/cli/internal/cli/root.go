...
type rootFlags struct {
	...
	// invocation. Mirrors the THREATLOCKER_NO_LEARN env var.
	noLearn bool
	...
	// orgID is the ThreatLocker ManagedOrganizationId (tenant GUID). It is
	// injected as the ManagedOrganizationId / OverrideManagedOrganizationId
	// request headers by newClient(). Resolved from --org, else the
	// THREATLOCKER_ORG_ID env var. Most Portal API data calls require it under
	// the New API version.
	orgID string
...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "threatlocker-cli",
		Short: `Threatlocker CLI — Every ThreatLocker Portal API feature, plus the write operations the read-only tools lack and a cross-tenant offline…`,
		Long: `Threatlocker CLI — Every ThreatLocker Portal API feature, plus the write operations the read-only tools lack and a cross-tenant offline store no other ThreatLocker tool has.
...
Highlights (not in the official API docs):
  ...
  • doctor   Diagnoses the #1 ThreatLocker integration pain: validates the raw 64-hex token format, the no-Bearer Authorization header, the ManagedOrganizationId header, New-vs-Old API mode, pings an authenticate…

Agent mode: add --agent to any command for JSON output + non-interactive mode.
Health check: run 'threatlocker-cli doctor' to verify auth and connectivity.
See README.md or the bundled SKILL.md for recipes.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("threatlocker-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.selectFields, "select", "", "Comma-separated fields to include in output (e.g. --select applicationFileId,applicationId,fullPath)")
	...
	rootCmd.PersistentFlags().StringVar(&flags.runProfileName, "profile", "", "Apply values from a saved run profile; this does not select a client (see 'threatlocker-cli profile list')")
	...
	rootCmd.AddCommand(newApprovalsCmd(flags))
	rootCmd.AddCommand(newAuditCmd(flags))
	rootCmd.AddCommand(newComputerGroupsCmd(flags))
	rootCmd.AddCommand(newComputersCmd(flags))
	rootCmd.AddCommand(newNetworkPoliciesCmd(flags))
	...
	rootCmd.AddCommand(newStoragePoliciesCmd(flags))
	rootCmd.AddCommand(newSystemAuditCmd(flags))
	...
	rootCmd.AddCommand(newApplicationFilesPromotedCmd(flags))
	rootCmd.AddCommand(newMaintenancePromotedCmd(flags))
	rootCmd.AddCommand(newOnlineDevicesPromotedCmd(flags))
	...
	addNovelCommandIfAbsent(rootCmd, newNovelDevicesCmd(flags))
...
func (f *rootFlags) newClient() (*client.Client, error) {
	...
	// target tenant. Shared with the MCP server via tenant_scope.go so the two
	// surfaces cannot drift apart.
	ApplyTenantScopeHeaders(cfg, ResolveTenantOrg(f.orgID))
