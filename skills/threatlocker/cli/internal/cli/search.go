...
In local mode: searches locally synced data only.`,
  ...
  threatlocker-cli search "error timeout"

  # Force local search only
  threatlocker-cli search "payment failed" --data-source local

  # Search a specific resource type locally
  threatlocker-cli search "critical" --type transactions --data-source local

  # JSON output for piping
  threatlocker-cli search "critical" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			query := args[0]
			// This API has a search endpoint: POST /ActionLog/ActionLogGetByParametersV2
			if flags.dataSource != "local" {
				c, err := flags.newClient()
				if err != nil {
					return err
				}
				data, _, getErr := c.Post(cmd.Context(), "/ActionLog/ActionLogGetByParametersV2", map[string]any{
					"q":                      query,
					"startDate":              "",
					"endDate":                "",
					"pageNumber":             1,
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'threatlocker-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
			...
			case "computer-groups":
				results, err = db.SearchComputerGroups(query, limit)
			case "tags":
			...
			case "":
				...
				{
					partial, searchErr := db.SearchComputerGroups(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search computer_groups failed: %w", searchErr)
					}
					for _, r := range partial {
						key := string(r)
						if !seen[key] {
