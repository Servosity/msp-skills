...
// threatlocker-cli CLI: `teach` (fire-and-forget, silent on success,
...
import (
	...
	"threatlocker-pp-cli/internal/learn/entities"
	"threatlocker-pp-cli/internal/learn/lookups"
	"threatlocker-pp-cli/internal/learn/patterns"
...
const noLearnEnvVar = "THREATLOCKER_NO_LEARN"
...
func learnDBPath(explicit string) string {
	...
	return defaultDBPath("threatlocker-cli")
}
...
Disabling: pass --no-learn or set ` + noLearnEnvVar + `=true.`,
		Example: `  threatlocker-cli teach --query "<question>" --resource-type <type> --resource <id> --resource <id> &`,
...
func newLearningsListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  threatlocker-cli learnings list --agent
  threatlocker-cli learnings list --query "<substring>"
  threatlocker-cli learnings list --warnings --agent`,
...
Requires at least one of --resource, --action, or --all.`,
		Example: `  threatlocker-cli learnings forget "<question>" --resource <id>
  threatlocker-cli learnings forget "<question>" --all`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if len(args) == 0 {
				return cmd.Help()
			}
...
a whole family.`,
		Example: `  threatlocker-cli teach-pattern --query-template "items in {entity}" --resource-template "GROUP-{entity:category}" --resource-type "items" --entity-kind "category" --strategy substitute`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dryRunOK(flags) {
...
cannot be taught — they are derived from the canonical input.`,
		Example: `  threatlocker-cli teach-lookup --kind country --canonical "United States" --value USA`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dryRunOK(flags) {
