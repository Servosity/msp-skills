...
// tenantScopeCacheHeaders are request headers that select WHICH tenant's data
// the API returns. They must partition the response cache: they are not
// representation headers (they do not vary the encoding of one resource, they
// change the resource), so canonicalRepresentationHeaders deliberately ignores
// them. Hand-wired for ThreatLocker; see skills/threatlocker/handfixes.json
// (multi-tenant-org-header).
var tenantScopeCacheHeaders = []string{"ManagedOrganizationId", "OverrideManagedOrganizationId"}
...
func (c *Client) cacheKeyFor(method, path string, params map[string]string, headers map[string]string, body []byte) string {
	...
	// Tenant partitioning. This CLI scopes requests to a customer tenant with
	// the ManagedOrganizationId header, which is NOT part of
	// canonicalRepresentationHeaders' whitelist. One MSP token addresses many
	// tenants, so without this two `--org` invocations sharing a path, params
	// and token would collide on one cache entry and serve tenant A's rows to a
	// request that targeted tenant B. Partition on the tenant explicitly.
	if c.Config != nil && len(c.Config.Headers) > 0 {
		for _, name := range tenantScopeCacheHeaders {
			if v := strings.TrimSpace(c.Config.Headers[name]); v != "" {
				key += "|" + name + "=" + v
			}
		}
	}
...
func (c *Client) doInternal(ctx context.Context, method, path string, params map[string]string, body any, headerOverrides map[string]string, readOnlyIntent bool, mutationIntent bool) (json.RawMessage, int, error) {
	...
	for attempt := 0; attempt <= maxRetries; attempt++ {
		...
		if req.Header.Get("User-Agent") == "" {
			if ua := os.Getenv("THREATLOCKER_USER_AGENT"); ua != "" {
				req.Header.Set("User-Agent", ua)
			} else {
				req.Header.Set("User-Agent", "threatlocker-cli/0.1.0")
			}
		}
...
func authPlaceholderCredentialError(cfg *config.Config) error {
	return authPlaceholderCredentialErrorWithSetup(cfg, "export THREATLOCKER_API_KEY=<your-token> or threatlocker-cli auth set-token <token>")
}
...
func (c *Client) maskCredentialText(text string, extraCredentials ...string) string {
	...
	if c != nil && c.Config != nil {
		...
		addCredential(c.Config.ThreatlockerApiKey)
	}
