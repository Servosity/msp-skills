# threatlocker skill - governance and safety model

> Unofficial. Community-built skill for the ThreatLocker API. Not affiliated with,
> endorsed by, or sponsored by the vendor.
> This page tells an MSP owner exactly what the threatlocker skill can touch and how to
> scope it, so you can decide what to let an AI agent do.

## What it authenticates as

The skill drives the `threatlocker-cli` binary (and `threatlocker-mcp`),
authenticating with `THREATLOCKER_API_KEY`. Credentials are read from the environment only -
never written to disk, never logged, never sent anywhere except the ThreatLocker API.
...
the ThreatLocker API, and you can read every line of how it does so. The skill is
