...
// unifi-network-cli CLI: `teach` (fire-and-forget, silent on success,
...
import (
	...
	"unifi-network-pp-cli/internal/learn/entities"
	"unifi-network-pp-cli/internal/learn/lookups"
	"unifi-network-pp-cli/internal/learn/patterns"
...
func learnDBPath(explicit string) string {
	...
	return defaultDBPath("unifi-network-cli")
}
...
Disabling: pass --no-learn or set ` + noLearnEnvVar + `=true.`,
		Example: `  unifi-network-cli teach --query "<question>" --resource-type <type> --resource <id> --resource <id> &`,
...
when learnings exist.`,
		Example: `  unifi-network-cli recall "<question>" --agent
  unifi-network-cli recall "<question>" --agent --min-confidence 2`,
...
func newLearningsListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  unifi-network-cli learnings list --agent
  unifi-network-cli learnings list --query "<substring>"
  unifi-network-cli learnings list --warnings --agent`,
		Annotations: map[string]string{"mcp:read-only": "true"},
		RunE: func(cmd *cobra.Command, args []string) error {
			if dryRunOK(flags) {
...
Requires at least one of --resource, --action, or --all.`,
		Example: `  unifi-network-cli learnings forget "<question>" --resource <id>
  unifi-network-cli learnings forget "<question>" --all`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if len(args) == 0 {
				return cmd.Help()
			}
...
a whole family.`,
		Example: `  unifi-network-cli teach-pattern --query-template "items in {entity}" --resource-template "GROUP-{entity:category}" --resource-type "items" --entity-kind "category" --strategy substitute`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dryRunOK(flags) {
...
cannot be taught — they are derived from the canonical input.`,
		Example: `  unifi-network-cli teach-lookup --kind country --canonical "United States" --value USA`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dryRunOK(flags) {
