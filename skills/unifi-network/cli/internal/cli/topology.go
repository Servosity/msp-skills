...
func newNovelTopologyCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:   "topology",
		Short: "Group every synced client under the device it is attached to, a device-to-client tree built entirely from local mirror data, no live crawl needed.",
		Long: "Groups every synced client under the device it is attached to " +
			...
			"at the top level. Run 'unifi-network-cli sync' first to populate the mirror.",
		Example:     "  unifi-network-cli topology --site default --json",
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			dbPath := defaultDBPath("unifi-network-cli")
			db, err := openNovelStore(ctx, dbPath)
			if err != nil {
				return err
			}
			if db == nil {
				fmt.Fprintf(cmd.ErrOrStderr(), "no local mirror at %s\nrun: unifi-network-cli sync\n", dbPath)
			...
			if len(view.Devices) == 0 {
				view.Note = "no devices in the local mirror for this site; run 'unifi-network-cli sync' to populate it"
			}
