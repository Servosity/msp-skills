...
var whichIndex = []whichEntry{
	{Command: "topology", Description: "Group every synced client under the device it is attached to, a device-to-client tree built entirely from local mirror data, no live crawl needed.", Group: "Local state that compounds", WhyItMatters: "Reach for this when an agent needs to see which clients sit behind which device without walking every device endpoint individually."},
	{Command: "drift", Description: "Show what changed in site config (networks, firewall, wifi, DNS) since the last drift run for this site.", Group: "Local state that compounds", WhyItMatters: "Use after a suspected config change to see exactly what moved, without manually diffing the controller UI."},
...
Exit codes:
  ...
  2  no confident match - the query did not score against any indexed capability; fall back to '--help' or 'search' if this CLI has one`,
		Example: `  unifi-network-cli which "stale tickets"
  unifi-network-cli which "bottleneck"
  unifi-network-cli which --limit 1 "send message"
  unifi-network-cli which                                # list the full capability index`,
