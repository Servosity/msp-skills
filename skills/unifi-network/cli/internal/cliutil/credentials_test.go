...
import (
	...
	"unifi-network-pp-cli/internal/cliutil/testenv"
...
func resetCredentialEnv(t *testing.T) (home, configPath string) {
	...
	return home, filepath.Join(home, ".config", "unifi-network-cli", "config.toml")
}
...
func TestCorruptCredentialsDoesNotOverrideLegacyConfig(t *testing.T) {
	...
	credentialsPath := filepath.Join(home, ".local", "share", "unifi-network-cli", "credentials.toml")
	if err := os.MkdirAll(filepath.Dir(credentialsPath), 0o700); err != nil {
		t.Fatalf("mkdir credentials dir: %v", err)
	}
...
func TestAuthWriteScrubsLegacyConfigWhenRelocated(t *testing.T) {
	...
	activeConfigPath := filepath.Join(newConfigDir, "unifi-network-cli", "config.toml")
	...
	credsPath := filepath.Join(newDataDir, "unifi-network-cli", "credentials.toml")
