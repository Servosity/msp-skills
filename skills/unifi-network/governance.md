...
> Unofficial. Community-built skill for the UniFi API. Not affiliated with,
...
The skill drives the `unifi-network-cli` binary (and `unifi-network-mcp`),
authenticating with `UNIFI_API_KEY`, `UNIFI_GATEWAY_HOST`. Credentials are read from the environment only -
never written to disk, never logged, never sent anywhere except the UniFi API.
...
| **Write (routine)** | Day-to-day mutations. | `sites acl-rules create`, `sites acl-rules update`, `sites acl-rules update-ordering`, `sites dns create-policy`, `sites dns update-policy`, `sites firewall create-policy`, `sites firewall create-zone`, `sites firewall patch-policy`, ... (18 total) | Preview with `--dry-run`, then an approved write (where a command documents its own confirm gate, use it too) |
| **Credential / security** | Touches tokens, keys, MFA. | (none detected) | Human-in-the-loop only |
| **Destructive** | Irreversible data or config loss. | `sites acl-rules delete`, `sites dns delete-policy`, `sites firewall delete-policy`, `sites firewall delete-zone`, `sites hotspot delete-voucher`, `sites hotspot delete-vouchers`, `sites networks delete`, `sites traffic-matching-lists delete`, ... (9 total) | Human-in-the-loop only, explicit confirmation |
...
the UniFi API, and you can read every line of how it does so. The skill is
