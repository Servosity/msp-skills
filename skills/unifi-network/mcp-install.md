# UniFi MCP - install for every agent that speaks MCP

This page wires the UniFi MCP server into any MCP client. If you use Claude
...
launch `unifi-network-mcp` directly on your machine - no hosting. *Remote* agents (ChatGPT,
Microsoft 365 Copilot / Copilot Studio, the Gemini app) only talk to an HTTPS
endpoint, so you expose `unifi-network-mcp` over HTTPS first.

## Prerequisite: install the MCP binary

Run the install command from [README.md](./README.md). It drops both `unifi-network-cli`
and `unifi-network-mcp` on your PATH. `unifi-network-mcp` is what the agents talk to.

```bash
unifi-network-mcp --help
```
...
Restart Gemini CLI; the UniFi tools appear in its tool list. (The **Gemini app /
...
All remote agents need `unifi-network-mcp` reachable as a public **HTTPS** endpoint. Run it
...
`unifi-network-mcp` you installed is not enough on its own. You also need a **Copilot Studio
license** and a **tenant admin** to enable it. This is a build-and-host task, not a
self-serve install.

Once `unifi-network-mcp` is hosted over HTTPS (above), the lowest-code route:
...
hermes mcp add unifi-network -- unifi-network-mcp

# OpenClaw
openclaw mcp set unifi-network '{"command":"unifi-network-mcp"}'
```
...
- `unifi-network-mcp: command not found`: the install dir is not on your PATH (the
