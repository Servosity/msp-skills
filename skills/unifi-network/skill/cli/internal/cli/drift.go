...
func newNovelDriftCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:   "drift",
		Short: "Show what changed in site config (networks, firewall, wifi, DNS) since the last drift run for this site.",
		Long: "Compares the current locally synced state against a snapshot " +
			...
			"the next run) — that is expected, not an error. Run 'unifi-network-cli " +
			"sync' before each 'drift' to pick up the latest config.",
		Example:     "  unifi-network-cli drift --site default --since 24h --json",
