# install.ps1 - install unifi-network-cli and unifi-network-mcp on Windows.
#
# Pulls prebuilt binaries from this skill's latest GitHub Release (unifi-network-v*).
...
$Skill   = "unifi-network"
$CliBin  = "unifi-network-cli.exe"
$McpBin  = "unifi-network-mcp.exe"
...
$ReleaseBase = if ($env:MSP_SKILLS_RELEASE_BASE) {
  ...
  # Each skill is versioned/tagged independently (unifi-network-vX.Y.Z),
...
Write-Host "  unifi-network-cli --version"
Write-Host ""
Write-Host "Next:"
Write-Host "  First command + auth: https://github.com/$Owner/$Repo/tree/main/skills/unifi-network#readme"
Write-Host "  Claude Desktop / ChatGPT wire-up: https://github.com/$Owner/$Repo/blob/main/skills/unifi-network/mcp-install.md"
