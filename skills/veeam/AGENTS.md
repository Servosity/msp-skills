# Veeam Service Provider Console Printed CLI Agent Guide

This directory is a generated `veeam-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
veeam-cli doctor --json
veeam-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
veeam-cli which "<capability>" --json
veeam-cli <command> --help
```
...
```bash
veeam-cli <command> --help
veeam-cli <command> --dry-run --agent
```
