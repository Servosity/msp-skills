...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".local", "share", "veeam-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("VEEAM_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("VEEAM_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "veeam-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.local/share/veeam-cli/feedback.jsonl.
When ` + "`VEEAM_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`VEEAM_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  veeam-cli feedback list
  veeam-cli feedback list --limit 5
  veeam-cli feedback list --json`,
