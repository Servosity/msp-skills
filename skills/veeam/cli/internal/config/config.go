...
type Config struct {
	...
	VeeamToken    string            `toml:"token"`
}

func Load(configPath string) (*Config, error) {
	cfg := &Config{
		BaseURL: "https://your-vspc-host:1280/api/v3",
	}

	// Resolve config path
	path := configPath
	if path == "" {
		path = os.Getenv("VEEAM_CONFIG")
	}
	if path == "" {
		home, _ := os.UserHomeDir()
		path = filepath.Join(home, ".config", "veeam-cli", "config.toml")
	}
	cfg.Path = path
	...
	// Env var overrides
	if v := os.Getenv("VEEAM_TOKEN"); v != "" {
		cfg.VeeamToken = v
		cfg.markEnvOverride("VeeamToken")
		cfg.AuthSource = "env:VEEAM_TOKEN"
	}
	...
	if cfg.AuthSource == "" && cfg.VeeamToken != "" {
		cfg.AuthSource = "config"
	}
	...
	if v := os.Getenv("VEEAM_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	return cfg, nil
}
...
func (c *Config) AuthHeader() string {
	...
	if c.VeeamToken != "" {
		if c.AuthSource == "" {
			c.AuthSource = "env:VEEAM_TOKEN"
		}
		return "Bearer " + c.VeeamToken
	}
	if c.AccessToken != "" {
...
func (c *Config) ClearTokens() error {
	...
	c.updateFileConfigField("ClientSecret")
	c.VeeamToken = ""
	delete(c.envOverrides, "VeeamToken")
	return c.save()
}
...
func (c *Config) configForSave() Config {
	out := *c
	if c.fileConfig != nil {
		if c.envOverrides["VeeamToken"] {
			out.VeeamToken = c.fileConfig.VeeamToken
		}
	}
...
func (c *Config) updateFileConfigField(field string) {
	...
	case "VeeamToken":
		c.fileConfig.VeeamToken = c.VeeamToken
	}
}
