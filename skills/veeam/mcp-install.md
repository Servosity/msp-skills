# Veeam MCP - install for every agent that speaks MCP

This page wires the Veeam MCP server into any MCP client. If you use Claude
...
launch `veeam-mcp` directly on your machine - no hosting. *Remote* agents (ChatGPT,
Microsoft 365 Copilot / Copilot Studio, the Gemini app) only talk to an HTTPS
endpoint, so you expose `veeam-mcp` over HTTPS first.

## Prerequisite: install the MCP binary

Run the install command from [README.md](./README.md). It drops both `veeam-cli`
and `veeam-mcp` on your PATH. `veeam-mcp` is what the agents talk to.
...
```json
{
  "mcpServers": {
    "veeam": {
      "command": "veeam-mcp",
      "env": {
        "VEEAM_BASE_URL": "<your-veeam_base_url>",
        "VEEAM_TOKEN": "<your-veeam_token>"
      }
    }
  }
}
...
Restart Gemini CLI; the Veeam tools appear in its tool list. (The **Gemini app /
...
All remote agents need `veeam-mcp` reachable as a public **HTTPS** endpoint. Run it
in HTTP mode with your credentials in the environment:

```bash
VEEAM_BASE_URL=<value> VEEAM_TOKEN=<value> veeam-mcp --transport http --addr :7777
```
...
`veeam-mcp` you installed is not enough on its own. You also need a **Copilot Studio
license** and a **tenant admin** to enable it. This is a build-and-host task, not a
self-serve install.

Once `veeam-mcp` is hosted over HTTPS (above), the lowest-code route:
...
hermes mcp add veeam -- veeam-mcp

# OpenClaw
openclaw mcp set veeam '{"command":"veeam-mcp"}'
```
...
- `veeam-mcp: command not found`: the install dir is not on your PATH (the
