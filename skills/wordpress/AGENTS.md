...
This directory is a generated `wordpress-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
wordpress-cli doctor --json
wordpress-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
wordpress-cli which "<capability>" --json
...
```bash
wordpress-cli <command> --help
wordpress-cli <command> --dry-run --agent
```
