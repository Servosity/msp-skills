...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  wordpress-cli workflow archive

  # Full re-archive (ignore previous sync state)
  wordpress-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"categories", "media", "pages", "posts", "tags", "users"}
			totalSynced := 0
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  wordpress-cli workflow status

  # Show status as JSON
  wordpress-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("wordpress-cli")
			}
