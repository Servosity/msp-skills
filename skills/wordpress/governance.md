# wordpress skill - governance and safety model

> Unofficial. Community-built skill for the WordPress API. Not affiliated with,
> endorsed by, or sponsored by the vendor.
> This page tells an MSP owner exactly what the wordpress skill can touch and how to
> scope it, so you can decide what to let an AI agent do.

## What it authenticates as

The skill drives the `wordpress-cli` binary (and `wordpress-mcp`),
authenticating with `WORDPRESS_BASIC_AUTH`. Credentials are read from the environment only -
never written to disk, never logged, never sent anywhere except the WordPress API.
...
the WordPress API, and you can read every line of how it does so. The skill is
