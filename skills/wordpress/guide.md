...
> **Claude Desktop bundle:** the `.mcpb` launches on macOS (Intel and Apple Silicon), Windows x64 and Linux x64. It contains only the MCP server; every tool shells out to the companion `wordpress-cli`, so run the installer above first (or set `WORDPRESS_CLI_PATH` to an existing binary). Bundling the CLI is tracked in [#331](https://github.com/Servosity/msp-skills/issues/331).
