# install.ps1 - install wordpress-cli and wordpress-mcp on Windows.
#
# Pulls prebuilt binaries from this skill's latest GitHub Release (wordpress-v*).
...
$Skill   = "wordpress"
$CliBin  = "wordpress-cli.exe"
$McpBin  = "wordpress-mcp.exe"
...
$ReleaseBase = if ($env:MSP_SKILLS_RELEASE_BASE) {
  ...
  # Each skill is versioned/tagged independently (wordpress-vX.Y.Z),
...
Write-Host "  wordpress-cli --version"
Write-Host ""
Write-Host "Next:"
Write-Host "  First command + auth: https://github.com/$Owner/$Repo/tree/main/skills/wordpress#readme"
Write-Host "  Claude Desktop / ChatGPT wire-up: https://github.com/$Owner/$Repo/blob/main/skills/wordpress/mcp-install.md"
