# Xero Printed CLI Agent Guide

This directory is a generated `xero-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
xero-cli doctor --json
xero-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
xero-cli which "<capability>" --json
...
```bash
xero-cli <command> --help
xero-cli <command> --dry-run --agent
```
