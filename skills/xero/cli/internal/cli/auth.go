...
func newAuthCmd(flags *rootFlags) *cobra.Command {
	cmd := &cobra.Command{
		Use:   "auth",
		Short: "Manage authentication for Xero Accounting",
...
func newAuthSetupCmd(_ *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: "  xero-cli auth setup\n  xero-cli auth setup --launch",
		RunE: func(cmd *cobra.Command, args []string) error {
			w := cmd.OutOrStdout()
			fmt.Fprintln(w, "See API docs for OAuth app registration: https://developer.xero.com")
			fmt.Fprintln(w, "")
			fmt.Fprintln(w, "Then run:")
			fmt.Fprintln(w, "  xero-cli login")
			if !launch {
				return nil
			}
			launchURL := "https://developer.xero.com"
...
func newAuthLoginCmd(flags *rootFlags) *cobra.Command {
	...
	cmd.Flags().StringVar(&clientID, "client-id", os.Getenv("XERO_CLIENT_ID"), "OAuth2 client ID")
	cmd.Flags().StringVar(&clientSecret, "client-secret", os.Getenv("XERO_CLIENT_SECRET"), "OAuth2 client secret")
...
func runOAuthLogin(cmd *cobra.Command, flags *rootFlags, clientID, clientSecret string, port int) error {
	...
	scopes := []string{"accounting.attachments", "accounting.attachments.read", "accounting.budgets.read", "accounting.contacts", "accounting.contacts.read", "accounting.journals.read", "accounting.reports.read", "accounting.reports.tenninetynine.read", "accounting.settings", "accounting.settings.read", "accounting.transactions", "accounting.transactions.read", "email", "openid", "paymentservices", "profile"}
	if len(scopes) > 0 {
...
func resolveOAuthCredentials(cmd *cobra.Command, flags *rootFlags, cfg *config.Config, clientID, clientSecret string) (string, string, error) {
	...
	if flags.noInput {
		return "", "", fmt.Errorf("OAuth2 client ID is required; pass --client-id, set XERO_CLIENT_ID, or run 'xero-cli login' without --no-input")
	}
...
func newAuthStatusCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if !authed {
				fmt.Fprintf(w, "  %s Not authenticated. Run 'xero-cli login' to authenticate.\n", red("FAIL"))
				return nil
			}
			...
			if cfg.TokenExpiry.IsZero() {
				...
				if cfg.RefreshToken != "" {
					...
					fmt.Fprintf(w, "  %s Token expired. Run 'xero-cli login' to re-authenticate.\n", red("FAIL"))
				}
			}
