...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  xero-cli workflow archive

  # Full re-archive (ignore previous sync state)
  xero-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"accounts", "bank-transactions", "contacts", "invoices", "items", "journals", "payments"}
			totalSynced := 0
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  xero-cli workflow status

  # Show status as JSON
  xero-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("xero-cli")
			}
