...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".local", "share", "xero-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("XERO_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("XERO_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "xero-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.local/share/xero-cli/feedback.jsonl.
When ` + "`XERO_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`XERO_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  xero-cli feedback list
  xero-cli feedback list --limit 5
  xero-cli feedback list --json`,
