...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "xero-cli",
		Short: `Xero CLI — Every Xero Accounting read plus a local SQLite ledger — aging, reconciliation, and GL tie-out that no other Xero tool c…`,
		Long: `Xero CLI — Every Xero Accounting read plus a local SQLite ledger — aging, reconciliation, and GL tie-out that no other Xero tool c…
...
Health check: run 'xero-cli doctor' to verify auth and connectivity.
See README.md or the bundled SKILL.md for recipes.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("xero-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.profileName, "profile", "", "Apply values from a saved profile (see 'xero-cli profile list')")
	...
	rootCmd.AddCommand(newBankTransactionsCmd(flags))
	...
	rootCmd.AddCommand(newNovelBankReconCmd(flags))
	...
	rootCmd.AddCommand(newNovelTieOutCmd(flags))
