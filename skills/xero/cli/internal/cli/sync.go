...
Resource scoping:
  ...
  xero-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  xero-cli sync --full

  # Incremental sync: only records from the last 7 days
  xero-cli sync --since 7d

  # Parallel sync with 8 workers
  xero-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  xero-cli sync --latest-only`,
...
func upsertSingleObject(db *store.Store, resource string, data json.RawMessage) error {
	...
	case "bank_transactions_history":
		return db.UpsertBankTransactionsHistory(data)
	case "contacts_history":
		return db.UpsertContactsHistory(data)
	case "invoices_history":
		return db.UpsertInvoicesHistory(data)
	case "items":
		return db.UpsertItems(data)
	case "items_history":
		return db.UpsertItemsHistory(data)
	case "payments_history":
		return db.UpsertPaymentsHistory(data)
	default:
