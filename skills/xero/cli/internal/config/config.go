...
func Load(configPath string) (*Config, error) {
	...
	path := configPath
	if path == "" {
		path = os.Getenv("XERO_CONFIG")
	}
	if path == "" {
		home, _ := os.UserHomeDir()
		path = filepath.Join(home, ".config", "xero-cli", "config.toml")
	}
	cfg.Path = path
	...
	if v := os.Getenv("XERO_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	if v := os.Getenv("XERO_AUTHORIZATION_URL"); v != "" {
...
func (c *Config) AuthHeader() string {
	...
	if c.AccessToken != "" {
		if c.AuthSource == "" {
			c.AuthSource = "env:XERO_ACCESS_TOKEN"
		}
...
const NoConfigWriteEnv = "XERO_NO_CONFIG_WRITE"
...
// material. It is the ONLY save path XERO_NO_CONFIG_WRITE suppresses;
