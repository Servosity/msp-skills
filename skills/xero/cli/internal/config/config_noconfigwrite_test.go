...
// touching disk. XERO_NO_CONFIG_WRITE makes the automatic token cache a
...
const (
	ncwToken   = "minted-token-XERO"
	ncwRefresh = "minted-refresh-XERO"
	ncwSecret  = "minted-secret-XERO"
)
...
func ncwMint(t *testing.T, value string, set bool) map[string]string {
	...
	home := t.TempDir()
	t.Setenv("HOME", home)
	t.Setenv("XERO_CONFIG", filepath.Join(home, "config.toml"))
	if set {
...
func TestNoConfigWriteTruthiness(t *testing.T) {
	...
	if NoConfigWriteEnv != "XERO_NO_CONFIG_WRITE" {
		t.Errorf("switch is named %q; the documented name is XERO_NO_CONFIG_WRITE", NoConfigWriteEnv)
	}
}
