...
| --- | --- | --- | --- |
| `XERO_ACCESS_TOKEN` | per_call | Yes | OAuth2 bearer access token (Custom Connections M2M token or pasted authorization-code token). The canonical credential var. |
| `XERO_TENANT_ID` | per_call | Yes | Tenant UUID sent as the required `Xero-Tenant-Id` header on every call. Get it from `GET /connections` after minting a token. |
| `XERO_ACCOUNTING_OAUTH2` | per_call | No | Slug-derived fallback for the bearer token; use `XERO_ACCESS_TOKEN` instead. |
| `XERO_OAUTH2` | per_call | No | Legacy fallback from the prior printed CLI; still read for back-compat. |
