# install.ps1 - install xero-cli and xero-mcp on Windows.
#
# Pulls prebuilt binaries from this skill's latest GitHub Release (xero-v*).
...
$ReleaseBase = if ($env:MSP_SKILLS_RELEASE_BASE) {
  ...
  # Each skill is versioned/tagged independently (xero-vX.Y.Z),
...
Write-Host "  xero-cli --version"
Write-Host ""
Write-Host "Next:"
Write-Host "  First command + auth: https://github.com/$Owner/$Repo/tree/main/skills/xero#readme"
Write-Host "  Claude Desktop / ChatGPT wire-up: https://github.com/$Owner/$Repo/blob/main/skills/xero/mcp-install.md"
