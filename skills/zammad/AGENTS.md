# Zammad Printed CLI Agent Guide

This directory is a generated `zammad-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
zammad-cli doctor --json
zammad-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
zammad-cli which "<capability>" --json
zammad-cli <command> --help
```
...
```bash
zammad-cli <command> --help
zammad-cli <command> --dry-run --agent
```
...
3. After answering, always fire `zammad-cli teach --query "<question>" --resource <id> --resource-type <type> &` in the background - teaching is unconditional and is the anchor that triggers playbook synthesis. Teach the structural question with identifiers stripped (no names, emails, phone numbers, account ids); the CLI warns on obvious PII shapes but does not block.
...
Annotations: `teach`, `teach-playbook`, `playbook amend`, `playbook list`, `recall`, and `learnings confirm` carry `mcp:local-write=true` (writes land only in the CLI's own local store); `learnings list`, `learnings candidates`, and `learnings stats` carry `mcp:read-only=true`, which is narrower than it sounds: each one still opens the learn store read-write and creates the database file when it is absent, but none records a learning row; `teach-pattern`, `teach-lookup`, `learnings forget`, and `learnings reject` keep the may-write/destructive defaults.
...
Disable the loop with `--no-learn` per-invocation or `ZAMMAD_NO_LEARN=true` for the whole session - useful for deterministic agent flows that don't want a learning row to silently change subsequent query results.
