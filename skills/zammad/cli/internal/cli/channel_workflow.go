...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  zammad-cli workflow archive

  # Full re-archive (ignore previous sync state)
  zammad-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"groups", "organizations", "overviews", "priorities", "states", "tags-tag-list", "tickets", "users"}
			totalSynced := 0
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  zammad-cli workflow status

  # Show status as JSON
  zammad-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("zammad-cli")
			}
