...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  zammad-cli doctor
  zammad-cli doctor --json
  zammad-cli doctor --fail-on warn
  zammad-cli doctor --fail-on stale`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if cfg != nil {
				...
				if header == "" {
					...
					report["auth_hint"] = "Set your API key with: export ZAMMAD_API_TOKEN=\"your-token-here\""
				} else {
					authConfigured = true
			...
			if os.Getenv("ZAMMAD_API_TOKEN") != "" {
				authEnvSet = append(authEnvSet, "ZAMMAD_API_TOKEN")
				...
				authEnvRequiredMissing = append(authEnvRequiredMissing, "ZAMMAD_API_TOKEN")
			}
			switch {
			...
			if cfg != nil && doctorBaseURLIsPlaceholder(cfg.BaseURL) {
				...
				if clientErr != nil {
					...
					if authHeader == "" {
						...
						authHeaders["User-Agent"] = "zammad-cli"
						verifyPath := "/users/me"
...
func collectCacheReport(ctx context.Context, staleAfterSpec string) map[string]any {
	report := map[string]any{}
	dbPath := defaultDBPath("zammad-cli")
	...
	if err != nil {
		if os.IsNotExist(err) {
			...
			report["hint"] = "Database not created yet; run 'zammad-cli sync' to hydrate."
			return report
		}
		report["status"] = "error"
	...
	if qerr != nil {
		...
		report["status"] = "unknown"
		report["hint"] = "No sync state recorded; run 'zammad-cli sync' to populate."
		return report
	}
	defer rows.Close()
	...
	case !haveAny && len(resources) == 0:
		report["status"] = "empty"
		report["hint"] = "Cache is empty; run 'zammad-cli sync' to hydrate."
	case fresh:
		report["status"] = "fresh"
	default:
		report["status"] = "stale"
		report["oldest_age"] = oldest.Round(time.Minute).String()
		report["hint"] = "Some resources are older than stale_after; run 'zammad-cli sync' to refresh."
	}
	return report
}
