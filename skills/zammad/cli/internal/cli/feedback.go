...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("ZAMMAD_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("ZAMMAD_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "zammad-cli/feedback")
...
When ` + "`ZAMMAD_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`ZAMMAD_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  zammad-cli feedback list
  zammad-cli feedback list --limit 5
  zammad-cli feedback list --json`,
