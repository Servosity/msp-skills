...
type rootFlags struct {
	...
	// invocation. Mirrors the ZAMMAD_NO_LEARN env var.
	noLearn bool
...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "zammad-cli",
		Short: `Zammad CLI — Every Zammad ticket, article, and Knowledge Base operation as one agent-native CLI — plus a team-management layer…`,
		Long: `Zammad CLI — Every Zammad ticket, article, and Knowledge Base operation as one agent-native CLI — plus a team-management layer (agent load, customer health, aging backlog, escalation triage, churn risk, feedback mining) the Zammad API can't answer in a single call.
...
Health check: run 'zammad-cli doctor' to verify auth and connectivity.
See README.md or the bundled SKILL.md for recipes.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("zammad-cli {{ .Version }}\n")
	...
	rootCmd.AddCommand(newKbCmd(flags))
	rootCmd.AddCommand(newOrganizationsCmd(flags))
	rootCmd.AddCommand(newOverviewsCmd(flags))
