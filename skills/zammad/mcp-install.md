# Zammad MCP - install for every agent that speaks MCP

This page wires the Zammad MCP server into any MCP client. If you use Claude
...
launch `zammad-mcp` directly on your machine - no hosting. *Remote* agents (ChatGPT,
Microsoft 365 Copilot / Copilot Studio, the Gemini app) only talk to an HTTPS
endpoint, so you expose `zammad-mcp` over HTTPS first.

## Prerequisite: install the MCP binary

Run the install command from [README.md](./README.md). It drops both `zammad-cli`
and `zammad-mcp` on your PATH. `zammad-mcp` is what the agents talk to.
...
Restart Gemini CLI; the Zammad tools appear in its tool list. (The **Gemini app /
...
All remote agents need `zammad-mcp` reachable as a public **HTTPS** endpoint. Run it
...
`zammad-mcp` you installed is not enough on its own. You also need a **Copilot Studio
license** and a **tenant admin** to enable it. This is a build-and-host task, not a
self-serve install.

Once `zammad-mcp` is hosted over HTTPS (above), the lowest-code route:
...
hermes mcp add zammad -- zammad-mcp

# OpenClaw
openclaw mcp set zammad '{"command":"zammad-mcp"}'
```
...
- `zammad-mcp: command not found`: the install dir is not on your PATH (the
